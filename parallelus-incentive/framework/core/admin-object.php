...
if ( !defined( $runway_framework_admin ) ) {
	class Runway_Admin_Object {
		...
		function __construct( $settings ) {
			...
			/* temp disabled */
			// add_action('current_screen', array(&$this, 'contextual_help_tab'));
		...
		// return parent file to highlight selected item
		// it is does not working because commented
		// needed lines in wordpress core :(
		...
		// currently deprecated because in wordpress 3.4.2
		// this functionality changed
		function contextual_help_tab() {

			global $theme_settings, $wp_version;

			// Check for user created help content
			if ( is_object( $theme_settings ) ) {
				$themeSettings = $theme_settings->get_objects( array( '_framework' ) );
			...
			if ( !isset( $help ) ) {
				...
				$help  = '<p>' . __( 'View the help documentation: ', THEME_NAME ) .
					'<a href="JavaScript:window.open("'. THEME_URL .'assets/docs/readme.html#'.$navigation.'", "helpWindow", status = 1, scrollbars = 1, width = 960, height = 700")">Help Document</a></p>';
			}

			// Only available in WP 3.3+
			$screen = get_current_screen();
			$screen->add_help_tab( array( 'content' => $help ) );
		...
		*/
		...
		function export_data() {
			...
			//
			// A new method of exporting data may be created in the future. Currently the settings and options for a theme
			// are mostly stored in the data files created by the Extensions and Options Builder when making child themes
			// to package and download. At this point we don't really need a method to export data but if we implement one
			// it would likely exsit as an Extension.
			//
			// ============================================================================================================
			...
			$saveFile = FRAMEWORK_DIR . "framework/utilities/download/save.php";
			...
			?>
				...
				<script type="text/javascript">
					function showCode() {
						jQuery('#GetFile').attr('src', '<?php echo FRAMEWORK_URL; ?>framework/utilities/download/getfile.php?file=save.php');
						return false;
					}
				</script>
				...
				<p style="display:none;"><textarea rows="15" class="large-text readonly" name="rules" id="rules" readonly="readonly" onclick="this.select();"><?php echo esc_html($export); ?></textarea></p>
			<?php
			$this->options_page_wrapper_footer();
...
?>
		...
		*/
		...
		function _field_template_path($field) {
		...
		function field_template_path($field) {
			...
			$field->template_path = null;

			if ( file_exists( THEME_DIR . 'data-types/' . $field->type . '/template.php' ) ) {
				$field->template_path = THEME_DIR . 'data-types/' . $field->type . '/template.php';
			} else {
				$field->template_path = get_theme_root().'/'.$theme_data['Template'].'/data-types/'.$field->type.'/template.php';
			}
				
			if(!file_exists($field->template_path))	
				$field->template_path = false;
		...
		function dynamic_template_field( $field ) {			
			...
			$field->template_path = $this->_field_template_path($field);						
			if( $field->template_path !== false) {
				...
				/* old data types loading */

				$field = $this->field_template_path($field);			

				// check is template ready for rendering
				if ( $field->template_path !== false ) {												
					
					$value = $this->get_val( $field->alias );					

					if ( isset( $value ) && !empty( $value ) ) {
						$field->value = $value;
					} else {
						$field->value = isset( $field->values ) ? $field->values : '';
					}
					...
					// render 
					return '<div>Field template file is absent.</div>';
				}
			}
		}

...
class VP_Option_Control_Set {
	...
	public function save( $option_key ) {
		...
		if ( update_option( $option_key, $opt ) ) {
			...
			$result['message'] = esc_html__( 'Saving successful', 'crazyblog' );
			...
			if ( $changed ) {
				...
				$result['message'] = esc_html__( 'Saving failed', 'crazyblog' );
			} else {
				$result['status'] = true;
				$result['code'] = self::SAVE_NOCHANGES;
				$result['message'] = esc_html__( 'No changes made', 'crazyblog' );
			}
		}

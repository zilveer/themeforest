	...
	*/
	...
	if ( $upload != "") { $val = $upload; } else {$val = $std;}
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.$id.'">Upload <i class="fa fa-upload"></i></span>';

			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'">Remove <i class="fa fa-remove"></i></span>';
		}
		else
		{

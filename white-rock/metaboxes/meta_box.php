...
class custom_add_meta_box {
	...
	function admin_enqueue_scripts() {
		...
		if ( meta_box_find_field_type( 'image', $this->fields ) )
		...
		$deps = meta_box_find_field_type( 'date', $this->fields ) || meta_box_find_field_type( 'slider', $this->fields ) ? array ( 'jqueryui' ) : null;
		...
		foreach ( $this->fields as $field) {
			...
			echo '<tr>
					...
					switch( $type ) {
						...
						// textarea
						case 'textarea':
							echo '<textarea name="' . $id . '" id="' . $id . '" cols="60" rows="4">' . esc_html( $meta ) . '</textarea>
									<br />' . $desc;
						break;
						// checkbox
						case 'checkbox':

<?php
add_action('tgmpa_register', 'jaw_register_required_plugins');
 ...
 */
function jaw_register_required_plugins() {

    /**
     * Array of plugin arrays. Required keys are name, slug and required.
     * If the source is NOT from the .org repo, then source is also required.
     */
    $force_active = ((defined('WP_ALLOW_MULTISITE') && WP_ALLOW_MULTISITE == true) || (defined('WP_DEBUG') && WP_DEBUG == true) || (defined('JAW_FORCE_PLUGINS') && JAW_FORCE_PLUGINS == true) ) ? false : true;
    $plugins = array(
        // This is an example of how to include a plugin pre-packaged with a theme
        array(
            'name' => 'JaW Shortcodes', // The plugin name
            'slug' => 'jaw-shortcodes', // The plugin slug (typically the folder name)
            'source' => get_stylesheet_directory() . '/framework/plugins/jaw-shortcodes.zip', // The plugin source
            ...
            'force_activation' => $force_active, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
            'external_url' => '', // If set, overrides default API URL and points to an external URL
        ),
        array(
            'name' => 'JaW Widgets', // The plugin name
            'slug' => 'jaw-widgets', // The plugin slug (typically the folder name)
            'source' => get_stylesheet_directory() . '/framework/plugins/jaw-widgets.zip', // The plugin source
            'required' => true, // If false, the plugin is only 'recommended' instead of required
            'version' => '1.3.9', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
            'force_activation' => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
            'external_url' => '', // If set, overrides default API URL and points to an external URL
        ),
        array(
            'name' => 'JaW Menu', // The plugin name
            'slug' => 'jaw-menu', // The plugin slug (typically the folder name)
            'source' => get_stylesheet_directory() . '/framework/plugins/jaw-menu.zip', // The plugin source
            'required' => true, // If false, the plugin is only 'recommended' instead of required
        ...
        array(
            'name' => 'JaW Customposts', // The plugin name
            'slug' => 'jaw-customposts', // The plugin slug (typically the folder name)
            'source' => get_stylesheet_directory() . '/framework/plugins/jaw-customposts.zip', // The plugin source
            'required' => true, // If false, the plugin is only 'recommended' instead of required
        ...
        array(
            ...
            'source' => get_stylesheet_directory() . '/framework/plugins/essential-grid.zip', // The plugin source
            'required' => false, // If false, the plugin is only 'recommended' instead of required
            'version' => '2.1.0.1', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
            'force_activation' => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
            'external_url' => '', // If set, overrides default API URL and points to an external URL
        ),
        array(
            ...
            'source' => get_stylesheet_directory() . '/framework/plugins/envato-wordpress-toolkit-master.zip', // The plugin source
            'required' => false, // If false, the plugin is only 'recommended' instead of required
        ...
        array(
            'name' => 'WooCommerce Sold Out Products',
            'slug' => 'woocommerce-sold-out-products',
            'version' => '1.0.0',
            'required' => false,
        ),
        array(
     ...
     */
    $config = array(
        'domain' => "jawtemplates", // Text domain - likely want to be the same as your theme.
        ...
        'strings' => array(
            'page_title' => __('Install Required Plugins', "jawtemplates"),
            'menu_title' => __('Install Plugins', "jawtemplates"),
            'installing' => __('Installing Plugin: %s', "jawtemplates"), // %1$s = plugin name
            'oops' => __('Something went wrong with the plugin API.', "jawtemplates"),
            ...
            'return' => __('Return to Required Plugins Installer', "jawtemplates"),
            'plugin_activated' => __('Plugin activated successfully.', "jawtemplates"),
            'complete' => __('All plugins installed and activated successfully. %s', "jawtemplates") // %1$s = dashboard link
        )
    );
		...
		protected function do_plugin_install() {
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) || ( isset( $_GET[sanitize_key( 'tgmpa-update' )] ) && 'update-plugin' == $_GET[sanitize_key( 'tgmpa-update' )] ) ) ) {
				...
				if ( ! WP_Filesystem( $creds ) ) {
					request_filesystem_credentials( esc_url($url), $method, true, false, $fields ); // Setup WP_Filesystem
					return true;
				}

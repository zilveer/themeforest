																	...
																	foreach($v['blocks'] as $vv) {
																		if( $vv['type'] == $block['type'] ) {
																			...
																			foreach($vv['options'] as $vvv) {
																				...
																				$vv['options'][$n]['id_clean'] = $vvv['id'];
																				$vv['options'][$n]['id_key'] = "_".$block['id'];
																				$opt = $vvv['type'];
	...
	*/
	function print_upload($value) {
		...
		?>
				<span>
					<div class="uploader">
						<input type="text" name="<?php echo $value['id'];?>" value="<?php echo $input_value;?>" class="upload" style="width:199px;  <?php if( $value['home'] == "yes" ) { ?>margin-left:483px;<?php } ?>" id="<?php echo $value['id'];?>" />
						<span id="<?php echo $value['id'];?>_button" class="action btn-upload" style="width:100px; float:right; z-index:500;">Choose File</span>
						<script type="text/javascript">
							jQuery(document).ready(function($){ loadUploader(jQuery("span#<?php echo $value['id'];?>_button"), "<?php echo THEME_FUNCTIONS_URL;?>upload-handler.php", "<?php echo THEME_UPLOADS_URL;?>");});
						</script>
					</div>
				</span>
			</div>

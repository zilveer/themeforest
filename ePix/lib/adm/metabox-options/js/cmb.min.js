...
window.CMB = (function(window, document, $, undefined){
	...
	cmb.init = function() {
		...
		$( l10n.meta_box_ids ).addClass('group_meta_box');
			
		$('#'+ postbox_id +' .group_meta_box').wrapAll('<div class="clearfix" id="nv_meta_boxes" /></div>');
			
		// Show tabs if more than one
		if( l10n.meta_box_count > 1 ) 
		{
			if( $('#' + postbox_id).length ) $('<h2 class="nav-tab-wrapper clearfix">'+ l10n.meta_box_menu +'</h2>').insertBefore('#nv_meta_boxes');
		}

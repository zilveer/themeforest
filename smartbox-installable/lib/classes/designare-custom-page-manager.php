 ...
 *
 */
Class DesignareCustomPageManager{
	 ...
	 */
	function DesignareCustomPageManager($custom_page, $id_prefix, $suffix, $default, $nonce){
	...
	function build_page(){
		...
		$this->templater=new DesignareTemplater();
		...
		echo '<input type="hidden" value="'.$this->taxonomy.'" id="taxnonomy_id" />
			  ...
			  <input type="hidden" value="'.wp_create_nonce(DESIGNARE_NONCE).'" id="designare_nonce" />
		      </div>';
	}
	...
	function _print_items($category){
		...
		$ordered_posts=designare_get_ordered_post_list($posts, $category,$this->custom_page->post_type);

...
class WPAlchemy_Taxonomy {
	...
	function WPAlchemy_Taxonomy( $arr ) {
		...
		$this->types = array( 'post', 'page', 'webinane_galleries' );
	...
	function _init() {
		...
		if ( true ) {//$this->can_output())
			...
			add_action( 'edited_category', array( $this, '_save' ), 10, 2 );

			add_action( 'create_category', array( $this, '_save' ), 10, 2 );



			add_action( 'edited_post_tag', array( $this, '_save' ), 10, 2 );

			add_action( 'create_post_tag', array( $this, '_save' ), 10, 2 );



			add_action( 'edited_product_cat', array( $this, '_save' ), 10, 2 );

			add_action( 'create_product_cat', array( $this, '_save' ), 10, 2 );
	...
	function vp_metabox_enqueue() {
		...
		if ( isset( $uri ) ) {
			...
			if ( $file == 'edit-tags.php' ) {
				...
				$loader->add_main_js( 'vp-option' );

				$loader->add_main_css( 'vp-option' );
			}
		}
	}
	...
	function vp_enqueue() {
		...
		if ( isset( $uri ) ) {
			...
			if ( $file == 'edit-tags.php' ) {
				...
				wp_register_script( 'select2', crazyblog_URI . 'core/duffers_panel/panel/public/js/vendor/select2.min.js', array(), '4.0', true );

				wp_register_script( 'cate', crazyblog_URI . 'core/duffers_panel/panel/public/js/vendor/category.js', array(), '4.0', true );

				wp_enqueue_style( 'select2', crazyblog_URI . 'core/duffers_panel/panel/public/css/vendor/select2.css', array(), '4.0', 'all' );

				wp_enqueue_script( array( 'jquery-ui-sortable', 'select2', 'cate' ) );

...
$flexbox_scheme = array(
    'main' => array(
        'left' => array(
            'stretch' => 'fxb-basis-auto fxb-sm-full',
        ),
    ),
);
...
if(zn_check_components('flags')) add_action( 'zn_head__top_right', 'zn_wpml_language_switcher_alt', 50 ); // WPML LANGUAGE SWITCHER


/**** MAIN LEFT */

if(zn_check_components('logo')) add_action( 'zn_head__main_left', 'zn_header_display_logo' ); // LOGO MARKUP


/**** MAIN CENTER */

if(zn_check_components('search_box')) add_action( 'zn_head__main_center', 'zn_header_searchbox_bord' ); // SEARCH BOX


/**** MAIN RIGHT */

add_action( 'zn_head__main_right', 'zn_header_calltoaction', 10 ); // CALL TO ACTION BUTTON
add_action( 'zn_head__main_right', 'zn_woocomerce_cart_init', 20 ); // ADD CART PANEL


/**** BOTTOM LEFT */

if(zn_check_components('mainmenu')) add_action( 'zn_head__bottom_left', 'zn_mainmenu_active_text', 10 ); // MAIN NAVIGATION

...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            ...
            $this->strings = array(
                'page_title'                      => __( 'Install Required Plugins', TGM_THEME_DOMAIN ),
                'menu_title'                      => __( 'Install Plugins', TGM_THEME_DOMAIN ),
                'installing'                      => __( 'Installing Plugin: %s', TGM_THEME_DOMAIN ),
                'oops'                            => __( 'Something went wrong.', TGM_THEME_DOMAIN ),
                ...
                'return'                          => __( 'Return to Required Plugins Installer', TGM_THEME_DOMAIN ),
                'plugin_activated'                => __( 'Plugin activated successfully.', TGM_THEME_DOMAIN ),
                'complete'                        => __( 'All plugins installed and activated successfully. %1$s', TGM_THEME_DOMAIN ),
            );
            ...
            if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) || ( isset( $_GET[sanitize_key( 'tgmpa-update' )] ) && 'update-plugin' == $_GET[sanitize_key( 'tgmpa-update' )] ) ) ) {
                ...
                if ( $this->is_automatic ) {
                    ...
                    if ( is_wp_error( $activate ) ) {
                        ...
                        echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( 'Return to Required Plugins Installer', TGM_THEME_DOMAIN ) . '</a></p>';
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', TGM_THEME_DOMAIN ) . '">' . __( 'Return to the Dashboard', TGM_THEME_DOMAIN ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
                        $msg = sprintf( __( 'The following plugin was activated successfully: %s.', TGM_THEME_DOMAIN ), '<strong>' . $plugin['name'] . '</strong>' );
            ...
            if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
                ...
                if ( ! empty( $message ) ) {
                    ...
                    foreach ( $message as $type => $plugin_groups ) {
                        ...
                        $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, TGM_THEME_DOMAIN ), $imploded, $count ) . '</p>'; // All messages now stored
                    }

                    /** Setup variables to determine if action links are needed */

                    $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, TGM_THEME_DOMAIN ) . '</a>' : '';
                    $show_activate_link = $activate_link ? '<a href="' . admin_url( 'plugins.php' ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, TGM_THEME_DOMAIN ) . '</a>'  : '';
                    $show_update_link  = $update_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, TGM_THEME_DOMAIN ) . '</a>' : '';
                    ...
                    $action_links = apply_filters(
                        ...
                        array(
                            ...
                            'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( 'Dismiss this notice', TGM_THEME_DOMAIN ) . '</a>',
                        )
                    );
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( isset( $plugin['external_url'] ) ) {
                    ...
                    $table_data[$i]['source'] = __( 'External Link', TGM_THEME_DOMAIN );
                }
                elseif ( isset( $plugin['source'] ) ) {
                    /** The plugin must be from a private repository */
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
                        $table_data[$i]['source'] = __( 'Private Repository', TGM_THEME_DOMAIN );
                    /** The plugin is pre-packaged with the theme */
                    else
                        $table_data[$i]['source'] = __( 'Pre-Packaged', TGM_THEME_DOMAIN );
                }
                /** The plugin is from the WordPress repository */
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', TGM_THEME_DOMAIN );
                }

                $table_data[$i]['type'] = $plugin['required'] ? __( 'Required', TGM_THEME_DOMAIN ) : __( 'Recommended', TGM_THEME_DOMAIN );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', TGM_THEME_DOMAIN ) );
                elseif ( is_plugin_inactive( $plugin['file_path'] ) )
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', TGM_THEME_DOMAIN ) );
                elseif ( version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<' ) )
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Updated', TGM_THEME_DOMAIN ) );
                else
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Active', TGM_THEME_DOMAIN ) );
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', TGM_THEME_DOMAIN ), admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', TGM_THEME_DOMAIN ),
                'source' => __( 'Source', TGM_THEME_DOMAIN ),
                'type'   => __( 'Type', TGM_THEME_DOMAIN ),
                'status' => __( 'Status', TGM_THEME_DOMAIN )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => __( 'Install', TGM_THEME_DOMAIN ),
                'tgmpa-bulk-activate' => __( 'Activate', TGM_THEME_DOMAIN ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) )
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, TGM_THEME_DOMAIN ), $imploded );
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', TGM_THEME_DOMAIN );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', TGM_THEME_DOMAIN );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', TGM_THEME_DOMAIN );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', TGM_THEME_DOMAIN );
                $this->strings['process_failed']      = __( 'Plugin install failed.', TGM_THEME_DOMAIN );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', TGM_THEME_DOMAIN );
            ...
            public function activate_strings() {
                ...
                $this->strings['activation_failed']  = __( 'Plugin activation failed.', TGM_THEME_DOMAIN );
                $this->strings['activation_success'] = __( 'Plugin activated successfully.', TGM_THEME_DOMAIN );
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', TGM_THEME_DOMAIN );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', TGM_THEME_DOMAIN ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', TGM_THEME_DOMAIN ) . '</span><span class="hidden">' . __( 'Hide Details', TGM_THEME_DOMAIN ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', TGM_THEME_DOMAIN );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', TGM_THEME_DOMAIN );
                }
                /** Default installation strings */
                else {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', TGM_THEME_DOMAIN );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', TGM_THEME_DOMAIN );
                    $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', TGM_THEME_DOMAIN );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', TGM_THEME_DOMAIN ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', TGM_THEME_DOMAIN ) . '</span><span class="hidden">' . __( 'Hide Details', TGM_THEME_DOMAIN ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', TGM_THEME_DOMAIN );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', TGM_THEME_DOMAIN );
                }
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', TGM_THEME_DOMAIN ) . '">' . __( 'Return to the Dashboard', TGM_THEME_DOMAIN ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }

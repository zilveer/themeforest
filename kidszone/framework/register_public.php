...
function dt_theme_footer_styles_scripts() {
	...
	wp_enqueue_script('jq.smooth', $template_uri.'/js/public/jquery-smoothscroll.js',array(),false,true);
	wp_enqueue_script('jq.easing', $template_uri.'/js/public/jquery-easing-1.3.js',array(),false,true);
	wp_enqueue_script('jq.inview', $template_uri.'/js/public/jquery.inview.js',array(),false,true);
	wp_enqueue_script('jq.validate', $template_uri.'/js/public/jquery.validate.min.js',array(),false,true);
	wp_enqueue_script('jq.caroufred', $template_uri.'/js/public/jquery.carouFredSel-6.2.0-packed.js', array(), false, true);
	...
	wp_enqueue_script('jq.mobilemenu', $template_uri.'/js/public/jquery.mobilemenu.js', array(), false, true);
	wp_enqueue_script('jq.contact', $template_uri.'/js/public/contact.js', array(), false, true);
	wp_enqueue_script('jq.donutchart', $template_uri.'/js/public/jquery.donutchart.js', array(), false, true);
	wp_enqueue_script('jq.totop', $template_uri.'/js/public/jquery.ui.totop.js',array(),false,true);
	...
	wp_enqueue_script('jq.custom', $template_uri.'/js/public/custom.js');
}
	...
	if (empty($disable_typo)) :
		for ($i = 1; $i <= 6; $i++) :
			...
			if (!empty($font) || (!empty($color) and $color != "#") || !empty($size)) :
				$output .= "H$i, H$i.hr-title, .dt-services H$i{";
				if (!empty($font)) {

...
function update_notifier_menu() {
	...
	$theme_data =  wp_get_theme('ThisWay'); // Get theme data from style.css (current version is what we want)
	if(version_compare($theme_data->Version, $xml->latest) == -1) {
		if(current_user_can('activate_plugins')){
		add_dashboard_page( $theme_data->Name . 'Theme Updates', 
							$theme_data->Name . '<span class="update-plugins count-1"><span class="update-count">1</span></span>', 
							'activate_plugins', 
							'thiswayupdates',
							'update_notifier');
		}
...
function update_notifier() {
	$xml = get_latest_theme_version(21600); // This tells the function to cache the remote call for 21600 seconds (6 hours)
	//$theme_data = get_theme_data(TEMPLATEPATH . '/style.css'); // Get theme data from style.css (current version is what we want)
	$theme_data =  wp_get_theme('ThisWay');
	?>
	<style>
...
function get_latest_theme_version($interval) {
	...
	$notifier_file_url = 'http://renklibeyaz.com/thisway.xml';

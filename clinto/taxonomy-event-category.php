...
<?php if ( have_posts() ) : ?>
	...
	<div class="navbar secondary-nav" >
		<div class="navbar-inner">
			<hr class="sexy_line">
			<h2 class="archive-title"><?php	printf( __( 'Event Tag Archives: %s', 'eventorganiser' ), '<span>' . single_cat_title( '', false ) . '</span>' );?></h2>

			<!---- If the tag has a description display it-->
			<?php
				$tag_description = category_description();
				if ( ! empty( $tag_description ) )
					echo apply_filters( 'category_archive_meta', '<p class="category-archive-meta center">' . $tag_description . '</p>' );
			?>
		</div>
	</div>

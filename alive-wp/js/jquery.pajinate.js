    ...
    $.fn.pajinate = function(options){
		...
		return this.each(function(){
			...
			while(number_of_pages > current_link){
				navigation_html += '<a class="page_link" href="" longdesc="' + current_link +'"></a>';
				current_link++;
			}
		...
		function goto(page_num){
			...
			if(firstPaginate == true) {
				...
				$items.animate({opacity:0}, { 
					...
					complete: function(){
						$items.hide();
						$items.slice(start_from, end_on).show();
						$items.animate({opacity:1}, {duration: pageFadeSpeed, queue:false});

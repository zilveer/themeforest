...
(function( window, $, undefined ) {
	...
	$.yit_portfolio_thumbs = function( options, element ) {
		this.element = $( element );
		this._init( options );
	};

	$.yit_portfolio_thumbs.defaults	= {
		elements : {
	...
	$.yit_portfolio_thumbs.prototype = {
		_init : function( options ) {
			this.options = $.extend( true, {}, $.yit_portfolio_thumbs.defaults, options );
		...
		_initLightboxSlider : function() {
            ...
            $('.work-thumbnail a[rel=lightbox_thumbs]').colorbox({
				transition:'elastic',
				rel:'lightbox_thumbs',
				fixed:true,
				maxWidth: '80%',
	...
	$.fn.yit_portfolio_thumbs = function( options ) {
		if ( typeof options === 'string' ) {
			var args = Array.prototype.slice.call( arguments, 1 );

			this.each(function() {
				var instance = $.data( this, 'yit_portfolio_thumbs' );
				if ( !instance ) {
				...
				if ( !$.isFunction( instance[options] ) || options.charAt(0) === "_" ) {
					console.error( "no such method '" + options + "' for yit_portfolio_thumbs instance" );
					return;
				}
			...
			this.each(function() {
				var instance = $.data( this, 'yit_portfolio_thumbs' );
				if ( !instance ) {
					$.data( this, 'yit_portfolio_thumbs', new $.yit_portfolio_thumbs( options, this ) );
				}
			});
		}
		return this;

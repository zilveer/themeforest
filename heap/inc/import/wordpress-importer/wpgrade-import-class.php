...
class Heap_import extends WPGrade_WP_Import {
	var $preStringOption;
	var $results;
	var $getOptions;
	var $saveOptions;
	...
	function import_posts_pages( $xml_file, $option_file, $stepNumber = 1, $numberOfSteps = 1 ) {
		...
		if ( $stepNumber == $numberOfSteps ) {
			...
			update_option( 'border_demo_data_imported', true );
	...
	function process_posts_stepped( $stepNumber = 1, $numberOfSteps = 1 ) {
		...
		foreach ( $currentPosts as $post ) {
			...
			$post_exists = get_page_by_title( $post['post_title'], null, $post['post_type'] );
	...
	function process_menus() {
		...
		foreach ( $this->posts as $post ) {
			...
			//			if ( ! post_type_exists( $post['post_type'] ) ) {
			//				printf( __( 'Failed to import &#8220;%s&#8221;: Invalid post type %s', 'lens' ),
	...
	function set_menus( $option_file ) {
		...
		if ( ! empty( $Heap_menus ) && ! empty( $menu_conf ) ) {
			foreach ( $Heap_menus as $Heap_menu ) {
				//check if we got a menu that corresponds to the Menu name array ($Heap_config->get('nav_menus')) we have set in menus.php
				if ( is_object( $Heap_menu ) && in_array( $Heap_menu->name, $menu_conf ) ) {
					$key = array_search( $Heap_menu->name, $menu_conf );

					if ( $key !== false ) {
						//if we have found a menu with the correct menu name apply the id to the menu location
						$locations[ $key ] = $Heap_menu->term_id;
					}
				}
			}
		}

...
<form class="variations_form cart clearfix" method="post" enctype='multipart/form-data' data-product_id="<?php echo $post->ID; ?>" data-product_variations="<?php echo esc_attr( json_encode( $available_variations ) ) ?>">
				...
				<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
					<tr>
						...
						<td class="value"><select id="<?php echo esc_attr( sanitize_title( $name ) ); ?>" name="attribute_<?php echo sanitize_title( $name ); ?>" data-attribute_name="attribute_<?php echo sanitize_title( $name ); ?>">
							<option value=""><?php echo __( 'Choose an option', LANGUAGE_ZONE ) ?>&hellip;</option>
							<?php
								if ( is_array( $options ) ) {
							...
							if ( sizeof( $attributes ) === $loop ) {
								echo '<a class="reset_variations" href="#reset">' . __( 'Clear selection', LANGUAGE_ZONE ) . '</a>';
							}
						?></td>
					</tr>
		        <?php endforeach;?>
	...
	<?php else : ?>
		...
		<p class="stock out-of-stock"><?php _e( 'This product is currently out of stock and unavailable.', LANGUAGE_ZONE ); ?></p>

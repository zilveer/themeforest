...
class WPAlchemy_MetaBox
{
	...
	function _head()
	{
		...
		<style type="text/css">
			...
			#wp-content-editor-container, #post-status-info, <?php if ($this->use_media_buttons) { ?> #content-html, #content-tmce <?php } else { ?> #wp-content-wrap <?php } ?>{ display: none; }
		...
		(
			...
			<script type="text/javascript">
				...
				(function($){ /* not using jQuery ondomready, code runs right away in footer */
					...
					var mb_id = '<?php echo (int) $this->id; ?>';
	...
	{
		echo esc_html($this->get_the_value( $n ));
	}
	...
	{
		echo esc_html($this->get_the_name( $n ));
	}
	...
	function the_index()
	{
		echo esc_attr($this->get_the_index());
	}
	...
	{
		echo esc_attr($this->get_the_checkbox_state( $n, $v, $is_default ));
	}
	...
	{
		echo esc_html($this->get_the_checkbox_state( $n, $v, $is_default ));
	}
	...
	{
		echo ecs_html($this->get_the_select_state( $n, $v, $is_default ));
	}

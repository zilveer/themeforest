 ...
 * @version 1.0
 */
class Learn_Theme_Options_Admin {
	/**
	 * Setup the options
	 *
	 * @since  1.0
	 ...
	 *
	 * @return Learn_Theme_Options_Admin
	 */
	...
	public function admin_menu() {
		$hook = add_theme_page( esc_html__( 'Theme Options', 'learn' ), esc_html__( 'Theme Options', 'learn' ), 'edit_theme_options', 'theme-options', array( $this, 'page' ) );
	...
	public function enqueue() {
		...
		function admin_theme_fonts_url() {
		    ...
		    $mon = _x( 'on', 'Roboto font: on or off', 'learn' );
		...
		*/
		function learn_admin_theme_scripts() {
		    wp_enqueue_style( 'admin-fonts', admin_theme_fonts_url(), array(), '1.0.0' );
		}
		add_action( 'wp_enqueue_scripts', 'learn_admin_theme_scripts' );

		wp_enqueue_script( 'ace', '//cdnjs.cloudflare.com/ajax/libs/ace/1.1.9/ace.js', array(), '1.1.9', true );
		?>
		<script>window.ace || document.write( '<script src="<?php echo LEARN_OPTIONS_URL ?>/framework/theme-options/js/ace.js" type="text/javascript"><\/script>' )</script>
		<?php
		wp_enqueue_style( 'alien-options', LEARN_OPTIONS_URL . '/framework/theme-options/css/admin.css' );
		wp_enqueue_script( 'alien-options', LEARN_OPTIONS_URL . '/framework/theme-options/js/admin.js', array( 'jquery', 'jquery-ui-datepicker', 'jquery-ui-sortable' ), '', true );
		wp_localize_script( 'alien-options', 'alienOptions', array(
			...
			'reset_notice'      => esc_html__( 'This action can\'t be undo. Are you sure want to reset?', 'learn' ),
			'import_notice'     => esc_html__( 'All previous data will be overwritten and can\'t be undo. Are you sure want to import?', 'learn' ),
			'media_title'       => esc_html__( 'Select Image', 'learn' ),
			'media_title_multi' => esc_html__( 'Select Images', 'learn' ),
		) );
	}
	...
	public function page() {
		$generator = new Learn_Theme_Options_Fields;
		$theme     = wp_get_theme();
		?>
		...
		<div class="wrap">
			...
			<div id="options-header" class="options-header">
				<a href="<?php echo esc_url( Learn_Theme_Options::$options['help']['support'] ); ?>" class="help" target="_blank"><i class="entypo-lifebuoy"></i><?php esc_html_e( 'Support', 'learn' ) ?>
				</a>
				<a href="<?php echo esc_url( Learn_Theme_Options::$options['help']['document'] ); ?>" class="help" target="_blank"><i class="entypo-book"></i><?php esc_html_e( 'Documentation', 'learn' ) ?>
				</a>

				<h1>
					<?php echo $theme->name ?> <?php esc_html_e( 'Theme Options', 'learn' ) ?>
					<span class="theme-version"><?php echo $theme->version ?></span>
					<span class="theme-author">
						<?php esc_html_e( ' by ', 'learn' ) ?>
			...
			<form method="POST" action="" id="theme-options" class="clearfix">
				...
				<div id="options-menu" class="options-menu">
					<ul class="menu">
						<?php
						foreach ( Learn_Theme_Options::$options['sections'] as $id => $menu ) {
				...
				<div id="options-content" class="options-content clearfix">
					<div class="toolbar">
						...
						<span class="quick-search">
							<input type="text" id="quick-search" placeholder="<?php esc_html_e( 'Quick Search', 'learn' ) ?>">
						</span>

						<button type="button" class="button reset-options"><?php esc_html_e( 'Reset Options', 'learn' ) ?></button>
						<button type="button" class="button button-primary save-options"><?php esc_html_e( 'Save Changes', 'learn' ) ?></button>
						<span class="spinner"></span>
					</div>

					<div id="options-sections" class="options-sections clearfix">
						<?php foreach ( Learn_Theme_Options::$options['fields'] as $section => $fields ): ?>
	...
	public function save() {
		...
		foreach ( Learn_Theme_Options::$defaults as $name => $default ) {
    ...
    public function custom_css() {
        ...
        do_action( 'learn_ajax_generate_custom_css' );
        die;
    }
}

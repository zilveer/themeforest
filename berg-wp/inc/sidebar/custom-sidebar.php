...
if (!class_exists('berg_custom_sidebar')) {

	class berg_custom_sidebar {
	
		var $sidebars  = array();
		var $stored    = "";

		function __construct() {
			$this->stored = 'berg_sidebars';
			$this->title = __('Custom Widget Area', 'BERG');

			add_action('load-widgets.php', array(&$this, 'load_assets') , 5);
			$this->register_custom_sidebars();
			add_action('wp_ajax_berg_ajax_delete_custom_sidebar', array(&$this, 'delete_sidebar_area'), 1000);
		}
		...
		function load_assets() {
			...
			wp_enqueue_script('berg_sidebar' , THEME_DIR_URI . '/admin/sidebar.js');
		}

		function template_add_widget_field() {
			$nonce =  wp_create_nonce('berg-delete-sidebar');
			$nonce = '<input type="hidden" name="berg-delete-sidebar" value="' . $nonce . '" />';

			echo "\n<script type='text/html' id='berg-add-widget'>\n";
			echo "	<form class='berg-add-widget' method='POST'>\n";
			echo "		<h3>" . $this->title . "</h3>\n";
			echo "		<span class='input_wrap'><input type='text' value='' placeholder = '" . __('Enter Name of the new Widget Area', 'BERG') . "' name='berg-add-widget' /></span>\n";
		...
		function add_sidebar_area() {
			if (!empty($_POST['berg-add-widget']) && ($_POST['berg-add-widget'] != '') && ($_POST['berg-add-widget'] != ' ')) {
				$this->sidebars = get_option($this->stored);
				$name = $this->get_name($_POST['berg-add-widget']);
		...
		function delete_sidebar_area() {
			check_ajax_referer('berg-delete-sidebar');
			if (!empty($_POST['name'])) {
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters('berg_custom_widget_args', $args);

			if (is_array($this->sidebars)) {
				foreach ($this->sidebars as $sidebar) {
					$args['name']  = $sidebar;
					$args['id']  = $sidebar;
					$args['class'] = 'berg-custom';

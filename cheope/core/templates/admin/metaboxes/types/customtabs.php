...
jQuery(document).ready(function($){
        ...
        if( $('#yit_custom_tabs').length )
        {
            var $yit_custom_tabs = $("#yit_custom_tabs").find(":input");
            $yit_custom_tabs.prop("disabled",false);
            $yit_custom_tabs.prop("readonly",false);
            $("#yit_custom_tabs img").remove();
        }
						...
						<table cellpadding="0" cellspacing="0" class="woocommerce_attribute_data">\
							<tbody>\
								<tr>\
									...
									<td rowspan="3">\
										<label><?php _e('Value', 'yit') ?>:</label>\
										<textarea name="<?php echo $name ?>[' + size + '][value]" cols="5" rows="5" placeholder="<?php _e('The content of the tab. (HTML is supported)','yit') ?>"></textarea>\
									</td>\
								</tr>\
							</tbody>\
						</table>\

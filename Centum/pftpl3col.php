...
<style type="text/css">
    ...
    @media only screen and (min-width: 1390px) {  #portfolio-wrapper iframe {min-height: 240px }     } 
	@media only screen and (min-width: 960px) and (max-width: 1389px) {  #portfolio-wrapper iframe {min-height: 180px }  #portfolio-wrapper img {min-height: 189px;}}
...
<div class="container">
		...
		<div <?php post_class('one-third column portfolio-item'); ?> id="post-<?php the_ID(); ?>" >
			...
			if($type == 'video' && $videothumbtype == 'video') {
				...
				if($videoembed) {
					...
					$post_embed = $wp_embed->run_shortcode('[embed  width="380" ]'.$videolink.'[/embed]') ;
	...
	<div class="pagination columns sixteen">
		<?php if ($wp_query->max_num_pages > 1) : ?>
		<nav id="nav-below" class="navigation">
			<div class="nav-previous"><?php next_posts_link(__('<i class="fa fa-chevron-left"></i>', 'centum')); ?></div>
			<div class="nav-next"><?php previous_posts_link(__('<i class="fa fa-chevron-right"></i>', 'centum')); ?></div>
		</nav><!-- #nav-below -->
		<?php endif;

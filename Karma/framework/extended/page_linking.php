...
function txfx_plt_add_meta_box( $page, $context ) {
	...
	if ( ( 'page' === $page ) && 'advanced' === $context )
		add_meta_box( 'page-links-to', 'Custom Page Linking', 'txfx_plt_meta_box', $page, 'side', 'low' );
}
...
function txfx_plt_meta_box() {
	...
	if ( !$url )
		$url = 'http://';
?>
	<p>Point this page to this URL: <input name="txfx_links_to" type="text" style="width:75%" id="txfx_links_to" value="<?php echo esc_attr( $url ); ?>" /></p>
...
function txfx_plt_save_meta_box( $post_ID ) {
  	...
  	if (!isset($_REQUEST['_txfx_pl2_nonce']))
	{
	//If nonce not posted, set it with null to prevent debug error.
	//mod by denzel
	$_REQUEST['_txfx_pl2_nonce'] = null;
	}  
...
function txfx_filter_links_to_pages ($link, $post) {
    ...
    global $post; // mod by denzel to prevent object error in WP_DEBUG mode.
	...
	if(!empty($post->ID)){ //mod by denzel to prevent object error.
	...
	if ( isset($page_links_to_cache[$id]) ) //mod by denzel wrapped with isset() check.
...
function txfx_page_links_to_highlight_tabs( $pages ) {
	...
	foreach ( (array) $page_links_to_cache as $id => $page ) {
		...
		if ( str_replace( 'http://www.', 'http://', $this_url ) == str_replace( 'http://www.', 'http://', $page ) || ( is_home() && str_replace( 'http://www.', 'http://', trailingslashit( home_url() ) ) == str_replace( 'http://www.', 'http://', trailingslashit( $page ) ) ) ) {
			$highlight = true;

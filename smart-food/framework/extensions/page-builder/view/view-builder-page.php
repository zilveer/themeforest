...
if ( ! current_user_can('edit_theme_options') )
	wp_die( __( 'Cheatin&#8217; uh?', 'smartfood' ) );
...
switch($action) {
	...
	case 'create' :
		...
		if(!is_wp_error($new_id)) {
			...
			$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'smartfood') . '<strong>' . $template_name . '</strong>' . __(' page template has been successfully created', 'smartfood') . '</p></div>';
		} else {
			$errors = '<ul>';
			...
			$messages[] = '<div id="message" class="error"><p>' . __('Sorry, the operation was unsuccessful for the following reason(s): ', 'smartfood') . '</p>' . $errors . '</div>';
		}
	...
	case 'clone' :
		...
		if(!is_wp_error($new_id)) {
			...
			$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'smartfood') . '<strong>' . $template_name . '</strong>' . __(' template has been successfully cloned', 'smartfood') . '</p></div>';
		} else {
			$errors = '<ul>';
			foreach( $new_id->get_error_messages() as $error ) {
				$errors .= '<li><strong>'. $error . '</strong></li>';
	...
	case 'update' :
		...
		$blocks = isset($_REQUEST['td_blocks']) ? $_REQUEST['td_blocks'] : '';
		...
		$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'smartfood') . '<strong>' . $template_name . '</strong>' . __(' page template has been updated', 'smartfood') . '</p></div>';
		break;
	...
	case 'delete' :
		...
		$messages[] = '<div id="message" class="updated"><p>' . __('The template has been successfully deleted', 'smartfood') . '</p></div>';
		break;
}
...
<div class="editor-wrap">
	<div class="editor-inner">
		...
		<div class="tdp-modal-title">
			<a href="#" class="tdp-editor-closer"><i class="fa fa-check fa-fw"></i></a>
			<h3 id="block-title-replace">Edit Text Content</h3>
			<div class="cf"></div>
		</div>
		
		<div class="tdp-editor-content">
			<?php wp_editor('','tdp-editor', array('textarea_rows' => 20, 'wpautop' => true, 'quicktags' => false)); ?>
		</div>
...
<div class="wrap">
	...
	<h2><span class="dashicons dashicons-tagcloud tdpb-icon"></span><?php echo $this->args['page_title'] ?></h2>
	...
	<div id="manage-templates" class="manage-templates">
		...
		<?php if($templates) { ?>
			...
			<form method="get" action="<?php echo admin_url('themes.php') ?>" >
				...
				<label for="template">
					<?php _e('Select a template to edit:', 'smartfood') ?> 

					<select name="template" id="">
						
						<?php if($selected_template_id === 0) { ?>
							<option value="0" selected="selected"><?php _e('-- Select --', 'smartfood') ?></option>
						<?php } ?>
				...
				<span class="add-new-template-action">
					...
					?>
					"><?php _e('Create a new template', 'smartfood') ?></a>
				</span>
		...
		<?php } else { ?>
			<span class="add-new-template-action">
				<?php _e('Create your new template below', 'smartfood') ?>
			</span>
		<?php } ?>
	...
	<div id="page-builder-frame">
		...
		<div id="page-builder-column" class="metabox-holder <?php echo $disabled ?>">
			<div id="page-builder-archive" class="postbox">
				<h3 class="hndle"><span><?php _e('All Available Blocks', 'smartfood') ?><small>Drag blocks into the right hand column to add content to your page.</small></span><span id="removing-block"><?php _e('Deleting', 'smartfood') ?></span><hr /><?php $this->blocks_filters(); ?></h3>
				<div class="inside">
		...
		<div id="page-builder-fixed">
				...
				<div class="tdpb-wrap aqpbdiv">
					...
					<form id="update-page-template" action="<?php echo $this->args['page_url'] ?>" method="post" enctype="multipart/form-data">
						...
						<div id="tdpb-header">
								...
								<div id="submitpost" class="submitbox">
									<div class="major-publishing-actions cf">
										...
										<label class="open-label howto" for="template-name">
											<span><?php _e('Template Name', 'smartfood') ?></span>
											<input name="template-name" id="template-name" type="text" class="template-name regular-text" title="Enter template name here" placeholder="<?php _e('Enter template name here', 'smartfood') ?>" value="<?php echo is_object($selected_template_object) ? $selected_template_object->post_title : ''; ?>">
										</label>
										...
										<div class="publishing-action">
											<?php submit_button( empty( $selected_template_id ) ? __( 'Create Template', 'smartfood') : __( 'Save Template', 'smartfood'), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_header' ) ); ?>
								...
								<input type="hidden" id="tdpb-nonce" name="tdpb-nonce" value="<?php echo wp_create_nonce('tdpb-settings-page-nonce') ?>"/>
						...
						<div id="tdpb-body">
							...
							<ul class="blocks cf" id="blocks-to-edit">
								...
								if($selected_template_id === 0) {
									...
									<br/><br/>
									When you have finished building your custom page template, make sure you click the Save Template button.', 'smartfood');
									echo '</p>';
							...
							<div class="major-publishing-actions cf">
								...
								<div class="delete-action">
									...
									echo '<a class="submitdelete deletion template-delete" href="' . esc_url(add_query_arg(
										...
										admin_url( 'themes.php' )
									)) . '">'. __('Delete Template', 'smartfood') .'</a>';
									?>
								...
								<div class="publishing-action">
									<?php if(!empty($selected_template_id)) {
										submit_button( __( 'Save Template', 'smartfood' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_footer' ) ); 
									} ?>
								</div><!-- END .publishing-action -->
							</div><!-- END .major-publishing-actions -->
			...
			<!-- 
			<p style="float:left"><small>Ebor Page Builder &copy; <?php echo date("Y") ?> by <a href="http://www.madeintdp.com">Tom Rhodes</a> & <a href="http://tdptgraphite.com">Syamil MJ</a></small></p>
			<p style="float:right"><small>Version <?php echo TDPB_VERSION ?></small></p>
			-->

<?php
	...
	*/	
	...
	if( !function_exists('gdlr_get_image_frame_item') ){
		function gdlr_get_image_frame_item($settings){
			...
			if( !empty($settings['link-type']) && $settings['link-type'] != 'none' ){
				...
				$ret .= '<span class="gdlr-image-link-icon"><i class="' . $icon . '"></i></span>';
				$ret .= '</a>';
			}
		...
		function gdlr_get_post_slider_item( $settings ){
			...
			if( $settings['post-type'] == 'cause' && $settings['cause-category'] ){
				$args['tax_query'] = array(array(
						'terms'=> explode(',', $settings['cause-category']), 
						'taxonomy'=> 'cause_category', 
						'field'=> 'slug' ));			
			}else if( $settings['post-type'] == 'post' ){
				$args['tax_query'] = array(array(
			...
			while($query->have_posts()){ $query->the_post();
				...
				if( !empty($image_id) ){
					...
					if( $settings['post-type'] == 'post' ){
						$slide_data[$image_id]['caption'] .= '<div class="blog-info blog-date"><i class="icon-time fa fa-clock-o"></i>';
	...
	if( !function_exists('gdlr_get_slider') ){
		function gdlr_get_slider( $slider_data, $thumbnail_size, $slider_type = 'flexslider', $featured = '' ){
			if( is_array($slider_data) ){
				$slide_order = $slider_data[0];
				$slide_data = $slider_data[1];
			}else{
			...
			if($slider_type == 'flexslider'){
				return gdlr_get_flex_slider($slides, array('size'=> $thumbnail_size, 'featured'=> $featured));
				...
				if( is_array($slide) ){
					...
					if( !empty($slide['title']) || !empty($slide['caption']) ){
						...
						if( !empty($settings['featured']) ){
							$caption .= '<div class="gdlr-caption-feature">';
							$caption .= '<span class="gdlr-left">' . $settings['featured'] . '</span>';
							$caption .= '<span class="gdlr-right">';
							$caption .= '<i class="icon-angle-left gdlr-flex-prev fa fa-angle-left"></i>';
							$caption .= '<i class="icon-angle-right gdlr-flex-next fa fa-angle-right"></i>';
							$caption .= '</span>';
							$caption .= '<div class="clear"></div>';
							$caption .= '</div>';
						}
						if( !empty($slide['title']) ){
							$caption .= '<div class="gdlr-caption-title">';
							$caption .= ($slide['slide-link'] == 'url')? '<a href="' . $slide['url'] . '" >': '';
							$caption .= $slide['title'];
							$caption .= ($slide['slide-link'] == 'url')? '</a>': '';
							$caption .= '</div>';
						}
						$caption .= empty($slide['caption'])? '': '<div class="gdlr-caption-text">' . $slide['caption'] . '</div>';
						$caption .= '</div>'; // gdlr-slider-caption

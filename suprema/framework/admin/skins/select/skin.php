...
class SelectSkin extends SupremaQodefSkinAbstract {
    /**
     * Skin constructor. Hooks to suprema_qodef_admin_scripts_init and suprema_qodef_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'select';

        //hook to admin register scripts
        add_action('suprema_qodef_admin_scripts_init', array($this, 'registerStyles'));
        add_action('suprema_qodef_admin_scripts_init', array($this, 'registerScripts'));

        //hook to admin enqueue scripts
        add_action('suprema_qodef_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('suprema_qodef_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('suprema_qodef_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('suprema_qodef_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            suprema_qodef_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            suprema_qodef_register_skin_style($styleHandle, $stylePath);
        }
	...
	public function renderOptions() {
        ...
        $tab    = suprema_qodef_get_admin_tab();
        $active_page = $suprema_qodef_Framework->qodeOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param SupremaQodefAdminPage $page current page to load
     *
     * @see QodeSkinAbstract::loadTemplatePart()
     */

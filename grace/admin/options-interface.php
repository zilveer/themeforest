	...
	foreach ( $options as $value ) {
		...
		case "color2":
			...
			while ($colorIndex < 3) {
				...
				if ( isset($stdVal) ) {
					if ( $colorStored[$el] !=  $stdVal )
						$default_color = ' data-default-color="' . $stdVal . '" ';
				}
				$output .= '<input name="' . esc_attr( $option_name . '[' . $value['id'] . '][' . $el . ']' ) . '" id="' . esc_attr( $value['id'] ) . $colorIndex . '" class="of-color"  type="text" value="' . esc_attr( $colorStored[$el] ) . '"' . $default_color .' />';
		...
		case 'gtypography':
			...
			$typography_options = array(
				'sizes' => of_recognized_font_sizes(),
				'faces' => of_recognized_gfont_faces(),
				'styles' => of_recognized_font_styles(),
				'color' => true
			);
			...
			case 'gfont':	
				...
				$faces = of_recognized_gfont_faces();
				foreach ( $faces as $key => $face ) {
					$output .= '<option value="' . esc_attr( $key ) . '" ' . selected( $font_stored['face'], $key, false ) . '>' . esc_html( $face ) . '</option>';
				}			

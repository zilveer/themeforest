...
class PeThemeFormElementLayout extends PeThemeFormElement {
	...
	protected function template() {
		...
		$preview = empty($this->data["preview"]) ? '' : '<div class="pe-preview"><div class="pe-controls"><input class="button addblock preview" type="button" value="Page Preview" /></div></div>';

		$html = <<<EOT
<div class="option" id="[ID]" data-prefix="[NAME]" data-allowed="[ALLOWED]">
	<div class="section">
...
<script>
	(window.jqpe35 || jQuery)("#[ID]").peFieldLayout({data:[JSON]});
</script>
EOT;
		return $html;
	}

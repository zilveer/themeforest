	...
	<div id="payment">
		<?php if ( $available_gateways = WC()->payment_gateways->get_available_payment_gateways() ) : ?>
			...
			<div class="form-row">
				...
				<input type="submit" class="button alt" id="place_order" value="<?php esc_attr_e( 'Add Payment Method', 'crazyblog' ); ?>" />
				<input type="hidden" name="woocommerce_add_payment_method" value="1" />
			</div>
		<?php else : ?>
			<p><?php esc_html_e( 'Sorry, it seems that there are no payment methods which support adding a new payment method. Please contact us if you require assistance or wish to make alternate arrangements.', 'crazyblog' ); ?></p>
		<?php endif; ?>
	</div>
</form>

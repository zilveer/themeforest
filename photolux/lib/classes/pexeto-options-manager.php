...
class PexetoOptionsManager{
	 ...
	 */
	function __construct($themename, $images_url, $version){
	...
	public function print_heading(){
		...
		echo '<div id="sidebar"><div class="logo-container"><div id="logo"></div></div><div id="navigation"><ul>';

		$i=1;
		foreach ($this->options as $value) {

			if($value['type']=='title'){
				echo '<li><span><a href="#navigation-'.$i.'"><span aria-hidden="true" class="font-icon '.$value['img'].'"></span>'.$value['name'].'</a></span></li>';
				$i++;
			}
		}
	...
	public function print_footer(){
		...
		<li><a href="http://themeforest.net/user/pexeto/follow" title="Follow Our Work on ThemeForest" class="icon-tf"><img src="'.$this->pexeto_images_url.'tf-icon.png" /></a></li>
		<li><a href="http://pexetothemes.com" class="icon-pexeto"><img src="'.$this->pexeto_images_url.'pex-icon.png" title="Visit Our Website" /></a></li>
	...
	public function print_multicheck($value){
		...
		foreach ($value['options'] as $option) {
			...
			echo '<div class="check-holder"><a href="" class="check font-icon'.$class.'" title="'.$option['id'].'"></a><span class="check-value">'.$option['name'].'</span></div>'; 
		} 
		echo '</div>';
	...
	public function print_upload($value){
		...
		$std = isset($value['std'])?$value['std']:"";
		$input_value = $this->get_field_value($value['id'], $std);
		echo '<input class="option-input upload pexeto-upload" name="'.$value['id'].'" id="'.$value['id'].'" type="text" value="'.$input_value.'" />';
		echo '<div id="'.$value['id'].'_button" class="upload-button upload-logo pexeto-upload-btn" ><a class="pex-button alignright"><span>Select Image</span></a></div><br/>';
	...
	public function print_checkbox($value){
		echo $this->before_option_title.$value['name'].$this->after_option_title;
		$std = isset($value['std'])?$value['std']:"";
		$input_value = esc_attr($this->get_field_value($value['id'], $std));
		if($input_value=='true'){
			$input_value='on';
		}
		...
		$add_class = $input_value=='off' ? ' off':'';
		echo '<div class="on-off'.$add_class.'"><em class="on-text">ON</em><span></span><em class="off-text">OFF</em></div>';
	...
	public function print_custom($value){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					echo '<script type="text/javascript">jQuery(document).ready(function($){
								pexetoOptions.loadUploader(jQuery("div#'.$field['id'].'_button"));
						});</script>';
					$preview=$field['id'];
					$is_textarea[]="false";
				break;
				...
				case 'imageselect':
					//print a textarea
					echo '<div class="styles-holder images-select-holder">';
					echo '<input  name="'.$field['id'].'" id="'.$field['id'].'" type="hidden" /><ul>';
				
					$counter=0;
					foreach ($field['options'] as $option) {
						//$style='background-image:url('.$option.');';
						echo '<li><a class="style-box" title="'.$option.'" href=""><img src="'.$option.'" /></a></li>';
					} 
					echo '</ul></div>';
					$is_textarea[]="false";
					break;
		...
		echo '<script type="text/javascript">
			jQuery(document).ready(function($){
				pexetoOptions.setCustomFieldsFunc("'.$value['id'].'", ["'.$idsString.'"], ["'.$namesString.'"], ['.$textareaString.'] , "'.$preview.'",  "'.PEXETO_TIMTHUMB_URL.'");
			});
		</script>';

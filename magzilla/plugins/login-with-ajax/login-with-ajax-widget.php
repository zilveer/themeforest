...
class LoginWithAjaxWidget extends WP_Widget {
    ...
    function LoginWithAjaxWidget() {
    	$this->defaults = array(
    		'title' => __('Log In','magzilla'),
    		'title_loggedin' => __( 'Hi', 'magzilla' ).' %username%',
    		'template' => 'default',
    		'profile_link' => 1,
    		'registration' => 1,
    		'remember' => 1
    	);
    	$widget_ops = array('description' => __( "Login widget with AJAX capabilities.", 'magzilla') );
    ...
    function form($instance) {
        ...
        ?>
			<p>
				<label for="<?php echo $this->get_field_id('title'); ?>"><?php echo sprintf(__('Title (%s)', 'magzilla'),__('Logged In','login-with-ajax')); ?>: </label>
				<input type="text" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo esc_attr($instance['title']); ?>" />
			</p>
			<p>
				<label for="<?php echo $this->get_field_id('title_loggedin'); ?>"><?php echo sprintf(__('Title (%s)', 'magzilla'),__('Logged Out','magzilla')); ?>: </label>
            ...
            <p>
            	<label for="<?php echo $this->get_field_id('profile_link'); ?>"><?php _e('Show profile link?', 'magzilla'); ?> </label>
            ...
            <p>
            	<label for="<?php echo $this->get_field_id('remember'); ?>"><?php _e('Recover Password?', 'magzilla'); ?> </label>
            ...
            <p>
            	<label for="<?php echo $this->get_field_id('registration'); ?>"><?php _e('AJAX Registration?', 'magzilla'); ?> </label>
			...
			<p>
            	<label for="<?php echo $this->get_field_id('template'); ?>"><?php _e('Template', 'magzilla'); ?> </label>

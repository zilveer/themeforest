...
/* Royal - Full width Posts */

 
class Royal_Full_Width_Posts extends WP_Widget {

    function Royal_Full_Width_Posts() {
		global $themename;
		$widget_ops = array('classname' => 'custom-small_sidebar', 'description' => __( "Full width - Recent category post", 'my-text-domain') );
		$control_ops = array('width' => 250, 'height' => 200);
		$this->WP_Widget('smallsidebar', __('6) Royal Full width - Recent category post', 'my-text-domain'), $widget_ops, $control_ops);
    }

    function widget($args, $instance) {
		global $wpdb, $shortname;
        extract( $args );
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Full width - Recent category post', 'my-text-domain') : $instance['title'], $instance, $this->id_base);
		...
		<ul class="widget_recent_posts_two">
			<?php foreach($posts as $post){
					...
					$time_ago = get_option('op_time_ago');
				   $post_time = human_time_diff( get_the_time('U'), current_time('timestamp') ) . ' ' . $time_ago .''; 
			?>
			<li class="widget_li">
            
			    <div class="widget_two_meta">
				<div class="widget_two_time"><?php echo $post_time ?> </div>
			    <div class="widget_two_views"><?php echo getPostViews($post->ID) ?> <?php echo get_option('op_views_text') ?></div>
			    </div>
				...
				<?php $image = aq_resize( $thumbnailSrc, 360, 200, true ); ?>
...
add_action('widgets_init', create_function('', 'return register_widget("Royal_Full_Width_Posts");'));

...
define( 'MAX_COUNT_SLIDER_IMAGE', 10 );
...
function startuply_load_multiple_post_thumbnail() {
	for ($image_idx = 1; $image_idx < MAX_COUNT_SLIDER_IMAGE + 1; $image_idx++) {
		new MultiPostThumbnails(
			array(
				'label' => "Product Image #{$image_idx}",
				'id' => 'product-image-'.str_pad($image_idx, 2, "0", STR_PAD_LEFT),
				'post_type' => 'download'
			)
		);
	}
}
add_action( 'wp_loaded', 'startuply_load_multiple_post_thumbnail' );

function startuply_get_post_thumbnails() {
	/*
	default sizes
	...
	*/
	...
	if (class_exists('MultiPostThumbnails')) {
		$thumb_attr = array(
			'class'	=> "col-xs-3 img-thumbnail thumb-img",
		);
		$full_attr = array(
		...
		for ($image_idx = 1; $image_idx < MAX_COUNT_SLIDER_IMAGE + 1; $image_idx++) {
			...
			$img_id = 'product-image-'.str_pad($image_idx, 2, "0", STR_PAD_LEFT);
			...
			$full = MultiPostThumbnails::get_the_post_thumbnail(
				get_post_type(),
				$img_id,
				null,
				$size = 'full',
			...
			if($thumb) {
				$thumbnails .= "<li class='product-pic' data-img-id='" . $img_id . "'>".$thumb."</li>";
			...
			if($full) {
				$fullimages .= "<li class='product-thumb' data-img-id='" . $img_id . "'>".$full."</li>";
			}

			$content = "<ul class='edd-product-main-pics'>".$fullimages."</ul>"."<ul class='edd-product-thumbs'>".$thumbnails."</ul>";

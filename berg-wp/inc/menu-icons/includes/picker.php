	...
	public static function _fields( $id, $item, $depth, $args ) {
		...
		?>
				<p class="field-custom description description-wide">
					<?php
						$label_check = isset($meta['label_hide']) ? $meta['label_hide'] : 0;
						
					?>
					<label for="edit-menu-item-nav-label-show-<?php echo $item_id; ?>">
						<?php _e( 'Don&rsquo;t show navigation label', 'BERG' ); ?>
						<input type="checkbox" name="menu-icons[<?php echo $item_id; ?>][label_hide]" <?php if ( $label_check == '1' ) { echo 'checked="checked"'; } ?> value="1" id="edit-menu-item-nav-label-show-<?php echo $item_id; ?>" />
					</label>
				</p>	
			<div class="field-icon description-wide menu-icons-wrap" data-id="<?php echo json_encode( $item->ID ); ?>">
				...
				<div class="_settings hidden">
					...
					foreach ( $fields as $field ) {
						...
						printf(
							'<label>%1$s: <input type="text" name="%2$s" class="_mi-%3$s" value="%4$s" disabled /></label><br />',

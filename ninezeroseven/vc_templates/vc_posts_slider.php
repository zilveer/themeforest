...
//Added to only get posts with featured images
...
while ( $my_query->have_posts() ) {
	...
	if ( $link != 'link_no' ) {
		if ( $link == 'link_post' ) {
			$link_image_start = '<a class="link_image" href="' . get_permalink( $post_id ) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'ninezeroseven' ), the_title_attribute( 'echo=0' ) ) . '">';
...
if ( $teasers ) {
	...
	$teasers = __( 'Nothing found.', 'ninezeroseven' );
}

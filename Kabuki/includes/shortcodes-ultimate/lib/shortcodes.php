<?php
	...
	function su_box_shortcode( $atts, $content = null ) {
		...
		return '<div class="su-box" style="border:1px solid ' . $styles['dark_color'] . '"><div class="su-box-title" style="background-color:' . $color . '; ' . $styles['light_color'] . '; ' . $styles['text_shadow'] . '">' . $title . '</div><div class="su-box-content">' . su_do_shortcode( $content, 'b' ) . '</div></div>';
	}
	...
	function su_note_shortcode( $atts, $content = null ) {
		...
		return '<div class="su-note" style="background-color:' . $styles['light_color'] . ';border:1px solid ' . $styles['dark_color'] . '"><div class="su-note-shell" style="color:' . $styles['text_color'] . '">' . su_do_shortcode( $content, 'n' ) . '</div></div>';
	}
	...
	// Shortcode: custom post list
	
	function su_display_posts_shortcode( $atts ) {
	...
	apply_filters( 'display_posts_shortcode_args', $args, $atts );
	
	$postlist = wp_get_recent_posts($args);
	$postcount = 0;
	$expmore = __('Expand &raquo;');
	$expclose = __('Close');
	$su_posts = '';
	global $more;
	foreach ($postlist as $postlistitem) {
	
	$more = 0; 
	$arc_year = get_the_time('Y', $postlistitem["ID"]); $arc_month = get_the_time('m', $postlistitem["ID"]); $arc_day = get_the_time('d', $postlistitem["ID"]);
	$postdate = get_the_time('F j, Y', $postlistitem["ID"]);;
	$cats = get_the_category($postlistitem["ID"]);
	if ( has_post_thumbnail($postlistitem["ID"]) ) { /* loads the post's featured thumbnail, requires Wordpress 3.0+ */ $postthumb = '<div class="featured-thumbnail">'.get_the_post_thumbnail($postlistitem["ID"]).'</div>'; } 
	$su_posts .= '<div class="post-single">
			<div class="post-wrapper">
				<div class="post-header">
				<h2><a href="'.get_permalink($postlistitem["ID"]).'" title="'.$postlistitem['post_title'].'" rel="bookmark">'.$postlistitem['post_title'].'</a></h2>
				</div><div class="clearleft"></div><!--.postHeader-->'.$postthumb.'
				<div class="post-content">';
				
				$content = preg_replace('/<!--more.*/s', '', $postlistitem['post_content']);
				$su_posts .= $content; 
 
				
				$su_posts .= '<div class="post-meta">
					<span class="meta-img"><img src="'.get_template_directory_uri().'/images/misc_icons/grey_light/clock_12x12.png" alt="date" /></span>
					<span class="meta-date">'.$postdate.'</span>
					<span class="meta-img"><img src="'.get_template_directory_uri().'/images/misc_icons/grey_light/user_9x12.png" alt="author" /></span>
					<span class="meta-author">'.$postlistitem['post_author'].'</span>
					<span class="meta-img"><img src="'.get_template_directory_uri().'/images/misc_icons/grey_light/list_12x11.png" alt="category" /></span>
					<span class="meta-cat">'.$cats[0]->cat_name.'</span>
				</div><!--.postMeta-->
	...
	$su_post_list = '<div id="shortcode-postlist">'.$su_posts.'</div>';

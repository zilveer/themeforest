...
class NMI_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$min_height = get_post_meta($item_id, '_menu-item-min_height', true );
		$disable_titles = get_post_meta($item_id, '_menu-item-disable_titles', true );

		$output .= '

			<p class="description description-thin">
				<label for="menu-item-design['.$item_id.']">
				Design<br>
				<select name="menu-item-design['.$item_id.']" class="widefat">
					<option value="">design</option>
					<option value="dropdown" '.selected('dropdown', $design, false).'>Dropdown</option>
					<option value="full-width" '.selected('full-width', $design, false).'>Full width</option>
					<option value="image-column" '.selected('image-column', $design, false).'>Image column</option>
					<option value="image-no-space" '.selected('image-no-space', $design, false).'>Image column no space</option>
				</select>
				</label>
			</p>
		
			<p class="description description-thin">
				<label for="menu-item-columns['.$item_id.']">
				Columns<br>
				<input type="number" id="menu-item-columns['.$item_id.']" class="widefat" name="menu-item-columns['.$item_id.']" value="'.$columns.'">
				</label>
			</p>
			...
			<p class="description description-thin">
				...
				Icon name (from fonts Awesome)<br>
				<input type="text" id="menu-item-icon['.$item_id.']" class="widefat" name="menu-item-icon['.$item_id.']" value="'.$icon.'">
				</label>
			</p>
		
			<p class="description description-thin">
				<label for="menu-item-label['.$item_id.']">
				Label<br>
				<select name="menu-item-label['.$item_id.']" class="option-tree-ui-select widefat">
					<option value="">none</option>
					<option value="hot" '.selected('hot', $label, false).'>Hot</option>
					<option value="sale" '.selected('sale', $label, false).'>Sale</option>
					<option value="new" '.selected('new', $label, false).'>New</option>
				</select>
				</label>
			</p>
			...
			<p class="description">
				<input type="checkbox" name="menu-item-disable_titles['.$item_id.']" id="menu-item-disable_titles['.$item_id.']" value="1" '.checked(1, $disable_titles, false).' />
				<label for="menu-item-disable_titles['.$item_id.']">
					Disable titles
				</label>
			</p>
		...
		// Append menu item upload link
		$output .= '</div><div class="nmi-upload-link nmi-div" style="display: none;">';

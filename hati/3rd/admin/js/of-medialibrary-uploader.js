...
/*-----------------------------------------------------------------------------------*/
	...
	var formfield,
        ...
        formfield = $(this).siblings('input,textarea').attr('id');
        formID = $(this).attr('rel');
		tpl = $(this).attr('data-tpl');
		
		//Change "insert into post" to "Use this Button"
		tbframe_interval = setInterval(function(){
			var c = jQuery('#TB_iframeContent').contents();
			var val = 'Use This Image'; // 'Use Image as ' + woo_title;
			c.find('.savesend .button').val(val).addClass('button-primary');
			c.find('#media-items').nextAll().hide(); // remove from Gallery all but images
		}, 1200);
	...
	window.send_to_editor = function(html) {
		...
		if (tpl) {
			var prev = $('#' + formfield).val();
			itemurl = tpl.replace('{{src}}', itemurl);

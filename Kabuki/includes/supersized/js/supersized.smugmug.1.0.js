...
(function($){
	...
	$.supersized = function( options ) {
		...
		var settings = {
			...
			//smugmug
			source					:	3,		//1-Keyword, 2-User(+keyword), 3-Gallery, 4-Category
			keyword                 :   '', 	//Comma-separated Smugmug keywords (combined) !!no space!!
			user					:	'',	    //Smugmug user nickname
			gallery					:	'', 	//Smugmug gallery ID_gallery key
			category				: 	'images',    //Smugmug category
			total_slides			:	100,	//How many pictures to pull (Between 1-100) This is currently the maximum allowed by the Google Feed API
			image_size              :   3,	//smugmug image Size 0 - Tiny, 1 - Thumb, 2 - Small, 3 - Medium, 4 - Large, 5 - XLarge, 6 - X2Large, 7 - X3Large, 8 - Original
			slides 					: 	[{}],	//Initiate slides array
			sort_by					:    0,		//0-None (original order), 1-Date Posted
			sort_direction			:    1		//0-descending, 1-ascending
	...
	if(options.sort_by) {
		var sort_order = 'publishedDate';
	}

    	//Determine where to pull images from
    	switch(options.source){
		    		
	    	case 1:		//From a Keyword
	    		var smugmugURL =  'http://api.smugmug.com/hack/feed.mg?Type=keyword&Data=' + options.keyword + '&format=rss200';
	    		break;
	    	case 2:		//From a User (with keyword)
	    		var smugmugURL =  'http://api.smugmug.com/hack/feed.mg?Type=userkeyword&NickName=' + options.user + '&Data=' + options.keyword + '&format=rss200';
	    		break;
	    	case 3:		//From a Gallery
	    		var smugmugURL =  'http://api.smugmug.com/hack/feed.mg?Type=gallery&Data=' + options.gallery + '&format=rss200';
	    		break;
			case 4:		//From a Category
					var smugmugURL =  'http://api.smugmug.com/hack/feed.mg?Type=keyword&Data=' + options.category + '&format=rss200';
					break;
	    }

		var smugmugLoaded = false;

		parseRSS(smugmugURL, prepareOutput);

function prepareOutput(result) {
		if (!result.error){ 	
    			//Build slides array from smugmug request
		if(options.sort_by) {
			switch(options.sort_direction){ //sorts the JSON object
				case 0: //sort descending
					sortJsonArrayByProp(result.entries, sort_order);
...
//	console.log("Object.keys(entry) = " + Object.keys(entry));
//	console.log("Date published = " + entry.publishedDate);
//	console.log("Length of entry.title = " + entry.title.length);
    			    //create image urls
					var indexLargestSize = (Object.keys(entry.mediaGroups[0].contents).length)-1;
					var indexRequestedSize = options.image_size;
					if (indexLargestSize < indexRequestedSize)
						var photoURL = entry.mediaGroups[0].contents[indexLargestSize].url; // if requested size is too large, use the largest one available
					else var photoURL = entry.mediaGroups[0].contents[indexRequestedSize].url;
    			    var thumbURL = entry.mediaGroups[0].contents[1].url;
    			   	var	photoLink = entry.link;
					var photoTitle = entry.title;
    			    if (i == 0){
...
function parseRSS(url, callback) { //uses the google feed API to parse the RSS feed from smugmug
  $.ajax({
    url: 'https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&num=' + options.total_slides + '&callback=?&q=' + encodeURIComponent(url),
    dataType: 'json',
...
//						return ( (a < b) ? -1 : ((a > b) ? 1 : 0) );
						return (Date.parse(a).compareTo(Date.parse(b))); //sort based on date
					});
				}
			}
...
//				console.log("slidecaptionWidth = " + slidecaptionWidth);

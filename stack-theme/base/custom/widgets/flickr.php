...
class Theme_Widget_Flickr extends WP_Widget {
	...
	function Theme_Widget_Flickr() {
		...
		$this->WP_Widget('flickr', THEME_NAME . ' - ' . __('Flickr', 'theme_admin'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="flickr-wrap">
			<script type="text/javascript" src="http://www.flickr.com/badge_code_v2.gne?count=<?php echo $count; ?>&amp;display=<?php echo $display; ?>&amp;size=s&amp;layout=x&amp;source=<?php echo $type;?>&amp;<?php echo ($type == 'user_set')?'set':$type; ?>=<?php echo $flickr_id; ?>"></script>
		</div>
		<div class="clear"></div>
		<?php
			echo $after_widget;
...
?>
		...
		<p>
			...
			<select name="<?php echo $this->get_field_name('type'); ?>" id="<?php echo $this->get_field_id('type'); ?>" class="widefat">
				...
				<option value="user_set" <?php selected($type,'user_set');?>>User's Set</option>
		...
		<p><label for="<?php echo $this->get_field_id('display'); ?>"><?php _e('Order', 'theme_admin'); ?>:</label>

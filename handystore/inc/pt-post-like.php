 ...
 */
function pt_like_scripts() {
	wp_enqueue_script( 'pt_like_post', get_template_directory_uri() . '/js/post-like.js', array('jquery'), '1.0', true );
	wp_localize_script( 'pt_like_post', 'ajax_var', array(
		'url' => admin_url( 'admin-ajax.php' ),
		'nonce' => wp_create_nonce( 'ajax-nonce' )
		)
	);
}
add_action( 'init', 'pt_like_scripts' );

/**
 *  Save like data
 */
add_action( 'wp_ajax_nopriv_pt-post-like', 'pt_post_like' );
add_action( 'wp_ajax_pt-post-like', 'pt_post_like' );
function pt_post_like() {
	$nonce = $_POST['nonce'];
    if ( ! wp_verify_nonce( $nonce, 'ajax-nonce' ) )
        die ( 'Nope!' );
	
	if ( isset( $_POST['pt_post_like'] ) ) {
 ...
 *  Front end button
 */
function pt_output_like_button( $post_id ) {
	...
	<div class="like-wrapper">
		<span><?php _e('Likes: ', 'plumtree'); ?></span>
		<a href="#" class="pt-post-like<?php echo esc_attr($class);?>" data-post_id="<?php echo esc_attr($post_id); ?>" title="<?php echo esc_attr($title); ?>">
 ...
 */
function pt_output_likes_counter($post_id) {

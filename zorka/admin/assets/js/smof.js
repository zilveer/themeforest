...
jQuery(document).ready(function($){
	...
	$('#expand_options').click(function(){
		...
		var with_wrapper = $('#main').width();

		if(flip == 0){
			flip = 1;
			$('#of_container #of-nav').hide();
			$('#of_container #content').width(with_wrapper - 28);
			...
			$('#of_container #content').width(with_wrapper - 188);
	...
	$('#of-popup-generate_less').center();
	$('#of-popup-install-data').center();
	...
	styleSelect = {
		...
		$('.of-typography-face').bind($.browser.msie ? 'click' : 'change', function(event) {
			var face_type = $(this).find('option:selected').attr('data-face-type');
			$(this).next('.of-typography-face-type').val(face_type);
	...
	/** AJAX Generate Less To Css */
	$('#of_generate_less').live('click',function() {
		...
		$.post(ajaxurl, data, function(response) {
			var success = $('#of-popup-generate_less');
			var fail = $('#of-popup-fail');
			var loading = $('.ajax-loading-img');
			loading.fadeOut();
	...
	/**	Tipsy @since v1.3 */
	if (jQuery().tipsy) {
		/*$('.typography-size, .typography-height, .typography-face, .typography-style, .of-typography-color,.typography-text-transform').tipsy({
			fade: true,
			gravity: 's',
			opacity: 0.7
		});*/

...
( function( $, window ) {
	...
	function import_demodata() {
		...
		var importButton = jQuery( '#Heap_import_demodata_button' ),
			container = jQuery( '#customize-control-heap_options-import_demodata_button_control' );

		var saveData = {
			container: container,
			ajaxUrl: $( 'input[name=Heap_import_ajax_url]', container ).val(),
			optionSlug: $( 'input[name=Heap_options_page_slug]', container ).val(),
			nonceImportPostsPages: $( 'input[name=Heap-nonce-import-posts-pages]', container ).val(),
			nonceImportThemeOptions: $( 'input[name=Heap-nonce-import-theme-options]', container ).val(),
			nonceImportWidgets: $( 'input[name=Heap-nonce-import-widgets]', container ).val(),
			ref: $( 'input[name=_wp_http_referer]', container ).val()
		};

		//this is the ajax queue
		var this_data = {},
			resultcontainer = $( '.Heap-import-results', this_data.container );
		qInst = $.qjax( {
			timeout: 3000,
			ajaxSettings: {
				type: "POST",
			...
			onQueueChange: function( length ) {
				...
				if ( length == 0 ) {
					if ( res.errors == false ) {
						...
						setTimeout( function() {
							resultcontainer.append( '<i>' + heap_admin_js_texts.import_all_done + '</i><br />' );
						}, 1000 );

						setTimeout( function() {
							resultcontainer.append( '<h3>' + heap_admin_js_texts.import_phew + '</h3><br /><p>' + heap_admin_js_texts.import_success_note + heap_admin_js_texts.import_success_reload + heap_admin_js_texts.import_success_warning + '</p>' );
						}, 1000 );
						...
						setTimeout( function() {
							resultcontainer.append( '<i>' + heap_admin_js_texts.import_failed + '</i><br />' );
						}, 1000 );
					}
		...
		importButton.bind( 'click', {set: saveData}, function( receivedData ) {
			...
			if ( button.is( '.Heap_button_inactive' ) ) return false;

			var activate = confirm( heap_admin_js_texts.import_confirm );

			if ( activate == false ) return false;

			//show loader
			$( '.Heap-loading-wrap', this_data.container ).css( {
				opacity: 0,
				display: "block",
				visibility: 'visible'
			...
			resultcontainer.append( '<br /><i>' + heap_admin_js_texts.import_working + '</i><br />' );
	...
	function ajax_import_posts_pages_stepped(resultcontainer, this_data) {
		...
		while ( stepNumber < numberOfSteps ) {
			...
			qInst.Queue( {
				...
				data: {
					action: 'Heap_ajax_import_posts_pages',
				...
				.fail( function( response ) {
					...
					resultcontainer.append( '<i style="color:red">' + heap_admin_js_texts.import_posts_failed + '</i><br />' );
				} )
					...
					if ( res != null && res.errors != null ) {
						if ( res.errors == false ) {
							if ( res.responses[0] != null ) {
								resultcontainer.append( '<i>' + heap_admin_js_texts.import_posts_step + ' ' +  + res.responses[0].supplemental.stepNumber + ' of ' + res.responses[0].supplemental.numberOfSteps + '</i><br />' );
								//for debuging purposes
								resultcontainer.append( '<div style="display:none;visibility:hidden;">Return data:<br />' + res.responses[0].data + '</div>' );
							} else {
								resultcontainer.append( '<i style="color:red">' + heap_admin_js_texts.import_posts_failed + '</i><br />' + heap_admin_js_texts.import_error + ' ' + res.responses[0].data );
							}
						}
						else {
							if ( res.responses[0] != null ) {
								resultcontainer.append( '<i style="color:red">' + heap_admin_js_texts.import_posts_failed + '</i><br />( ' + res.responses[0].errors[0].message + ' )<br/>' );
							} else {
								resultcontainer.append( '<i style="color:red">' + heap_admin_js_texts.import_posts_failed + '</i><br />' + heap_admin_js_texts.import_error + ' ' + res.responses[0].data );
							}
						}
					} else {
						resultcontainer.append( '<i style="color:red">' + heap_admin_js_texts.import_posts_failed + ' ' + heap_admin_js_texts.import_try_reload + ' </i><br />' );
					}
				} );
		}
	}
	...
	function ajax_import_theme_options(resultcontainer, this_data) {
		...
		qInst.Queue( {
			...
			url: this_data.ajaxUrl,
			data: {
				action: 'Heap_ajax_import_theme_options',
			...
			.fail( function( response ) {
				responseRaw = response;
				res = wpAjax.parseAjaxResponse( response, 'notifier' );
				resultcontainer.append( '<i style="color:red">' + heap_admin_js_texts.import_theme_options_failed + '</i><br />' );
			} )
			.done( function( response ) {
				responseRaw = response;
				res = wpAjax.parseAjaxResponse( response, 'notifier' );
				if ( res != null && res.errors != null ) {
					if ( res.errors == false ) {
						resultcontainer.append( '<i>' + heap_admin_js_texts.import_theme_options_done + '</i><br />' );
						//for debuging purposes
						resultcontainer.append( '<div style="display:none;visibility:hidden;">Return data:<br />' + res.responses[0].data + '</div>' );
					}
					else {
						resultcontainer.append( '<i style="color:red">' + heap_admin_js_texts.import_theme_options_error + ': ' + res.responses[0].errors[0].message + ' )<br/><br/>' );
					}
				} else {
					resultcontainer.append( '<i style="color:red">' + heap_admin_js_texts.import_theme_options_failed + '</i><br />' );
				}
	...
	function ajax_import_widgets(resultcontainer, this_data) {
		...
		qInst.Queue( {
			...
			url: this_data.ajaxUrl,
			data: {
				action: 'Heap_ajax_import_widgets',
			...
			.fail( function() {
				responseRaw = response;
				res = wpAjax.parseAjaxResponse( response, 'notifier' );
				resultcontainer.append( '<i style="color:red">' + heap_admin_js_texts.import_widgets_failed + '</i><br />' );
			} )
			.done( function( response ) {
				responseRaw = response;
				res = wpAjax.parseAjaxResponse( response, 'notifier' );
				if ( res != null && res.errors != null ) {
					if ( res.errors == false ) {
						resultcontainer.append( '<i>' + heap_admin_js_texts.import_widgets_done + '</i><br />' );

						//for debuging purposes
						resultcontainer.append( '<div style="display:none;visibility:hidden;">Return data:<br />' + res.responses[0].data + '</div>' );
					}
					else {
						resultcontainer.append( '<i style="color:red">' + heap_admin_js_texts.import_widgets_error + ': '  + res.responses[0].errors[0].message + ' )<br/><br/>' );
					}
				} else {
					resultcontainer.append( '<i style="color:red">' + heap_admin_js_texts.import_widgets_failed + '</i><br />' );
				}

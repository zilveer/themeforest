...
get_header();
the_post();
$portfolio_grid_no = (empty($smof_data['portfolio_grid_no'])) ? 4 : $smof_data['portfolio_grid_no'];
$show_details_outside = $smof_data['portfolio_details_outside'];
...
<?php while(have_posts()) :
		...
		if ( (function_exists('has_post_thumbnail')) && has_post_thumbnail() ) :
			$full_image_url = wp_get_attachment_url( get_post_thumbnail_id(), 'full' );
			 //$thumb_image_url = aq_resize( $full_image_url, 680, 320, true );
		$thumb_image_url = aq_resize( $full_image_url, 960, 640, true );
		else :
			$full_image_url = get_template_directory_uri().'/img/940x480.gif';
			$thumb_image_url = get_template_directory_uri().'/img/960x640.gif';

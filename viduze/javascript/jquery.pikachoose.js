	     ...
	     finishAnimating: function(data){
			...
			if(data.clickThrough != ""){
     		...
     		if(this.options.showCaption && data.caption != "" && data.caption != null){
     			this.caption.html(data.caption).fadeTo('fast',0.8);
				if( typeof(Cufon) != 'undefined' ){
					var cp_font_fam = this.caption.find('.cp-title').css('font-family');
					cp_font_fam = cp_font_fam.replace(/'/g,"");
					if( Cufon.hasFont( cp_font_fam ) ){
						Cufon.replace(this.caption.find('.cp-title'), {fontFamily: cp_font_fam});		
					}
				}				
     		}
...
jQuery(document).ready(function(){
	...
	if( PIKACHOOSE.transition == 'fade out then in'){
		PIKACHOOSE.transition = [0];
	}else if( PIKACHOOSE.transition == 'cross fade'){
		PIKACHOOSE.transition = [1];
	}else if( PIKACHOOSE.transition == 'boxes'){
		PIKACHOOSE.transition = [2];
	}else if( PIKACHOOSE.transition == 'wallpaper'){
		PIKACHOOSE.transition = [3];
	}else if( PIKACHOOSE.transition == 'billboard slide'){
		PIKACHOOSE.transition = [4];
	}else if( PIKACHOOSE.transition == 'box fades'){
		PIKACHOOSE.transition = [5];
	}else if( PIKACHOOSE.transition == 'full frame slide'){
		PIKACHOOSE.transition = [6];
	}else{
		PIKACHOOSE.transition = [1];
	}
	PIKACHOOSE.text = { play: "", stop: "", previous: "", next: "", loading: "" };
	PIKACHOOSE.hoverPause = (PIKACHOOSE.hoverPause == 'enable')? true: false;	
	PIKACHOOSE.carousel = true;
	PIKACHOOSE.carouselOptions = {wrap:'circular'};
	
	jQuery("ul#pikame").PikaChoose(PIKACHOOSE);
});
...
var KlFontSize = React.createClass({
	mixins: [
		...
		return (
			<div className="ae-container ae-has-dropdown ae-pos-static">
				<button className={cssClass} data-type="button-klheadings" onClick={this.togglePopup} tabIndex={this.props.tabIndex}>
					<span className="ae-icon-font-size"></span>
				</button>
	...
	getValue(){
		...
		return span ? parseInt( span.getStyle( 'font-size' ).replace('px', '') ) : '0';
		...
		return (
			<div className="ae-dropdown ae-dropdown--dropup ae-rounded-top">
				<div className="ae-container ae-slideArea">
					...
					<span className="ae-slideArea-smallVal ae-fontsize-smaller">A</span>
						<Slider className="ae-slideArea-slider" tipTransitionName="rc-slider-tooltip-zoom-down" step={1} defaultValue={initVal} onChange={this.onChange} />
					<span className="ae-slideArea-bigVal ae-fontsize-bigger" >A</span>
				</div>
			</div>
		);
	},
...
AlloyEditor.Buttons[KlFontSize.key] = AlloyEditor.KlFontSize = KlFontSize;
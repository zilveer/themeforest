...
if ( ! class_exists( 'MthemeTexameta') ) :
...
class MthemeTexameta {
  ...
  public function load_scripts_styles() {
    ...
    if (in_array($taxnow,$this->_meta_box['pages'])){
      ...
      wp_enqueue_style( 'tax-meta-clss', MTHEME_CSS . 'mtheme.texameta.css' );
      // Enqueue Meta Box Scripts
      wp_enqueue_script( 'tax-meta-clss', MTHEME_JS . 'mtheme.texameta.js', array( 'jquery' ), null, true );
  ...
  public function insert_images() {
    ...
    foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
      ...
      $li   .= "<a title='" . __( 'Delete this image','mtheme') . "' class='at-delete-file' href='#' rel='{$nonce}|{$term_id}|{$id}|{$attachment_id}'><img src='" . MTHEME_IMAGES . "delete-16.png' alt='" . __( 'Delete','mtheme') . "' /></a>";
  ...
  public function add() {
    ...
    foreach ( $this->_meta_box['pages'] as $page ) {
	  ...
	  // Manage gallery_category_columns
	  add_filter('manage_edit-'.$page.'_columns', array( $this,'cat_columns'));
	  // this saves the add fields
	  add_filter('manage_'.$page.'_custom_column', array( $this,'manage_cat_columns'), 10, 3);
    }
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $c = 0;
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        <img src="';
        if ($this->_Local_images){
          echo MTHEME_IMAGES.'remove.png';
        }else{
          echo 'http://i.imgur.com/g8Duj.png';
        }
        echo '" alt="'.__('Remove','mtheme').'" title="'.__('Remove','mtheme').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '<img src="';
    if ($this->_Local_images){
      echo MTHEME_IMAGES.'add.png';
    }else{
      echo 'http://i.imgur.com/w5Tuc.png';
    }
    echo '" alt="'.__('Add','mtheme').'" title="'.__('Add','mtheme').'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_file( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      if ( !empty( $meta )  && count($meta) > 0 && !$this->is_array_empty($meta) ) {
        ...
        echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files','mtheme') . '</strong></div>';
        echo '<ol class="at-upload">';
        foreach ( $meta as $att ) {
          // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
          echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}||{$field['id']}|{$att}'>" . __( 'Delete','mtheme' ) . "</a>)</li>";
        }
        echo '</ol>';
      }
    ...
    echo "<div class='at-file-upload-label'> \n
      <strong>" . __( 'Upload new files','mtheme' ) . "</strong>\n
    </div>\n";
    echo "<div class='new-files'>\n
      <div class='file-input'>\n
        <input type='file' name='{$field['id']}[]' />\n
      </div><!-- End .file-input -->\n
      <a class='at-add-file button' href='#'>" . __( 'Add more files','mtheme' ) . "</a>\n
      </div><!-- End .new-files -->\n";
    echo "</td>";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_color( $field, $meta ) {
	...
	$this->show_field_begin( $field, $meta );
	/*echo '<input name="'.$field['id'].'" id="'.$field['id'].'" type="text" value="'.$meta.'" class="mtheme-colorpicker" />';*/
	echo "<input class='at-color' type='text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='8' />";
    echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color','mtheme' ) . "'/>";
  ...
  public function footer_js(){
	...
	*/
	...
	function cat_columns($theme_columns) {
		$theme_columns = MthemeCore::$components['texa_column'];
 ...
 */
  ...
  if (!function_exists('get_tax_meta')){
    function get_tax_meta($term_id,$key,$default='',$multi = false){
      $t_id = (is_object($term_id))? $term_id->term_id: $term_id;
      $m = get_option( 'tax_meta_'.$t_id);  
      if (isset($m[$key])){
        return $m[$key];

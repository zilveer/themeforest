...
if ( ! function_exists( 'yit_get_bloginfo' ) ) {
    /**
     * Force the URL to https://, if we are in SSL
     *
     * @since 1.0.0
     */
    function yit_get_bloginfo( $name ) {
        ob_start();
        bloginfo( $name );
        return ob_get_clean();
    }
        ...
        jQuery( document ).ready( function( $ ) {
            ...
            $( '#install-sampledata' ).click( function( e ) {
                ...
                if ( accept ) {
                    ...
                    var version = $('#sample_data_version').length > 0 ? $('#sample_data_version').val() : 'default';
...
if( !function_exists( 'yit_install_sampledata_callback' ) ) {
    ...
    function yit_install_sampledata_callback() {
        ...
        $filename = YIT_THEME_NAME . '.gz';
        if( isset($_POST['version']) && $_POST['version'] != 'default' ) {
            $filename = YIT_THEME_NAME . '_light.gz';
        }

        $gz = YIT_THEME_ASSETS_DIR . '/sample-data/' . $filename;
        if( file_exists( $gz ) ) {

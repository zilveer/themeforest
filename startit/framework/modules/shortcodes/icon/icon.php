<?php
namespace QodeStartit\Modules\Shortcodes\Icon;

use QodeStartit\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package QodeStartit\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
            ...
            'params'                    => array_merge(
                ...
                array(
                    ...
                    array(
                        ...
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'startit'),
                    ...
                    array(
                        ...
                        'value'       => '',
                        'description' => '',
                        'dependency'  => array('element' => 'type', 'value' => array('circle', 'square', 'circle checked'))
                    ),
                    array(
                        'type'        => 'colorpicker',
                        'heading'     => 'Icon Color',
    ...
    public function render($atts, $content = null) {
        ...
        $html = qode_startit_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.qode_startit_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type'])) {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.qode_startit_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.qode_startit_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.qode_startit_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.qode_startit_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.qode_startit_filter_px($params['border_radius']).'px';
                }
            }
        }

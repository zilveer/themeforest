...
if ( ! class_exists( 'AT_Meta_Box_Ext') ) :
  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme']))
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = get_stylesheet_directory_uri() . '/engine/metaboxes';
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = get_post_meta($post->ID,$field['id'],true);
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        <span class="re-control"><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','vivaco').'" title="'.__('Remove','vivaco').'" id="remove-'.$field['id'].'"></span>
    ...
    echo '" alt="'.__('Add','vivaco').'" title="'.__('Add','vivaco').'" id="add-'.$field['id'].'"><br/></div>';
    ...
    echo '</table><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','vivaco').'" title="'.__('Remove','vivaco').'" id="remove-'.$field['id'].'"></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
  ...
  public function show_field_color( $field, $meta ) {
    ...
    if( wp_style_is( 'wp-color-picker', 'registered' ) ) { //iris color picker since 3.5
      ...
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color' ,'vivaco') . "'/>";
  ...
  function Validate_upload_file_type($file) {
    if (isset($_POST['uploadeType']) && !empty($_POST['uploadeType']) && isset($_POST['uploadeType']) && $_POST['uploadeType'] == 'my_meta_box'){
      ...
      if (!in_array($ext, (array)$allowed)){
        $file['error'] = __("Sorry, you cannot upload this file type for this field.", 'vivaco');
        return $file;
      }
  ...
  public function load_textdomain(){
    ...
    load_textdomain( 'vivaco', dirname(__FILE__) . '/lang/' . get_locale() .'mo' );
  }
} // End Class
...
class AT_Meta_Box extends AT_Meta_Box_Ext {
  ...
  */
  ...
  $meta = get_post_meta($post->ID,$field['id'],true);
    ...
    echo '<span class="re-control"><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','vivaco').'" title="'.__('Remove','vivaco').'" id="remove-'.$field['id'].'"></span>';
    echo '</table>';
  ...
  echo '<span><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove me','vivaco').'" title="'.__('Remove me','vivaco').'" id="remove-'.$field['id'].'" /></span></table></div>';
  $counter = 'countadd_'.$field['id'];
  $js_code = ob_get_clean ();
  $js_code = str_replace("\n","",$js_code);
  $js_code = str_replace("\r","",$js_code);
 ...
 /**
  ...
  $plugin_path = $this->SelfPath;
    ...
    foreach ( $field['options'] as $key => $value ) {
      ...
      echo "<span>
        <input id='img_{$field['id']}_{$i}' type='radio' ".( isset($field['style'])? "style='{$field['style']}' " : '' )." class='at-radio img-radio {$key}".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' value='{$key}'" . $checked . " /> <div class='vsc-radio-label'>{$value}</div>

...
class XMenuWalker extends Walker_Nav_Menu {
	...
	function custom_style_menu_item($item_id, $menu_data, $args){
		$item_style = '';
		$submenu_style = '';
		$icon_style = '';
		...
		if (isset($menu_data['custom-style-menu-bg-color']) && $menu_data['custom-style-menu-bg-color']) {
			$item_style .= 'background-color:' . $menu_data['custom-style-menu-bg-color'] . ';';
		}
		...
		//add style
		if (!empty($item_style)) {
			$this->xmenu_custom_style .= '#menu-item-' . $menu_id_prefix . $item_id . ' > a {' . $item_style . '}';
		}
		...
		if (!empty($item_style)) {
			$this->xmenu_custom_style .= '#menu-item-' . $menu_id_prefix . $item_id . ' > a:hover,'
				...
				. '#menu-item-' . $menu_id_prefix . $item_id . '.current-menu-ancestor' . ' > a {' . $item_style_active . '}';
		}
	...
	function add_custom_style ($args) {
		...
		if ( $menu && ! is_wp_error($menu) && !isset($menu_items) ) {
			...
			if ($menu_items) {
				$responsive_breakpoint = G5Plus_Global::get_option('mobile_header_responsive_breakpoint', '991');
				...
				foreach	($menu_items as $key => $menu_item) {
					...
					if ($menu_data) {
						...
						$this->custom_style_menu_item($menu_item->ID, $menu_data, $args);
					}
				}
	...
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if (isset($xmenu_meta['widget-area']) && !empty($xmenu_meta['widget-area']) && ($xmenu_meta['widget-area'] != '-1')) {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		$item_classes [] = 'x-item-menu-' . $sub_menu_type;
		...
		if (isset($xmenu_meta['image-layout']) && ($xmenu_meta['image-layout'] != 'below')) {
			$item_output .= $this->get_menu_image($depth, $xmenu_meta, $item);
		}
	...
	function get_menu_image($depth, $xmenu_meta, $item) {
		$output = '';
		if (($depth > 0) && ((isset($xmenu_meta['image-url']) && !empty($xmenu_meta['image-url'])) || (isset($xmenu_meta['image-feature']) && ($xmenu_meta['image-feature'] == '1') && ($item->type != 'custom')))) {
			...
			if ($attachment_id) {
				$image_thumbnail_name = isset($xmenu_meta['image-size']) ? $xmenu_meta['image-size'] : 'full';

<?php
/*
* Original plugin by Autommattic, forked by truethemes to include reCAPTCHA and Site Option settings. since Karma version 2.7.0
* Updated on Karma Version 4.0.5
*/
...
*/
define( 'TT_GRUNION_PLUGIN_DIR', TRUETHEMES_EXTENDED . "/grunion-contact-form/");
define( 'TT_GRUNION_PLUGIN_URL', get_template_directory_uri()."/framework/extended/grunion-contact-form/");


if ( is_admin() )
	require_once TT_GRUNION_PLUGIN_DIR . '/admin.php';
	
/*
* mod by tt, require two more files.
*/
require_once TT_GRUNION_PLUGIN_DIR . '/truethemes-recaptchalib.php'; //reCAPTCHA library mod by tt.	
require_once TT_GRUNION_PLUGIN_DIR . '/truethemes-recaptcha-functions.php'; //reCAPTCHA functions by tt.


// take the content of a contact-form shortcode and parse it into a list of field types
function tt_contact_form_parse( $content ) {
...
function tt_contact_form_render_field( $field ) {
...
*/
	...
	if ( $field['type'] == 'email' ) {
		$r .= "\n<div>\n";
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='grunion-field-label ".esc_attr($field['type']) . ( tt_contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __($contact_required) . '</span>' : '' ) . "</label>\n";
		$r .= "\t\t<input type='text' name='".esc_attr($field_id)."' id='".esc_attr($field_id)."' value='".esc_attr($field_value)."' class='".esc_attr($field['type'])."'/>\n";
		$r .= "\t</div>\n";
	} elseif ( $field['type'] == 'textarea' ) {
		$r .= "\n<div>\n";
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='".esc_attr($field['type']) . ( tt_contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __($contact_required) . '</span>' : '' ) . "</label>\n";
		...
		$r .= "\t<div><label class='". ( tt_contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __($contact_required) . '</span>' : '' ) . "</label>\n";
		foreach ( $field['options'] as $option ) {
			$r .= "\t\t<input type='radio' name='".esc_attr($field_id)."' value='".esc_attr($option)."' class='".esc_attr($field['type'])."' ".( $option == $field_value ? "checked='checked' " : "")." />\n";
 			$r .= "\t\t<label class='".esc_attr($field['type']) . ( tt_contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>". htmlspecialchars( $option ) . "</label>\n";
		...
		$r .= "\t\t<label class='".esc_attr($field['type']) . ( tt_contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>\n";
...
function tt_contact_form_validate_field( $field ) {
	...
	if ( $field['required'] && $field['type'] == 'email' ) {
		if ( !is_email( $field_value ) ) {
			...
			$contact_form_errors->add( $field_id, sprintf( __( '%s requires a valid email address','truethemes_localize' ), $field['label'] ) );
		}
		...
		$contact_form_errors->add( $field_id, sprintf( __('%s is required','truethemes_localize'), $field['label'] ) );
	}
	
	$contact_form_values[ $field_id ] = $field_value;
}

function tt_contact_form_is_error( $field_id ) {
...
function tt_contact_form_field( $atts, $content, $tag ) {
	...
	if ( $_POST )
		tt_contact_form_validate_field( $field );
	
	return tt_contact_form_render_field( $field );
}

add_shortcode('contact-field', 'tt_contact_form_field');


function tt_contact_form_shortcode( $atts, $content ) {
	global $post;
	...
	extract( shortcode_atts( array(
		...
		'widget' => 0 //This is not exposed to the user. Works with tt_contact_form_widget_atts
	), $atts ) );
	...
	$body = tt_contact_form_parse( $content );

	$r = "<div role='form' id='contact-form-$id'>\n";
...
*/
	...
	$errors = array();
	if ( is_wp_error( $contact_form_errors ) && $errors = (array) $contact_form_errors->get_error_codes() ) {
		$r .= "<div class='form-error karma_notify message_yellow' id='tt-contact-notify'><p>\n";
		foreach ( $contact_form_errors->get_error_messages() as $message )
			$r .= "\t- $message<br />";
		$r .= "</p>";
		$r .= "\n</div>\n\n";
...
*/
    ...
    //check recaptcha
	require_once TT_GRUNION_PLUGIN_DIR . '/truethemes-recaptchalib.php';
	...
	$resp = tt_recaptcha_check_answer ($captcha_private,$_SERVER["REMOTE_ADDR"],$_POST["recaptcha_challenge_field"],$_POST["recaptcha_response_field"]); //verify captcha entered with server.
 	    ...
 	    if (!$resp->is_valid) {
			...
			$new_contact_form_errors->add("$id-recaptcha", sprintf( __('Invalid CAPTCHA','truethemes_localize'), '' ) );
...
/**end of recaptcha verify mod by tt**/
	...
	$message_sent = tt_contact_form_send_message( $to, $subject, $widget );
...
$contact_required = get_option('ka_contact_required');
		...
		echo '<div class="karma_notify message_green tt-contact-notify"><p>'.$contact_successmsg.'</p></div>';
...
add_shortcode( 'contact-form', 'tt_contact_form_shortcode' );

function tt_contact_form_send_message( $to, $subject, $widget ) {
	global $post;
...
*/
	...
	$spam = '';
	$akismet_values = tt_contact_form_prepare_for_akismet( $contact_form_values );
	...
	$time = date_i18n( __('l F j, Y \a\t g:i a','truethemes_localize'), current_time( 'timestamp' ) );
...
***/
	...
	add_filter( 'wp_insert_post_data', 'tt_grunion_insert_filter', 10, 2 );
	...
	remove_filter( 'wp_insert_post_data', 'tt_grunion_insert_filter' );
...
function tt_contact_form_prepare_for_akismet( $form ) {
...
// submit an array to Akismet. If you're accepting a new item via $_POST, run it through tt_contact_form_prepare_for_akismet() first
function tt_contact_form_is_spam_akismet( $form ) {
	...
	if ( 'true' == trim( $response[1] ) ) // 'true' is spam
		$result = true;
	return apply_filters( 'tt_contact_form_is_spam_akismet', $result, $form );
}

// submit a comment as either spam or ham
// $as should be a string (either 'spam' or 'ham'), $form should be the comment array
function tt_contact_form_akismet_submit( $as, $form ) {
	global $akismet_api_host, $akismet_api_port;
...
function tt_contact_form_widget_atts( $text ) {
	static $widget = 0;
...
add_filter( 'widget_text', 'tt_contact_form_widget_atts', 0 );

function tt_contact_form_widget_shortcode_hack( $text ) {
...
function tt_contact_form_init() {
	if ( function_exists( 'akismet_http_post' ) ) {
		add_filter( 'contact_form_is_spam', 'tt_contact_form_is_spam_akismet', 10 );
		add_action( 'contact_form_akismet', 'tt_contact_form_akismet_submit', 10, 2 );
	}
	if ( !has_filter( 'widget_text', 'do_shortcode' ) )
		add_filter( 'widget_text', 'tt_contact_form_widget_shortcode_hack', 5 );

	// custom post type we'll use to keep copies of the feedback items
	register_post_type( 'feedback', array(
		'labels'	=> array(
			'name'			=> __( 'Messages','truethemes_localize' ),
			'singular_name'	=> __( 'Message','truethemes_localize' ),
			'search_items'	=> __( 'Search Message','truethemes_localize' ),
			'not_found'		=> __( 'No message found','truethemes_localize' ),
			'not_found_in_trash'	=> __( 'No message found','truethemes_localize' )
		),
		//'menu_icon'		=> TT_GRUNION_PLUGIN_URL . '/images/grunion-menu.png',  <-- @since 4.0 - vector icon font used.
		'show_ui'		=> TRUE,
		'public'		=> FALSE,
		'rewrite'		=> FALSE,
		'query_var'		=> FALSE,
	...
	//wp_register_style('grunion.css', TT_GRUNION_PLUGIN_URL . 'css/grunion.css');
}
add_action( 'init', 'tt_contact_form_init' );

/**
 * Add a contact form button to the post composition screen
 */
add_action( 'media_buttons', 'tt_grunion_media_button', 999 );
function tt_grunion_media_button( ) {
	global $post_ID, $temp_ID;
	$iframe_post_id = (int) (0 == $post_ID ? $temp_ID : $post_ID);
	$title = esc_attr( __( 'Add a custom form','truethemes_localize' ) );
	$plugin_url = esc_url( TT_GRUNION_PLUGIN_URL );
...
if ( !empty( $_GET['grunion'] ) && $_GET['grunion'] == 'form-builder' ) {
	add_action( 'parse_request', 'tt_parse_wp_request' );
	add_action( 'wp_ajax_grunion_form_builder', 'tt_parse_wp_request' );
}

function tt_parse_wp_request( $wp ) {
	tt_display_form_view( );
	exit;
}

function tt_display_form_view( ) {
	require_once TT_GRUNION_PLUGIN_DIR . 'grunion-form-view.php';
}

/* @since 4.0 - removed for new WP Dashboard styling

function menu_alter() {
    echo '
	<style>
	#menu-posts-feedback .wp-menu-image img { display: none; }
	#adminmenu .menu-icon-feedback:hover div.wp-menu-image, #adminmenu .menu-icon-feedback.wp-has-current-submenu div.wp-menu-image, #adminmenu .menu-icon-feedback.current div.wp-menu-image { background: url("' .TT_GRUNION_PLUGIN_URL . 'images/grunion-menu-hover.png") no-repeat 6px 7px !important; }
	#adminmenu .menu-icon-feedback div.wp-menu-image, #adminmenu .menu-icon-feedback div.wp-menu-image, #adminmenu .menu-icon-feedback div.wp-menu-image { background: url("' . TT_GRUNION_PLUGIN_URL . 'images/grunion-menu.png") no-repeat 6px 7px !important; }
	</style>';
}
...
function tt_grunion_insert_filter( $data, $postarr ) {
	global $do_grunion_insert;

 ...
 * @return string
 */
function option_tree_slider1( $value, $settings, $int ) 
{ 
?>
      ...
      <div class="element">
        ...
        if ( !empty( $settings[$value->item_id] ) ) {
          foreach( $settings[$value->item_id] as $image ) { ?>
            <li><?php option_tree_slider_view1( $value->item_id, $image, $int, $count ); ?></li><?php 
            $count++; 
          }
        } 
        ?>
 ...
 * @return string
 */
function option_tree_slider_view1( $id, $image, $int, $count ) {
  // required fileds

...
c._validateChangeStep(e,b)&&c._gotoStep(b)});b.steps.on("click.yit",".next, .prev",function(b){b.preventDefault();b=a(this).data("next");var e=c.current_step.data("step");c._validateChangeStep(e,b)&&c._gotoStep(b)});b.ship_to_billing.billing.on("change.yit",function(c){c=a(this).prop("checked");b.ship_to_billing.shipping.prop("checked",c).change()});b.ship_to_billing.shipping.on("change.yit",function(d){d=a(this).prop("checked");b.ship_to_billing.billing.prop("checked",d);c._updateBillingShippingLabels(d)});
...
this.options.elements.steps.filter("[data-step=2]").find("input.next"),a=this.options.elements.steps.filter("[data-step=4]").find("input.prev");b?(c.val(c.data("billing")).data("next",4),a.val(a.data("billing")).data("next",2)):(c.val(c.data("shipping")).data("next",3),a.val(a.data("shipping")).data("next",3))},_validateChangeStep:function(b,a){steps=this.options.steps;return b<a&&a<=steps?this._validateStep(a):this.user_logged_in&&1<a&&a<b?this._validateStep(a):!this.user_logged_in&&0<a&&a<b?this._validateStep(a):
!1},_validateStep:function(a){return!0},_gotoStep:function(b){var c=this.element,d=this.options.elements.steps.filter("[data-step="+b+"]");this.current_step.fadeOut("slow",function(){d.fadeIn("slow").addClass("current");a(this).removeClass("current");c.trigger("slide_change.yit",{step:b})});this._updateSection(b);this._updateBar(b);this.current_step=d},_updateSection:function(b){var c=this.options.elements.sections;c.removeClass("current").removeClass("passed").filter("[data-step="+b+"]").addClass("current");
c.filter(function(){return parseInt(a(this).data("step"))<parseInt(b)}).addClass("passed")},_updateBar:function(a){a=100*(parseInt(a)+1)/(this.options.steps+1);this.options.elements.progress_bar.css("width",a+"%")}};a.fn.yit_checkout=function(b){if("string"===typeof b){var c=Array.prototype.slice.call(arguments,1);this.each(function(){var d=a.data(this,"yit_checkout");d?a.isFunction(d[b])&&"_"!==b.charAt(0)?d[b].apply(d,c):console.error("no such method '"+b+"' for yit_checkout instance"):console.error("cannot call methods on yit_checkout prior to initialization; attempted to call method '"+

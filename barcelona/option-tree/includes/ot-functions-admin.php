    ...
    if ( 'background' == $type ) {
      ...
      foreach( $input as $key => $value ) {
        ...
        if ( $key == 'width' && ! empty( $value ) && ! is_numeric( $value ) ) {
          ...
          add_settings_error( 'option-tree', 'invalid_border_width', sprintf( esc_html__( 'The %s input field for %s only allows numeric values.', 'option-tree' ), '<code>width</code>', '<code>' . $field_id . '</code>' ), 'error' );
      ...
      if ( 0 === preg_match( '/^#([a-f0-9]{6}|[a-f0-9]{3})$/i', $input ) && 0 === preg_match( '/^rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9\.]{1,4})\s*\)/i', $input ) ) {
        ...
        add_settings_error( 'option-tree', 'invalid_hex', sprintf( esc_html__( 'The %s Colorpicker only allows valid hexadecimal or rgba values.', 'option-tree' ), '<code>' . $field_id . '</code>' ), 'error' );
      ...
      if ( isset( $errors ) ) {
        ...
        foreach( $errors as $error ) {
          ...
          add_settings_error( 'option-tree', 'invalid_dimension_' . $error, sprintf( esc_html__( 'The %s input field for %s only allows numeric values.', 'option-tree' ), '<code>' . $error . '</code>', '<code>' . $field_id . '</code>' ), 'error' );
      ...
      if ( isset( $errors ) ) {
        ...
        foreach( $errors as $error ) {
          ...
          add_settings_error( 'option-tree', 'invalid_spacing_' . $error, sprintf( esc_html__( 'The %s input field for %s only allows numeric values.', 'option-tree' ), '<code>' . $error . '</code>', '<code>' . $field_id . '</code>' ), 'error' );
  ...
  function ot_social_links_settings( $id ) {
    ...
    $settings = apply_filters( 'ot_social_links_settings', array(
      ...
      array(
        'id'        => 'href',
        'label'     => 'Link',
        'desc'      => sprintf( esc_html__( 'Enter a link to the profile or page on the social website. Remember to add the %s part to the front of the link.', 'option-tree' ), '<code>http://</code>' ),
        'type'      => 'text',
      )
    ), $id );
  ...
  function ot_insert_css_with_markers( $field_id = '', $insertion = '', $meta = false ) {
    ...
    if ( file_exists( $filepath ) ) {
      ...
      if ( ! is_writable( $filepath ) ) {
        add_settings_error( 'option-tree', 'dynamic_css', sprintf( esc_html__( 'Unable to write to file %s.', 'option-tree' ), '<code>' . $filepath . '</code>' ), 'error' );
        return false;
      }
...
if ( ! function_exists( 'ot_loop_through_option_types' ) ) {
  ...
  function ot_loop_through_option_types( $type = '', $child = false ) {
    ...
    foreach( $types as $key => $value )
      $content.= '<option value="' . esc_attr( $key ) . '" ' . selected( $type, $key, false ) . '>'  . $value . '</option>';
  ...
  function ot_list_item_view( $name, $key, $list_item = array(), $post_id = 0, $get_option = '', $settings = array(), $type = '' ) {
    ...
    <div class="option-tree-setting">
      ...
      foreach( $settings as $field ) {
        ...
        /* option label */
        echo '<div id="setting_' . $_args['field_id'] . '" class="' . esc_attr( $class ) . '"' . $conditions . '>';
    ...
    <div class="option-tree-setting">
      ...
      foreach( $settings as $field ) {
        ...
        if ( isset( $field['condition'] ) && ! empty( $field['condition'] ) ) {
          ...
          $conditions = ' data-condition="' . esc_attr( $field['condition'] ) . '"';
          $conditions.= isset( $field['operator'] ) && in_array( $field['operator'], array( 'and', 'AND', 'or', 'OR' ) ) ? ' data-operator="' . esc_attr( $field['operator'] ) . '"' : '';
    ...
    echo '<form method="post" id="option-tree-options-layouts-form">';
      ...
      if ( is_array( $layouts ) && count( $layouts ) > 1 ) {
        ...
        foreach( $layouts as $key => $data ) {
          ...
          echo '<input type="hidden" name="' . esc_attr( ot_layouts_id() ) . '[' . $key . ']" value="' . ( isset( $data ) ? $data : '' ) . '" />';

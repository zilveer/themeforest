...
class VP_Metabox extends WPAlchemy_MetaBox {
	...
	function _save( $post_id ) {
		...
		if ( $post_type == 'product' ) {
			$meta = crazyblog_set( crazyblog_set( $new_data, 'sh_product_options' ), '0' );
			if ( crazyblog_set( $meta, 'offer_opt' ) == 1 && crazyblog_set( $meta, 'offer' ) != '' ) {
				update_post_meta( $post_id, 'shineblog_product_offer_date', crazyblog_set( $meta, 'offer' ) );
			}
		}
		...
		if ( $this->mode == WPALCHEMY_MODE_EXTRACT ) {
			...
			if ( !empty( $new_fields ) ) {
				...
				/* if ($post_type == 'sh_room' && crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'beds') != '') {
				  add_post_meta($post_id, 'sh_room_beds', crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'beds'));
				  }
				  if ($post_type == 'sh_room' && crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'adults') != '') {
				  add_post_meta($post_id, 'sh_room_adults', crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'adults'));
				  }

				  if ($post_type == 'sh_room' && crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'bath_room') != '') {
				  add_post_meta($post_id, 'sh_room_bath_room', crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'bath_room'));
				  }

				  if ($post_type == 'sh_room' && crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'kitchen') != '') {
				  add_post_meta($post_id, 'sh_room_kitchen', crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'kitchen'));
				  }
				  if ($post_type == 'sh_room' && crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'beds') != '') {
				  add_post_meta($post_id, 'sh_room_beds', crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'beds'));
				  } */
			...
			if ( is_null( $new_data ) ) {
				...
				/* if ($post_type == 'sh_room' && crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'beds') != '') {
				  update_post_meta($post_id, 'sh_room_beds', crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'beds'));
				  }
				  if ($post_type == 'sh_room' && crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'adults') != '') {
				  update_post_meta($post_id, 'sh_room_adults', crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'adults'));
				  }
				  if ($post_type == 'sh_room' && crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'children') != '') {
				  update_post_meta($post_id, 'sh_room_children', crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'children'));
				  }
				  if ($post_type == 'sh_room' && crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'bath_room') != '') {
				  update_post_meta($post_id, 'sh_room_bath_room', crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'bath_room'));
				  }
				  if ($post_type == 'sh_room' && crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'kitchen') != '') {
				  update_post_meta($post_id, 'sh_room_kitchen', crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_room_options'), 0), 'kitchen'));
				  }
				  if ($post_type == 'sh_booking' && crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_booking_options'), 0), 'booking_status') != '') {
				  update_post_meta($post_id, 'sh_booking_status', crazyblog_set(crazyblog_set(crazyblog_set($new_data, 'sh_booking_options'), 0), 'booking_status'));
				  } */
			}

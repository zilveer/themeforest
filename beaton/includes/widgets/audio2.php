<?php
class wize_widget_audio2 extends WP_Widget {
	
    /*--------------------------------------------------*/
    /* CONSTRUCT THE WIDGET
    /*--------------------------------------------------*/
	
    function wize_widget_audio2() {
        $widget_opts = array(
            'classname' => 'widget_audio_two',
            'description' => __('This widget shows an player with album.', 'wizedesign')
        );
        parent::__construct('widget-audio#2', esc_html__('BEATON - Audio #2', 'wizedesign'), $widget_opts);
    }
	...
	<div id="wd-ad2">';
        while ($wp_query->have_posts()):
            ...
            if ($arrImages) {
                foreach ($arrImages as $attachment) {
                    $playlist .= '
					<li><a href="' . esc_url(wp_get_attachment_url($attachment->ID)) . '" class="fap-single-track no-ajax" title="' . esc_attr($attachment->post_title) . '" rel="' . esc_attr($cover[0]) . '" data-meta="#player-meta-widget2">';
					if (strlen($attachment->post_title) > 40) {
						$playlist .= substr($attachment->post_title, 0, 40) . '...';
					} else {
        ...
        echo '
			<div class="wd-ad2-bg"></div>
			<div class="wd-ad2-title">
			...
			if ($genre) {
                echo '
				<div class="wd-ad2-gen">' . esc_html($genre, "wizedesign") . '</div>';
            }
            if ($date) {
                echo '
				<div class="wd-ad2-date">' . esc_html($day, "wizedesign") . ' ' . esc_html($month, "wizedesign") . ' ' . esc_html($year, "wizedesign") . '</div>';
            }
			echo '
			</div><!-- end .wd-ad2-title -->
			
			<div class="wd-ad2-songs">
				<ul class="songs-list">' . $playlist . ' 
				</ul><!-- end ul.songs-list -->
			</div><!-- end .wd-ad2-songs -->
			<div class="wd-ad2-buy">
				<span>' . esc_html__("Buying", "wizedesign") . '</span>';
            if ($beatport) {
                echo ' 
				<a href="' . esc_url($beatport) . '" class="wd-ad2-beatport no-ajax" target="_blank"></a>';
            }
            if ($amazon) {
                echo ' 
				<a href="' . esc_url($amazon) . '" class="wd-ad2-amazon no-ajax" target="_blank"></a>';
            }
            if ($amazon) {
                echo ' 
				<a href="' . esc_url($itunes) . '" class="wd-ad2-itunes no-ajax" target="_blank"></a>';
            }
            if ($other) {
                echo '
				<a href="' . esc_url($other) . '" class="wd-ad2-other no-ajax" target="_blank">' . esc_html($other_text, "wizedesign") . '</a>';
            }
            echo '
			</div><!-- end .wd-ad2-buy -->
		</div><!-- end .wd-ad2cover -->';
...
add_action('widgets_init', create_function('', 'register_widget("wize_widget_audio2");'));
...
class Options_Framework_Interface {
	...
	static function optionsframework_tabs() {
		...
		foreach ( $options as $value ) {
			...
			if ( $value['type'] == "heading" ) {
				...
				if ( $past_type == 'heading_second' ) { $menu .= '</ul>'; $past_type = $value['type']; }
			...
			} else if ( $value['type'] == "heading_parent" ) {
				$counter++;
				$class = '';
				$class = ! empty( $value['id'] ) ? $value['id'] : $value['name'];
				$class = preg_replace( '/[^a-zA-Z0-9._\-]/', '', strtolower($class) ) . '-tab';
				$menu .= '<li><a id="options-group-'.  $counter . '-tab" class="dropdown-toggle nav-tab ' . $class .'" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#options-group-'.  $counter ) . '">' . esc_html( $value['name'] ) . '</a></li>' . "\n";
				$menu .= '<ul data-id="options-group-'.  $counter . '-tab" class="dropdown-menu">';
			} else if ( $value['type'] == "heading_second" ) {
				$counter++;
				$class = '';
				$class = ! empty( $value['id'] ) ? $value['id'] : $value['name'];
				$class = preg_replace( '/[^a-zA-Z0-9._\-]/', '', strtolower($class) ) . '-tab';
				$menu .= '<li><a id="options-group-'.  $counter . '-tab" class="nav-tab ' . $class .'" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#options-group-'.  $counter ) . '">' . esc_html( $value['name'] ) . '</a></li>';
				$past_type = $value['type'];
			}
		}
	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			// Wrap all options
			if ( ( $value['type'] != "heading" ) && ( $value['type'] != "heading_parent" ) && ( $value['type'] != "heading_second" ) && ( $value['type'] != "info" ) ) {
				...
				if ( isset( $value['name'] ) ) {
					$output .= '<h4 class="heading">' . esc_html( $value['name'] ) . '<i class="minimize"></i></h4>' . "\n";
				}
			...
			switch ( $value['type'] ) {
				...
				case "checkbox":
					...
					$output .= '<label class="pseudo-checkbox" for="' . esc_attr( $value['id'] ) . '"><i></i></label>';
					break;

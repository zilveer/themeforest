...
if($isrc&&$sf!='no') { ?>
	...
	if (preg_match('%(?:youtube\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $murl, $match)) {
		...
		if($pos === false) { } else {
			...
			echo '<div class="'.$fr.' '.$roundy.'"><div class="'.$frin.' cb5_media"><iframe class="cb5_media" width="'.$w.'" height="'.$h.'" src="http://player.vimeo.com/video/'.$video.'?title=0&amp;byline=0&amp;portrait=0" title="'.$alt.'" frameborder="0"></iframe></div></div>';
		}

		if($video==''&&$pos===false&&$murl!='') {
			if($cb_type=='audio') $h='45';
			echo '<div class="'.$fr.' '.$roundy.'"><div class="'.$frin.' cb5_media"><video id="media-'.$post->ID.'" class="video-js vjs-default-skin cb5_media" controls preload="none" width="'.$w.'" height="'.$h.'" poster="" data-setup=> <source src="'.$murl.'" type="video/mp4" /> </video></div></div>';
		}
		?>
	<div class="cl"></div>

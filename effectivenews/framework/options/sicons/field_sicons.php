    ...
    class ReduxFramework_sicons extends ReduxFramework{
        ...
        public function render() {
            ...
            if (isset($this->value) && is_array($this->value)) {
                ...
                echo '<input type="hidden" class="mom_icon_holder selected_icon" data-id="'.$x.'" id="selected-icon-'.$x.'" name="' . $this->field['name'] . '[' . $x . '][icon]" id="' . $this->field['id'] . '-selected_icon_' . $x . '" value="' . $icon['icon'] . '" /><span class="button upload_custom_icon" id="add_' . $x . '" data-x="'. $x . '">' . __('Upload Icon', 'redux-framework') . '</span>';
                echo '</div>';
                    ...
                    echo '<li><label>Background Color on hover: </label><br><input type="text" id="' . $this->field['id'] . '-bgcolorh_' . $x . '" name="' . $this->field['name'] . '[' . $x . '][bgcolorh]" value="' . esc_attr($icon['bgcolorh']) . '" class="redux-color redux-color-init  wp-color-picker" placeholder="'.$placeholder.'" /></li>';
...
*/
                echo '<li><a href="javascript:void(0);" class="button deletion redux-sicons-remove">' . __('Delete icon', 'redux-framework') . '</a></li>';
                echo '</ul></div></fieldset></div>';
            }

...
// Latest Sermons
class TB_Latest_Sermons extends WP_Widget {
	
	function TB_Latest_Sermons() {
		$widget_ops = array('classname' => 'tb_sermons', 'description' => __( 'Latest Sermons', 'grace') );		
		$this->WP_Widget('TB_Latest_Sermons', __('ThemeBlossom: Latest Sermons', 'grace'), $widget_ops);	
	}
	...
	function widget( $args, $instance ) {
		...
		$qSermons = new WP_Query(array('post_type' => TB_SERMON_CPT, 'posts_per_page' => $number_of_posts, 'no_found_rows' => true, 'post_status' => 'publish'));
...
function tb_register_latest_sermons() {
	
	register_widget('TB_Latest_Sermons');
	
	do_action('widgets_init');
}

add_action('init', 'tb_register_latest_sermons', 1);

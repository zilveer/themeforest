		...
		<div class="gravatar">
      	...
      	<div class="right">
			...
			<?php
				...
				{
			?>
					<h7 style="float:left;margin-top:1px"><?php echo $comment->comment_author; ?></h7>
			<?php
				}
			?>
...
'reply_text' =>  __('Reply', THEMEDOMAIN), 'login_text' => __('Login to Reply', THEMEDOMAIN), 'max_depth' => $args['max_depth']))) ?>
    ...
    <?php 
        ...
        {
    ?>
    <br class="clear"/><hr/><div style="height:30px"></div>
    <?php
    	}
    ?>
<?php
	...
	if(!empty($posts))
	{
		...
		$count_post = count($posts);
			...
			foreach($posts as $post)
			{
				...
				$return_html.= '<strong class="title"><a href="'.get_permalink($post->ID).'">'.$post->post_title.'</a></strong><br/><span class="post_attribute">'.get_the_time(THEMEDATEFORMAT, $post->ID).'</span>';
				$return_html.= '</li>';
...
function tg_apply_builder($page_id) {
	...
	if(isset($ppb_form_item_arr[0]) && !empty($ppb_form_item_arr[0]))
	{
	    ...
	    foreach($ppb_form_item_arr as $key => $ppb_form_item)
	    {
	    	...
	    	if(isset($ppb_form_item_data_obj->$ppb_shortcode_content_name))
	    	{
	    		...
	    		{
	    			$ppb_shortcode_code.= 'title="'.htmlentities(urldecode($ppb_form_item_data_obj->$ppb_shortcode_title_name)).'" ';
	    		}

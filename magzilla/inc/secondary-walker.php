...
class favethemes_secondary_nav extends Walker_Nav_Menu {
...
// add main/sub classes to li's and links
 ...
 function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
	    ...
	    if( is_rtl() ) { $nav_arrow = 'fa-caret-left'; } else { $nav_arrow = 'fa-caret-right'; }
		...
		if ( $args->has_children ) {
			...
			if($depth > 0){
				$item_output = sprintf( '%1$s<a%2$s>%3$s%4$s%5$s</a>%6$s',
					...
					$attributes,
					$args->link_before,
					apply_filters( 'the_title', $item->title.' <i class="fa '.$nav_arrow.'"></i>', $item->ID ),
					$args->link_after,
					$args->after

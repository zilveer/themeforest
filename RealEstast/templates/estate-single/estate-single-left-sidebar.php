	...
	<div class="property_detail">
		...
		if (have_posts()) : while (have_posts()) : the_post();
			...
			<div class="infotext-detail">
				<h3><?php the_title(); ?></h3>
				<span class="price">
					<?php echo PGL_Addon_Estate::format_price(get_post_meta($post->ID, 'estate_price', TRUE), get_post_meta($post->ID, 'estate_purpose', true));?>
				</span>
				<div class="row title-info">
					...
					<?php
					if ( $agent_id = get_post_meta($post->ID,'agent_id') ) :?>
						<div class="col-sm-6 col-md-6">
	                            <span class="line-top">
		                        <?php _e('Agent', PGL);?>
                                <span>
                                    <?php foreach($agent_id as $id):?>
                                    <?php echo '<a href="'.get_permalink($id).'">'.get_the_title($id).'</a>';?><?php if($id !== end($agent_id)):?>, <?php endif;?>
                                    <?php endforeach;?>
		                        </span>
		                        </span>
						</div>

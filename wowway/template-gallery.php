...
get_header();
	...
	while (have_posts()) : the_post();
		...
		$thumbs_style = get_option( 'krown_gal_hover_style', 'style-1' );
	...
	<section id="portfolio" class="folioGrid <?php echo $thumbs_ratio; ?> get-ratio disable-resize-parent clearfix" data-url="<?php echo substr( get_permalink(), strpos( get_permalink(), '/', 9) ); ?>" data-type="gallery" data-gal="yes" data-hover="<?php echo $thumbs_style; ?>">
		...
		<a id="post-<?php the_ID(); ?>" class="folioItem <?php krown_categories( $post->ID, 'gallery_category', ' ', 'slug' ); ?> <?php echo $thumbs_style; ?> isotope-hidden" data-custom-filter="0" data-slug="<?php echo $post->post_name; ?>" data-title="<?php echo get_the_title() . ' | ' . get_bloginfo( 'name') ; ?>" <?php echo $item_href; ?>>
			...
			<div class="folioTextHolder">
				<div class="folioText">
					<h3><?php the_title(); ?></h3>
					<p>
						<?php if ( $thumbs_style == 'style-1' ) {
							krown_excerpt( 'krown_excerptlength_small', 'krown_excerptmore' ); 
						} else {
							krown_categories( $post->ID, 'gallery_category', ', ', 'name' ) ;
						} ?>
					</p>
				</div>
			</div>

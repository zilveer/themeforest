...
if (!function_exists('mad_standard_post_filter')) {
	...
	function mad_standard_post_filter($this_post) {
		...
		if (has_post_thumbnail($this_id)) {
			$thumbnail = MAD_HELPER::get_the_post_thumbnail($this_id, $image_size, $thumbnail_atts);
			$before = "<div class='image-overlay'><a href='{$link}' data-group='entry-". $this_id ."' title='". sprintf(esc_attr__('%s', 'flatastic'), the_title_attribute('echo=0')) ."' class='single-image entry-media photoframe {$jackbox} {$zoom_image}'>{$thumbnail}</a></div>";
		}
...
if (!function_exists('mad_quote_post_filter')) {
	...
	function mad_quote_post_filter($this_post) {
		...
		$output = '<div class="post-quote entry-media image-overlay">';
			$output .= is_single() ? '' : '<a href="'. esc_url($this_post['url']) .'" class="whole-link" title="' . sprintf( esc_attr__('Permalink to %s', 'flatastic'), the_title_attribute('echo=0') ) .'"></a>';

...
if (!class_exists('MAD_HELPER')) {
	...
	class MAD_HELPER {
		...
		public static function main_navigation( $theme_location = 'primary' ) {
			...
			$nav_menu_primary = rwmb_meta( 'mad_nav_menu', '', mad_post_id() );
			$nav_menu_secondary = rwmb_meta( 'mad_nav_menu_secondary', '', mad_post_id() );

			if ( !empty($nav_menu_primary) && is_numeric($nav_menu_primary) && $theme_location == 'primary' ) {
				$defaults['menu'] = $nav_menu_primary;
			}

			if ( empty($nav_menu_primary) && $theme_location == 'primary' ) {

				$nav_menu_primary =  mad_custom_get_option('nav-menu-primary', '');

				if ( isset($nav_menu_primary) ) {
					if ( !empty($nav_menu_primary) && $theme_location == 'primary' ) {
						$defaults['menu'] = $nav_menu_primary;
					}
				}
			...
			if ( !empty($nav_menu_secondary) && is_numeric($nav_menu_secondary) && $theme_location == 'secondary' ) {
				$defaults['menu'] = $nav_menu_secondary;
			}

			if ( empty($nav_menu_secondary) && $theme_location == 'secondary' ) {

				$nav_menu_secondary =  mad_custom_get_option('nav-menu-secondary', '');

				if ( isset($nav_menu_secondary) ) {
					if ( !empty($nav_menu_secondary) && $theme_location == 'secondary' ) {
						$defaults['menu'] = $nav_menu_secondary;
					}
				}
			...
			$frontpage = get_permalink(get_option('page_on_front'));
					...
					<div class="col-sm-7 col-xs-5">
						<?php if ($pages > 1): ?>
							<p class="d_inline_middle f_size_medium">
								<?php printf(__("Results %d of %d", 'flatastic'), $paged, $pages) ?>
							</p>
						<?php endif; ?>
					</div>
				...
				<div class="row m_xs_bottom_30">
					...
					<div class="col-sm-7 col-xs-5">
						<?php if ($total == 1 && $max > 1): ?>
							...
							<p class="d_inline_middle f_size_medium">
								<?php printf(__("Results %d of %d", 'flatastic'), $current, $max) ?>
							</p>
...
if (!function_exists('mad_shop_corenavi')) {

	function mad_shop_corenavi($pages = "", $a = array()) {

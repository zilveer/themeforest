    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            ...
            $this->strings = array(
                ...
                'installing'                     => __( 'Installing Plugin: %s', 'weddingindustry' ),
                'oops'                           => __( 'Something went wrong.', 'weddingindustry' ),
                ...
                'dashboard'                      => __( 'Return to the dashboard', 'weddingindustry' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'weddingindustry' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'weddingindustry' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'weddingindustry' ),
                'dismiss'                        => __( 'Dismiss this notice', 'weddingindustry' ),
            );
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'weddingindustry' ) . '">' . __( 'Return to the Dashboard', 'weddingindustry' ) . '</a>' ) . '</p>';
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'weddingindustry' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'weddingindustry' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'weddingindustry' ) . '</a>'  : '';
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'weddingindustry' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'weddingindustry' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'weddingindustry' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'weddingindustry' ) : __( 'Recommended', 'weddingindustry' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'weddingindustry' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'weddingindustry' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'weddingindustry' ) . ' %2$s">' . __( 'Install', 'weddingindustry' ) . '</a>',
                        esc_url(
                            wp_nonce_url(
                                add_query_arg(
                                    array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'weddingindustry' ) . ' %2$s">' . __( 'Activate', 'weddingindustry' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( TGM_Plugin_Activation::$instance->menu ),
        ...
        public function column_cb( $item ) {
            ...
            if ( __( 'Private Repository', 'weddingindustry' ) === $item['source'] ) {
                // Escape external URLs
                $plugin_url = esc_url( $plugin_url );
            } elseif ( __( 'Pre-Packaged', 'weddingindustry' ) === $item['source'] ) {
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'weddingindustry' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'weddingindustry' ),
                'source' => __( 'Source', 'weddingindustry' ),
                'type'   => __( 'Type', 'weddingindustry' ),
                'status' => __( 'Status', 'weddingindustry' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => __( 'Install', 'weddingindustry' ),
                'tgmpa-bulk-activate' => __( 'Activate', 'weddingindustry' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'weddingindustry' ), $imploded );
                }
            ...
            class TGM_Bulk_Installer extends WP_Upgrader {
                ...
                public function run( $options ) {
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package']          = __( 'Install package not available.', 'weddingindustry' );
                        $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'weddingindustry' );
                        $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'weddingindustry' );
                        $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'weddingindustry' );
                        $this->strings['process_failed']      = __( 'Plugin install failed.', 'weddingindustry' );
                        $this->strings['process_success']     = __( 'Plugin installed successfully.', 'weddingindustry' );
                    ...
                    public function activate_strings() {
                        ...
                        $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'weddingindustry' );
                        $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'weddingindustry' );
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        ...
                        if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'weddingindustry' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'weddingindustry' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'weddingindustry' ) . '</span><span class="hidden">' . __( 'Hide Details', 'weddingindustry' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'weddingindustry' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'weddingindustry' );
                        }
                        // Default installation strings.
                        else {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'weddingindustry' );
                            $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'weddingindustry' );
                            $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'weddingindustry' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'weddingindustry' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'weddingindustry' ) . '</span><span class="hidden">' . __( 'Hide Details', 'weddingindustry' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'weddingindustry' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'weddingindustry' );
                        }
                    ...
                    public function bulk_footer() {
                        ...
                        if ( empty( $complete ) ) {
                            echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'weddingindustry' ) . '">' . __( 'Return to the Dashboard', 'weddingindustry' ) . '</a>' ) . '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }

...
jQuery(document).ready(function($){
	...
	//(un)fold options in a checkbox-group
  	...
  	jQuery( '[data-advanced-folding]' ).each( function( i, el ) {
	  	var $el        = $( el ),
	  		toggle_el  = $el.data( 'advanced-folding' ),
	  		$toggles   = jQuery( '[data-advanced-folding-match^="' + toggle_el + ':"]' );
  	...
  	$( '.tab-pane[data-tab-id]' ).each( function( i, el ) {
	  	var $tabContainer = $( el ),
	  		$tabChildren = jQuery( '[data-tab-item-of="' + $tabContainer.data( 'tabId' ) + '"]' );
	...
	// event.preventDefault();
		...
		if ( $(this).parent().hasClass( 'changelog' ) ) {
			window.open( 'https://laborator.ticksy.com/article/7530/?print' );
			return;
		}
	...
	/** AJAX Save Options */
	$('.of-save-sticky').on('click', function(ev){
		ev.preventDefault();
	...
	$('#of_save').live('click',function() {
		...
		$this.add( $('.of-save-sticky') ).addClass( 'is-loading' ).data( 'busy', true );
		...
		$.post(ajaxurl, data, function(response) {
			...
			$this.data( 'busy', false ).add( $('.of-save-sticky') ).removeClass( 'is-loading' );
	...
	jQuery(".cb-enable").click(function(){
		...
		jQuery($fold).stop()[ fold_reverse ? 'slideUp' : 'slideDown' ]('normal', "swing");
	});
	jQuery(".cb-disable").click(function(){
		...
		var obj = jQuery(this);
		var $fold='.f_'+obj.data('id');
		jQuery($fold).stop()[ fold_reverse ? 'slideDown' : 'slideUp' ]('normal', "swing");
	});

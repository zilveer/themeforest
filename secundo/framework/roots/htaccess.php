  ...
  function roots_htaccess_writable() {
    if (!is_writable(get_home_path() . '.htaccess')) {
      if (current_user_can('administrator')) {
        add_action('admin_notices', create_function('', "echo '<div class=\"error\"><p>" . sprintf(wp_kses(__('Please make sure your <a href="%s">.htaccess</a> file is writable ', 'ct_theme'),array('a'=>'')), admin_url('options-permalink.php')) . "</p></div>';"));
      }
    }
  }
  ...
  function roots_add_h5bp_htaccess($content) {
    ...
    if ((!file_exists($htaccess_file) && is_writable($home_path) && $wp_rewrite->using_mod_rewrite_permalinks()) || is_writable($htaccess_file)) {
      if ($mod_rewrite_enabled) {
        ...
        if ($h5bp_rules === array()) {
          $filename = CT_THEME_LIB_DIR . '/roots/h5bp-htaccess';

 ...
 */
if (!class_exists('LiberoStickyNavigationWalker')) {
	class LiberoStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if($depth == 0){
				$out_div = '<div class="second'.$background_image_class.'"><div class="inner">';
			}else{
				$out_div = '';
			}

			// build html
			$output .= "\n" . $indent . $out_div  .'<ul '.libero_mikado_get_inline_style($menu_image_background).'>' . "\n";
		}
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($item->hide == ""){
				...
				$item_output .= '</span><span class="bottom-border"><span class="bottom-border-inner"></span></span></a>';
			}

...
jQuery(document).ready(function($) {
    ...
    $(window).on('load', function(){
    	var img_value_select = $('#section-header_layout img.of-radio-img-selected').parent().find('input').val();
    	nuvo_admin_show_slider(img_value_select);
		nuvo_admin_show_custom_header(img_value_select);
    });
    
    $('#section-header_layout').on('click', 'span', function() {
		var img_value = $(this).find('input').val();
		nuvo_admin_show_slider(img_value);
		nuvo_admin_show_custom_header(img_value);
	});
    
    function nuvo_admin_show_slider(_id) {
		if(_id == 'v2'){
			$('#section-cs-header-revsliders').css('display', 'block');
		} else {
			$('#section-cs-header-revsliders').css('display', 'none');
		}
	}
    
    function nuvo_admin_show_custom_header(_id){
    	if(_id == 'custom'){
    		$('#section-cs-header-id').css('display', 'block');
    	} else {
    		$('#section-cs-header-id').css('display', 'none');
    	}
    }
});

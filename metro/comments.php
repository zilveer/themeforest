...
if ( post_password_required() ) {
	...
	?><div class="discussion" id="comments"><?php
	...
	if ( have_comments() ) {
	  ...
	  if ( ! empty($comments_by_type['pings']) ) { // if there are pings 
		...
		?><p class="nocomments"><?php _e('No comments yet.', 'om_theme') ?></p><?php
	}
	...
	if ( comments_open() ) {
		...
		if(false)
			...
			<div class="new-comment" id="respond">
				<div class="new-comment-caption"><?php comment_form_title( __('Leave a Reply', 'om_theme'), __('Leave a Reply to %s', 'om_theme') ); ?></div>
					...
					<?php } else { ?>
						...
						<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
							...
							<?php if ( is_user_logged_in() ) { ?>
								...
								<div><?php printf(__('Logged in as %1$s. %2$sLog out &raquo;%3$s', 'om_theme'), '<a href="'.get_option('siteurl').'/wp-admin/profile.php">'.$user_identity.'</a>', '<a href="'.(function_exists('wp_logout_url') ? wp_logout_url(get_permalink()) : get_option('siteurl').'/wp-login.php?action=logout" title="').'" title="'.__('Log out of this account', 'om_theme').'">', '</a>') ?></div><br/>
						
							<?php } else { ?>
	
								<div class="one-third">
									<input type="text" name="author" id="author" value="<?php echo esc_attr($comment_author); ?>" tabindex="1" placeholder="<?php echo htmlspecialchars(__('Name', 'om_theme')) ?> <?php if ($req) echo htmlspecialchars(__("*", 'om_theme')); ?>" <?php if ($req) echo ' class="required"'; ?> />
								</div>
								<div class="one-third">
									<input type="text" name="email" id="email" value="<?php echo esc_attr($comment_author_email); ?>" tabindex="2" placeholder="<?php echo htmlspecialchars(__('Mail (will not be published)', 'om_theme')) ?> <?php if ($req) echo htmlspecialchars(__("*", 'om_theme')); ?>" <?php if ($req) echo ' class="required email"'; ?> />
								</div>
							...
							<div><textarea name="comment" id="comment" rows="4" tabindex="4" placeholder="<?php echo htmlspecialchars(__('Message', 'om_theme')) ?>" class="required"></textarea></div>

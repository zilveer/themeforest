...
var DialogHelper = {
    ...
    loadShortcodeDetails: function () {
        if (SelectedShortcodeType) {
            ...
            jQuery( '#tmnf-options-table' ).html( '' );
            jQuery.getScript(shortcode_generator_url + "shortcodes/" + SelectedShortcodeType + ".js", function () {
                a.initializeDialog();
    ...
    initializeDialog: function () {
        if (typeof ShortcodeMeta == "undefined") {
            jQuery( '#tmnf-options' ).append( "<p>Error loading details for shortcode: " + SelectedShortcodeType + "</p>" );
        } else {
            if (ShortcodeMeta.disablePreview) {
                jQuery( '#woo-preview' ).remove();
                jQuery( '#woo-btn-preview' ).remove()
            }
            var a = ShortcodeMeta.attributes,
            ...
            jQuery( '#tmnf-options select' ).each( function ( i ) {
   ...
   getTextKeyValue: function (a) {
    ...
    makeShortcode: function () {
        ...
        if (ShortcodeMeta.customMakeShortcode) return ShortcodeMeta.customMakeShortcode(a);
        var c = a.content ? a.content : ShortcodeMeta.defaultContent,
            f = "";
        for (var d in a) {
            var g = a[d];
        ...
        switch ( ShortcodeMeta.shortcodeType ) {

            case 'text-replace':

                var shortcode = "[" + ShortcodeMeta.shortcode + f + "]" + (c ? c + "[/" + ShortcodeMeta.shortcode + "]" : " ")

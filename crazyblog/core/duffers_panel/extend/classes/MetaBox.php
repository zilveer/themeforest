...
class WPAlchemy_MetaBox1 {
	...
	function _init() {
		...
		if ( !WPAlchemy_MetaBox1::_is_post() AND ! WPAlchemy_MetaBox1::_is_page() )
			return;
			...
			<script type="text/javascript">
				...
				jQuery(function ($)
				{
					...
					$(document).on('click', '[class*=docopy-]', function (e)
					{
						...
						var clone_rem1 = $(the_group).prev().find('div.vp-fontawesome');
						var clone_rem2 = $(the_group).prev().find('div.vp-select');
						var clone_rem3 = $(the_group).prev().find('div.vp-icon');
						var counter1 = $(clone_rem1).children().children().children('div.select2-container').length;
						if (counter1 == 2) {
							$(clone_rem1).children().children().children('div.select2-container:first').remove();
						}
						var counter2 = $(clone_rem2).children().children().children('div.select2-container').length;
						if (counter2 == 2) {
							$(clone_rem2).children().children().children('div.select2-container:first').remove();
						}
						var counter3 = $(clone_rem3).children().children().children('div.select2-container').length;
						if (counter3 == 2) {
							$(clone_rem3).children().children().children('div.select2-container:first').remove();
						}
					});
	...
	function _meta( $post_id = NULL, $internal = FALSE ) {
		/* if ( ! is_numeric($post_id))
		  ...
		  $this->meta = $meta; */
		//$theme_options = is_array(get_option('crazyblog'.'_theme_options')) ? get_option('crazyblog'.'_theme_options') : array();
		if ( is_array( get_option( 'wp_crazyblog_theme_options' ) ) ) {
			$theme_options = array();
			$db_options = get_option( 'wp_crazyblog_theme_options' );
			if ( !is_array( $db_options ) )
				$db_options = array();
			//printr($db_options);
			foreach ( $db_options as $db_option ) {
				if ( is_array( $db_option ) )
					$theme_options = array_merge( $theme_options, $db_option );
			}
		}else {
			$theme_options = array();
		}
		$meta = (array) crazyblog_set( $theme_options, $this->id );
		array_pop( $meta );
		$theme_options[$this->id][$this->id] = $meta;
		$this->meta = crazyblog_set( $theme_options, $this->id );

...
class Twitter
{
	...
	function get($count = 20)
	{
		...
		$tweet_option_id = 'pp_twitter_'.$username.'_'.$count;
		$tweet_option_timer_id = 'pp_twitter_'.$username.'_'.$count.'_timer';
		$pp_twitter_cache = get_option($tweet_option_id);
		
		if(empty($pp_twitter_cache))
		{
			add_option( $tweet_option_id, '' ); 
			add_option( $tweet_option_timer_id, intval(time()));
		}
		
		//var_dump(time());
		//var_dump(intval(get_option($tweet_option_timer_id)));
		$tweets_cache_timer = intval((time()- intval(get_option($tweet_option_timer_id)))/60);
		//var_dump($tweets_cache_timer);
		
		if($tweets_cache_timer > 15)
		{
			...
			{
				update_option( $tweet_option_id, serialize($array_tweets) );
				update_option( $tweet_option_timer_id, intval(time()) );
			}
		}
		else
		{
			error_reporting(0);
			$file = $pp_twitter_cache;
			//var_dump($file);

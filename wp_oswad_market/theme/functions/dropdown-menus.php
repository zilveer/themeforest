...
function dropdown_menu( $args ) {
	...
	$args[ 'items_wrap' ] = '<select data-native-menu="true" id="%1$s" class="%2$s ' . apply_filters( 'dropdown_menus_class', 'dropdown-menu' ) . '">%3$s</select>';
...
class DropDown_Menu_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'dropdown-menu-widget', 'description' => __( 'Use this widget to add one of your custom menus as a dropdown.','wpdance' ) );
		parent::__construct( 'dropdown_menu', __('Dropdown Menu','wpdance'), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		if ( ! $menus ) {
			echo '<p>'. sprintf( __( 'No menus have been created yet. <a href="%s">Create some</a>.','wpdance' ), admin_url( 'nav-menus.php' ) ) .'</p>';
			return;
		}
		?>
		<p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'nav_menu' ); ?>"><?php _e( 'Select Menu','wpdance' ); ?>:</label>

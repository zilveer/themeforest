    ...
    removeFile: function () {
     ...
     $('.mlu_remove').live('click', function(event) { 
      ...
      $('.mlu_remove_multi').live('click', function(event) { 
        ...
        //$(this).parents('.multi-screenshot').siblings('.of-background-properties').hide(); //remove background properties
        return false;
      });
      
      /*$('textarea.displayall-upload').change(function(e){
      	$(this).parents().children('.multi-screenshot').slideUp();
      	//alert($(this).val());
      	 var urlArray = $(this).val().split('http');
          ...
          $(this).parents().children('.multi-screenshot').slideDown();
...
/*-----------------------------------------------------------------------------------*/
	// zrobic ta fukcje dla multi zeby dodawala tekst do text fielda zeby bylo wiele urlow
	// zrobic zeby usuwany byl obrazek i link klikniety
	// zrobic zeby wyswietlane bylo wiele obrazkow
...
/*-----------------------------------------------------------------------------------*/
	...
	/*var foo = "http://localhost:8888/incorporated/wp-content/themes/incorporated/images/slider/02.jpg" + "\n";
	foo += "http://localhost:8888/incorporated/wp-content/themes/incorporated/images/slider/03.jpg" + "\n";
	foo += "http://localhost:8888/incorporated/wp-content/themes/incorporated/images/slider/04.jpg" + "\n";
	
	
	var fooArray = foo.split('http');
	//var index = 0;
	if(fooArray.length < 10){
		$.each(fooArray,function (i, val) {
			fooArray[i] = "http" + val;
			//alert(fooArray[i]);
		//	index++;
		}); 
	...
	window.send_to_editor = function(html) {
        ...
        /*  if (itemurl.match(image)) {
            btnContent = '<div class="multi-screenshot"><div class="mp-image-frame"></div><img src="'+itemurl+'" alt="" /><a href="#" class="mlu_remove_multi button">Remove Image</a><div>';
          } else {
...
/*-----------------------------------------------------------------------------------*/
	...
	$(document).ready(function () {
		...
		//optionsframeworkMLU.recreateMultiFileField();
		optionsframeworkMLU.mediaUpload();
		optionsframeworkMLU.multiMediaUpload();

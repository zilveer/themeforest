...
function yiw_addp($str)
{
    ...
    $exclude = 'dropcap|highlight|b|size';

    $str = preg_replace( '/(\[(' . $exclude . ')\])/', '/<\/p>[$1]<\/p>/', $str );
...
function yiw_get_attachment_id( $url ) {
    ...
    foreach( $ids as $id )
        if( !wp_get_attachment_image_src($id, 'full') && $url == array_shift( wp_get_attachment_image_src($id, 'full') ) )
            return $id;
...
function yiw_post_id() {
    global $post, $wp_query, $woocommerce;

    $post_id = 0;
    if ( $wp_query->is_posts_page ) $post_id = get_option( 'page_for_posts' );
    elseif ( isset( $woocommerce ) && ( ( function_exists( 'is_shop' ) && is_shop() ) || is_product_category() || is_product_tag() ) ) $post_id = function_exists('wc_get_page_id') ? wc_get_page_id( 'shop' ) : woocommerce_get_page_id( 'shop' );
    elseif ( get_option( 'jigoshop_shop_page_id' ) != false && function_exists( 'is_shop' ) && is_shop() ) $post_id = get_option( 'jigoshop_shop_page_id' );

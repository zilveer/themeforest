jQuery(document).ready(function($){
    ...
    if ( optionsframework_upload ) {
      ...
      optionsframework_upload.on( 'select', function() {
        ...
        if ( attachment.attributes.type == 'image' ) {
          optionsframework_selector.find('.screenshot').empty().hide().append('<img src="' + attachment.attributes.url + '"><a class="mlu_remove remove-image button">Remove</a>').slideDown('fast');
        }
  ...
  $('.remove-image, .remove-file, .mlu_remove').live("click", function () {
    optionsframework_remove_file( $(this).parents('.section') );
    });

   $('input.upload_button').live("click", function (event) {
      optionsframework_add_file(event, $(this).parents('.section'));
    });

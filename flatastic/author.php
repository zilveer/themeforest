...
<?php if ( have_posts() ) : ?>
	...
	<?php
		...
		$heading      = __("About", 'flatastic') ." ". $name;
		$description  = get_the_author_meta('description', $id);

		if (current_user_can('edit_users') || get_current_user_id() == $id) {
			$description .= " <a href='" . admin_url( 'profile.php?user_id=' . $id ) . "'>". __( '[ Edit the profile ]', 'flatastic' ) ."</a>";
		}
	?>
	...
	the_posts_pagination( array(
		'prev_text'          => __( 'Previous page', 'flatastic' ),
		'next_text'          => __( 'Next page', 'flatastic' ),
		'show_all' => true,
		'mid_size' => 2
	) );
	?>

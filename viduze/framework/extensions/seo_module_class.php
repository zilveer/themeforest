...
//		if (!wp_verify_nonce($nonce, 'edit-aioseop-nonce')) die ( 'Security Check - If you receive this in error, log out and back in to WordPress');
	...
	function edit_category_form() {
		...
		<?php _e('SEO Module', 'all_in_one_seo_pack') ?>
		</th>
		</tr>
		<tr>
	...
	function add_meta_tags_textinput() {
	 ...
	 <?php if (substr($this->wp_version, 0, 3) >= '2.5') { ?>
				...
				<a target="__blank" href="http://semperfiwebdesign.com/gallery/wordpress/wordpress-plugins/all-in-one-seo-pack/"><?php _e('Click here for Support', 'all_in_one_seo_pack') ?></a>
				<input value="aiosp_edit" type="hidden" name="aiosp_edit" />
	...
	function options_panel() {
		...
		if(!$aioseop_options['aiosp_cap_cats']) {
			...
			if (isset($_POST['action']) && $_POST['action'] == 'aiosp_update' && isset($_POST['Submit_Default'])) {
				...
				$message = __("SEO Settings Reset.", 'all_in_one_seo_pack');
		...
		if(isset($_POST['action'])&&isset($_POST['Submit'])){
			if ($_POST['action'] && $_POST['action'] == 'aiosp_update' && $_POST['Submit']!='') {
				$nonce = $_POST['nonce-aioseop'];
				if (!wp_verify_nonce($nonce, 'aioseop-nonce')) die ( 'Security Check - If you receive this in error, log out and back in to WordPress');
				$message = __("SEO Settings Updated.", 'all_in_one_seo_pack');
				
				//Mod by TT
				//adding all isset to $_POST variable to prevent WP_DEBUG error!
				if(isset($_POST['aiosp_can'])):
...
?>
	...
	<?php
	_e('The &lt;title&gt; of your homepage. This is independent of any other option. If not set, the default blog title will get used.', 'all_in_one_seo_pack');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<span class="description">
	<?php
	_e("A comma separated list of your most important keywords for your site that will be written as &lt;meta&gt; keywords on your homepage.", 'all_in_one_seo_pack');
	 ?>
	</span>
	<p></p>
	</td>
	...
	<input type="submit" class='button-primary' name="Submit" value="<?php _e('Update Settings', 'all_in_one_seo_pack')?>" /> 

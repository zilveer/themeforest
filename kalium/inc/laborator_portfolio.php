...
function kalium_get_portfolio_query( $opts = array() ) {
	...
	$args = array(
		...
		'layouts'                     => array(
			...
			'type_1'                     => array(
				...
				'hover_layer_icon'          => get_data( 'portfolio_type_1_hover_layer_icon' ),
				'hover_layer_icon_image'	=> get_data( 'portfolio_type_1_hover_layer_icon_custom' ),
				'hover_layer_icon_width'	=> get_data( 'portfolio_type_1_hover_layer_icon_custom_width' ),
	...
	if ( ! empty( $post_id ) && $vc_mode == false && ( $portfolio = get_post( $post_id ) ) ) {
		...
		if ( $portfolio->masonry_style_portfolio ) {
			$args['custom_query']        = array();
			$args['layout_type']         = 'type-2';
			$args['masonry_items']       = get_field( 'masonry_items_list', $portfolio );
	...
	if ( empty( $opts['no_query'] ) ) {
		...
		if ( ! defined( 'DOING_AJAX' ) ) {
			...
			if ( $hover_color = $args['layouts']['type_2']['hover_color'] ) {
				generate_custom_style( $portfolio_instance_id_attr . '.portfolio-holder .item-box .thumb .hover-state', "background-color: {$hover_color} !important;" );
				generate_custom_style( $portfolio_instance_id_attr . '.portfolio-holder .item-box .thumb .hover-state .likes .like-btn > *', "color: {$hover_color} !important;" );
			}
...
function kalium_portfolio_endless_pagination( $portfolio_args ) {
	...
	?>
	<div class="portfolio-endless-pagination endless-pagination endless-pagination-alignment-<?php echo $portfolio_args['pagination']['align']; echo when_match( $portfolio_args['pagination']['max_num_pages'] <= 1, 'not-visible' ); ?>">
		<div class="show-more<?php echo " type-{$loader_type}"; echo when_match( $portfolio_args['pagination']['type'] == 'endless-reveal', 'auto-reveal' ); echo when_match( $portfolio_args['pagination']['max_num_pages'] <= 1, 'is-finished' ); ?>" data-endless="true">
			<div class="button">
...
function kalium_portfolio_generate_gallery_gap( $gap = 30, $type = '' ) {
	...
	if ( $type == 'carousel' ) {
		generate_custom_style( '.single-portfolio-holder.portfolio-type-3 .gallery-slider .gallery-item', "padding-left:{$gap_half}px; padding-right:{$gap_half}px;" );
	} else {		
		generate_custom_style( '.single-portfolio-holder .gallery .photo, .single-portfolio-holder .gallery .comparison-image-slider', "margin-bottom:{$gap}px;" );
		generate_custom_style( '.single-portfolio-holder .gallery .row', "margin-left:-{$gap_half}px; margin-right:-{$gap_half}px;" );
		generate_custom_style( '.single-portfolio-holder .gallery .row > div', "padding-left:{$gap_half}px; padding-right:{$gap_half}px;" );
	}
}
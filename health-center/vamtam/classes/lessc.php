...
class WpvLessc {
	...
	protected function compileProp($prop, $block, $out) {
		...
		case 'mixin':
			...
			if ($mixins === null) {
				...
				$this->throwError("{$prop[1][0]}($arg_count) is undefined");
			}
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new WpvLesscParser($this, __METHOD__);
	...
	protected function makeParser($name) {
		$parser = new WpvLesscParser($this, $name);
	...
	protected function newFormatter() {
		$className = "WpvLesscFormatter_lessjs";
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
				return $this->formatterName;
			$className = "WpvLesscFormatter_$this->formatterName";
		}
...
class WpvLesscParser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('WpvLessc', 'preg_quote'),
					array_keys(self::$precedence))).')';

			$commentSingle = WpvLessc::preg_quote(self::$commentSingle);
			$commentMultiLeft = WpvLessc::preg_quote(self::$commentMultiLeft);
			$commentMultiRight = WpvLessc::preg_quote(self::$commentMultiRight);
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("WpvLessc", "preg_quote"), $directives));
	...
	protected function expressionList(&$exps) {
		...
		$exps = WpvLessc::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName = null) {
		...
		$value = WpvLessc::compressList($values, ', ');
		return true;
	}
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("WpvLessc", "preg_quote"), $stop);
	...
	protected function literal($what, $eatWhitespace = null) {
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = WpvLessc::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.WpvLessc::preg_quote($what), $m, !$until)) return false;
...
class WpvLesscFormatter_classic {
	public $indentChar = "  ";
...
class WpvLesscFormatter_compressed extends WpvLesscFormatter_classic {
...
class WpvLesscFormatter_lessjs extends WpvLesscFormatter_classic {
	public $disableSingle = true;

<?php
if (!class_exists("sfThemeBundledPlugins")) {
	class sfThemeBundledPlugins {
		...
		public function __construct($plugins) {
			...
			$this->messages = 
				array(
					  "not-installed" => __("Install",'swiftframework'),
					  "installed" => __("Activate",'swiftframework'),
					  "active" => __("Active",'swiftframework')
					  );
		...
		public function options() {
			$plugins = $this->status();
			$html = '<table class='swiftframework'>';
		...
		public static function init($plugins) {
			self::$instance = new sfThemeBundledPlugins($plugins);

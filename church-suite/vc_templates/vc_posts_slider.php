...
while ( $my_query->have_posts() ) {
    ...
    // if ( $thumbnail == '' ) $thumbnail = esc_html__("No Featured image set.", "js_composer");
    ...
    if ( $slides_content != '' && $content != '' && ( $type == ' wpb_flexslider flexslider_fade flexslider' || $type == ' wpb_flexslider flexslider_slide flexslider' ) ) {
        ...
        if ($slides_title==true) $description .= "<h6>$categories / $date</h6>".'<h2 class="post-title">' . $link_image_start . $post_title . $link_image_end .'</h2>';
        $description .= $content;
        $description .= '</div>';
    }
...
else { $teasers = esc_html__("Nothing found." , "js_composer"); }

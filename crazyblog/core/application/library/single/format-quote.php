...
$post_img = (crazyblog_set( wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'crazyblog_1170x590' ), '0' ));
$quoteText = crazyblog_set( crazyblog_set( crazyblog_set( $post_meta, 'crazyblog_post_format_options' ), '0' ), 'crazyblog_quote_post' );
    ...
    <div <?php post_class( 'default-template ' . $no_image ); ?> itemscope itemtype="http://schema.org/Article">
		<?php if ( !empty( $quoteText ) ): ?>
			<div class="single-quote">
				<div class="quote-box" style="background:url(<?php echo esc_url( $post_img ); ?>); ">
					<blockquote><p>
							<?php echo esc_html( $quoteText ); ?>
						</p></blockquote>
				</div>	
				<ul class="short-info">
		...
		<div class="mfn-gallery">
			<?php
			do_shortcode( crazyblog_getContent() );
			wp_link_pages();
			?>
		</div>

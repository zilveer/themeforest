<?php
namespace Libero\Modules\Header\Lib;
/**
 * Class MkdHeaderConnector
 *
...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('libero_mikado_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('libero_mikado_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('libero_mikado_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        do_action('libero_mikado_after_header_connect');
    }
     ...
     *
     * Hooks to libero_mikado_title_content_padding filter
     *
     * @return int
     */

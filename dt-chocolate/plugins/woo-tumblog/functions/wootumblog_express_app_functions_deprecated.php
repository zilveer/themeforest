...
function express_uploadFile($args) {
	...
	if ( !current_user_can('upload_files') ) {
		...
		return new IXR_Error(401, __('You are not allowed to upload files to this site.', LANGUAGE_ZONE));
	}
	...
	if ( ! empty($upload['error']) ) {
		$errorString = sprintf(__('Could not write file %1$s (%2$s)', LANGUAGE_ZONE), $name, $upload['error']);

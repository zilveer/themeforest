 ...
 * @version   2.5.2
 * >>> @modifications  by Olevmedia: dismiss notice key verification upon registered plugin versions added & LayerSlider update hook removed
	...
	class TGM_Plugin_Activation {
		...
		protected function do_plugin_install() {
			...
			if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
				...
				if ( 'update' === $install_type ) {
					//remove LayerSlider update manipulation
					global $wp_filter;
					if(isset($wp_filter['pre_set_site_transient_update_plugins'])) {
						foreach($wp_filter['pre_set_site_transient_update_plugins'] as $k => $v ) {
							foreach($v as $k2=>$v2) {
								if(isset($v2['function']) && is_array($v2['function']) && isset($v2['function'][0]) && is_object($v2['function'][0]) ) {
									if(get_class($v2['function'][0]) == 'KM_PluginUpdatesV3') {
										unset($wp_filter['pre_set_site_transient_update_plugins'][$k][$k2]);
									}
								}
							}
						}
		...
		public function notices() {
			...
			if ( $this->is_tgmpa_page() || get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice_' . $this->id, true ) == $this->get_dismiss_notice_key() ) {
				return;
			}

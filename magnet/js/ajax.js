...
function checkSizesAjax(){
	
	$width = $j(window).width();
	if($width > size1_width){
	...
	if($width < size2_width){
		$j('.big-slider-slide .image, .portfolio_holder_v3 article .image').stop(true).unbind();
		$j('.big-slider-slide .image img, .portfolio_holder article .image img').stop(true).unbind();
		$j('.big-slider-slide .image img, .portfolio_holder article .image img').removeAttr('style');
	}
	
	if($width < size3_width){
		$j('.big-slider-slide .image, .portfolio_holder article .image').stop(true).unbind();
		$j('.big-slider-slide .image img, .portfolio_holder article .image img').stop(true).unbind();
		$j('.big-slider-slide .image img, .portfolio_holder article .image img').removeAttr('style');
	}
...
function slideOutOldPage(content, direction, animationTime, callbacks) { 
	...
	}else if($j('.content_inner').hasClass('curtain')){
		var animation = 'ajax_curtain';
	...
	}else if(animation == "ajax_curtain"){
		content.css('min-height','auto');
		
		if($j(window).scrollTop() > 0){
			$j('html, body').animate({scrollTop: 0}, 300, function(){
				content.stop().animate({height: "0px", opacity: '0'}, animationTime, 'easeInOutQuad', function(){
					$j(this).hide().remove();
					callCallback(callbacks,"oncomplete", null, []);
				});
			});
...
function slideInNewPage(text, direction, animationTime) {
	...
	if(newHTML.find('.content_inner').hasClass('updown')){
		...
		var animation = 'ajax_updown_fade';
	}else if(newHTML.find('.content_inner').hasClass('curtain')){
		var animation = 'ajax_curtain';
	...
	newContent.waitForImages(function() {
		...
		if($j(window).height() > newHeight){
			viewport.css('min-height',newHeight + 5);
		}else{
		...
		if(animation == "ajax_updown" || animation == "ajax_updown_fade"){
			...
			newContent.css({visibility: 'visible', height:'0px'}).stop().animate({height: height}, PAGE_TRANSITION_SPEED, 'easeInOutQuad',function(){
				newContent.css('height','auto');
				newContent.css('min-height',height + 5);
				initPortfolioSingleInfo();
			});
...
function showActivePage(){
	...
	var page_id = '';
	if ((document.location.href.indexOf("?s=") >= 0) || (document.location.href.indexOf("?animation=") >= 0)) {
...
$j(document).ready(function() {
	...
	if($j('body').hasClass('woocommerce') || $j('body').hasClass('woocommerce-page')){	
		...
		$j(document).on('click','a[target!="_blank"]',function(click){
			...
			if($j(this).hasClass('bx-pager-link')) return false;

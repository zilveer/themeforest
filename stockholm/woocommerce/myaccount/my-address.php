...
if ( ! wc_ship_to_billing_address_only() && wc_shipping_enabled() ) {
	...
	$get_addresses = apply_filters( 'woocommerce_my_account_get_addresses', array(
		'billing' => __( 'Billing Address', 'qode' ),
		'shipping' => __( 'Shipping Address', 'qode' )
	), $customer_id );
} else {
...
<p class="myaccount_address">
	<?php echo apply_filters( 'woocommerce_my_account_my_address_description', __( 'The following addresses will be used on the checkout page by default.', 'qode' ) ); ?>
</p>
...
<?php foreach ( $get_addresses as $name => $title ) : ?>
	...
	<div class="u-column<?php echo ( ( $col = $col * -1 ) < 0 ) ? 1 : 2; ?> col-<?php echo ( ( $oldcol = $oldcol * -1 ) < 0 ) ? 1 : 2; ?> woocommerce-Address">
		<header class="woocommerce-Address-title title">
			...
			<a href="<?php echo esc_url( wc_get_endpoint_url( 'edit-address', $name ) ); ?>" class="edit"><?php _e( 'Edit', 'qode' ); ?></a>
		</header>
		<address>
			<?php
				...
				if ( ! $formatted_address )
					_e( 'You have not set up this type of address yet.', 'qode' );
				else

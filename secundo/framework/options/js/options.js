jQuery(document).ready(function () {
    ...
    //tabs
    jQuery('#nhp-main-tabs a').click(function () {
        var $a = jQuery(this);
        jQuery('#nhp-main-tabs li').removeClass('active');
        $a.parents('li').addClass('active');

        jQuery('.nhp-main-wrapper-items').hide();
        var target = jQuery('#' + $a.data('rel')).fadeIn('medium');
    ...
    function initSubtabs($context) {

        var current = jQuery('.nhp-opts-group-tab-link-li.active', $context)
        if (!current.length) {
            current = jQuery('.nhp-opts-group-tab-link-li:eq(0)', $context).addClass('active');

        }

        if ($context == undefined) {
            jQuery('a[data-rel=' + current.parents('.nhp-main-wrapper-items').attr('id') + ']').trigger('click');
        }
        jQuery('.nhp-opts-group-tab-link-a', current).trigger('click');
    }

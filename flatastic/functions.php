...
define('MAD_THEME_VERSION', '1.4.8');
	...
	class MAD_BASE_FUNCTIONS {
		...
		public function init() {
			...
			if (!is_admin()) {
				...
				add_action('wp_print_styles', array(&$this, 'wp_dynamic_styles'), 4);
...
//						add_action('wp_head', array(&$this, 'top_cookie_alert_localize'), 3);
		...
		function body_class($classes) {
			...
			if (mad_custom_get_option('compatible_with_mega_menu')) {
				$classes[] = 'compatible_with_mega_menu';
			}

			if (!mad_custom_get_option('sidebar_mobile')) {
				$classes[] = 'touch_sidebar_disable';
			}
			return $classes;
		}
		...
		public function register_styles() {
			...
			wp_register_style(MAD_PREFIX . 'magnific', MAD_BASE_URI . 'js/magnific-popup/magnific-popup.css' );
			wp_register_style(MAD_PREFIX . 'heapbox', MAD_BASE_URI . 'js/heapbox/heapbox.css');

			wp_register_style(MAD_PREFIX . 'chosen-drop-down', MAD_BASE_URI . 'js/chosen/chosen.css');
		}
		...
		public function register_scripts() {
			...
			wp_register_script(MAD_PREFIX . 'magnific', MAD_BASE_URI . 'js/magnific-popup/jquery.magnific-popup.min.js', array('jquery'), '', true );
			...
			wp_register_script(MAD_PREFIX . 'chosen-drop-down', MAD_BASE_URI . 'js/chosen/chosen.jquery.min.js', array('jquery'));
		}
		...
		public static function wp_dynamic_styles() {

			global $mad_theme_data;
			$prefix_name = sanitize_file_name($mad_theme_data['name']);
		...
		public function localize_script() {
			wp_localize_script('jquery', 'global', array(
				...
				'megaMenuConfig' => mad_custom_get_option('compatible_with_mega_menu', 1)
			));
		}

		public function top_cookie_alert_localize() {
			wp_localize_script('jquery', 'cwmessageobj', array(
				'cwmessage' => __("Please note this website requires cookies in order to function correctly, they do not store any specific information about you personally.", 'flatastic'),
				'cwagree' => __("Accept Cookies", 'flatastic'),
				'cwmoreinfo' =>  sanitize_text_field(__("Read more...", 'flatastic')),
				'cwmoreinfohref' => is_ssl() ? "https://" : "http://" . "www.cookielaw.org/the-cookie-law"
			));
		}

		public function top_cookie_alert_body_prepend() {
			$cookie_alert_message = mad_custom_get_option('cookie_alert_message', __('Please note this website requires cookies in order to function correctly, they do not store any specific information about you personally.', 'flatastic'));
			...
			<script type="text/javascript">
				(function ($) {
					...
					var cwmessageobj = {
						'cwmessage' : "<?php _e("Please note this website requires cookies in order to function correctly, they do not store any specific information about you personally.", 'flatastic') ?>",
						'cwagree' : "<?php _e("Accept Cookies", 'flatastic') ?>",
						'cwmoreinfo' :  "<?php _e("Read more...", 'flatastic') ?>",
		...
		public static function load_textdomain () {
			load_theme_textdomain('flatastic', MAD_BASE_PATH  . 'lang');
		}
...
if (class_exists('WooCommerce')) {
	...
	if ( ! function_exists( 'mad_woo_config' ) ) {
		...
		function mad_woo_config() {
			...
			mad_get_template( 'config-woocommerce/config.php' );
...
if ( class_exists('mega_main_init') ) {
	if ( mad_custom_get_option('compatible_with_mega_menu', 1) ) {
		require_once( MAD_BASE_PATH . 'config-megamenu/config.php' );
	}
}

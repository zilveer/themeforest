jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(MORNING_RECORDS_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == MORNING_RECORDS_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(MORNING_RECORDS_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				morning_records_message_success('', MORNING_RECORDS_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(MORNING_RECORDS_STORAGE['strings']['editor_caption_close']);
			} else {
				morning_records_message_warning(rez.error, MORNING_RECORDS_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		morning_records_message_confirm(MORNING_RECORDS_STORAGE['strings']['editor_delete_post'], MORNING_RECORDS_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(MORNING_RECORDS_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					morning_records_message_success('', MORNING_RECORDS_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = MORNING_RECORDS_STORAGE['site_url'];
						}, 1000);
				} else {
					morning_records_message_warning(rez.error, MORNING_RECORDS_STORAGE['strings']['editor_delete_error']);
				}
			});

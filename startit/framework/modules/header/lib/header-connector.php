...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('qode_startit_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('qode_startit_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('qode_startit_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        if(!empty($config['affect_slider'])) {
            add_filter('qode_startit_slider_item_padding', array($this, 'sliderItemPaddingFilter'));
            add_filter('qode_startit_slider_navigation_padding', array($this, 'sliderNavigationPaddingFilter'));
        }

        do_action('qode_startit_after_header_connect');
    }
     ...
     *
     * Hooks to qode_startit_title_content_padding filter
     *
     * @return int
     */
     ...
     *
     * Hooks to qodef_themename_filter_slider_item_padding filter
     *
     * @return int
     */
     ...
     * Returns padding value calculated from header height.
     *
     * Hooks to qode_startit_slider_item_padding filter
     *
     * @return int
     */

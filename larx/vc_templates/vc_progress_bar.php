...
$class_to_filter = 'text-left vc_progress_bar wpb_content_element';
...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<h5 class="vc_label"' . $line['txtcolor'] . '>' . $line['label'] . $unit . '</h5>';
	$output .= '<div class="progress vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
			' vc_progress-bar-color-' . $line['color'] : '' )
		. '">';
	$output .= '<span class="vc_label"' . $line['txtcolor'] . '>';
	if ( $max_value > 100.00 ) {
		$percentage_value = (float) $line['value'] > 0 && $max_value > 100.00 ? round( (float) $line['value'] / $max_value * 100, 4 ) : 0;
	} else {
		$percentage_value = $line['value'];
	}
	$output .= '<span class="vc_bar progress-bar' . esc_attr( implode( ' ', $bar_options ) ) . '" data-percentage-value="' . esc_attr( $percentage_value ) . '" data-value="' . esc_attr( $line['value'] ) . '"' . $line['bgcolor'] . '>' . $line['value'] . $units . '</span>';
	$output .= '</span></div>';
}

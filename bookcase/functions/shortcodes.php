...
function ellipsis_button( $atts, $content = null ) {
...
add_shortcode('button', 'ellipsis_button');

/*-----------------------------------------------------------------------------------*/
/*	Lightbox Shortcodes
/*-----------------------------------------------------------------------------------*/

function ellipsis_lightbox( $atts, $content = null ) {
    extract(shortcode_atts(array(
    'link'	=> '#',
    ), $atts));
...
add_shortcode('lightbox', 'ellipsis_lightbox');
...
function ellipsis_divider( $atts, $content = null ) {
    extract(shortcode_atts(array(
    ), $atts));
...
add_shortcode('divider', 'ellipsis_divider');
...
function ag_featuredfulltabs( $atts, $content = null ) {
	...
	$out .= '<div class="clear"></div><div id="tabs" class="nobreak"><div class="one_third alpha tablink">';
	...
	foreach ($atts as $tab) {
		if($count == 1){$first = 'ui-state-active';}else{$first = '';}
		$out .= '
		<li class="'.$first.'"> <a href="#tabs-'.$tab_counter_1.'"><span class="tabimage"></span><span class="text"><span>'.$tab.'</span></span></a> </li>'; 
...
function tabfullpanes( $atts, $content = null ) {
	...
	$out .= '<div class="two_third column-last">';
	global $tab_counter_2;

	$out .= '<div id="tabs-'.$tab_counter_2.'" class="panel">'. do_shortcode($content) .'</div></div>';

        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( 'Options panel created using %1$s', '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . 'Redux Framework' . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = 'Options';
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = 'Options';
                }
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
                    ...
                    if ( true === $this->args['allow_sub_menu'] ) {
                        ...
                        if ( true == $this->args['system_info'] ) {
                            add_submenu_page(
                                $this->args['page_slug'], 'System Info', 'System Info', $this->args['page_permissions'], $this->args['page_slug'] . '&tab=system_info_default', '__return_null'
                            );
                        }
                    }
                }
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    // Construct message
                    $msg = sprintf( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', $event, strtolower( $s ) ) . '<br/><br/>' . $url;
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = 'Default' . ": " . substr( $default_output, 0, - 2 );
                }
...
//                }
            ...
            public function ajax_save() {
                ...
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" ) ) {
                    json_encode( array(
                        'status' => 'Invalid security credential, please reload the page and try again.',
                        'action' => 'reload'
                    ) );
                    die();
                }
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => 'Your panel has no fields. Nothing to save.' ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {

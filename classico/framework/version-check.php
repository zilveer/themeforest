...
class ET_Version_Check {
	...
	private function api_get_version() {
		...
		$raw_response = wp_remote_get($this->api_url . '?theme=' . ET_THEME_SLUG); 
	...
	public function add_notice() {
		...
		if ( ! get_user_meta($user_id, $this->ignore_key) ) {
	        ...
	        printf(__('There is a new version %1$s of %2$s Theme available on <a href="%3$s" target="_blank">ThemeForest</a> | <a href="%4$s" target="_blank">View changelog.</a> | <a href="%5$s">Dismiss Notice</a>'), $this->new_version, ET_THEME_NAME, ET_TF_LINK, ET_CHANGELOG_LINK, '?' . $this->ignore_key . '=0');
	        echo "</p></div>";
		}
	}

...
if ( ! class_exists( "Envato_WP_Theme_Updater" ) ) {
	class Envato_WP_Theme_Updater {
		...
		public function check( $updates ) {
			$this->username = apply_filters( 'om_theme_update_username', $this->username );
			$this->apiKey   = apply_filters( 'om_theme_update_apiKey', $this->apiKey );
			$this->authors  = apply_filters( 'om_theme_update_authors', $this->authors );

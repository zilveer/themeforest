...
var pexetoOptions={
		...
		setOnOffFunc:function(){
			jQuery('div.on-off').each(function(){
				if(jQuery(this).siblings('input[type=hidden]:first').attr('value')==='on'){
					jQuery(this).find('span').css({marginLeft:49});
				}
			});
			...
			jQuery('div.on-off').bind('click', function(){
				...
				if(hiddenInput.attr('value')=='on'){
					...
					jQuery(this).find('span').animate({marginLeft:49});
		...
		loadUploader: function(element, pathToPhp, uploadsUrl){
			...
			new AjaxUpload(button, {
				...
				onSubmit : function(file, ext){
					...
					buttonSpan=button.find('span').text('Upload');
				...
				onComplete: function(file, response){
					imgUrl=response=='-1'?'error':uploadsUrl+'/'+response;
					
					button.siblings('input.upload').attr('value', imgUrl);
		...
		addItem:function(optionsData) {
			...
			for ( var i = 0; i < length; i++) {
				if(optionsData.preview && optionsData.inputIds[i]==='#'+optionsData.preview){
					html+=pexetoOptions.generatePreview(jQuery(optionsData.inputIds[i]).attr("value"));
				}

...
>
    ...
    ?>
        <div class="wishlist-title <?php echo ( $wishlist_meta['is_default'] != 1 && $is_user_owner ) ? 'wishlist-title-with-form' : ''?>">
            <?php echo apply_filters( 'yith_wcwl_wishlist_title', '<h2 class="product-title">' . $page_title . '</h2>' ); ?>
            <?php if( $wishlist_meta['is_default'] != 1 && $is_user_owner ): ?>
                <a class="btn button show-title-form">
                    ...
                    <?php esc_html_e( 'Edit title', 'flatastic' ) ?>
                </a>
            <?php endif; ?>
        </div>
        <?php if( $wishlist_meta['is_default'] != 1 && $is_user_owner ): ?>
	...
	<div class="table_wrap">
		...
		<thead>
			<tr>
				<th class="product-thumbnail product_image_col"><?php esc_html_e('Product Image', 'flatastic') ?></th>

				<th class="product-name product_title_col">
					<span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product Name and Category', 'flatastic' ) ) ?></span>
				</th>
				...
				<?php if( $show_price ) : ?>
					...
					<th class="product-price product_price_col">
						<span class="nobr">
							<?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Price', 'flatastic' ) ) ?>
						</span>
					</th>
				...
				<?php if( $show_stock_status ) : ?>
					...
					<th class="product-stock-stauts">
						<span class="nobr">
							<?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock Status', 'flatastic' ) ) ?>
						</span>
					</th>
				...
				<?php if( $show_last_column ) : ?>
					...
					<th class="product-add-to-cart"><span class="nobr"><?php esc_html_e('Action', 'flatastic') ?></span></th>
		...
		if( count( $wishlist_items ) > 0 ) :
			foreach( $wishlist_items as $item ) :
				...
				if( $product !== false && $product->exists() ) :
					...
					<tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>">
						...
						<td class="product-thumbnail" data-title="<?php esc_html_e('Product Image', 'flatastic') ?>">
							<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>">
								<?php echo $product->get_image(array(120, 120)) ?>
							</a>
						</td>

						<td class="product-name" data-title="<?php esc_html_e('Product Name and Category', 'flatastic') ?>">
							<h4>
								<a class="product-name" href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>"><?php echo apply_filters( 'woocommerce_in_cartproduct_obj_title', $product->get_title(), $product ) ?></a>
							</h4>
							<?php echo $product->get_categories(); ?>
						</td>

						<?php if( $show_price ) : ?>
							<td class="product-price" data-title="<?php esc_html_e('Price', 'flatastic') ?>">
								<?php
								...
								elseif( $product->price != '0' ) {
									...
									echo apply_filters( 'yith_free_text', esc_html__( 'Free!', 'flatastic' ) );
								}
								?>
							</td>
						<?php endif ?>
						...
						<?php if( $show_stock_status ) : ?>
							<td class="product-stock-status" data-title="<?php esc_html_e('Stock Status', 'flatastic') ?>">
								<?php
								if( $stock_status == 'out-of-stock' ) {
									$stock_status = "Out";
									echo '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of Stock', 'flatastic' ) . '</span>';
								} else {
									$stock_status = "In";
									echo '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'flatastic' ) . '</span>';
								}
								?>
							</td>
						<?php endif ?>
						...
						<?php if( $show_last_column ): ?>
							<td class="product-add-to-cart" data-title="<?php esc_html_e('Action', 'flatastic') ?>">
								<!-- Date added -->
								<?php
								if( $show_dateadded && isset( $item['dateadded'] ) ):
									echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on : %s', 'flatastic' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
								endif;
								?>
								...
								<?php if( $available_multi_wishlist && is_user_logged_in() && count( $users_wishlists ) > 1 && $move_to_another_wishlist ): ?>
									<select class="change-wishlist selectBox">
										<option value=""><?php esc_html_e( 'Move', 'flatastic' ) ?></option>
										<?php
										foreach( $users_wishlists as $wl ):
											...
											<option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
												...
												if( $wl['wishlist_privacy'] == 1 ){
													$wl_privacy = esc_html__( 'Shared', 'flatastic' );
												}
												elseif( $wl['wishlist_privacy'] == 2 ){
													$wl_privacy = esc_html__( 'Private', 'flatastic' );
												}
												else{
													$wl_privacy = esc_html__( 'Public', 'flatastic' );
												}
								...
								<?php if( $is_user_owner && $repeat_remove_button ): ?>
									<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist remove" title="<?php esc_html_e( 'Remove this product', 'flatastic' ) ?>"><?php _e( 'Remove', 'flatastic' ) ?></a>
								<?php endif; ?>
			...
			<tr>
				<td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php esc_html_e( 'No products were added to the wishlist', 'flatastic' ) ?></td>
			</tr>
		<?php endif; ?>
			...
			<td colspan="<?php echo esc_attr( $column_count ) ?>">
				<?php if( $show_cb ) : ?>
					<div class="custom-add-to-cart-button-cotaniner">
						<a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_meta['wishlist_token'] ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php esc_html_e( 'Add the selected products to the cart', 'flatastic' ) ?></a>
					</div>
				<?php endif; ?>
				...
				<?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
					<div class="ask-an-estimate-button-container">
						<a href="<?php echo ( $additional_info ) ? '#ask_an_estimate_popup' : esc_url($ask_estimate_url) ?>" class="btn button ask-an-estimate-button" <?php echo ( $additional_info ) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : '' ?> >
							...
							<?php esc_html_e( 'Ask for an estimate', 'flatastic' ) ?>
						</a>
					</div>
				<?php endif; ?>

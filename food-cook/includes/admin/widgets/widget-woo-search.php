...
class Woo_Search extends WP_Widget {
	...
	function Woo_Search() {
		$widget_ops = array( 'description' => 'This is a WooThemes standardized search widget.' );
		 WP_Widget::__construct( false, __( 'Dahz - Search', 'woothemes' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
		if ( isset( $title ) && $title ) { echo $before_title . apply_filters( 'widget_title', $title, $instance, $this->id_base ) . $after_title; }
			include( get_template_directory() . '/includes/templates/widget/search-form.php' );
		echo $after_widget;
	}

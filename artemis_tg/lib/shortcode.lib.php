...
function flickr_func($atts, $content) {
	...
	if(!empty($flickr_id))
	{
		...
		{
		    $return_html.= '<li>';
		    $return_html.= '<a href="'.$photo['url'].'" title="'.$photo['title'].'"><img src="'.$photo['thumb_url'].'" alt="" class="flickr" rel="fancybox-thumb" width="75" height="75" /></a>';$return_html.= '</li>';
		}
...
function expand_page_func($atts, $content) {
	...
	$return_html = '<input type="hidden" id="expand_height" name="expand_height" value="'.$height.'" />';
	
	return $return_html;
}
add_shortcode('expand_page', 'expand_page_func');

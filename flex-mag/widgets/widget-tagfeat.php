<?php
/**
 * Plugin Name: Tag Featured Widget
 */

add_action( 'widgets_init', 'mvp_tagfeat_load_widgets' );

function mvp_tagfeat_load_widgets() {
	register_widget( 'mvp_tagfeat_widget' );
}

class mvp_tagfeat_widget extends WP_Widget {

	/**
	 * Widget setup.
	 */
	function mvp_tagfeat_widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'mvp_tagfeat_widget', 'description' => __('A widget that displays a list of posts from a tag of your choice.', 'mvp-text') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'mvp_tagfeat_widget' );

		/* Create the widget. */
		$this->__construct( 'mvp_tagfeat_widget', __('Flex Mag: Tag Featured Widget', 'mvp-text'), $widget_ops, $control_ops );
	}
					...
					<div class="feat-widget-wrap left relative">
						...
						<div class="feat-widget-img left relative">
							<?php if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail())  ) { ?>
								<?php the_post_thumbnail('mvp-post-thumb', array( 'class' => 'widget-img-main' )); ?>
								<?php the_post_thumbnail('mvp-medium-thumb', array( 'class' => 'widget-img-side' )); ?>
							<?php } ?>

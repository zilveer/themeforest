        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                /*if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'mk_framework' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'mk_framework' ) . '</a> v' . self::$_version ) . '</span>';
                }*/

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'mk_framework' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'mk_framework' );
                }
...
// __construct()
            ...
            /*private function _internationalization() {
                ...
                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'mk_framework' );
                ...
                load_textdomain( 'mk_framework', self::$_dir . 'languages/' . $locale . '.mo' );
            }*/
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
                    ...
                    if ( true === $this->args['allow_sub_menu'] ) {
                        ...
                        if ( true == $this->args['system_info'] ) {
                            add_submenu_page(
                                $this->args['page_slug'], __( 'System Info', 'mk_framework' ), __( 'System Info', 'mk_framework' ), $this->args['page_permissions'], $this->args['page_slug'] . '&tab=system_info_default', '__return_null'
                            );
                        }
                    }
                }
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = __( 'Enable', 'mk_framework' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'mk_framework' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'mk_framework' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'mk_framework' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'mk_framework' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'mk_framework' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'mk_framework' ) . ": " . substr( $default_output, 0, - 2 );
                }
...
//                }
            ...
            public function ajax_save() {
                ...
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" ) ) {
                    json_encode( array(
                        'status' => __( 'Invalid security credential, please reload the page and try again.', 'mk_framework' ),
                        'action' => 'reload'
                    ) );
                    die();
                }
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'mk_framework' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {

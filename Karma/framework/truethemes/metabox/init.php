...
//define('CMB_META_BOX_URL', get_template_directory_uri() . '/framework/truethemes/metabox/');  no longer needed? works same with either
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear' => __( 'Clear','truethemes_localize' ),
					'defaultString' => __( 'Default','truethemes_localize' ),
					'pick' => __( 'Select Color','truethemes_localize' ),
					'current' => __( 'Current Color','truethemes_localize' ),
				) );
			}
		} else {

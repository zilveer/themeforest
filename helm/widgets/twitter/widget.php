	...
	class tp_widget_recent_tweets extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				MTHEME_NAME . ' - Recent Tweets', // Name
				array( 'description' => __( 'Display recent tweets', 'mtheme_local' ), ) // Args
			);
		}
					...
					if(!empty($tp_twitter_plugin_tweets)){
						...
						<div class="mtheme-tweets">
							...
							foreach($tp_twitter_plugin_tweets as $tweet){
								...
								print '<li><span class="tweet_avatar"><img src="'.$tweet['profile_image_url'].'" width="32px" height="32px" alt="avatar" /><span class="tweet_time"><a class="twitter_time" target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">'.relative_time($tweet['created_at']).'</a></span> <span class="tweet_text">'.convert_links($tweet['text']).'</span></span></li>';

...
if ($this->parent->dev_mode_forced) {
    ...
    $tip_msg    = esc_html__('Redux has enabled developer mode because', 'redux-framework') . ' ' . $debug_bit . $conjunction_bit . $localhost_bit . '.';
} else {
...
<div id="redux-header">
	<?php if ( ! empty( $this->parent->args['display_name'] ) ) { ?>
		<div class="display_header">
			...
			<h2><?php echo sprintf('%s', $this->parent->args['display_name']); ?></h2>

			<?php if ( ! empty( $this->parent->args['display_version'] ) ) { ?>
				<span><?php echo sprintf('%s', $this->parent->args['display_version']); ?></span>
                        <?php } ?>

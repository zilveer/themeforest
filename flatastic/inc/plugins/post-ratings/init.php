...
//      load_plugin_textdomain(self::ID, false, MAD_INC_PLUGINS_URI . 'post-ratings/lang');
  ...
  */
  public function CreateMenu(){
    add_theme_page(__('Post Ratings', 'flatastic'), __('Post Ratings', 'flatastic'), 'manage_options', 'flatastic', array($this, 'SettingsPage'), 62);
  }
  ...
  */
  public function PluginSettingsLink($links, $file){
    ...
    if (MAD_INC_PLUGINS_URI . 'post-ratings/' === $file) {
      $settings_link = '<a href="'.add_query_arg(array('page' => self::ID), admin_url('options-general.php')).'">'.__('Settings', 'flatastic').'</a>';
  ...
  */
  public function SettingsPage(){
    ...
    $generic_pages = array(
      'home'     => __('Home', 'flatastic'),
      'archive'  => __('Archives', 'flatastic'),
      'singular' => __('Single pages', 'flatastic'),
      'search'   => __('Search results', 'flatastic'),
    );

    ?>
    <div class="wrap metabox-holder">
      <h2><?php _e('Post Ratings', 'flatastic'); ?></h2>

      <form method="post" action="options.php" style="position:relative;">

        <div class="postbox" style="position:absolute;right:0;top:0;">
          <h3><span><?php _e('Global stats', 'flatastic'); ?></span></h3>
          <div class="inside">
            <p><?php printf(__('%1$s votes (on %2$s posts)', 'flatastic'), sprintf('<strong>%d</strong>', $num_votes), sprintf('<strong>%d</strong>', $num_rated_posts)); ?></p>
            <p><?php printf(__('Average vote count per post: %s', 'flatastic'), sprintf('<strong>%.2F</strong>', @($options['num_votes'] / $options['num_rated_posts']))); ?></p>
            <p><?php printf(__('Average rating per post: %s', 'flatastic'), sprintf('<strong>%.2F</strong>', $avg_rating)); ?></p>
          </div>
        </div>
        ...
        <table class="form-table">
          ...
          <tr valign="top">
            <th scope="row"><?php _e('Access level', 'flatastic'); ?></th>
            <td>
              <label for="anonymous_vote">
                ...
                <?php _e('Allow unregistered users to vote', 'flatastic'); ?>
             </label>
            </td>
          </tr>

          <tr valign="top">
            <th scope="row"><?php _e('Maximum rating', 'flatastic'); ?></th>
            <td>
              <input type="text" size="3" name="<?php echo self::ID; ?>[max_rating]" value="<?php echo $max_rating; ?>" />
              <p><span class="description"><?php _e('Changing this option will reset existing post rating records', 'flatastic'); ?></span></p>
            </td>
          </tr>

          <tr valign="top">
            <th scope="row">
              <?php _e('Bayesian rating (score) formula', 'flatastic'); ?>
            </th>
            <td>
              ...
              <fieldset>
               ...
               <label for="bayesian_formula_1">
                 <input id="bayesian_formula_1" name="<?php echo self::ID; ?>[bayesian_formula]" type="radio" value="<?php echo self::BR1; ?>" <?php checked($bayesian_formula, self::BR1); ?> />
                  ...
                  <code style="font-size: 14px;">(<em>v</em> / (<em>v</em> + <strong>MV</strong>)) * <em>r</em> + (<strong>MV</strong> / (<em>v</em> + <strong>MV</strong>)) * <strong>R</strong></code> (<?php printf(__('from %s', 'flatastic'), '<a href="http://en.wikipedia.org/wiki/Internet_Movie_Database#User_ratings_of_films" target="_blank">IMDB</a>'); ?>)
               </label>
               </p>
               ...
               <label for="bayesian_formula_2">
                 ...
                 <code style="font-size: 14px;">((<strong>AV</strong> * <strong>R</strong>) + (<em>v</em> * <em>r</em>)) / (<strong>AV</strong> + <em>v</em>)</code> (<?php printf(__('from %s', 'flatastic'), '<a href="https://gist.github.com/44522/" target="_blank">thebroth</a>'); ?>)
               </label>
               </p>
               ...
               <label for="user_formula">
                 ...
                 <?php _e('I have my own formula:', 'flatastic'); ?>
               ...
               </label> <a href="#" onclick="jQuery('#legend').toggle();">(<?php _e('Legend', 'flatastic'); ?>)</a>
               </p>

               <div id="legend" style="display:none;">
                <p>
                 <code style="font-size: 14px;"><strong>AV</strong></code> = <?php _e('Global average number of votes per post', 'flatastic'); ?>
                 <br />
                 <code style="font-size: 14px;"><strong>&nbsp;V</strong></code> = <?php _e('Global number of votes (from all posts)', 'flatastic'); ?>
                 <br />
                 <code style="font-size: 14px;"><em>&nbsp;v</em></code> = <?php _e('Number of votes from the current post', 'flatastic'); ?>
                 <br />
                 <code style="font-size: 14px;"><strong>&nbsp;R</strong></code> = <?php printf(__('Global average rating per post (from 1 to %d)', 'flatastic'), $max_rating); ?>
                 <br />
                 <code style="font-size: 14px;"><em>&nbsp;r</em></code> = <?php printf(__('Average rating of the current post (from 1 to %d)', 'flatastic'), $max_rating); ?>
                 <br />
                 <code style="font-size: 14px;"><strong>MV</strong></code> = <?php printf(__('Minimum vote count per post to consider (%d by default)', 'flatastic'), self::MIN_VOTES); ?>
                 <br />
                 <code style="font-size: 14px;"><strong>MR</strong></code> = <?php printf(__('Maximum rating, see option above (currently %d)', 'flatastic'), $max_rating); ?>
               </p>
              </div>
          ...
          <tr valign="top">
            <th scope="row"><?php _e('Allow ratings on', 'flatastic'); ?></th>
            <td>
              <fieldset>
          ...
          <tr valign="top">
            <th scope="row"><?php _e('Locations of the rate links', 'flatastic'); ?></th>
            <td>
              ...
              <fieldset>
               <label for="location_before_post">
                 ...
                 <?php _e('Before post content', 'flatastic'); ?>
               </label>
               <br />
               <label for="location_after_post">
                 ...
                 <?php _e('After post content ', 'flatastic'); ?>
               </label>
               <br />
               <br />
               <label for="custom_filter">
                  <?php _e('I have my own action hook:', 'flatastic'); ?>
          ...
          <tr valign="top">
            <th scope="row"><?php _e('Page visibility', 'flatastic'); ?></th>
            <td>
              <fieldset>
          ...
          <tr><td colspan="2">&nbsp;</td></tr>
            ...
            <th scope="row" colspan="2">
              ...
              <input type="submit" class="button-primary" value="<?php _e('Save Changes', 'flatastic'); ?>" />
              &nbsp;
              <label for="remove_all">
                <input id="remove_all" type="checkbox" value="1" name="<?php echo self::ID; ?>[remove_ratings]" />
                <?php _e('Delete rating records from all posts', 'flatastic'); ?>
              </label>
      ...
      <div style="background:#eee;padding: 5px 10px;margin: 10px 5px;">
        <?php printf(__('Found a bug, having a feature request or just looking for help on using this plugin? Then head on to the %s.', 'flatastic'), '<a href="'.self::PROJECT_URI.'">'.__('Post Ratings support forums', 'flatastic').'</a>'); ?>
      </div>
    </div>
    <?php
  }
  ...
  */
  public function assets(){
    ...
    if(!class_exists('Atom') || (class_exists('Atom') && (!defined('Atom::VERSION'))))
      wp_localize_script(self::ID, 'post_ratings',
		  array(
          	'blog_url' => home_url('/'),
			'noRatedMsg' => esc_html__('Not rated yet!', 'flatastic')
		  )
      );
  ...
  */
  public function Run(){
    ...
    // this is our $.ajax request
      ...
      if(!$post){
        $error = __("Invalid vote! Cheatin' uh?", 'flatastic');
        ...
        if($this->currentUserCanRate($post_id)){
          ...
          $error = __('You cannot rate this post!', 'flatastic');
        }
  ...
  */
  public function ControlBlockHook($content = ''){
    ...
    if($control){
      ...
      if(empty($post->ID)){
        printf(__("Your '%s' action is must run in a post's context!", 'flatastic'), $custom_filter);
        return $content;
      }

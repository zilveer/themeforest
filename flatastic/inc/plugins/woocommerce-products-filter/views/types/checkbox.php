...
if (!function_exists('woof_draw_checkbox_childs'))
{
	...
	function woof_draw_checkbox_childs($taxonomy_info, $tax_slug, $childs, $show_count)
	{
		...
		if ($hide_childs == -1)
		{
			$hide_childs = (int) get_option('mad_woof_checkboxes_slide');
		}

		//excluding hidden terms
		$hidden_terms = array();
		if (isset($MAD_WOOF->settings['excluded_terms'][$tax_slug])) {
			$hidden_terms = explode(',', $MAD_WOOF->settings['excluded_terms'][$tax_slug]);
		}
		?>
		<ul class="woof_childs_list" <?php if ($hide_childs == 1): ?>style="display: none;"<?php endif; ?>>
			...
			<?php foreach ($childs as $term) : $inique_id = uniqid(); ?>
				...
				if (!in_array($term['slug'], $current_request)) {
					if ($show_count) {
						$count = $MAD_WOOF->dynamic_count($term, 'checkbox', $_REQUEST['additional_taxes']);
				...
				<li>
					...
					if (!empty($term['childs'])) {
						woof_draw_checkbox_childs($taxonomy_info, $tax_slug, $term['childs'], $show_count);
					}
					?>

...
?>
<h2><?php _e( 'Order Details', LANGUAGE_ZONE ); ?></h2>
<table class="shop_table order_details">
	<thead>
		<tr>
			<th class="product-name"><?php _e( 'Product', LANGUAGE_ZONE ); ?></th>
			<th class="product-total"><?php _e( 'Total', LANGUAGE_ZONE ); ?></th>
		</tr>
	</thead>
	<tbody>
		<?php
							...
							<?php
								...
								if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
									...
									foreach ( $download_files as $download_id => $file ) {
										...
										$links[] = '<small><a href="' . esc_url( $file['download_url'] ) . '">' . sprintf( __( 'Download file%s', LANGUAGE_ZONE ), ( count( $download_files ) > 1 ? ' ' . $i . ': ' : ': ' ) ) . esc_html( $file['name'] ) . '</a></small>';
									}
	...
	<?php
				...
				if ( $has_refund && $key === 'order_total' ) {
					...
					if ( wc_tax_enabled() && 'incl' == $order->tax_display_cart ) {
						...
						if ( ! empty( $tax_del_array ) ) {
							$refunded_tax_del .= ' ' . sprintf( __( '(Includes %s)', LANGUAGE_ZONE ), implode( ', ', $tax_del_array ) );
						}

						if ( ! empty( $tax_ins_array ) ) {
							$refunded_tax_ins .= ' ' . sprintf( __( '(Includes %s)', LANGUAGE_ZONE ), implode( ', ', $tax_ins_array ) );
						}
					}
		...
		if ( $has_refund ) { ?>
			<tr>
				<th scope="row"><?php _e( 'Refunded:', LANGUAGE_ZONE ); ?></th>
		...
		if ( '' != $order->customer_note ) { ?>
			<tr>
				<th scope="row"><?php _e( 'Note:', LANGUAGE_ZONE ); ?></th>
...
<header>
	<h2><?php _e( 'Customer details', LANGUAGE_ZONE ); ?></h2>
</header>
<table class="shop_table shop_table_responsive customer_details">
<?php
	if ( $order->billing_email ) {
		echo '<tr><th>' . __( 'Email:', LANGUAGE_ZONE ) . '</th><td data-title="' . __( 'Email', LANGUAGE_ZONE ) . '">' . $order->billing_email . '<//td></tr>';
	}
	if ( $order->billing_phone ) {
		echo '<tr><th>' . __( 'Telephone:', LANGUAGE_ZONE ) . '</th><td data-title="' . __( 'Telephone', LANGUAGE_ZONE ) . '">' . $order->billing_phone . '</td></tr>';
	}
...
<?php endif; ?>
		...
		<header class="title">
			<h3><?php _e( 'Billing Address', LANGUAGE_ZONE ); ?></h3>
		</header>
		<address><p>
			<?php
				if ( ! $order->get_formatted_billing_address() ) _e( 'N/A', LANGUAGE_ZONE ); else echo $order->get_formatted_billing_address();
			?>
		</p></address>
...
<?php if ( get_option( 'woocommerce_ship_to_billing_address_only' ) === 'no' && get_option( 'woocommerce_calc_shipping' ) !== 'no' ) : ?>
	...
	<div class="col-2">
		...
		<header class="title">
			<h3><?php _e( 'Shipping Address', LANGUAGE_ZONE ); ?></h3>
		</header>
		<address><p>
			<?php
				if ( ! $order->get_formatted_shipping_address() ) {
					_e( 'N/A', LANGUAGE_ZONE );
				} else {

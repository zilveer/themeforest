...
if( !function_exists('thb_text_format') ) {
	function thb_text_format( $str, $add_paragraphs=false ) {
		...
		if ( ! $add_paragraphs ) {
			remove_filter( 'thb_text_format', 'wpautop', 10 );
		}

		$str = apply_filters('thb_text_format', $str);

		if ( ! $add_paragraphs ) {
			add_filter( 'thb_text_format', 'wpautop' );
		}
...
add_filter( 'thb_text_format', 'wptexturize'        );
add_filter( 'thb_text_format', 'convert_smilies'    );
add_filter( 'thb_text_format', 'convert_chars'      );
add_filter( 'thb_text_format', 'wpautop'            );
add_filter( 'thb_text_format', 'shortcode_unautop'  );
add_filter( 'thb_text_format', 'prepend_attachment' );
 ...
 * @return string
 */
if( !function_exists('thb_text_twitterify') ) {
	function thb_text_twitterify( $tweet ) {

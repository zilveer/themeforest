...
(function ($) {
	 ...
	 */
	var Dependency = function ( selector, qualifiers, $subject ) {
        this.selector = $subject.data('dependency');
        this.$dependencyObj = $( '#' + $subject.data('dependency') );
	 ...
	 */
	var DependencySet = function ( dependencies, $subject ) {
		...
		for ( d in dependencies ) {
			...
			this.dependencies.push( new Dependency(d, dependencies[d], $subject) );
	...
	DependencyCollection.prototype.addSet = function ( set ) {
		...
		this.dependencySets.push( new DependencySet( set, this.$subject ) );
	...
	DependencyCollection.prototype.enable = function ( noFade ) {
		...
		} else if ( this.$subject[0].nodeName.toLowerCase() !== 'input' && this.$subject[0].nodeName.toLowerCase() !== 'textarea' && this.$subject[0].nodeName.toLowerCase() !== 'select') {
		...
		if ( this.settings.hide ) {
			...
			if ( $hideObject.css('display') === 'none' ) {
                ...
                if ( noFade ) {
					...
					$hideObject.slideDown( this.settings.duration );
                    
                    if( this.$subject.hasClass('ut-has-subdependency') ) {                        
                        
                        this.$subject.find( 'input, textarea, select' ).trigger('change');
	...
	DependencyCollection.prototype.disable = function ( noFade ) {
		...
		if ( this.settings.hide ) {
			...
			if ( noFade ) {
				...
				$hideObject.slideUp( this.settings.duration );
                
                if( this.$subject.hasClass('ut-has-subdependency') ) {
                    
                    $("[data-dependency='" + this.$subject.data('panel-for') + "']").parent().slideUp( this.settings.duration ); 

<?php
/*
 * Library taken from Rotating Tweets (Twitter widget & shortcode) plugin, v1.3.13, http://wordpress.org/extend/plugins/rotatingtweets/
 *
 */
 ...
 */
if ( ! class_exists( 'WP_OAuthConsumer' ) ):
...
class WP_OAuthSignatureMethod_HMAC_SHA1 extends WP_OAuthSignatureMethod {
  ...
  public function build_signature($request, $consumer, $token) {
    ...
    return base64_encode(hash_hmac('sha1', $base_string, $key, true)); // Completely Safe. Builds HMAC-SHA1 signature for oAuth.
  }
}
...
abstract class WP_OAuthSignatureMethod_RSA_SHA1 extends WP_OAuthSignatureMethod {
  ...
  public function build_signature($request, $consumer, $token) {
    ...
    return base64_encode($signature); // Completely Safe. Builds RSA-SHA1 signature for oAuth.
  }

  public function check_signature($request, $consumer, $token, $signature) {
    $decoded_sig = base64_decode($signature); // Completely Safe. Used for checking RSA-SHA1 signature for oAuth.
    ...
    if (!$parameters) {
      ...
      if ($http_method == "POST"
        ...
        $post_data = WP_OAuthUtil::parse_parameters(
          file_get_contents(self::$POST_INPUT) // Completely Safe. Retrieves oAuth request parameters from self.
        );

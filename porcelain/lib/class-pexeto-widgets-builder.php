...
class PexetoWidgetsBuilder {
	...
	protected function print_before_field( $field, $multi=false ) {
		...
		$html = '<div class="'.$this->option_class.$multiclass.' option-'.$field['type'].$additional_class.'"'.$data.'>';
	...
	public function print_field( $field, $saved = null ) {
		...
		case 'multioption':
			...
			foreach ( $field["fields"] as $sub_field ) {
				...
				$this->print_field( $new_field , $saved_val[$sub_field['id']] );
			}
	...
	public function print_upload( $field, $saved_val ) {
		$this->print_before_field( $field );
		echo '<div class="upload-wrapper"><input class="option-input upload pexeto-upload" name="'.$field['id'].'" id="'.$field['id'].'" type="text" value="'.htmlspecialchars( $saved_val ).'" />';
		echo '<a id="'.$field['id'].'_button" class="upload-button pexeto-upload-btn"><span>Select Image</span></a></div>';
	...
	public function print_image_radio( $field, $saved_val ) {
		...
		if ( sizeof( $field['options'] )>0 ) {
			foreach ( $field['options'] as $sub_option ) {
				...
				echo '<div class="option-image-radio"><input type="radio" name="'.$field['id'].'" value="'.$sub_option['id'].'" '.$checked.'/><img src="'.$sub_option['img'].'" title="'.$sub_option['title'].'"/></div>';
			}
		}

...
class cmb_Meta_Box {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'taxonomy_select':
					...
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
				case 'custom_taxonomy_multicheck':
					echo '<ul>';
					$i = 1;
					foreach ( $field['options'] as $value => $name ) {
						// Append `[]` to the name to get multiple values
						...
						$args = array(
							...
							if ($attachments) {
								...
								foreach ($attachments as $attachment) {
									echo '<li>'.wp_get_attachment_link($attachment->ID, 'thumbnail', 0, 0, __( 'Download', 'morphis' ) );
									echo '<span>';
					...
					echo '<div id="', $field['id'], '_status" class="cmb_upload_status">';	
						if ( $meta != '' ) { 
							...
							if ( $check_image ) {
								...
								echo '<a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">' . __( 'Remove Image', 'morphis' ) . '</a>';
								echo '</div>';
							} else {
								$parts = explode( '/', $meta );
								...
								echo 'File: <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta, '" target="_blank" rel="external">' . __( 'Download', 'morphis' ) . '</a> / <a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">' . __( 'Remove', 'morphis' ) . '</a>)';
							}	
						}
					echo '</div>'; 
				break;
	...
	function save( $post_id)  {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( in_array( $field['type'], array( 'taxonomy_select', 'taxonomy_radio', 'taxonomy_multicheck', 'custom_taxonomy_multicheck' ) ) )  {	

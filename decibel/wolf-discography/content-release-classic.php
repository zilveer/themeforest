...
<article id="post-<?php the_ID(); ?>" <?php post_class( array( 'wolf-release', 'clearfix' ) ); ?>>
	...
	<div class="entry-content">
		...
		<div class="wolf-release-meta">
			...
			<?php // Title
			if ( $release_title ) : ?>
			<strong><?php _e( 'Title', 'wolf' ); ?></strong> : <?php echo sanitize_text_field( $release_title ); ?><br>
			<?php endif; ?>

			<?php // Date
			if ( $display_date ) : ?>
			<strong><?php _e( 'Release Date', 'wolf' ); ?></strong> : <?php echo sanitize_text_field( $display_date ); ?><br>
			<?php endif; ?>
			...
			<?php // Catalog number
			if ( $release_catalog ) : ?>
			<strong><?php _e( 'Catalog ref.', 'wolf' ); ?></strong> : <?php echo sanitize_text_field( $release_catalog ); ?><br>
			<?php endif; ?>

			<?php // Type
			if ( $release_type && wolf_get_release_option( 'display_format' ) ) : ?>
			<strong><?php _e( 'Format', 'wolf' ); ?></strong> : <?php echo sanitize_text_field( $release_type ); ?><br>
			<?php endif; ?>
		</div>

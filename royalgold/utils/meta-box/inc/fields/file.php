	...
	class RWMB_File_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_localize_script( 'rwmb-file', 'rwmbFile', array(
				'maxFileUploadsSingle' => __( 'You may only upload maximum %d file', 'royalgold' ),
				'maxFileUploadsPlural' => __( 'You may only upload maximum %d files', 'royalgold' ),
			) );
		}
		...
		static function wp_ajax_delete_file()
		{
			...
			if ( $ok )
				...
				wp_send_json_error( __( 'Error: Cannot delete file', 'royalgold' ) );
		}
		...
		{
			$i18n_title = apply_filters( 'rwmb_file_upload_string', _x( 'Upload Files', 'file upload', 'royalgold' ), $field );
			$i18n_more  = apply_filters( 'rwmb_file_add_string', _x( '+ Add new file', 'file upload', 'royalgold' ), $field );
		...
		{
			$i18n_delete = apply_filters( 'rwmb_file_delete_string', _x( 'Delete', 'file upload', 'royalgold' ) );
			$i18n_edit   = apply_filters( 'rwmb_file_edit_string', _x( 'Edit', 'file upload', 'royalgold' ) );
			$li = '
			<li>

...
<div class="defaultsContainer"><?php if(!empty($modifiable)): foreach($modifiable as $m): $sb_name = $allsidebars[$m]['name'];?>
                <p><?php echo esc_attr($sb_name); ?>: 
                    <select name="search_page_<?php echo esc_attr($m);?>">
                        <option value=""></option>
                    <?php foreach($allsidebars as $key => $sb):?>
                        <option value="<?php echo esc_attr($key); ?>" <?php echo (isset($defaults['search'][$m]) && $defaults['search'][$m]==$key) ? 'selected="selected"' : ''; ?>>

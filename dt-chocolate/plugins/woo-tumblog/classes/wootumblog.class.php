...
class WooTumblog {
	...
	function woo_tumblog_contextual_help($contextual_help, $screen_id, $screen) {
		...
		if ($screen_id == $tumblog_plugin_hook) {
			//Help HTML
			$contextual_help = '<p><h3><strong>'.__('How to add Tumblog functionality to your theme!', LANGUAGE_ZONE).'</strong></h3></p>';
			//Template Tags
			$contextual_help .= '<p><strong>'.__('Included Template Tags:', LANGUAGE_ZONE).'</strong></p>';
			...
			//Example Code
			$contextual_help .= '<p><strong>'.__('Twenty Ten example implementation:', LANGUAGE_ZONE).'</strong></p>';
			$contextual_help .= '<p><ul>';
	...
	function woo_tumblog_plugin_install_notice() {
		if (0) {
			echo "
			<div id='woo-tumblog-warning' class='updated fade'><p><strong>".__('Woo Tumblog is almost ready.', LANGUAGE_ZONE)."</strong> ".sprintf(__('You must <a href="%1$s">configure your settings</a> for it to work.', LANGUAGE_ZONE), "edit.php?page=tumblog")."</p></div>
			";
		}
	}
	...
	function woo_tumblog_plugin_options_notice() {
		if (0) {
			echo "
			<div id='woo-tumblog-warning' class='updated fade'><p><strong>".__('NOTE	:', LANGUAGE_ZONE)."</strong> ".__('You can add the custom tumblog output to your theme by following the instructions in ', LANGUAGE_ZONE).sprintf(__('the contextual help in the <a href="%1$s">Tumblog Options</a> page.', LANGUAGE_ZONE), "edit.php?page=tumblog")."</p></div>
			";
		}
	}
	...
	function woo_tumblog_options() {
		...
		<div id="woo_container" class="wrap">
			...
			<div id="main">
				...
				//Admin messages
				echo "
					<div id='woo-tumblog-warning' class='updated fade'><p><strong>".__('NOTE	:', LANGUAGE_ZONE)."</strong> ".__('Add the tumblog output to your theme by following the instructions in the contextual help menu above.', LANGUAGE_ZONE)."</p></div>
					";
				//Check if submit
		...
		if ($default_feed_option == 'yes') {
			...
			if (get_option('woo_tumblog_content_method') == 'post_format') {
				...
				if ( has_post_format( 'aside' , $post_id )) {
					...
					if(!empty($link_url)) {
						$content .= '<p><a href="'.$link_url.'" rel="bookmark" title="'.get_the_title($post_id).'" target="_blank">'.__('Play Audio', LANGUAGE_ZONE).'</a></p>';
					}
					...
					$content .= '<p><cite>'.get_post_meta($post_id,'quote-copy',true).__(' ~ ', LANGUAGE_ZONE).'<a href="'.get_post_meta($post_id,'quote-url',true).'" title="'.get_the_title($post_id).'">'.get_post_meta($post_id,'quote-author',true).'</a></cite></p>';

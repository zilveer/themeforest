	...
	<footer class="entry-meta text-center">
		...
		<small>
			<?php
				...
				$categories_list = get_the_category_list( __( ', ', 'humbleshop' ) );
				if ( $categories_list && humbleshop_categorized_blog() ) :
			?>
			<span class="cat-links">
				<?php printf( __( 'This entry was posted in %1$s', 'humbleshop' ), $categories_list ); ?>
			</span>
			<?php endif; // End if categories ?>

			<?php
				/* translators: used between list items, there is a space after the comma */
				$tags_list = get_the_tag_list( '', __( ', ', 'humbleshop' ) );
				if ( $tags_list ) :
			?>
			<span class="tags-links">
				<?php printf( __( 'Tagged %1$s', 'humbleshop' ), $tags_list ); ?>
			</span>

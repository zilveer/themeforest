...
define( 'NOTIFIER_THEME_NAME', 'Pioneer' ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', get_bloginfo('stylesheet_directory') ); // The theme folder name
define( 'NOTIFIER_XML_FILE', 'http://www.epicthemes.net/updates/notifier_pioneer.xml' ); // The remote notifier XML file containing the latest version of the theme and changelog
define( 'NOTIFIER_CACHE_INTERVAL', 6000 ); // The time interval for the remote XML cache in the database (21600 seconds = 6 hours)
...
function update_notifier_menu() {  
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (float)$xml->latest > (float)$theme_data['Version']) { // Compare current theme version with the remote XML version
			...
			add_submenu_page('epicadmin.php', NOTIFIER_THEME_NAME . ' Theme Updates',  NOTIFIER_THEME_NAME . ' <span class="update-plugins count-1"><span class="update-count">New Updates</span></span>', 'administrator', 'theme-update-notifier','update_notifier');
		}
	}	
}
...
function update_notifier_bar_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (float)$xml->latest > (float)$theme_data['Version']) { // Compare current theme version with the remote XML version
			$wp_admin_bar->add_menu( array( 'id' => 'update_notifier', 'title' => '<span>' . NOTIFIER_THEME_NAME . ' <span id="ab-updates">New Updates</span></span>', 'href' => get_admin_url() . 'admin.php?page=theme-update-notifier' ) );
		}
	}
}
...
function update_notifier() { 
	...
	<style>
		...
		h3.title {margin: 30px 0 0 0; padding: 30px 0 0 0; border-top: 1px solid #ddd; color:#f9f9f9;}
	</style>
	<div class="wrap rm_wrap">
		...
		<div id="instructions">
		    ...
		    <p><strong>Please note:</strong> make a <strong>backup</strong> of the theme inside your WordPress installation folder <strong><?php echo NOTIFIER_THEME_FOLDER_NAME; ?>/</strong></p>
		    <p>To update the theme, login to <a href="http://www.epicthemes.net/">themeforest.net</a>,  and re-download the theme.</p>

...
Picker.core=function(o,e,xy,z,fu) {
	...
	function commit(v) { if(fu) fu(v);
		...
		if(o=='mCur') { var W=parseInt($S('mSpec').width), W2=W/2, W3=W2/2; 
			...
			$('mHEX').innerHTML=color.HSV_HEX(hsv); color.cords(W);
			
			var darker = darkerColor("#"+$('mHEX').innerHTML, .4);
			var lighter = lighterColor("#"+$('mHEX').innerHTML, .2);
			//alert(darker);
		    jQuery('#origami-slider').css("background-color", "#" + $('mHEX').innerHTML);
		    
		    jQuery('body').css("background-color", ''+darker+'');
		    jQuery('#origami-slider').css("border-bottom:",'solid 5px '+darker+'');
		    jQuery('#origami-slider').css("border-top-color",''+darker+'');
		    jQuery('#footer').css("background-color", ''+lighter+'');
		    
		    jQuery('#footer').css("border-top", 'solid 4px '+darker+'');
		    jQuery('#footer .button').css("background-color:", ''+lighter+'');
		    
		    //jQuery('#origami-slider { border-bottom: solid 5px #'.$darker.';border-top: solid 5px #'.$darker.'; background: #'.get_option('themeteam_origami_full_color_bg').' url('.get_bloginfo('template_directory').'/images/bg/'.get_option('themeteam_origami_pattern_bg').');}';
			//jQuery('#page-title{ border-bottom: solid 5px #'.$darker.';border-top: solid 5px #'.$darker.'; background: #'.get_option('themeteam_origami_full_color_bg').' url('.get_bloginfo('template_directory').'/images/bg/'.get_option('themeteam_origami_pattern_bg').');}';
			//jQuery('body { background: #'.$darker.';}';
			//jQuery('#footer{background-color:#'.$lighter.';}';
			//jQuery('#footer input[type=text], #footer input[type=email], #footer textarea { background-color:#'.$darker.'; border-radius: 2px; -moz-border-radius:2px; -webkit-border-radius: 2px; border: none; padding: 10px; width: 195px; box-shadow: 1px 1px 0 #'.$darker.'; -moz-box-shadow: 1px 1px 0 #'.$darker.'; -webkit-box-shadow: 1px 1px 0 #'.$darker.'; color:#'.$lighter.';}';
			//jQuery('#footer { border-top: solid 4px #'.$darker.';}';
			//jQuery('#footer, #footer .button { background-color:#'.$lighter.'; }';
		
			//jQuery('.box-v4 header { border: solid 1px #'.get_option('themeteam_origami_full_color_bg').'; border-bottom: solid 5px #'.$darker.'; background:#'.get_option('themeteam_origami_full_color_bg').'; }';
		

		}
		else if(o=='mSize') { var b=Math.max(Math.max(v[0],v[1])+oH,75); color.cords(b);

			$S('demo').height=(b+28)+'px'; $S('demo').width=(b+20)+'px';
...
/* LOAD */
$S('demo').left= (document.body.clientWidth -180)+'px';
$S('demo').top='130px';
toggle('demo','block');
<?php
namespace Flow\Modules\CallToAction;
...
class CallToAction implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		$call_to_action_button_IconCollections = flow_elated_icon_collections()->iconCollections;
		...
		vc_map( array(
				...
				'params' => array_merge(
					...
					flow_elated_icon_collections()->getVCParamsArray(array('element' => 'type', 'value' => array('with-icon'))),
					array(
						array(
							'type' 			=> 'textfield',
						...
						array(
							...
							'value' => array_merge(array('No Icon' => ''),flow_elated_icon_collections()->getIconCollectionsVC()),
							'save_always' => true,
							'dependency' => array('element' => 'show_button', 'value' => array('yes'))
						)
					),
	...
	public function render($atts, $content = null) {
		...
		foreach (flow_elated_icon_collections()->iconCollections as $collection_key => $collection) {

			$call_to_action_icons_form_fields['button_' . $collection->param ] = '';

		}

		$args = array_merge($args, flow_elated_icon_collections()->getShortcodeParams(),$call_to_action_icons_form_fields);
		...
		$html = flow_elated_get_shortcode_module_template_part('templates/call-to-action-template', 'calltoaction', '', $params);
	...
	private function getTextWrapperClasses($params) {
		return ( $params['show_button'] == 'yes') ? 'eltd-call-to-action-column1 eltd-call-to-action-cell' : '';
	}
	...
	private function getCallToActionIcon($params) {
		...
		$icon = flow_elated_icon_collections()->getIconCollectionParamNameByKey($params['icon_pack']);
		$iconStyles = array();
		$iconStyles['icon_attributes']['style'] = $this->getIconStyles($params);
		$call_to_action_icon = '';
		if(!empty($params[$icon])){			
			$call_to_action_icon = flow_elated_icon_collections()->renderIcon( $params[$icon], $params['icon_pack'], $iconStyles );
		}
	...
	private function getButtonParameters($params) {
		...
		if(!empty($params['button_icon_pack'])) {
			...
			$iconPackName = flow_elated_icon_collections()->getIconCollectionParamNameByKey($params['button_icon_pack']);

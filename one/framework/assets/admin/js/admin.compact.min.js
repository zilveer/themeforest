...
else{$.thb.load(url,params);}};$.thb.pageChange=function(url,params){params=$.extend({},{loadingClass:"thb-ajax-loading",preload:true,filter:false,waitfor:false,before:function(){},after:function(){},complete:function(){},afterComplete:function(){},error:function(){}},params);params.afterComplete=function(){if(window.history&&window.history.pushState){window.history.pushState({},'',url);}};$.thb.loadUrl(url,params);};$.thb.loadImage=function(obj,params){params=$.extend({},{allLoaded:function(){},imageLoaded:function(){}},params);var images=[],loadedImages=0;function imgSrc(img){var src=img.attr('src'),lazy=(src===''||src==='data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==')&&img.data('src')&&img.data('src')!=='';if(lazy){src=img.data('src');}
...
$('.tiptop').css({'top':top,'bottom':bottom,'left':left,'right':right});});}};$.fn[pluginName]=function(options){return this.each(function(){if(!$.data(this,pluginName)){$.data(this,pluginName,new TipTop(this,options));}});};})(jQuery,window,document);;var THB_Template=function(template_string,data){var version=parseFloat(thb.wp_version),settings={evaluate:/<#([\s\S]+?)#>/g,interpolate:/\{\{\{([\s\S]+?)\}\}\}/g,escape:/\{\{([^\}]+?)\}\}(?!\})/g};if(version<4.5){return _.template(template_string,data,settings);}
var template_function=_.template(template_string,settings);return function(data){return template_function(data);};}
...
var media_popup=wp.media({multiple:defaults.multiple,title:defaults.title,library:{type:'image'}});media_popup.states.models[0].on('select',function(){self.close(this);});media_popup.on('open',function(){var selection=media_popup.state().get('selection');if(options.ids){jQuery.each(options.ids,function(id){attachment=wp.media.attachment(options.ids[id]);attachment.fetch();selection.add(attachment?[attachment]:[]);});}});media_popup.open();},close:function(frame){var selection=frame.get('selection'),images=[];jQuery.each(selection.models,function(){images.push({url:this.attributes.url,id:this.attributes.id});});if(images.length>0){this.valorize(images);}},valorize:function(images){}});var THB_DuplicableContainer=Backbone.View.extend({el:".thb-fields-container.duplicable",initialize:function(){var self=this;this.$el.data("container",this);this.fields_container=this.$('.thb-container');this.fields_counter=this.$('.thb-field').length;this.static_counter=this.fields_counter;if(this.$el.hasClass('sortable')){var sortable_args={distance:10,helper:'clone',stop:function(){self.reorderFields();}};if(this.$el.hasClass('withHandle')){sortable_args["handle"]=".handle";}
...
return true;});this.obj.find(".thb-modal-close").on("click",function(){self.hide();return false;});this.obj.find("form").on("submit",function(){return false;});this.obj.find(".thb-btn-save").on("click",function(){self.close();return false;});this.obj.show();$.ajax({'method':'POST','url':ajaxurl,'data':{'action':'thb_modal_content','data':data,'slug':slug},'success':function(html){self.obj.find("form").html(html);self.obj.addClass("thb-tabs-count-"+self.obj.find(".thb-tab-content").length);if(self.obj.find(".thb-tab-content").length>1){self.obj.addClass("thb-modal-has-tabs");}
setTimeout(function(){thb_boot_fields(self.obj);self.obj.removeClass("thb-modal-loading");},10);}});};this.hide=function(){if(onHide!==undefined){var serialized=this.obj.find("form").serializeObject();onHide(serialized,this);}
...
modal.open(modal_data);return false;});field.find(".thb-row").each(function(){self.bindRow(field,$(this));});};this.updateData=function(){var builder_container=$("#thb-fields-container-thb_builder_rows"),fields=builder_container.find(".thb-field-section");$.each(fields,function(i,field){field=$(field);var $data=field.find(".thb-section-data"),data={'rows':[],'appearance':{'class':'','width':''}},rows=field.find(".thb-row");$.each(rows,function(k,row){var columns=$(row).find(".thb-column"),row_data={'columns':[]};$.each(columns,function(i,column){var blocks=$(column).find(".thb-block"),column_data={'size':$(column).attr("data-size"),'appearance':$.parseJSON($(column).attr("data-appearance")),'blocks':[]};$.each(blocks,function(j,block){var block_data=$.parseJSON($(block).attr("data-data"));if(block_data.is_title!==undefined&&block_data.is_title=='1'){$(block).addClass("title_block");}
column_data['blocks'].push({'data':block_data,'type':$(block).attr("data-type")});});row_data['columns'].push(column_data);});data['rows'].push(row_data);});var appearance=$.parseJSON($data.attr("data-appearance"));if(appearance!==null){data['appearance']=appearance;}
var param=$.param(data);param=param.replace(/\+/g,'%20');param=param.replace(/\'/g,'%27');$data.val(param);});};this.bind();};$.fn.thbSection=function(){return this.each(function(){$(this).data("thb-field-section",new THB_Section($(this)));});};})(jQuery);(function($){var THB_Slide=function(field){var self=this;this.bind=function(){field.find(".thb-btn-edit").on("click",function(e){var subtype=$(this).attr("data-subtype"),key=$(this).attr("data-key"),title=$(this).attr("title");self.edit(false,key,subtype,title);return false;});field.find(".thb-btn-clone").on("click",function(){self.clone_self();return false;});};this.clone_self=function(){var container=field.parents(".thb-fields-container.duplicable").first().data("container");var cloned_slide=field.clone();field.after(cloned_slide);thb_boot_fields(cloned_slide);container.reorderFields();return false;},this.edit=function(new_slide,key,subtype,title){var modal_key=key+'_edit_slide_'+subtype;if(subtype==""){modal_key=key+'_edit';}
var modal=new THB_Modal(modal_key,title,function(data){$.each(data,function(name,value){field.find("[data-name='"+name+"']").attr("value",value);});if(subtype=="video"){var url=data.id;if(new_slide&&url===""){field.remove();return false;}
...
function thb_add_multiple_slides(container,tpl){var media=new THB_Media();media.valorize=function(images){if(images.length>0){var temp=THB_Template(jQuery("script[data-tpl='"+tpl+"']").html()),template=temp(),view_upload=jQuery(template).find(".thb-view-upload"),size=view_upload.data("image-size");container.addLoadingClass();jQuery.get(ajaxurl,{action:"thb_images_upload_get_sizes",id:_.pluck(images,'id'),size:size},function(srcs){jQuery.each(srcs,function(i){var html=container.addField(tpl),upload_view=html.find(".thb-view-upload"),img=this;if(jQuery(upload_view).data('THB_Upload')!==undefined){jQuery(upload_view).data('THB_Upload').valorize(img,img.image);}});container.removeLoadingClass();});}};media.open({title:jQuery("[data-action='thb_add_multiple_slides']").data('title'),multiple:true});}

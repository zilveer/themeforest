...
class Auto_Importer extends WP_Importer
{
	 ...
	 */
	function do_import($import_atts=true) {

		$this->handle_upload();
		
		if($import_atts==true) $this->fetch_attachments = true;
		else $this->fetch_attachments = false;
...
function auto_import( $args,$att) {
	...
	if($att=='0') $autoimport->do_import(false);
	else $autoimport->do_import(true);

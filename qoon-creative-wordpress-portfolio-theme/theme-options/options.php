...
// Default: Options
$args['menu_title'] = __('Theme Options', "qoon-creative-wordpress-portfolio-theme");

// Set a custom page title for the options page.
// Default: Options
$args['page_title'] = __('Theme Options', "qoon-creative-wordpress-portfolio-theme");
...
add_filter('redux/options/oi_qoon_options/compiler', 'compiler_action', 10, 3);
...
if (!isset($args['global_variable']) || $args['global_variable'] !== false ) {
	...
	$args['intro_text'] = sprintf( __('<p><strong>Theme Options</strong> Panel!</p>', "qoon-creative-wordpress-portfolio-theme" ), $v );
} else {
	$args['intro_text'] = __('<p>This text is displayed above the options panel. It isn\'t required, but more info is always better! The intro_text field accepts all HTML.</p>', "qoon-creative-wordpress-portfolio-theme");
}
...
$redux_path_images = get_template_directory_uri() . '/theme-options/options/img/';
...
include($uri.'/theme-options/options/blog.php');
	include($uri.'/theme-options/options/single.php');
include($uri.'/theme-options/options/portfolio.php');
...
function generate_options_css( $oi_qoon_options ) {
   global $oi_qoon_options;
   ...
   if( $wp_filesystem ) {
        $wp_filesystem->put_contents(
            $css_dir.'options.css', $css, FS_CHMOD_FILE 
        );
    }
...
function oi_theme_css_compiler() {
	global $oi_qoon_options;
	generate_options_css( $oi_qoon_options );
}

function compiler_action($options, $css, $changed_values) {
    global $wp_filesystem;
    $filename = get_template_directory() . '/theme-options/style.css';
...
add_action('redux-compiler-oi_qoon_options', 'oi_theme_css_compiler');
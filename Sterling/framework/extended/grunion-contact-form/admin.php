...
function grunion_post_type_columns_filter( $cols ) {
	$cols = array(
		...
		'feedback_from'		=> __( 'From', 'tt_theme_framework' ),
		'feedback_message'		=> __( 'Message', 'tt_theme_framework' ),
		'feedback_date'			=> __( 'Date', 'tt_theme_framework' )
	);
	...
	switch ( $col ) {
		...
		case 'feedback_message':
			...
			if ( $post->post_status == 'trash' ) {
				...
				echo '"><a title="';
				echo esc_attr__( 'Resore this item from the Trash', 'tt_theme_framework');
				...
				echo '">' . __( 'Restore', 'tt_theme_framework' ) . '</a></span> | ';

				echo "<span class='delete'> <a class='submitdelete' title='";
				echo esc_attr( __( 'Delete this item permanently', 'tt_theme_framework') );
				echo "' href='" . get_delete_post_link( $post->ID, '', true );
				echo "'>" . __( 'Delete Permanently', 'tt_theme_framework' ) . "</a></span>";
?>
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as spam', 'tt_theme_framework');
				...
				echo '">';
				echo '<a class="submitdelete" title="' . esc_attr__( 'Trash', 'tt_theme_framework' );
				echo '" href="' . get_delete_post_link( $post_id );
				echo '">' . __( 'Trash', 'tt_theme_framework' ) . '</a></span>';
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as NOT spam', 'tt_theme_framework' );
...
<?php
		...
		case 'feedback_date':
			echo get_the_date( __( 'Y-m-d @ g:i:s A', 'tt_theme_framework'));
			break;
	}
}
...
function grunion_ajax_spam() {
	...
	if ( !current_user_can("edit_page", $post_id) )
		wp_die( __( 'You are not allowed to manage this item.', 'tt_theme_framework') );
	...
	if ( $_POST['make_it'] == 'spam' ) {
		...
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item out of the Trash.', 'tt_theme_framework') );

		if ( ! wp_untrash_post($post_id) )
			wp_die( __('Error in restoring from Trash.', 'tt_theme_framework') );

	} elseif( $_POST['make_it'] == 'trash' ) {
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item to the Trash.', 'tt_theme_framework') );

		if ( ! wp_trash_post($post_id) )
			wp_die( __('Error in moving to Trash.', 'tt_theme_framework') );
	...
	if ( isset( $status['publish'] ) ) {
		...
		$status_html .= '>' . __( 'Messages', 'tt_theme_framework' ) . ' <span class="count">';
	...
	if ( isset( $status['trash'] ) ) {
		...
		$status_html .= '>' . __( 'Trash', 'tt_theme_framework' ) . ' <span class="count">';
	...
	if ( isset( $status['spam'] ) ) {
		...
		$status_html .= '>' . __( 'Spam', 'tt_theme_framework' ) . ' <span class="count">';

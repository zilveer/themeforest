...
if ( is_admin() ) {
	...
	$of_page= 'appearance_page_ss-theme-options';
...
if ( ! function_exists( 'optionsframework_mlu_init' ) ) :
	...
	function optionsframework_mlu_init () {
		register_post_type( 'optionsframework', array(
			'labels'            => array(
				'name' => __( 'Theme Options Media', 'ss_framework' ),
			),
			'public'            => true,
			'show_ui'           => false,
...
if ( ! function_exists( 'optionsframework_medialibrary_uploader' ) ) :
	...
	function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload', 'ss_framework' ) . '" rel="' . $int . '" />' . "\n";
		...
		if ( $value != '' ) { 
			$remove = '<a href="javascript:(void);" class="mlu_remove button">' . __( 'Remove', 'ss_framework' ) . '</a>';
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'ss_framework' );
...
if ( ! function_exists( 'optionsframework_mlu_js_popup' ) ) :
	...
	jQuery(function($) {
		...
		$( '.savesend input.button[value*="<?php _e( 'Insert into Post', 'ss_framework' ); ?>"], .media-item #go_button' ).attr( 'value', '<?php _e( 'Use this File', 'ss_framework' ); ?>' );
...
if ( ! function_exists( 'optionsframework_mlu_modify_tabs' ) ) :
	...
	function optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', 'ss_framework' ), __( 'Previously Uploaded', 'ss_framework' ), $tabs['gallery'] );
		return $tabs;
	}

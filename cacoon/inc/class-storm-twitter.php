	...
	class StormTwitter {
		...
		private function cropTweets($result, $count) {
			if( is_array($result) ){
				...
				$last_error = 'Something went wrong with the twitter request!';
				$return = array('error' => $last_error);
			}
		...
		private function checkValidCache($screenname,$options) {
			$cache_option_id = 'mc_tweetcache_'.trim( esc_attr($screenname) );

			if( get_option( $cache_option_id ) ){
				$cache = get_option( $cache_option_id );
				$cache = @json_decode( base64_decode($cache), true );

				if ( !isset($cache) ) {
					delete_option( $cache_option_id );
					return false;
				}
				...
				if ( !isset($cache[$cachename]['time']) || !isset($cache[$cachename]['tweets']) ) {
					delete_option( $cache_option_id );
					add_option( $cache_option_id, base64_encode( json_encode($cache) ) );
					return false;
				}
		...
		private function oauthGetTweets($screenname, $options) {
			...
			if ( ( is_array($result) && !isset($result['errors']) ) || ( !isset($result->errors) ) ) {

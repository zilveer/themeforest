...
function optionsframework_admin_init() 
{
	...
	global $of_options, $options_machine, $options_data, $smof_details;
	...
	do_action('optionsframework_admin_init_before', array(
			...
			'smof_data'			=> $options_data
		));
	
	if (empty($options_data['smof_init'])) { // Let's set the values if the theme's already been active
...
function of_ajax_callback() 
{
	...
	elseif($save_type == 'restore_options')
	{
		...
		$options_data = of_get_options(BACKUPS);

		of_save_options($options_data);
	...
	elseif($save_type == 'import_options'){
		...
		$options_data = unserialize(base64_decode($_POST['data'])); //100% safe - ignore theme check nag
		of_save_options($options_data);

 ...
 * @return array
 */
function of_get_options($key = null, $unik_data = null) {
	global $smof_data;

	do_action('of_get_options_before', array(
		'key'=>$key, 'data'=>$unik_data
	));
	if ($key != null) { // Get one specific value
		$unik_data = get_theme_mod($key, $unik_data);
	} else { // Get all values
		$unik_data = get_theme_mods();	
	}
	$unik_data = apply_filters('of_options_after_load', $unik_data);
	if ($key == null) {
		$smof_data = $unik_data;
	} else {
		$smof_data[$key] = $unik_data;
	}
 ...
 *
 * @param $unik_data Options array to save
 * @author Jonah Dahlquist
 * @since 1.4.0
 * @uses update_option()
 * @return void
...
function of_save_options($unik_data, $key = null) {
	global $smof_data;
    if (empty($unik_data))
        return;	
	...
	$unik_data = apply_filters('of_options_before_save', $unik_data);
	if ($key != null) { // Update one specific value
		if ($key == BACKUPS) {
			unset($unik_data['smof_init']); // Don't want to change this.
		}
		set_theme_mod($key, $unik_data);
	} else { // Update all values in $unik_data
		foreach ( $unik_data as $k=>$v ) {

...
<form id="yith-wcwl-form" action="<?php echo esc_url( YITH_WCWL()->get_wishlist_url( 'view' . ( $wishlist_meta['is_default'] != 1 ? '/' . $wishlist_meta['wishlist_token'] : '' ) ) ) ?>" method="post">
    ...
    ?>
        <div class="wishlist-title <?php echo ( $wishlist_meta['is_default'] != 1 && $is_user_owner ) ? 'wishlist-title-with-form' : ''?>">
            <?php echo apply_filters( 'yith_wcwl_wishlist_title', '<h3 class="headline" style="margin-top:0px;">' . $page_title . '</h3><span class="line" style="margin-bottom:35px;"></span><div class="clearfix"></div>' ); ?>
    ...
    <table class="shop_table cart cart-table wishlist_table responsive-table" cellspacing="0" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo ( is_user_logged_in() ) ? esc_attr( $wishlist_meta['ID'] ) : '' ?>" data-token="<?php echo ( ! empty( $wishlist_meta['wishlist_token'] ) && is_user_logged_in() ) ? esc_attr( $wishlist_meta['wishlist_token'] ) : '' ?>">
        ...
        <tr>
            ...
            <th class="product-name">
                <span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Product Name', 'trizzy' ) ) ?></span>
            </th>
            ...
            <?php if( $show_price ) : ?>
                ...
                <th class="product-price">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Unit Price', 'trizzy' ) ) ?>
                    </span>
                </th>
            ...
            <?php if( $show_stock_status ) : ?>
                ...
                <th class="product-stock-stauts">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock Status', 'trizzy' ) ) ?>
                    </span>
                </th>
        ...
        if( count( $wishlist_items ) > 0 ) :
                ...
                if( $product !== false && $product->exists() ) :
                    ...
                    <tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>">
                        ...
                        <td class="product-remove">
                            <div>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove remove_from_wishlist" title="<?php _e( 'Remove this product', 'trizzy' ) ?>">&times;</a>
                            </div>
                        </td>
                        <?php endif; ?>
                        ...
                        <td class="product-add-to-cart">
                            ...
                            if( $show_dateadded && isset( $item['dateadded'] ) ):
                                echo '<span class="dateadded">' . sprintf( __( 'Added on : %s', 'trizzy' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
                            endif;
                            ?>
                            ...
                            <select class="change-wishlist selectBox">
                                <option value=""><?php _e( 'Move', 'trizzy' ) ?></option>
                                <?php
                                ...
                                ?>
                                    <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
                                        ...
                                        if( $wl['wishlist_privacy'] == 1 ){
                                            $wl_privacy = __( 'Shared', 'trizzy' );
                                        }
                                        elseif( $wl['wishlist_privacy'] == 2 ){
                                            $wl_privacy = __( 'Private', 'trizzy' );
                                        }
                                        else{
                                            $wl_privacy = __( 'Public', 'trizzy' );
                                        }
                            ...
                            <?php if( $is_user_owner && $repeat_remove_button ): ?>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php _e( 'Remove this product', 'trizzy' ) ?>"><?php _e( 'Remove', 'trizzy' ) ?></a>
                            <?php endif; ?>
                        </td>
                    <?php endif; ?>
                    </tr>
            ...
            <tr>
                <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php _e( 'No products were added to the wishlist', 'trizzy' ) ?></td>
            </tr>
        <?php
        endif;
            ...
            <td colspan="<?php echo esc_attr( $column_count ) ?>">
                <?php if( $show_cb ) : ?>
                    <div class="custom-add-to-cart-button-cotaniner">
                        <a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_meta['wishlist_token'] ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', __( 'Add the selected products to the cart', 'trizzy' ) ) ?></a>
                    </div>
                <?php endif; ?>
                ...
                <?php if ( is_user_logged_in() && $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
                    <div class="ask-an-estimate-button-container">
                        ...
                        <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', __( 'Ask for an estimate', 'trizzy' ) ) ?>
                    </a>
                    </div>
                <?php endif; ?>
    ...
    <div id="ask_an_estimate_popup">
        <form action="<?php echo $ask_estimate_url ?>" method="post" class="wishlist-ask-an-estimate-popup">
            ...
            <button class="btn button ask-an-estimate-button ask-an-estimate-button-popup" >
                <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_icon', '<i class="fa fa-shopping-cart"></i>' )?>
                <?php _e( 'Ask for an estimate', 'trizzy' ) ?>
            </button>
        </form>
    </div>
<?php endif; ?>
	...
	do_action( 'woothemes_testimonials_before', $args );
		...
		if ( ! is_wp_error( $query ) && is_array( $query ) && count( $query ) > 0 ) {
			...
			if ( $args['pagination'] == true && count( $query ) > 1 && $args['effect'] != 'none' ) {
				...
				$html .= '<a href="#" class="btn-prev">' . apply_filters( 'woothemes_testimonials_prev_btn', '&larr; ' . __( 'Previous', ET_DOMAIN ) ) . '</a>' . "\n";
		        $html .= '<a href="#" class="btn-next">' . apply_filters( 'woothemes_testimonials_next_btn', __( 'Next', ET_DOMAIN ) . ' &rarr;' ) . '</a>' . "\n";
		        $html .= '</div><!--/.pagination-->' . "\n";
			}

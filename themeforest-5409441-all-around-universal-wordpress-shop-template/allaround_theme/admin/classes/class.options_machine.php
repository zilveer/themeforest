...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'slider':
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slide_add_button">' . __('Add New Link', 'allaround') . '</a></div>';
	...
	public static function optionsframework_contact_function($id,$std,$oldorder,$order,$int){
		...
		if (!empty($val['name'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>'. __('Team Member', 'allaround') .' '. $order .'</strong>';
		}
		...
		$slider .= '<a href="#" class="button network_add_button">'. __('Add Network', 'allaround').'</a>';

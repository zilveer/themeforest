...
class CS_Tweets_Widget extends WP_Widget {
	...
	function __construct()
	{
		...
		parent::__construct('cs-tweets-widget', esc_html__('CS Twitter','wp_nuvo'), $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance)
	{
		...
		$transName = '_list_tweets_'.$args['widget_id'];
		
		$twitter_data = get_transient($transName);
		...
		if(!$twitter_data) {
			...
			$twitter_data = wp_remote_retrieve_body($response);
			
			set_transient($transName, $twitter_data, 60);
		}
		
		$twitter = json_decode($twitter_data, true);
	...
	function form($instance)
	{
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id('show_date'); ?>" class="widefat" name="<?php echo $this->get_field_name('show_date'); ?>">
				<option value="no" <?php if($instance['show_date'] == 'no'){ echo "selected='selected'"; } ?>><?php echo esc_html__('No','wp_nuvo'); ?></option>
				<option value="yes" <?php if($instance['show_date'] == 'yes'){ echo "selected='selected'"; } ?>><?php echo esc_html__('Yes','wp_nuvo'); ?></option>
			</select>
		</p>

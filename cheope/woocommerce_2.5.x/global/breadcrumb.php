...
$wrap_before = '<p id="yit-breadcrumb" itemprop="breadcrumb">';
$wrap_after = '</p>';
...
if ( get_option('woocommerce_prepend_shop_page_to_urls') == "yes" && wc_get_page_id( 'shop' ) && get_option( 'page_on_front' ) !== wc_get_page_id( 'shop' ) )
	$prepend =  $before . '<a href="' . get_permalink( wc_get_page_id('shop') ) . '">' . get_the_title( wc_get_page_id('shop') ) . '</a> ' . $after . $delimiter;
else
	$prepend = '';

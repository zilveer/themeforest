...
class pulp_shortcodes
{
	...
	function __construct( $popup )
	{
		...
		{
			$this->append_error( __( 'Config file does not exist', 'morphis' ) );
		}
	}
	...
	function formate_shortcode()
	{
		...
		if( isset( $pulp_shortcodes[$this->popup]['child_shortcode'] ) )
			$this->has_child = true;
		
		if( isset( $pulp_shortcodes ) && is_array( $pulp_shortcodes ) )
		{
			// get shortcode config stuff
			$this->params = $pulp_shortcodes[$this->popup]['params'];
			$this->shortcode = $pulp_shortcodes[$this->popup]['shortcode'];
			$this->popup_title = $pulp_shortcodes[$this->popup]['popup_title'];
			...
			if( isset( $pulp_shortcodes[$this->popup]['no_preview'] ) && $pulp_shortcodes[$this->popup]['no_preview'] )
			{
			...
			foreach( $this->params as $pkey => $param )
			{
				...
				switch( $param['type'] )
				{
					...
					case 'color' :
						...
						// prepare
						$output  = $row_start;						
						$output .= '<div class="farb-popup-wrapper"><input type="text" class="zilla-form-color zilla-input popup-colorpicker" name="' . $pkey . '" id="' . $pkey . '" value="' . $param['std'] . '" />' . "\n";
						$output .= '<div class="farb-popup"><div class="farb-popup-inside"><div id="' . $pkey . 'picker" class="color-picker"></div></div></div></div>';						
						$output .= $row_end;
			...
			{
				// set child shortcode
				$this->cparams = $pulp_shortcodes[$this->popup]['child_shortcode']['params'];
				$this->cshortcode = $pulp_shortcodes[$this->popup]['child_shortcode']['shortcode'];
			
				// popup parent form row start
				$prow_start  = '<tbody>' . "\n";
				$prow_start .= '<tr class="form-row has-child">' . "\n";
				$prow_start .= '<td><a href="#" id="form-child-add" class="button-secondary">' . $pulp_shortcodes[$this->popup]['child_shortcode']['clone_button'] . '</a>' . "\n";
				...
				foreach( $this->cparams as $cpkey => $cparam )
				{
					...
					switch( $cparam['type'] )
					{
						...
						case 'color' :
							...
							// prepare
							$coutput  = $crow_start;						
							$coutput .= '<div class="farb-popup-wrapper"><input type="text" class="zilla-form-color zilla-input popup-colorpicker" name="' . $cpkey . '" id="' . $cpkey . '" value="' . $cparam['std'] . '" />' . "\n";
							$coutput .= '<div class="farb-popup"><div class="farb-popup-inside"><div id="' . $cpkey . 'picker" class="color-picker"></div></div></div></div>';						
							$coutput .= $crow_end;

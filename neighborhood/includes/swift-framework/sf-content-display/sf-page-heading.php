        ...
        function sf_page_heading() {
            ...
            if ( sf_woocommerce_activated() ) {
	            if ( is_wc_endpoint_url( 'view-order' ) ) {
	                $page_title = __('View Order', 'swiftframework');    
	            }
	            if ( is_wc_endpoint_url( 'orders' ) ) {
	                $page_title = __('Orders', 'swiftframework');       
	            }
	            if ( is_wc_endpoint_url( 'downloads' ) ) {
	                $page_title = __('Downloads', 'swiftframework');       
	            }
	            if ( is_wc_endpoint_url( 'edit-account' ) ) {
	                $page_title = __('Edit Account', 'swiftframework');   
	            }
	            if ( is_wc_endpoint_url( 'edit-address' ) ) {
	                $page_title = __('Edit Address', 'swiftframework');
	            }
	            if ( is_wc_endpoint_url( 'lost-password' ) ) {
	                $page_title = __('Lost Password', 'swiftframework');      
	            }
	            if ( is_wc_endpoint_url( 'customer-logout' ) ) {
	                $page_title = __('Logout', 'swiftframework');    
	            }
	            if ( is_wc_endpoint_url( 'order-pay' ) ) {
	                $page_title = __('Order Payment', 'swiftframework');      
	            }
	            if ( is_wc_endpoint_url( 'order-received' ) ) {
	                $page_title = __('Order Received', 'swiftframework');     
	            }
	            if ( is_wc_endpoint_url( 'add-payment-method' ) ) {
	                $page_title = __('Add Payment Method', 'swiftframework');     
	            }
	        }

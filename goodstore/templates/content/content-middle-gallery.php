...
?>
<article id="post-<?php the_ID(); ?>"  <?php post_class(array('element', $class, 'col-lg-4', 'content-middle', 'format-gallery')); ?>   
    ...
    <div class="box ">
        <div class="image ">
            ...
            if (jwUtils::has_post_thumbnail()) {
                ...
                $url = wp_get_attachment_image_src($gallery[0]->id, 'post-size-middle');
                if (isset($url[0])) {
                    $i['url'] = $url[0];
                    ...
                    switch (jwOpt::get_option('std_post_image_clickable', '0')) {
                        case '1': echo '<a href="' . get_permalink() . '"  title="' . jwUtils::crop_length(get_the_title(), jaw_template_get_var('letter_excerpt_title', 60)) . '">';
                            break;
                        case '2': echo '<a href="' . $gallery[0]->url . '"  rel="prettyPhoto[posts-' . jaw_template_get_counter('pagination') . ']" title="' . jwUtils::crop_length(get_the_title(), jaw_template_get_var('letter_excerpt_title', 60)) . '">';
                            break;
                    }
                    ?>
                    <img src="<?php echo $i['url']; ?>" alt="<?php echo $i['caption']; ?>" title="<?php echo $i['caption']; ?>" width="307"  alt="<?php the_title(); ?>"/>
                    <?php
                    if (jwOpt::get_option('std_post_image_clickable', '0') != '0') {
                        echo '</a>';
                    }

...
// Get all ID start with themeum_wi
var elements = [];
jQuery("div[id^='themeum_wi']").each(function(){ elements.push(this.id); });


jQuery.each( elements, function( key, value ) {
  	jQuery( '#'+value ).prepend( '<form  method="POST" id="widget-delete" ><input type="hidden" name="sidebar-id" value="'+value+'" /><input type="hidden" name="action-del" value="themeum_widget_form_del"><input type="submit" value="Delete" /></form>' );
});

<?php
	...
	function gdl_gallery_shortcode( $atts, $content = null ){
		extract( shortcode_atts(array('title'=>'', 'width'=>'200', 'height'=>'200', 'margin'=>'20', 'row_num'=>'100'), $atts) );
		$gdl_gallery = "";
		...
		if( !empty( $slider_xml_string ) ){
			...
			foreach( $slider_xml_dom->documentElement->childNodes as $slider ){
				...
				if( $link_type == 'Link to URL' ){
					...
					$gdl_gallery = $gdl_gallery . '<a data-rel="prettyPhoto[bkpGallery]" href="' . $image_full[0] . '"  title="">';
					$gdl_gallery = $gdl_gallery . '<img class="gdl-gallery-image" src="' . $image_url[0] . '" alt="' . $alt_text . '" />';
					$gdl_gallery = $gdl_gallery . '</a>';
				}else{
					$gdl_gallery = $gdl_gallery . '<img class="gdl-gallery-image" src="' . $image_url[0] . '" alt="' . $alt_text . '" />';
	...
	function gdl_column_shortcode( $atts, $content = null ){
		...
		$is_last = ($last == 'false')? '': '<div class="clear"></div>';
	
		switch($col){
			case '1/4':
				return '<div class="shortcode1-4">' . do_shortcode($content) . '</div>' . $is_last;
			case '1/3':
				return '<div class="shortcode1-3">' . do_shortcode($content) . '</div>' . $is_last;
			case '1/2':
				return '<div class="shortcode1-2">' . do_shortcode($content) . '</div>' . $is_last;
			case '2/3':
				return '<div class="shortcode2-3">' . do_shortcode($content) . '</div>' . $is_last;
			case '3/4':
				return '<div class="shortcode3-4">' . do_shortcode($content) . '</div>' . $is_last;
			default : 
			case '1/1':
				return '<div class="shortcode1">' . do_shortcode($content) . '</div>' . $is_last;
		}			
	...
	function gdl_vimeo_shortcode( $atts, $content = null ){
		...
		$vimeo = $vimeo . '<iframe src="http://player.vimeo.com/video/' . $id[1] . '?title=0&byline=0&portrait=0" width="' . $width . '" height="' . $height . '" frameborder="0" webkitAllowFullScreen allowfullscreen></iframe>';
		$vimeo = $vimeo . '</div>';
	...
	function gdl_button_shortcode( $atts, $content = null ){
		...
		extract( shortcode_atts(array("color" => '', "background" => '', "size" => 'large', "src"=> '#'), $atts) );	
		...
		return '<a href="' . $src . '" class="gdl-button shortcode-' . $size . '-button" style="color:' . $color . '; background-color:' . $background . '; border-color:' . $button_border . '; ">' . $content . '</a>';
	}	
	...
	function gdl_frame_shortcode( $atts ){
		...
		$frame = "<img src='" . $src . "' width='" . $width . "' height='" . $height . "' alt='' />";
		...
		$frame = "<div class='gdl-image-frame shortcode-image-" . $align . "' style='float: " . $align . ";'>" . $frame . "</div>";
	...
	function fix_shortcodes($content){   
		...
		$shortcode_tags = array(
			'gdl_gallery' => 'gdl_gallery_shortcode', 'column' => 'gdl_column_shortcode', 'accordion' => 'gdl_accordion_shortcode', 'acc_item' => 'gdl_acc_item_shortcode', 'toggle_box' => 'gdl_toggle_box_shortcode', 'toggle_item' => 'gdl_toggle_item_shortcode', 'tab' => 'gdl_tab_shortcode', 'tab_item' => 'gdl_tab_item_shortcode', 'divider' => 'gdl_divider_shortcode', 'space' => 'gdl_space_shortcode', 'youtube' => 'gdl_youtube_shortcode', 'vimeo' => 'gdl_vimeo_shortcode', 'button' => 'gdl_button_shortcode', 'list' => 'gdl_list_shortcode', 'social' => 'gdl_social_shortcode', 'quote' => 'gdl_quote_shortcode', 'dropcap' => 'gdl_dropcap_shortcode', 'testimonial' => 'gdl_testimonial_shortcode', 'message_box' => 'gdl_message_box_shortcode', 'frame' => 'gdl_frame_shortcode', 'price-item' => 'gdl_price_item_shortcode'
		);

...
function optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		case 'number':
			...
			//assign the additional HTML5 attributes to the inputs of type number
			if($value['id'] === 'slider_height') {
			    $attr = 'max="1000" min="100" step="1"';
			}
			elseif($value['id'] === 'transition_speed' || $value['id'] === 'pf_transition_speed') {
				$attr = 'min="0" step="50"';
			}
			elseif($value['id'] === 'slider_pause' || $value['id'] === 'pf_slider_pause') {
				$attr = 'max="10000" min="0" step="50"';
			}
			elseif($value['id'] === 'pp_default_width' || $value['id'] === 'pp_default_height') {
			    $attr = 'max="2000" min="100" step="1"';
			}
			else {
				$attr = 'min="50" step="50"';
			}
			   
			$output .= '<input id="' . esc_attr( $value['id'] ) . '" type="number" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" ' . $attr . ' value="' . esc_attr( $val ) . '" />';
		break;

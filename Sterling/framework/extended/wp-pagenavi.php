...
//mod by denzel, add in $custom_query variable to allow replace of $wp_query

function wp_pagenavi($custom_query = '',$before = '', $after = '') {

//mod by denzel to replace $wp_query with $custom_query if there is any...
if($custom_query){
global $wpdb;
...
//	global $wpdb, $wp_query; //comment out original code, put in if statement above, no changes to codes below!
			...
			switch(intval($pagenavi_options['style'])) {
				...
				if(!empty($pages_text)) {
					...
					if ($start_page >= 2 && $pages_to_show < $max_page) {
						...
						echo '<a href="'.esc_url(get_pagenum_link()).'" class="first" title="&laquo;">&laquo;</a>';
					...
					if ($end_page < $max_page) {
						...
						echo '<a href="'.esc_url(get_pagenum_link($max_page)).'" class="last" title="&raquo;">&raquo;</a>';
					}
					break;
				case 2;
...
function pagenavi_init() {
	...
	$pagenavi_options['pages_text'] = __('Page %CURRENT_PAGE% of %TOTAL_PAGES%','tt_theme_framework');
	$pagenavi_options['current_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['page_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['first_text'] = __('&laquo;','tt_theme_framework');
	$pagenavi_options['last_text'] = __('&raquo;','tt_theme_framework');
	$pagenavi_options['next_text'] = __('&raquo;','tt_theme_framework');
	$pagenavi_options['prev_text'] = __('&laquo;','tt_theme_framework');
	$pagenavi_options['dotright_text'] = __('...','tt_theme_framework');
	$pagenavi_options['dotleft_text'] = __('...','tt_theme_framework');

...
if(!class_exists('AQ_Block')) {
	class AQ_Block {
		...
		function form_callback($instance = array()) {
			...
			if(isset($instance['content']))$instance['content']=str_replace(";cbsp#21&;","\r\n",$instance['content']);
			if(isset($instance['content']))$instance['content']=str_replace(";cbsp#21;","\r\n",$instance['content']);
			//display the block
		...
		function after_form($instance) {
			...
			$cols_o=array(''=>'none','fadein'=>'Fade','slideleft'=>'Slide Left','slideright'=>'Slide Right','slidetop'=>'Slide Top','slidedown'=>'Slide Down');	?>
<div class="cl"></div>
<p class="description">
	<label for="<?php echo $this->get_field_id('fade') ?>">Fade effect<br />
	<?php echo aq_field_select('fade', $block_id, $cols_o, $fade) ?> </label>
</p>
	<?php
	...
	echo '</div>',
		...
		function before_block($instance) {
			...
			$fade_class = $fade ? 'animatefade-'.$fade : '';
			echo '<div id="aq-block-'.$template_id.'-'.$number.'" class="aq-block aq-block-'.$id_base.' aq_'.$size.' '.$column_class.' '.$fade_class.' cf">';
		}

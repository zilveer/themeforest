...
class Woothemes_Testimonials {
	...
	public function register_post_type () {
		$labels = array(
			'name' => _x( 'Testimonials', 'post type general name', ETHEME_DOMAIN ),
			'singular_name' => _x( 'Testimonial', 'post type singular name', ETHEME_DOMAIN ),
			'add_new' => _x( 'Add New', 'testimonial', ETHEME_DOMAIN ),
			'add_new_item' => sprintf( __( 'Add New %s', ETHEME_DOMAIN ), __( 'Testimonial', ETHEME_DOMAIN ) ),
			...
			'parent_item_colon' => '',
			'menu_name' => __( 'Testimonials', ETHEME_DOMAIN )
	...
	public function register_custom_column_headings ( $defaults ) {
		$new_columns = array( 'image' => __( 'Image', ETHEME_DOMAIN ) );
	...
	public function updated_messages ( $messages ) {
	  ...
	  $messages[$this->token] = array(
	    ...
	    1 => sprintf( __( 'Testimonial updated. %sView testimonial%s', ETHEME_DOMAIN ), '<a href="' . esc_url( get_permalink( $post_ID ) ) . '">', '</a>' ),
	    2 => __( 'Custom field updated.', ETHEME_DOMAIN ),
	    3 => __( 'Custom field deleted.', ETHEME_DOMAIN ),
	    4 => __( 'Testimonial updated.', ETHEME_DOMAIN ),
	    /* translators: %s: date and time of the revision */
	    5 => isset($_GET['revision']) ? sprintf( __( 'Testimonial restored to revision from %s', ETHEME_DOMAIN ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
	    6 => sprintf( __( 'Testimonial published. %sView testimonial%s', ETHEME_DOMAIN ), '<a href="' . esc_url( get_permalink( $post_ID ) ) . '">', '</a>' ),
	    7 => __('Testimonial saved.', ETHEME_DOMAIN),
	    8 => sprintf( __( 'Testimonial submitted. %sPreview testimonial%s', ETHEME_DOMAIN ), '<a target="_blank" href="' . esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) . '">', '</a>' ),
	    9 => sprintf( __( 'Testimonial scheduled for: %1$s. %2$sPreview testimonial%3$s', ETHEME_DOMAIN ),
	      // translators: Publish box date format, see http://php.net/date
	      '<strong>' . date_i18n( __( 'M j, Y @ G:i', ETHEME_DOMAIN  ), strtotime( $post->post_date ) ) . '</strong>', '<a target="_blank" href="' . esc_url( get_permalink($post_ID) ) . '">', '</a>' ),
	    10 => sprintf( __( 'Testimonial draft updated. %sPreview testimonial%s', ETHEME_DOMAIN ), '<a target="_blank" href="' . esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) . '">', '</a>' ),
	  );
	...
	public function meta_box_setup () {
		add_meta_box( 'testimonial-data', __( 'Testimonial Details', ETHEME_DOMAIN ), array( &$this, 'meta_box_content' ), $this->token, 'normal', 'high' );
	} // End meta_box_setup()
	...
	public function enter_title_here ( $title ) {
		if ( get_post_type() == $this->token ) {
			$title = __( 'Enter the customer\'s name here', ETHEME_DOMAIN );
		}
		return $title;
	...
	public function get_custom_fields_settings () {
		...
		$fields['gravatar_email'] = array(
		    'name' => __( 'Gravatar E-mail Address', ETHEME_DOMAIN ),
		    'description' => sprintf( __( 'Enter in an e-mail address, to use a %sGravatar%s, instead of using the "Featured Image".', ETHEME_DOMAIN ), '<a href="' . esc_url( 'http://gravatar.com/' ) . '" target="_blank">', '</a>' ),
		    'type' => 'text',
		    'default' => '',
		    'section' => 'info'
		);
		...
		$fields['byline'] = array(
		    'name' => __( 'Byline', ETHEME_DOMAIN ),
		    'description' => __( 'Enter a byline for the customer giving this testimonial (for example: "CEO of WooThemes").', ETHEME_DOMAIN ),
		    'type' => 'text',
		    'default' => '',
		    'section' => 'info'
		);
		...
		$fields['url'] = array(
		    'name' => __( 'URL', ETHEME_DOMAIN ),
		    'description' => __( 'Enter a URL that applies to this customer (for example: http://woothemes.com/).', ETHEME_DOMAIN  ),
		    'type' => 'url',
		    'default' => '',
		    'section' => 'info'
		);

...
function storeAddress(){
    ...
    // Validation
    if(!$_GET['email']){ return __("No email address provided", BFI_I18NDOMAIN); } 

    if(!preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*$/i", $_GET['email'])) {
        return __("Email address is invalid", BFI_I18NDOMAIN); 
    }

    // grab an API Key from http://admin.mailchimp.com/account/api/
    $api = new MCAPI(bfi_get_option(BFI_OPTIONMAILCHIMPAPIKEY));
    
    // grab your List's Unique Id by going to http://admin.mailchimp.com/lists/
    // Click the "settings" link for the list - the Unique Id is at the bottom of that page. 
    $list_id = bfi_get_option(BFI_OPTIONMAILCHIMPLISTKEY);

    if($api->listSubscribe($list_id, $_GET['email'], '') === true) {
        // It worked!
        return __('Success! Please check your email to confirm your sign up.', BFI_I18NDOMAIN) . 
            "<span class='success' style='display: none; width: 0;'></span>";
    }else{
        // An error ocurred, return error message   
        return sprintf(__('Error: %s', BFI_I18NDOMAIN), $api->errorMessage);
    }
    
}

// If being called via ajax, autorun the function
if(array_key_exists('ajax', $_GET) && $_GET['ajax']){ echo storeAddress(); }

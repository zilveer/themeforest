	...
	class MAD_ADMIN_PAGES extends MAD_FRAMEWORK {
		...
		function admin_menu() {
			...
			foreach ($this->globalObject->option_pages as $key => $data_set) {
				if ($key === 0) {
					...
					$page = add_menu_page( $the_title, $the_title, 'manage_options', 'mad', array(&$this, 'create_page'));
				}
		...
		public function localize_popup_text() {
			wp_localize_script('modernizr', 'madLocalize', array(
				'errorText' => __('Data is not preserved!', 'flatastic'),
				'successText' => __('All options are saved successfully!', 'flatastic'),
				'importsuccessText' => __('Import demo successfully!', 'flatastic'),
				'importsuccessOptions' => __('Import options successfully!', 'flatastic'),
				'resetText' => __('Are you sure you want to delete all of the options?', 'flatastic'),
				'importText' => __('By importing the dummy data all your current theme option settings will be overwritten. Continue anyway?', 'flatastic')
			));
		}

...
*/
class BoxyWidgetBase extends WP_Widget {
...
function widget_field_text($obj, $instance, $fieldname, $fieldtitle) {
	...
	?>
	<p>
		<label for="<?php echo esc_attr($obj->get_field_id($fieldname)); ?>"><?php echo esc_attr($fieldtitle); ?>:</label>
		<input class="widefat" id="<?php echo esc_attr($obj->get_field_id($fieldname)); ?>" name="<?php echo esc_attr($obj->get_field_name($fieldname)); ?>" type="text" value="<?php echo esc_attr($value); ?>" />
	</p>
	<?php
}
...
function widget_field_textarea($obj, $instance, $fieldname, $fieldtitle) {
	...
	<p>
		<label for="<?php echo esc_attr($obj->get_field_id($fieldname)); ?>"><?php echo esc_attr($fieldtitle); ?>:</label>
		<textarea class="widefat" id="<?php echo esc_attr($obj->get_field_id($fieldname)); ?>" name="<?php echo esc_attr($obj->get_field_name($fieldname)); ?>" style="height: 150px;" type="text"><?php echo esc_attr($value); ?></textarea>
	</p>
	<?php
}

function widget_field_desc($obj, $instance, $fieldname, $fieldtitle) {
	?><p><small style="display:block; line-height:14px; font-size:11px; color:#666; padding:0 2px;"><?php echo esc_attr($fieldtitle); ?></small></p><?php
}
...
function widget_field_select($obj, $instance, $fieldname, $fieldtitle, $field_array) {
	...
	?>
	<p>
		<label for="<?php echo esc_attr($obj->get_field_id($fieldname)); ?>"><?php echo esc_attr($fieldtitle); ?>:</label><br />
		<select name="<?php echo esc_attr($obj->get_field_name($fieldname)); ?>" id="<?php echo esc_attr($obj->get_field_id($fieldname)); ?>" style="width: 100%;">
			<?php foreach ($field_array['options'] as $val => $name) : ?>
				<option value="<?php echo esc_attr($val); ?>" <?php echo ($val == esc_attr($value)) ? 'selected="selected"' : ''; ?>><?php echo esc_attr($name); ?></option>
			<?php endforeach; ?>
		</select>
	</p>
	<?php
...
function widget_field_customCategories($obj, $instance, $fieldname, $fieldtitle, $field_array){
	...
	<p>
		<label for="<?php echo esc_attr($obj->get_field_id($fieldname)); ?>"><?php echo esc_attr($fieldtitle); ?>:</label><br />
		<select multiple size="5" name="<?php echo esc_attr($obj->get_field_name($fieldname)); ?>[]" id="<?php echo esc_attr($obj->get_field_id($fieldname)); ?>" style="height:98px; width: 100%;">
			<?php foreach ($categories as $category) : ?>
				<option value="<?php echo esc_attr($category->cat_ID); ?>" <?php if (!empty($value) && in_array($category->cat_ID,$value)) { echo 'selected="selected"'; } ?>><?php echo esc_attr($category->name); ?></option>
			<?php endforeach; ?>
		</select>
	</p>
	<?php }
	...
	<p>
		...
		<select name="<?php echo esc_attr($obj->get_field_name($fieldname)); ?>[]" id="<?php echo esc_attr($obj->get_field_id($fieldname)); ?>" style="width: 100%;">
			...
			<?php foreach ($posts as $post) : ?>
				<option value="<?php echo esc_attr($post->ID); ?>" <?php if (!empty($value) && in_array($post->ID,$value)) { echo 'selected="selected"'; } ?>><?php echo get_the_title($post->ID); ?></option>
			<?php endforeach; ?>
		</select>
	</p>
	<?php
...
function widget_field_set($obj, $instance, $fieldname, $fieldtitle, $field_array) {
	...
	<p>
		<label for="<?php echo esc_attr($obj->get_field_id($fieldname)); ?>"><?php echo esc_attr($fieldtitle); ?>:</label><br />
		<?php foreach ($field_array['options'] as $val => $name) : ?>
			<input type="checkbox" name="<?php echo esc_attr($obj->get_field_name($fieldname)); ?>[]" value="<?php echo esc_attr($val); ?>" <?php echo (!(in_array($val, $value) === FALSE)) ? 'checked="checked"' : ''; ?>>&nbsp;<?php echo esc_attr($name); ?><br />
		<?php endforeach; ?>
	</p>
	<?php
}

...
while ( $my_query->have_posts() ) {
    ...
    if($this->getTeaserData('enable', $post->id) === '1') {
        ...
        $post->content = wpautop(get_the_content()); //smoked
        $post->excerpt = wpautop(get_the_excerpt());
        $post->thumbnail_data = $this->getPostThumbnail($post->id, $grid_thumb_size);
        $post->thumbnail = $post->thumbnail_data && isset($post->thumbnail_data['thumbnail']) ? $post->thumbnail_data['thumbnail'] : '';
...
<div class="<?php echo apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $css_class, $this->settings['base']) ?>">
    <div class="wpb_wrapper">
        ...
        <div class="teaser_grid_container">
            ...
            ?>
            <ul class="genre-filter open categories_filter vc_clearfix">
            ...
            <ul class="wpb_thumbnails wpb_thumbnails-fluid vc_clearfix" data-layout-mode="<?php echo $grid_layout_mode ?>">
                ...
                <?php foreach($posts as $post): ?>
                    ...
                    <?php if($post->post_type === A13_CUSTOM_POST_TYPE_WORK || $post->post_type === A13_CUSTOM_POST_TYPE_GALLERY) : ?>
                        <li class="isotope-item g-item <?php echo apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $li_span_class, 'vc_teaser_grid_li').$post->categories_css ?>"<?php echo $block_style ?>>

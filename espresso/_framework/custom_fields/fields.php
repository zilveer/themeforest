...
class ECF_Field {
	...
	function custom_class($custom_class) {
	...
	function render_row($field_html) {
		...
		$custom_class = isset($this->custom_class) ? ' '.$this->custom_class : '';

		$field_has_options = $this->is_multiply || $this->is_subfield;

		$html = '
		<div class="ecf-field-container'.$custom_class.'"> 
			<div class="ecf-label">
            ...
            jQuery(function ($) {
                ...
                function adjust_panels(selectVal){
	            	if (selectVal == 'audio'){
	            		...
	            		$('#button_link_panel').hide();
	            		$('#slide_video_panel').hide();
	            		$('#slide_address_panel').hide();
	            		$('#slide_image_panel').hide();
	            	} else if (selectVal == 'video'){
	            		$('#slide_video_panel').fadeIn('fast');
	            		$('#button_link_panel').hide();
	            		$('#slide_audio_panel').hide();
	            		$('#slide_address_panel').hide();
	            		$('#slide_image_panel').hide();
	            	} else if (selectVal == 'image'){
	            		$('#slide_image_panel').fadeIn('fast');
	            		$('#button_link_panel').fadeIn('fast');
	            		$('#slide_address_panel').hide();
	            		$('#slide_audio_panel').hide();
	            		$('#slide_video_panel').hide();
	            	}
...
class ECF_FieldLinkSelect extends ECF_Field {
	...
	function print_js() {
        ...
        <script type="text/javascript" charset="utf-8">
            jQuery(function ($) {
                ...
                // Hide Everything
                $('input[name="_button_link_custom"]').parents('.ecf-field-container').hide();
                $('select[name="_button_link_page"]').parents('.ecf-field-container').hide();
                $('select[name="_button_link_post"]').parents('.ecf-field-container').hide();
                $('select[name="_button_link_gallery"]').parents('.ecf-field-container').hide();
                $('select[name="_button_link_video"]').parents('.ecf-field-container').hide();
                $('select[name="_button_link_audio"]').parents('.ecf-field-container').hide();
                $('select[name="_button_link_event"]').parents('.ecf-field-container').hide();
                ...
                function adjust_panels(selectVal){
                	if (selectVal == ''){
	            		...
	            		$('input[name="_button_link_custom"]').parents('.ecf-field-container').fadeIn('fast');
	            		$('select[name="_button_link_page"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_post"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_gallery"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_video"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_audio"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_event"]').parents('.ecf-field-container').hide();
	            	} else if (selectVal == 'page'){
	            		$('select[name="_button_link_page"]').parents('.ecf-field-container').fadeIn('fast');
	            		$('input[name="_button_link_custom"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_post"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_gallery"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_video"]').parents('.ecf-field-container').hide();
	            		...
	            		$('select[name="_button_link_post"]').parents('.ecf-field-container').fadeIn('fast');
	            		$('input[name="_button_link_custom"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_page"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_gallery"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_video"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_audio"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_event"]').parents('.ecf-field-container').hide();
					} else if (selectVal == 'gallery'){
	            		$('select[name="_button_link_gallery"]').parents('.ecf-field-container').fadeIn('fast');
	            		$('input[name="_button_link_custom"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_page"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_post"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_video"]').parents('.ecf-field-container').hide();
	            		...
	            		$('select[name="_button_link_video"]').parents('.ecf-field-container').fadeIn('fast');
	            		$('input[name="_button_link_custom"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_page"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_gallery"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_post"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_audio"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_event"]').parents('.ecf-field-container').hide();
					} else if (selectVal == 'audio'){
	            		$('select[name="_button_link_audio"]').parents('.ecf-field-container').fadeIn('fast');
	            		$('input[name="_button_link_custom"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_page"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_gallery"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_video"]').parents('.ecf-field-container').hide();
	            		...
	            		$('select[name="_button_link_event"]').parents('.ecf-field-container').fadeIn('fast');
	            		$('input[name="_button_link_custom"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_page"]').parents('.ecf-field-container').hide();
	            		$('select[name="_button_link_gallery"]').parents('.ecf-field-container').hide();
                		$('select[name="_button_link_video"]').parents('.ecf-field-container').hide();
...
class ECF_FieldFile extends ECF_Field {
	...
	function load() {
		...
		if (isset($_GET['delete_field']) && $_GET['delete_field']==$this->name && isset($_GET['delete_value']) && is_admin() ) {
			...
			header('Location: ' . esc_url_raw(remove_query_arg(array('delete_field', 'delete_value'))));
		}
	}
	function set_value_from_input() {
...
class ECF_FieldMediaImage extends ECF_Field {
	function render() {
		if ( !empty($this->value) && strpos($this->value, 'http://') === false ) {
			$image_dir = site_url().'/wp-content/uploads';
			$this->value = $image_dir.'/'.$this->value;
		}

		$html = '<div class="carbon-field" data-type="Image">';
		...
		$html .= '<span 
				...
				data-window-label="Images"
				data-window-button-label="Select Image"
				data-type="image"
				data-value-type="url"
			>Select Image</span>';
	...
	function description(){
		...
		if ( $this->value != '' ) {
			...
			$description .= '<img src="' . esc_url( $this->value ) . '" alt="" class="carbon-view_image" style="max-width: 150px; max-height: 150px"/>';
			$description .= '<span class="carbon-file-remove">&#215</span>';
			$description .= '</div>';
		} else {
			$description = '<img src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" alt="" class="carbon-view_image blank" style="max-width: 150px; max-height: 150px"/>';
		}

		$description .= '<div class="cl"></div>';

		return '<div class="carbon-description">' . $description . '</div>';
	}
}
...
class ECF_FieldImage extends ECF_FieldFile {
	...
	function get_file_description() {
	    return '<img src="' . home_url() . '/wp-content/uploads/' . $this->value . '" alt="" height="100" class="ecf-view_image"/>';
	}
	function save_file($file) {
		// Build destination path
		$upload_path = $this->get_upload_path();
		...
		if ( move_uploaded_file($file['tmp_name'], $file_dest) != FALSE ) {
			...
			if ( !($this->width == null && $this->height == null)) {
				...
				$resized = wp_get_image_editor( $file['tmp_name'] );
				if ( ! is_wp_error( $resized ) ) {
				    $resized->resize( $this->width, $this->height, true );
				    $resized->save( $file_dest );
				}
...
class ECF_FieldSep extends ECF_Field {
	function render() { return '<hr />'; }
}
?>
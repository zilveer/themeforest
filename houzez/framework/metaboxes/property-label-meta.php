<?php
/**
 * Add metaboxes to property Label
 * Created by PhpStorm.
 * User: waqasriaz
 * Date: 06/08/16
 * Time: 11:25 PM
...
if ( !function_exists( 'houzez_property_label_add_meta_fields' ) ) :
    function houzez_property_label_add_meta_fields() {
        $houzez_meta = houzez_get_property_label_meta();
        ?>
...
add_action( 'property_label_add_form_fields', 'houzez_property_label_add_meta_fields', 10, 2 );
...
if ( !function_exists( 'houzez_property_label_edit_meta_fields' ) ) :
    function houzez_property_label_edit_meta_fields( $term ) {
        $houzez_meta = houzez_get_property_label_meta( $term->term_id );
        ?>
        <?php
...
add_action( 'property_label_edit_form_fields', 'houzez_property_label_edit_meta_fields', 10, 2 );


if ( !function_exists( 'houzez_save_property_label_meta_fields' ) ) :
    function houzez_save_property_label_meta_fields( $term_id ) {
        ...
        if ( isset( $_POST['fave'] ) ) {
            ...
            houzez_update_property_label_colors( $term_id, $houzez_meta['color'], $houzez_meta['color_type'] );
        }

    }
endif;

add_action( 'edited_property_label', 'houzez_save_property_label_meta_fields', 10, 2 );
add_action( 'create_property_label', 'houzez_save_property_label_meta_fields', 10, 2 );

...
if ( !function_exists( 'cg_content_nav' ) ) :
    ...
    function cg_content_nav( $nav_id ) {
        ...
        <nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo $nav_class; ?>">
            <h1 class="screen-reader-text"><?php _e( 'Post navigation', 'commercegurus' ); ?></h1>
        ...
        if ( 'pingback' == $comment->comment_type || 'trackback' == $comment->comment_type ) :
            ...
            <li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
                <div class="comment-body">
                    <?php _e( 'Pingback:', 'commercegurus' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'commercegurus' ), '<span class="edit-link">', '</span>' ); ?>
                </div>
                ...
                <article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
                    <footer class="comment-meta">
                        <div class="comment-author vcard">
                            ...
                            <?php printf( __( '%s <span class="says">says:</span>', 'commercegurus' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
                        </div><!-- .comment-author -->

                        <div class="comment-metadata">
                            <a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
                                <time datetime="<?php comment_time( 'c' ); ?>">
                                    <?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'commercegurus' ), get_comment_date(), get_comment_time() ); ?>
                                </time>
                            </a>
                            <?php edit_comment_link( __( 'Edit', 'commercegurus' ), '<span class="edit-link">', '</span>' ); ?>
                        </div><!-- .comment-metadata -->

                        <?php if ( '0' == $comment->comment_approved ) : ?>
                            <p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'commercegurus' ); ?></p>
                        <?php endif; ?>
                    </footer><!-- .comment-meta -->
...
// Use shortcodes in text widgets.
    ...
    function wpcg_numeric_posts_nav() {

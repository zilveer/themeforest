...
if (!function_exists('mad_widgets_register')) {
	function mad_widgets_register () {
		...
		register_sidebar(array(
			...
			'description'   => __('For all pages and posts.', 'flatastic'),
...
if (!function_exists('mad_add_to_mailchimp_list')) {
	...
	function mad_add_to_mailchimp_list() {
		...
		$result['text']  = __('You\'ve been added to our sign-up list. We have sent an email, asking you to confirm the same.', 'flatastic');
...
if (!class_exists('mad_widget_popular_widget')) {
	...
	class mad_widget_popular_widget extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( 'popular-widget', strtoupper('flatastic') .' '. __('Widget Popular and Latest Posts', 'flatastic'),
				array('classname' => 'widget_popular_posts', 'description' => __("Display most popular and latest posts", 'flatastic'))
			);
		...
		function get_latest_posts() {
			...
			if ($posts == false) {
				...
				$posts = new WP_Query(apply_filters('pop_get_latest_posts_args', $args));
				$posts = $posts->posts;
...
if (!class_exists('mad_widget_social_links')) {
	...
	class mad_widget_social_links extends WP_Widget {
		...
		function __construct() {
			$settings = array('classname' => __CLASS__, 'description' => __('Displays website social links', 'flatastic'));
			parent::__construct(__CLASS__, strtoupper('flatastic') .' '. __('Widget Social Links', 'flatastic'), $settings);
		}
...
if (!class_exists('mad_widget_advertising_area')) {
	...
	class mad_widget_advertising_area extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( __CLASS__,  strtoupper('flatastic').' '. __('Advertising Area', 'flatastic'), $widget_ops );
		}
		...
		function widget($args, $instance) {
			...
			if (empty($instance['image_url'])) {
				$image_url = '<span>'.__('Advertise here', 'flatastic').'</span>';
			} else {
...
if (!class_exists('mad_widget_contact_us')) {
	...
	class mad_widget_contact_us extends WP_Widget {
		...
		function __construct() {
			$settings = array('classname' => 'widget_contact_us', 'description' => __('Displays contact us', 'flatastic'));
			parent::__construct( __CLASS__, strtoupper('flatastic') .' '. __('Widget Contact Us', 'flatastic'), $settings );
		}
		...
		function form($instance) {
			...
			$defaults = array(
				'title' => __('Contact Us', 'flatastic'),
				'address' => __('8901 Marmora Road, Glasgow, D04 89GR.', 'flatastic'),
				'phone' => '800-559-65-80',
				'email' => 'info@companyname.com',
				'hours' => wp_kses(__('Monday - Friday: 08.00-20.00 <br> Saturday: 09.00-15.00 <br> Sunday: closed', 'flatastic'), array('br' => array()))
			);
			$instance = wp_parse_args( (array) $instance, $defaults );
			?>

			<p>
				<label><?php _e('Title', 'flatastic');?>:
					<input id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" class="widefat" type="text" />
				</label>
			</p>

			<p>
				<label><?php _e('Address', 'flatastic');?>:
			...
			<p>
				<label><?php _e('Working hours', 'flatastic');?>:
...
if (!class_exists('mad_widget_mailchimp')) {
	...
	class mad_widget_mailchimp extends WP_Widget {
		...
		function __construct() {
			$settings = array('classname' => 'widget_zn_mailchimp', 'description' => __('Use this widget to add a mailchimp newsletter to your site.', 'flatastic'));
			parent::__construct('widget-zn-mailchimp', strtoupper('flatastic') .' '. __('Widget Newsletter', 'flatastic'), $settings);
...
if (!class_exists('mad_widget_flickr')) {
	...
	class mad_widget_flickr extends WP_Widget {
		...
		function __construct() {
			$settings = array('classname' => 'widget_flickr', 'description' => __('Flickr feed widget', 'flatastic'));
			parent::__construct(__CLASS__,  strtoupper('flatastic') .' '. __('Widget Flickr feed', 'flatastic'), $settings);
		}
		...
		function form($instance) {
			...
			<p>
				<label for="<?php echo $this->get_field_id('username'); ?>"><?php _e('Flickr Username', 'flatastic') ?>:</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('imagescount'); ?>"><?php _e('Number of images', 'flatastic') ?>:</label>

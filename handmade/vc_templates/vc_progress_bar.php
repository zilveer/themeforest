 ...
 */
$output = $layout_style = $title = $values = $units = $bgcolor = $custombgcolor = $customtxtcolor = $options = $el_class = $css = '';
...
wp_enqueue_style('handmade_vc_progress_bar_css', THEME_URL . 'assets/vc-extend/css/vc_progress_bar' . $min_suffix . '.css', array(), false);
...
$output = '<div class="handmade-h-progress-bar ' . esc_attr($layout_style) . ' ' . esc_attr($css_class) . '">';
...
foreach ($graph_lines_data as $line) {
    ...
    if ($layout_style == 'style1') {
        $output .= '<small class="vc_label"' . $line['txtcolor'] . '><p class="progress-bar-title">' . $line['label'] . $unit . '</p></small>';
    } elseif ($layout_style == 'style2') {
        $output .= '<small class="vc_label"' . $line['txtcolor'] . '><p class="progress-bar-title">' . $line['label'] . '</p>' . $unit . '</small>';
    } else {
        $output .= '<small class="vc_label"' . $line['txtcolor'] . '><p class="progress-bar-title">' . $line['label'] . '</p></small>';
    }

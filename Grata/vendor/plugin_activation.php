...
function my_theme_register_required_plugins() {
	...
	$plugins = array(
		array(
			...
			'info_url' 			    => 'http://codecanyon.net/item/slider-revolution-responsive-wordpress-plugin/2751380', // If set, overrides default API URL and points to an external URL
		),
	);
		...
		public function site_transient_update_plugins( $trans ) {
			...
			foreach ( $this->plugins as $plugin ) {
				...
				if ( version_compare( $installed_plugins[ $plugin_basename ]['Version'], $plugin['version'], '<' ) ) {
					...
					$trans->response[ $plugin_basename ]->url         = $plugin['info_url'];

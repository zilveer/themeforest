...
// ALSO CHECK ENVATO ITEM ID
            $purchased_themes  = $this->filter_purchased_themes_by_name($purchased_themes, $theme_name);

// DO NOT CHECK ALL INSTALLED THEMES - CHECK CURRENT THEME VERSION
...
// OR PARENT TEMPLATE IF CHILD

            $current_theme = wp_get_theme();
            $parent_template = $current_theme->template;
						$parent_theme = wp_get_theme($parent_template);
						$version = $parent_theme->Version;
// OK NOW
            $updated_themes = array();
            $is_newer = $this->is_newer_version_available( $version, $purchased_themes[0]->version );
            if($is_newer) $updated_themes[] = $purchased_themes[0];
            $result->updated_themes = $updated_themes;
...
//                if ( $entry->theme_name != $theme_name )
                if ( $entry->theme_name != $theme_name && $entry->item_id != IRON_ENVATO_ITEM_ID)
                    unset($result[$i]);
            }

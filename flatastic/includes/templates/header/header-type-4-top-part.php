...
<?php ob_start(); ?>
	...
	<?php if ( mad_is_shop_installed() ): ?>
		...
		<?php else: ?>
			<div class="bar-login">
				<a class="to-login" data-href="<?php echo esc_attr($accountPage) ?>" href="<?php echo esc_url($accountPage); ?>"><?php _e('Login', 'flatastic'); ?></a>
				<span> | </span>
				<a href="<?php echo esc_url($accountPage); ?>"><?php _e('Register', 'flatastic'); ?></a>
			</div><!--/ .bar-login-->
		<?php endif; ?>
	...
	<?php else: ?>
		...
		<?php if ( is_user_logged_in() ): ?>
			...
			<div class="invert_string">
				...
				<span class="welcome_username"><?php echo _e('Welcome visitor', 'flatastic') . ', ' . esc_html($user_name) ?></span>

				<a href="<?php echo wp_logout( esc_url(home_url()) ); ?>"><?php _e('Logout', 'flatastic') ?></a>
			</div>
			...
			<div class="row">
				...
				<div class="col-md-4 col-sm-5 t_align_c t_xs_align_c">
					<span class="top-message">
						<?php echo mad_custom_get_option('promo_message', __('Promo message will be shown here!', 'flatastic'), true) ?>
					</span>
				</div>

			...
			?>
				...
				<?php if ( is_singular( 'portfolio' ) ) : ?>
 					...
 					<?php 
						...
						//self hosted videos
						if( $portfolio_video_m4v ){
							do_action("create_media_output",
								array(
									'id' => 'video-'.get_the_ID(),
									'type' => "video",
									'file_mp4' => $portfolio_video_m4v,
									'file_webm' => $portfolio_video_webm,
									'poster'=> $video_image
								)
							);
						}
						...
						if ($video){
							...
							if( strpos($video, 'youtube')  ) { //youtube
								echo '<div class="video-container"><iframe src="//www.youtube.com/embed/'.rt_find_tube_video_id($video).'" allowfullscreen></iframe></div>';
							}
							
							if( strpos($video, 'vimeo')  ) { //vimeo
								echo '<div class="video-container"><iframe  src="//player.vimeo.com/video/'.rt_find_tube_video_id($video).'?color=d6d6d6&title=0&amp;byline=0&amp;portrait=0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';
							}

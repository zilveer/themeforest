...
function widget($args, $instance) {	
	...
	$offset 		= apply_filters('widget_offset', $instance['offset']);
	$thumbnail_size = apply_filters('widget_thumbsize', $instance['thumbnail_size']);
					...
					echo $before_title . $title . $after_title; ?>
						...
						<?php
							...
							foreach( $myposts as $post ) : setup_postdata($post); ?>
								<li <?php if(!empty($thumbnail_size)) { $size[0] = $thumbnail_size + 8; $size[1] = $thumbnail_size + 8; } else { $size[0]=60; $size[1]=60; } ?> >
									<?php if($thumbnail == true) { ?>
										<a href="<?php the_permalink(); ?>" style="float: left; margin: 0 5px 0 0;"><?php the_post_thumbnail(array($thumbnail_size,$thumbnail_size));?></a>
									<?php } ?>
									<a href="<?php the_permalink(); ?>" class="recent-post-title"><?php the_title(); ?></a><br/>
									<span class="time"><?php echo the_time('F j, Y'); ?></span>
...
function form($instance) {	
	...
	$def = array('title'=>'Recent Posts', 'cat'=>'', 'number'=>'5', 'offset'=>'0', 'thumbnail_size'=>'60', 'thumbnail'=>'1', 'posttype'=>'post');

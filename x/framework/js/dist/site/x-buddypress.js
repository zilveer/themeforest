...
jq(document).ready( function() {
	...
	jq('.x-item-list-tabs-subnav').find('li.last select option').prepend(BP_DTheme.x_show);
	...
	if ( $whats_new.length && bp_get_querystring('r') ) {
		...
		jq('#whats-new-form').addClass('active'); // X addition

		// jq('#whats-new-options').animate({ // X comment out
		// 	height:'40px'
		// });
	...
	$whats_new.blur( function(){
		if ( document.activeElement !== this ) {
			if (!this.value.match(/\S+/)) {
				this.value = '';
				jq('#whats-new-form').removeClass('active'); // X addition
				// jq('#whats-new-options').animate({ // X comment out
				// 	height:'40px'
				// });
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.hasClass('acomment-reply') || target.parent().hasClass('acomment-reply') ) {
			...
			form.slideDown( 200 );
			// jq.scrollTo( form, 500, { // X comment out
			// 	offset:-100,
			// 	easing:'swing'
			// } );

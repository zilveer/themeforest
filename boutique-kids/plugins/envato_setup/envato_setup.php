...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function get_logo_image() {
			...
			if ( $logo_image_id ) {
				...
				$image_url = get_theme_mod( 'logo_header_image', get_template_directory_uri() . '/images/' . get_theme_mod( 'boutique_site_color', $this->get_default_theme_style() ) . '/logo.png' );
			}
		...
		public function admin_menus() {
			...
			if ( $this->is_submenu_page() ) {
				...
				$add_subpage_function( $this->parent_slug, esc_html__( 'Setup Wizard', 'boutique-kids' ), esc_html__( 'Setup Wizard', 'boutique-kids' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			} else {
				add_theme_page( esc_html__( 'Setup Wizard', 'boutique-kids' ), esc_html__( 'Setup Wizard', 'boutique-kids' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			}
		...
		public function init_wizard_steps() {
			...
			$this->steps = array(
				'introduction' => array(
					'name'    => esc_html__( 'Introduction', 'boutique-kids' ),
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text'      => esc_html__( '...verifying', 'boutique-kids' ),
			) );
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			echo '<t';
			echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Wizard', 'boutique-kids' ) . '</ti' . 'tle>'; ?>
		...
		<?php if ( 'next_steps' === $this->step ) : ?>
			<a class="wc-return-to-dashboard"
			   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'boutique-kids' ); ?></a>
		<?php endif; ?>
		</body>
		<?php
		...
		public function envato_setup_introduction() {
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'boutique-kids' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'boutique-kids' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'boutique-kids' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Not right now', 'boutique-kids' ); ?></a>
				</p>
				<?php
			} else if ( get_option( 'envato_setup_complete', false ) ) {
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'boutique-kids' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you have already run the setup wizard. Below are some options: ', 'boutique-kids' ); ?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again', 'boutique-kids' ); ?></a>
					</li>
					<li>
						<form method="post">
							<input type="hidden" name="reset-font-defaults" value="yes">
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e( 'Reset font style and colors', 'boutique-kids' ); ?>" name="save_step"/>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Cancel', 'boutique-kids' ); ?></a>
				</p>
				<?php
			} else {
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'boutique-kids' ), wp_get_theme() ); ?></h1>
				<p><?php printf( esc_html__( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. It should only take 5 minutes.', 'boutique-kids' ), wp_get_theme() ); ?></p>
				<p><?php esc_html_e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'boutique-kids' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'boutique-kids' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
		...
		public function envato_setup_introduction_save() {
			...
			if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
				...
				// reset site color
				remove_theme_mod( 'boutique_site_color' );

				if ( class_exists( 'boutique_customize_save_hook' ) ) {
					$site_color_defaults = new boutique_customize_save_hook();
				...
				?>
				<p>
					<strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'boutique-kids' ); ?></strong>
				</p>
				<?php
				return true;
			}
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php esc_html_e( 'Default Plugins', 'boutique-kids' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:', 'boutique-kids' ); ?></p>
					...
					echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.', 'boutique-kids' ) . '</strong></p>';
				} ?>

				<p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.', 'boutique-kids' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_plugins"><?php esc_html_e( 'Continue', 'boutique-kids' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'boutique-kids' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
			<?php
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'boutique-kids' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Activating Plugin', 'boutique-kids' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Updating Plugin', 'boutique-kids' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Installing Plugin', 'boutique-kids' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'boutique-kids' ) ) );
			}
			exit;
		...
		private function _content_default_get() {
			...
			foreach ( $available_content as $post_type => $post_data ) {
				if ( count( $post_data ) ) {
					...
					$content[ $post_type ] = array(
						...
						'description'      => sprintf( esc_html__( 'This will create default %s as seen in the demo.', 'boutique-kids' ), $post_type_title ),
						'pending'          => esc_html__( 'Pending.', 'boutique-kids' ),
						'installing'       => esc_html__( 'Installing.', 'boutique-kids' ),
						'success'          => esc_html__( 'Success.', 'boutique-kids' ),
			...
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'boutique-kids' ),
				'description'      => esc_html__( 'Insert default sidebar widgets as seen in the demo.', 'boutique-kids' ),
				'pending'          => esc_html__( 'Pending.', 'boutique-kids' ),
				'installing'       => esc_html__( 'Installing Default Widgets.', 'boutique-kids' ),
				'success'          => esc_html__( 'Success.', 'boutique-kids' ),
			...
			$content['settings'] = array(
				'title'            => esc_html__( 'Settings', 'boutique-kids' ),
				'description'      => esc_html__( 'Configure default settings.', 'boutique-kids' ),
				'pending'          => esc_html__( 'Pending.', 'boutique-kids' ),
				'installing'       => esc_html__( 'Installing Default Settings.', 'boutique-kids' ),
				'success'          => esc_html__( 'Success.', 'boutique-kids' ),
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php esc_html_e( 'Default Content', 'boutique-kids' ); ?></h1>
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
					<p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'boutique-kids' ); ?></p>
				<?php } else { ?>
					<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'boutique-kids' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				<table class="envato-setup-pages" cellspacing="0">
					<thead>
					<tr>
						<td class="check"></td>
						<th class="item"><?php esc_html_e( 'Item', 'boutique-kids' ); ?></th>
						<th class="description"><?php esc_html_e( 'Description', 'boutique-kids' ); ?></th>
						<th class="status"><?php esc_html_e( 'Status', 'boutique-kids' ); ?></th>
					</tr>
					</thead>
					<tbody>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_content"><?php esc_html_e( 'Continue', 'boutique-kids' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'boutique-kids' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function ajax_content() {
			...
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found', 'boutique-kids' ) ) );
			}
			...
			if ( $json ) {
				...
				wp_send_json( array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'boutique-kids' ),
					'logs'    => $this->logs,
					'errors'  => $this->errors,
				) );
			}
			...
			switch ( $post_type ) {
				...
				default:
					...
					$replace_post_id_keys = array(
						'parallax_image',
						'boutique_row_image_top',
						'boutique_row_image_bottom',
						'image',
						'item', // vc grid
						'post_id',
					);
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( ! $upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'boutique-kids' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size', 'boutique-kids' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'boutique-kids' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'boutique-kids' ), size_format( $max_size ) ) );
				}
			}
		...
		public function envato_setup_logo_design() {
			...
			?>
			<h1><?php esc_html_e( 'Logo &amp; Design', 'boutique-kids' ); ?></h1>
			<form method="post">
				<p><?php printf( esc_html__( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard. Try %sEnvato Studio%s if you need a new logo designed.', 'boutique-kids' ), '<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">', '</a>' ); ?></p>
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'boutique-kids' ); ?></a>
						</td>
					</tr>
				</table>

				<?php
				$demo_styles = apply_filters( 'boutique_default_styles', array() );
				if ( ! $this->get_default_theme_style() || count( $demo_styles ) <= 1 ) {

				} else {
					?>

					<p><?php esc_html_e( 'Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.', 'boutique-kids' ); ?></p>

					<div class="theme-presets">
						<ul>
							<?php
							$current_demo = get_theme_mod( 'boutique_site_color', $this->get_default_theme_style() );
				...
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next"
					       value="<?php esc_attr_e( 'Continue', 'boutique-kids' ); ?>" name="save_step"/>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'boutique-kids' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function envato_setup_logo_design_save() {
			...
			if ( $new_style ) {
				...
				if ( isset( $demo_styles[ $new_style ] ) ) {
					set_theme_mod( 'boutique_site_color', $new_style );
					if ( class_exists( 'boutique_customize_save_hook' ) ) {
						$site_color_defaults = new boutique_customize_save_hook();
		...
		public function envato_setup_updates() {
			?>
			<h1><?php esc_html_e( 'Theme Updates', 'boutique-kids' ); ?></h1>
			<?php if ( function_exists( 'envato_market' ) ) { ?>
				<form method="post">
					...
					if ( count( $my_items ) ) {
						...
						<p class="envato-setup-actions step">
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next button-primary"><?php esc_html_e( 'Continue', 'boutique-kids' ); ?></a>
						</p>
						<?php
					} else {
						?>
						<p><?php esc_html_e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'boutique-kids' ); ?></p>
						<p>When an update becomes available it will show in the Dashboard with an option to install.</p>
						<p>
						...
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e( 'Login with Envato', 'boutique-kids' ); ?>" name="save_step"/>
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'boutique-kids' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
		...
		public function envato_setup_customize() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'boutique-kids' ); ?></a>
			</p>
		...
		public function envato_setup_help_support() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue', 'boutique-kids' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
			<?php
		}
		...
		public function envato_setup_ready() {
			...
			<h1><?php esc_html_e( 'Your Website is Ready!', 'boutique-kids' ); ?></h1>
			...
			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php esc_html_e( 'Next Steps', 'boutique-kids' ); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large"
						                             href="https://twitter.com/dtbaker"
						                             target="_blank"><?php esc_html_e( 'Follow @dtbaker on Twitter', 'boutique-kids' ); ?></a>
						</li>
						<li class="setup-product"><a class="button button-next button-large"
						                             href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!', 'boutique-kids' ); ?></a>
						</li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php esc_html_e( 'More Resources', 'boutique-kids' ); ?></h2>
					<ul>
						<li class="documentation"><a href="http://dtbaker.net/envato/documentation/"
						                             target="_blank"><?php esc_html_e( 'Read the Theme Documentation', 'boutique-kids' ); ?></a>
						</li>
						<li class="howto"><a href="https://wordpress.org/support/"
						                     target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'boutique-kids' ); ?></a>
						</li>
						<li class="rating"><a href="http://themeforest.net/downloads"
						                      target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'boutique-kids' ); ?></a></li>
						<li class="support"><a href="http://dtbaker.net/envato/"
						                       target="_blank"><?php esc_html_e( 'Get Help and Support', 'boutique-kids' ); ?></a></li>
					</ul>
				</div>
			</div>
			<?php
		...
		public function envato_market_admin_init() {
			...
			if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$class         = 'error';
					echo "<div class=\"$class\"><p>" . sprintf( esc_html__( 'Something went wrong while trying to retrieve oauth token: %s', 'boutique-kids' ), $error_message ) . '</p></div>';
				} else {
			...
			add_settings_section(
				...
				sprintf( esc_html__( 'Login for %s updates', 'boutique-kids' ), $this->envato_username ),
			...
			add_settings_field(
				...
				esc_html__( 'oAuth Login', 'boutique-kids' ),

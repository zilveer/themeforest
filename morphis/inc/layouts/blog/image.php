		...
		<div class="three columns alpha clearfix">
			<ul class="entry-meta meta1 clearfix">
				...
				<li><img src="<?php echo get_template_directory_uri() . '/images/icons/glyphish/86-camera.png'; ?>" class="post-format-icon" title="<?php _e( 'Image Post Format', 'morphis' ); ?>"/></li>
				<?php endif; ?>
		...
		<?php endif; ?>		
			...
			<div class="post-body">						
				<h2 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr( __( 'Permalink to %s', 'morphis' ) ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2>			
				...
				<div class="entry-content">
					...
					<?php if ( $image_pf != '' ) { ?>
					<?php printf( '<div class="overlay squared half-bottom"><figure><div class="overlay-mask"><a class="icon-view" href="%1$s" rel="prettyPhoto" title=""></a><a class="icon-link" href="'. get_permalink() .'"></a></div><img src="%1$s" alt="' . get_the_title() . '" title="' . get_the_title() . '" /></figure></div>', $image_pf ); ?>					
					<?php } else { ?>

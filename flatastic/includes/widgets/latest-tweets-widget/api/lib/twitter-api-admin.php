...
function twitter_api_admin_render_header( $subheader, $css = '' ){
    ?>
     <div class="wrap">
        <h2><?php echo esc_html__('Twitter API Authentication Settings','flatastic')?></h2>
...
function twitter_api_admin_render_form(){
    ...
    <form action="<?php echo twitter_api_admin_base_uri()?>" method="post">
        <p>
            <label for="twitter-api--consumer-key"><?php echo __('OAuth Consumer Key','flatastic');?>:</label><br />
            <input type="text" size="64" name="saf_twitter[consumer_key]" id="twitter-api--consumer-key" value="<?php echo esc_html($consumer_key)?>" />
        </p>
        <p>
            <label for="twitter-api--consumer-secret"><?php echo __('OAuth Consumer Secret','flatastic');?>:</label><br />
        ...
        <p>
            <label for="twitter-api--access-key"><?php echo __('OAuth Access Token','flatastic');?>:</label><br />
            <input type="text" size="64" name="saf_twitter[access_key]" id="twitter-api--access-key" value="<?php echo esc_html($access_key)?>" />
        </p>
        <p>
            <label for="twitter-api--access-secret"><?php echo __('OAuth Access Secret','flatastic');?>:</label><br />
        ...
        <p class="submit">
            <input type="submit" class="button-primary" value="<?php echo esc_html__('Save settings','flatastic')?>" />
        </p>
        <small>
            <?php echo esc_html__('These details are available in','flatastic')?> 
            <a href="https://dev.twitter.com/apps"><?php echo esc_html__('your Twitter dashboard','flatastic')?></a>
        </small>
    </form>
    <?php
}
...
function twitter_api_admin_render_login( $consumer_key, $consumer_secret ){
    ...
    echo '<p><a class="button-primary" href="',esc_html($href),'">'.esc_html__('Connect to Twitter','flatastic').'</a></p>';
    echo '<p>&nbsp;</p>';
}
...
function twitter_api_admin_render_page(){
    if ( ! current_user_can('manage_options') ){
        twitter_api_admin_render_header( __("You don't have permission to manage Twitter API settings",'flatastic'),'error');
    ...
    try {
        ...
        if( ! $consumer_key || ! $consumer_secret ){
            throw new Exception( __('Twitter application not fully configured','flatastic') );
        }
        ...
        if( ! $access_key || ! $access_secret ){
            twitter_api_admin_render_header( __('Plugin not yet authenticated with Twitter','flatastic'), 'error' );
        ...
        // else we have auth - verify that tokens are all still valid
            ...
            twitter_api_admin_render_header( sprintf( __('Authenticated as @%s','flatastic'), $me['screen_name'] ), 'updated' );
        }
...
function twitter_api_admin_menu() {
    $title = __('Twitter API','flatastic');

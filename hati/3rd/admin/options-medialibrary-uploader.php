...
if ( ! function_exists( 'optionsframework_mlu_init' ) ) {
	function optionsframework_mlu_init () {
		register_post_type( 'optionsframework', array(
			'labels' => array(
				'name' => 'Theme Options Internal Container',
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'optionsframework_medialibrary_uploader' ) ) {
	...
	function optionsframework_medialibrary_uploader( $_id, $_value, $_tpl = null, $_desc = '', $_postid = 0, $_name = '') {
		...
		if ( $_tpl ) {
			$output .= '<textarea id="' . $id . '" class="uploadplus' . $class . '" name="'.$name.'" cols="64" rows="5">' . $value . "</textarea>\n<br>\n";
			$output .= '<input id="upload_' . $id . '" class="upload_button button button-primary" type="button" data-tpl="'. $_tpl .'" value="Upload" rel="' . $int . '" />' . "\n";
		} else { // normal upload (== null)
			$output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="'.$name.'" value="' . $value . '" />' . "\n";
			$output .= '<input id="upload_' . $id . '" class="upload_button button button-primary" type="button" value="Upload" rel="' . $int . '" />' . "\n";
		}

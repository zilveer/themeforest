...
class Custom_WC_Widget_Layered_Nav_Filters extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Shows active layered nav filters so users can see and deactivate them.', 'homeshop' );
		$this->widget_id          = 'woocommerce_layered_nav_filters';
		$this->widget_name        = __( 'WooCommerce Layered Nav Filters', 'homeshop' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Active Filters', 'homeshop' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( count( $_chosen_attributes ) > 0 || $min_price > 0 || $max_price > 0 ) {
			...
			if (!is_null($_chosen_attributes)){
				foreach ( $_chosen_attributes as $taxonomy => $data ) {
					...
					foreach ( $data['terms'] as $term_id ) {
						...
						echo '<li class="chosen"><a title="' . __( 'Remove filter', 'homeshop' ) . '" href="' . esc_url( $link ) . '">' . $term->name . '</a></li>';
					}
				}
			}
			...
			if ( $min_price ) {
				...
				echo '<li class="chosen"><a title="' . __( 'Remove filter', 'homeshop' ) . '" href="' . esc_url( $link ) . '">' . __( 'Min', 'homeshop' ) . ' ' . wc_price( $min_price ) . '</a></li>';
			}

			if ( $max_price ) {
				$link = remove_query_arg( 'max_price' );
				echo '<li class="chosen"><a title="' . __( 'Remove filter', 'homeshop' ) . '" href="' . esc_url( $link ) . '">' . __( 'Max', 'homeshop' ) . ' ' . wc_price( $max_price ) . '</a></li>';
			}

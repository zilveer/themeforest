...
class PeThemeUtils {
	...
	public static function fix_serialize_str($matches) {
		$str = $matches[1];
		return sprintf('s:%d:"%s";',strlen($str),$str);
	}

	// http://www.bangheadonwall.net/?attachment_id=227
	public static function &fix_serialize($broken) {
		// /e modifier is deprecated in php 5.5
		//$fixed = preg_replace('!s:(\d+):"(.*?)";!e', "'s:'.strlen('$2').':\"$2\";'", $broken);
		$fixed = preg_replace_callback('!s:\d+:"(.*?)";!', array('PeThemeUtils','fix_serialize_str'), $broken);
		return $fixed;
	}

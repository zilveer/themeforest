...
class VP_Option extends WPAlchemy_MetaBox {
	...
	public function __construct( array $configs ) {
		...
		extract( $test = array_merge( array(
			...
			'layout' => 'fluid',
			'page_title' => esc_html__( 'Vafpress Options', 'crazyblog' ),
			'menu_label' => esc_html__( 'Vafpress Options', 'crazyblog' ),
			'priority' => 10,
				), $configs ) );
		...
		if ( isset( $option_key ) )
			...
			throw new Exception( esc_html__( 'Option Key is required', 'crazyblog' ), 1 );
		if ( isset( $template ) )
			$this->set_template( $template );
		else
			throw new Exception( esc_html__( 'Template Array/File is required', 'crazyblog' ), 1 );
		if ( isset( $page_slug ) )
			$this->set_page_slug( $page_slug );
		else
			throw new Exception( esc_html__( 'Page Slug is required', 'crazyblog' ), 1 );
	...
	public function register_menu_page() {
		if ( is_array( $this->get_menu_page() ) ) {
			...
			$hook_suffix = add_theme_page(
					$this->get_page_title(), $this->get_menu_label(), $this->get_minimum_role(), $this->get_page_slug(), array( $this, 'option_page_display' ), $menu_page['icon_url'], $menu_page['position']
			);
		}
		else {
			$hook_suffix = add_theme_page(
					//$this->get_menu_page(),
					$this->get_page_title(), $this->get_menu_label(), $this->get_minimum_role(), $this->get_page_slug(), array( $this, 'option_page_display' )
			);
		}
	...
	public function dev_mode_notice() {
		VP_WP_Util::admin_notice( esc_html__( "Development Mode is Active, options' values won't be saved into database.", 'crazyblog' ), false );
	}
	...
	public function enqueue_scripts_and_styles() {
		...
		$opt_loader->add_main_css( 'vp_flateicon' );
	...
	function ajax_save() {//printr($_POST);
	...
	function ajax_import_option() {
		...
		if ( $result['status'] ) {
			...
			if ( empty( $option ) ) {
				$result['status'] = false;
				$result['message'] = esc_html__( "Can not be empty", 'crazyblog' );
			} else {
				...
				if ( is_array( $option ) ) {
					...
					$result['status'] = false;
					$result['message'] = esc_html__( "Invalid data", 'crazyblog' );
				}
			}
		}
	...
	function ajax_export_option() {
		...
		if ( $result['status'] ) {
			...
			$result = array(
				'status' => true,
				'message' => esc_html__( "Successful", 'crazyblog' ),
				'option' => $sr_options,
			);
		}
	...
	function vp_verify_nonce() {
		...
		if ( $verify ) {
			...
			$result['status'] = false;
			$result['message'] = esc_html__( "Unverified Access.", 'crazyblog' );
		}
		return $result;
	}
	...
	public function init_options_set() {
		...
		if ( is_string( $this->get_template() ) and is_file( $this->get_template() ) )
			...
			throw new Exception( esc_html__( 'Invalid template supplied', 'crazyblog' ), 1 );
...
//			$util_menu->set_title( esc_html__( 'Demo Installation', 'crazyblog' ) );
//			$util_menu->set_name( 'menu_util' );
//			$util_menu->set_icon( 'font-awesome:fa fa-toggle-on' );
//
//			// setup restore default section
//			$restore_section = new VP_Option_Control_Group_Section();
//			$restore_section->set_title( esc_html__( 'Restore Default', 'crazyblog' ) );

...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			//Start Heading
			 if ($value['type'] != "heading") 
			 {
				...
				if(($value['name'])&&($value['type'] != "separator")) $output .= '<h3 class="heading">'. $value['name'] .'</h3>'."\n";
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					if( isset( $value['id'] ) && isset( $smof_data[$value['id']] ) ) $t_value = stripslashes($smof_data[$value['id']]);
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {
						$selected = (isset($data[$value['id']])) ? selected($data[$value['id']], $option, false) : '';
						$output .= '<option id="' . $select_ID . '" value="'.$option.'" ' . $selected . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;

				//select option with key value settings
				case 'select2':
					$mini ='';
					if(!isset($value['mod'])) $value['mod'] = '';
					if($value['mod'] == 'mini') { $mini = 'mini';}
					...
					$output .= '<select class="select of-input" name="'.$value['id'].'" id="'. $value['id'] .'">';
					foreach ($value['options'] as $select_ID => $option) {			
						$selected = (isset($data[$value['id']])) ? selected($data[$value['id']], $select_ID, false) : '';
				...
				case 'textarea':	
					...
					if(isset($smof_data[$value['id']])) $ta_value = stripslashes($smof_data[$value['id']]);				
				...
				//	Options separator
				case "separator":
						$output .= '<div class="option_sep"><p>'.$value['name'].'</p></div>';				
				break;
				...
				//colorpicker option
				case 'color':
					$color = (!empty($smof_data[$value['id']])) ? $smof_data[$value['id']] : '';
				...
				case 'slider':
					...
					$slides = (!empty($smof_data[$value['id']])) ? $smof_data[$value['id']] : array();
					$count = count($slides);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
				...
				case 'new_slider':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_new_slider_function($value['id'],$value['std'],$oldorder,$order,$int);
					} else {
						$i = 0;
						foreach ($slides as $slide) {
							$oldorder = $slide['order'];
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button new_slide_add_button">Add New Slide</a></div>';
					
				break;

				//drag & drop Progress bar manager
				case 'progress':
					$_id = strip_tags( strtolower($value['id']) );
					$int = '';
					$int = optionsframework_mlu_get_silentpost( $_id );
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_progress_function($value['id'],$value['std'],$oldorder,$order,$int);
					} else {
						$i = 0;
						foreach ($slides as $slide) {
							$oldorder = $slide['order'];
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button progress_add_button">Add New Progress</a></div>';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'flatbox').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
			...
			if (( $value['type'] != 'heading')&&($value['type'] != 'separator')) { 
				...
				if ($value['type'] == 'separator') { 
					$output .= '<div class="clear"> </div></div></div></div><div class="clear"> </div>'."\n";
				}
	...
	/**
	 * Drag and drop NEW slides manager
	 *
	 * @uses get_option()
	 *
	 * @access public
	 ...
	 * @return string
	 */
	public static function optionsframework_new_slider_function($id,$std,$oldorder,$order,$int){
		...
		//initialize all vars
		$slidevars = array('title','url','text1','text2','link','link2','description');
		...
		$slider .= '</div>';	
		$slider .= '<label>button 1 text</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][text1]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['text1'] .'" />';

		$slider .= '<label>Link URL (optional)</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][link]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['link'] .'" />';

		$slider .= '<label>button 2 text</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][text2]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['text2'] .'" />';

		$slider .= '<label>Link URL 2 (optional)</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][link2]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['link2'] .'" />';
	 ...
	 * @return string
	 */
	public static function optionsframework_progress_function($id,$std,$oldorder,$order,$int){
		...
		//initialize all vars
		$slidevars = array('title','url','link','color','description');
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>Progress '.$order.'</strong>';
		}
		...
		$slider .= '<label>Progress Text</label>';
		$slider .= '<input class="slide of-input of-slider-title" name="'. $id .'['.$order.'][title]" id="'. $id .'_'.$order .'_slide_title" value="'. stripslashes($val['title']) .'" />';
	 ...
	 //    	$slider .= '</a>';
		...
		// 	}
		// $slider .= '</div>';	
		$slider .= '<label>Prgress Percent ( between 0 & 100 )</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][link]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['link'] .'" />';
		

		//$color = (!empty($smof_data[$value['id']])) ? $smof_data[$value['id']] : '';
		$slider .= '<label>Progress Color</label></br>';
		$slider .= '<div id="'. $id .'_'.$order .'_picker" class="colorSelector"><div style="background-color: '.$val['color'].'"></div></div>';
		$slider .= '<input class="of-color" name="'. $id .'['.$order.'][color]" id="'. $id .'_'.$order .'" type="text" value="'. $val['color'] .'" />';

...
// Set default panel
if ($panel == '' && in_array($widget->position, array('slider','top-a', 'top-b', 'bottom-a', 'bottom-b', 'main-top', 'main-bottom', 'sidebar-a', 'sidebar-b'))) {
...
// Set title
if (in_array($widget->position, array('headerbar', 'toolbar-r' ,'toolbar-l', 'footer','slider'))) {
	$title = '';
...
if (in_array($widget->position, array('breadcrumbs', 'logo', 'logo-small', 'search', 'debug')) || (($widget->position == 'offcanvas') && $widget->menu)) {
	...
	if (isset($params['animation'])) {
		$animation = 'data-uk-scrollspy="{cls:\''.$params['animation'].'\''.((isset($params['animation_repeat'])) ?', repeat: true':'').'}"';
	} else {
		$animation = '';
	}

...
class TB_Latest_Posts extends WP_Widget {
	...
	function TB_Latest_Posts() {
		$widget_ops = array('classname' => 'tb_latest_posts', 'description' => __( 'Latest posts with thumbnails', 'grace') );		
		$this->WP_Widget('TB_Latest_Posts', __('ThemeBlossom: Latest Posts', 'grace'), $widget_ops);	
	}
	...
	function widget( $args, $instance ) {
		...
		$latestPosts = new WP_Query(array('posts_per_page' => $number_of_posts, 'no_found_rows' => true, 'post_status' => 'publish', 'ignore_sticky_posts' => true, 'order' => 'DESC'));
		
		if ($latestPosts->have_posts()) : ?>
        
		<?php
        echo $before_widget;
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Latest Posts', 'grace') : $instance['title'], $instance, $this->id_base);		
	...
	function form( $instance ) {
		...
		$instance = wp_parse_args( (array) $instance, array( 'number_of_posts' => 2, 'title'=>'Latest Posts', 'number_of_words' => 10 ) );

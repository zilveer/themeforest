...
add_filter( 'wp_footer', 'btp_isotope_wp_footer' );
?>
<div class="isotope-wrapper">
<?php get_template_part( '/lib/works/templates/isotope_toolbar' ); ?>
<div class="collection works view-as-grid one-half filterable">
<ul><!-- --><?php while ( $query->have_posts() ): $query->the_post(); ?><li class="item <?php btp_entry_render_filters( 'btp_work_category' );?>">            
        	<article class="pid-<?php the_ID(); ?>">
            	<?php if ( $elems['featured_media'] ) { btp_entry_render_featured_media( 'one_half', $lightbox_group ); } ?>

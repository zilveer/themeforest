...
# ------------------------------------------------------------------------------------------------------------------------
		// FOR USE IN THEME SUB-FOLDER (or as extension)
		if ($is_extension = true) {
			//Determine Base URL
			$ext_path = str_replace( "\\", '/', dirname( dirname( __FILE__ ) ) );
			$ext_path = str_replace( str_replace( "\\", '/', get_template_directory() ), "",$ext_path );
			$ext_url = get_template_directory_uri().$ext_path.'/';
			$the_asset_path = $ext_url;
		} else {
			$the_asset_path = trailingslashit(plugins_url('/mtekk_adminkit_tabs.js', dirname(__FILE__) ));
		}
# ------------------------------------------------------------------------------------------------------------------------

		//Register JS for tabs
		wp_register_script('mtekk_adminkit_tabs', $the_asset_path . 'includes/mtekk_adminkit_tabs.js', array('jquery-ui-tabs'));
		//Register CSS for tabs
		wp_register_style('mtekk_adminkit_tabs', $the_asset_path . 'includes/mtekk_adminkit_tabs.css');
		//Register options
	...
	function filter_plugin_actions($links, $file)
	{
		...
		{ 
			...
			$links[] = '<a href="' . $this->admin_url() . '">' . __('Settings', 'framework') . '</a>';
		}
		return $links;
	}
	/**

...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	...
	define( 'GRID_THEME_PLUGINS_URL', get_template_directory_uri() . '/admin/woocommerce-grid-list-toggle' );
	/**
	 * WC_List_Grid class
	 **/
	if (!class_exists('WC_List_Grid')) {
		...
		class WC_List_Grid {
			...
			public function __construct() {
				...
				$this->settings = array(
					array(
						'name' => __( 'Default catalog view', 'homeshop' ),
						'type' => 'title',
						'id' => 'wc_glt_options'
					),
					array(
						'name' 		=> __( 'Default catalog view', 'homeshop' ),
						'desc_tip' 	=> __( 'Display products in grid or list view by default', 'homeshop' ),
						'id' 		=> 'wc_glt_default',
						'type' 		=> 'select',
						'options' 	=> array(
							'grid'  => __('Grid', 'homeshop'),
							'list' 	=> __('List', 'homeshop')
						)
					),
			...
			function setup_scripts_styles() {
				if ( is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() ) {
					wp_enqueue_style( 'grid-list-layout', GRID_THEME_PLUGINS_URL . '/assets/css/style.css' );
					//wp_enqueue_style( 'grid-list-button', GRID_THEME_PLUGINS_URL . '/assets/css/button.css' );
				}
			}
			function setup_scripts_script() {
				if ( is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() ) {
					//wp_enqueue_script( 'cookie', GRID_THEME_PLUGINS_URL . '/assets/js/jquery.cookie.min.js', array( 'jquery' ) );
					//wp_enqueue_script( 'grid-list-scripts', GRID_THEME_PLUGINS_URL . '/assets/js/jquery.gridlistview.js', array( 'jquery' ) );
			...
			function gridlist_toggle_button() {
				?>
					<nav class="gridlist-toggle">
						<a href="#" id="grid" title="<?php _e('Grid view', 'homeshop'); ?>">&#8862; <span><?php _e('Grid view', 'homeshop'); ?></span></a><a href="#" id="list" title="<?php _e('List view', 'homeshop'); ?>">&#8863; <span><?php _e('List view', 'homeshop'); ?></span></a>
					</nav>
				<?php
			}

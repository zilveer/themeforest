...
tagBox = {
	...
	get : function(id) {
		...
		$.post(ajaxurl, {'action':'shiba-get-tagcloud', 'tax':tax}, function(r, stat) {
			if ( 0 == r || 'success' != stat )
				r = wpAjax.broken;
...
*/
		...
		// tag cloud
		$('a.tagcloud-link').off('click');
		$('a.tagcloud-link').click(function(event){
			if ( ! $('.the-tagcloud', $(this).parent()).length )
				tagBox.get( $(this).attr('id') );
			$(this).siblings('.the-tagcloud').toggle();
			return false;
		});

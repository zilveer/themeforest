...
class TwitterApiClient {
    ...
    public static function create_instance( $default = true ){
        ...
        if( $default ){
            if( ! $consumer_key || ! $consumer_secret || ! $access_key || ! $access_secret ){
                trigger_error( __('Twitter application is not fully configured', 'candidate') );
            }
    ...
    public function call( $path, array $_args, $http_method ){
        ...
        if( ! $this->has_auth() ){
            throw new TwitterApiException( __('Twitter client not authenticated', 'candidate'), 0, 401 );
        }
        ...
        foreach( $_args as $key => $val ){
            if( is_string($val) ){
                ...
                throw new TwitterApiException( __('Invalid Twitter parameter', 'candidate').' ('.gettype($val).') '.$key.' in '.$path, -1 );
            }
            else {
                $args[$key] = (string) $val;
            }
        ...
        if( isset( $data['errors'] ) ) {
            while( $err = array_shift($data['errors']) ){
                ...
                if( $data['errors'] ){
                    $message = sprintf( __('Twitter error #%d', 'candidate'), $err['code'] ).' "'.$err['message'].'"';
    ...
    public function oauth_exchange( $endpoint, array $args ){
        ...
        if( ! is_array($params) || ! isset($params['oauth_token']) || ! isset($params['oauth_token_secret']) ){
            throw new TwitterApiException( __('Malformed response from Twitter', 'candidate'), -1, $stat );
        }
        return $params;   
    }
    ...
    public static function http_request( $endpoint, array $conf ){
        ...
        if( empty($http['response']) ){
            throw new TwitterApiException( __('Wordpress HTTP request failure', 'candidate'), -1 );
        }
        return $http;
    }
...
class TwitterOAuthToken {
    ...
    public function __construct( $key, $secret = '' ){
        if( ! $key ){
           throw new Exception( __('Invalid OAuth token', 'candidate').' - '.__('Key required even if secret is empty') );
        }
        $this->key = $key;
        $this->secret = $secret;
    }
...
function _twitter_api_http_status_text( $s ){
    ...
    // unknown status    
    return sprintf( __('Status %u from Twitter', 'candidate'), $s );
}

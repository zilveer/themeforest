 ...
 *
 *
 * @package 	proStore/library/admin/update-notifier.php
 * @file	 	1.0
 */
?>
<?php
...
define( 'NOTIFIER_THEME_NAME', 'proStore' ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', 'proStore' ); // The theme folder name
define( 'NOTIFIER_XML_FILE', 'http://rchour.net/notifier.xml' ); // The remote notifier XML file containing the latest version of the theme and changelog
...
function update_notifier_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (float)$xml->latest > (float)$theme_data['Version']) { // Compare current theme version with the remote XML version
			add_submenu_page('optionsframework', NOTIFIER_THEME_NAME . '-  Theme Updates', 'Theme Update', 'administrator', 'theme-update-notifier', 'update_notifier');
		}
	}
}
add_action('admin_menu', 'update_notifier_menu');

function update_admin_notice(){
		...
		if( (float)$xml->latest > (float)$theme_data['Version']) {
			...
			if ($page!="admin.php&page=theme-update-notifier") {
		?>
				<div id="message" class="updated"><p><strong>There is a new version of the <?php echo NOTIFIER_THEME_NAME; ?> theme available.</strong> You have version <?php echo $theme_data['Version']; ?> installed. Update to version <?php echo $xml->latest; ?>.</p></div>
		<?php
			}
		}
}
add_action('admin_notices', 'update_admin_notice');
...
function update_notifier() {
	...
	<div class="wrap">
		...
		<div id="instructions" style="float:left;display:inline-block;">
...
function get_latest_theme_version($interval) {
	...
	$db_cache_field = 'prostore_notifier-cache';
	$db_cache_field_last_updated = 'prostore_notifier-cache-last-updated';

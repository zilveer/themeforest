...
if ( $theQuery->have_posts() ) while ( $theQuery->have_posts() ) : $theQuery->the_post(); ?>
		...
		<div id="post-<?php $postID = get_the_ID(); echo $postID; ?>" <?php post_class(); ?>>
			...
			<div class="entry-summary">
				...
				if (isset($venue)) $addressArray[] = $venue;
				if (isset($address)) $addressArray[] = $address;
				if (isset($location)) $addressArray[] = $location;
				...
				if (isset($startDate)) $infoLine[] = "<li><span aria-hidden='true' class='icon-calendar'></span> " . date_i18n(get_option('date_format'), $startDate) . "</li>";
				if (isset($startTime)) $infoLine[] = "<li><span aria-hidden='true' class='icon-time'></span> $startTime</li>";

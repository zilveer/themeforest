<?php
$output = $title = $type = $onclick = $custom_links = $img_size = $custom_links_target = $images = $el_class = $interval = $randomize = '';
...
if ( $type == 'nivo' ) {
    ...
    $el_start = '<li class="single-column-item">';
    $el_end = '</li>';
    $slides_wrap_start = '<ul class="single_column_ul">';
    $slides_wrap_end = '</ul>';
} else if ( $type == 'r_slider' ) {
	wp_enqueue_script( 'jquery.royalslider.min' );

    $el_start = '<div class="rsContent">';
    $el_end = '</div>';
    $slides_wrap_start = '<div class="gallery-slider royalSlider rsMinW">';
    $slides_wrap_end = '</div>';
}
...
foreach ( $images as $attach_id ) {
    ...
    if ($attach_id > 0) {
		if ( $type == 'r_slider' ) {
			$post_thumbnail = wpb_getImageBySize(array( 'attach_id' => $attach_id, 'thumb_size' => $img_size, 'class' => 'rsImg' ));
		} else {
	...
	else if ( $onclick == 'link_magnificpopup' ) {
        $link_start = '<a class="magnific-popup" href="'.$p_img_large[0].'">';
        $link_end = '</a>';
    }
    ...
    $gal_images .= $el_start . $link_start . $thumbnail . $link_end . $attachment_caption . $el_end;
}

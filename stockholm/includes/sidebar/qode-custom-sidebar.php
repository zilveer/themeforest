...
if( ! class_exists( 'qode_sidebar' ) ){
	...
	class qode_sidebar{
		...
		function template_add_widget_field(){
			...
			echo "\n  <h3>". $this->title ."</h3>";
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".__('Enter Name of the new Widget Area', 'qode')."' name='qode-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".__('Add Widget Area', 'qode')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}

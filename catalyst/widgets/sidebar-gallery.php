<?php
class Fables_SidebarGallery_Widget extends WP_Widget {
	function Fables_SidebarGallery_Widget() {
		$widget_ops = array('classname' => 'widget_fables_sidebar_gallery', 'description' => 'Generates a gallery to the sidebar or footer.' );
		$this->WP_Widget('fables_sidebar_gallery', MTHEME_NAME . '- Widget Gallery', $widget_ops);
	}
		...
		if ( !empty( $title ) ) { 
			...
			switch ($listchoice) {
				...
				case "portfolio" :
					...
					$lightboximage_id = get_post_thumbnail_id(($post->ID), 'full'); 
					$lightboximage_url = wp_get_attachment_image_src($lightboximage_id,'full');
					$lightboximage_url = $lightboximage_url[0];  
					
					$image_id = get_post_thumbnail_id(($post->ID), 'sidebar-gallery'); 
					$image_url = wp_get_attachment_image_src($image_id,'sidebar-gallery');  
					...
					?>
						...
						if ($clickaction=="lightbox") { ?>
							<a rel="prettyPhoto[widget]" href="<?php echo $lightboximage_url; ?>" title="<?php echo $imageTitle; ?>">
							<?php
							} else {
							?>
				...
				case "page" :
					...
					$lightboximage_array = wp_get_attachment_image_src( $attatchmentID , 'full', false);
					$lightboximage_url = $lightboximage_array[0];

					$imagearray = wp_get_attachment_image_src( $attatchmentID , 'sidebar-gallery', false);
	...
	function form($instance) {
		...
		$portfolio_options[] = '<option value="BLANK">Select a Portfolio</option>';
...
register_widget('Fables_SidebarGallery_Widget');
?>
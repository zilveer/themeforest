<?php
/**
 * Code that handles the demo theme options import (via AJAX)
 *
 * @package Pile
 * @since   Pile 2.0
 */
...
if ( $Heap_importerError !== false ) {
	...
	if ( ! is_file( $import_filepath . '.php' ) ) {
		$response['id'] = new WP_Error( 'import_theme_options_nofile', 'The PHP file containing the theme options data could not be found or could not be read in <pre>' . get_template_directory() . 'inc/import/demo-data' . '</pre><br/> You might want to try to set the file permission to 777.' );
	} else {
		ob_start();

		$wp_import                    = new Heap_import();

...
class TwitterApiClient {
    ...
    public static function create_instance( $default = true ){
        ...
        if( $default ){
            if( ! $consumer_key || ! $consumer_secret || ! $access_key || ! $access_secret ){
                trigger_error( __('Twitter application not fully configured','flatastic') );
            }
    ...
    public function call( $path, array $_args, $http_method ){
        ...
        if( ! $this->has_auth() ){
            throw new TwitterApiException( __('Twitter client not authenticated','flatastic'), 0, 401 );
        }
        ...
        foreach( $_args as $key => $val ){
            if( is_string($val) ){
                ...
                throw new TwitterApiException( __('Invalid Twitter parameter','flatastic').' ('.gettype($val).') '.$key.' in '.$path, -1 );
            }
            else {
                $args[$key] = (string) $val;
            }
        ...
        if( isset( $data['errors'] ) ) {
            while( $err = array_shift($data['errors']) ){
                $err['message'] = __( $err['message'], 'flatastic' );
                if( $data['errors'] ){
                    $message = sprintf( __('Twitter error #%d','flatastic'), $err['code'] ).' "'.$err['message'].'"';
        ...
        if( isset($data['error']) ){
            ...
            $message = sprintf( __('Twitter error #%d','flatastic'), $code ).' "'.$data['error'].'"';
    ...
    public function oauth_exchange( $endpoint, array $args ){
        ...
        if( ! is_array($params) || ! isset($params['oauth_token']) || ! isset($params['oauth_token_secret']) ){
            throw new TwitterApiException( __('Malformed response from Twitter','flatastic'), -1, $stat );
        }
        return $params;   
    }
    ...
    public static function http_request( $endpoint, array $conf ){
        ...
        if( empty($http['response']) ){
            throw new TwitterApiException( __('Wordpress HTTP request failure','flatastic'), -1 );
        }
        return $http;
    }
...
class TwitterOAuthToken {
    ...
    public function __construct( $key, $secret = '' ){
        if( ! $key ){
           throw new Exception( __('Invalid OAuth token','flatastic').' - '.__('Key required even if secret is empty','flatastic') );
        }
        $this->key = $key;
        $this->secret = $secret;
    }
...
function _twitter_api_http_status_text( $s ){
    ...
    if( $text ){
        twitter_api_load_textdomain( null, 'flatastic' );
        return __( $text, 'flatastic' );
    }
    // unknown status    
    return sprintf( __('Status %u from Twitter','flatastic'), $s );
}

...
wp.customize('use_sidebar',function( value ) {
	value.bind(function(to) {
		if(to){
			$('#sidebar').show();
			$('#content-wrapper-inside').addClass('sidebar--true');
		}else{
			$('#sidebar').hide();
			$('#content-wrapper-inside').removeClass('sidebar--true');
		}
		$(document).trigger('sleek:resize');
	});
});
...
wp.customize('independent_sidebar',function( value ) {
	value.bind(function(to) {
		if(to){
			$('body').removeClass('independent-sidebar--false').addClass('independent-sidebar--true');
		}else{
			$('body').removeClass('independent-sidebar--true').addClass('independent-sidebar--false');
		}
	});
});
...
wp.customize('bg_header',function( value ) {
	value.bind(function(to) {
		if(to){
			$('.header .nano-content').attr('style', 'background:'+to);
		}else{
			$('.header .nano-content').attr('style', 'background:none;');
		}
	});
});
...
wp.customize('bg_content',function( value ) {
	value.bind(function(to) {
		if(to){
			$('.main-content .nano-content').attr('style', 'background:'+to);
		}else{
			$('.main-content .nano-content').attr('style', 'background:none;');
		}
	});
});
...
wp.customize('bg_sidebar',function( value ) {
	value.bind(function(to) {
		if(to){
			$('.sidebar').attr('style', 'background:'+to);
		}else{
			$('.sidebar').attr('style', 'background:none;');
		}
	});
});
...
wp.customize('bg_sidebar_dark',function( value ) {
	value.bind(function(to) {
		if(to){
...
function apply_typo_setting(setting, el){
	setting = setting.split('|');

	// family
	var family = '\"'+setting[0]+'\",sans-serif';
	var weight = '400';
	var style  = 'normal';
	var size   = '16';
	var line   = '1.4';
	...
	// weight / style
	// if string starts with reg = regular
	if(setting[1].substring(0,3)=='reg'){
		weight = '400';
		style = 'normal';
	}else{
		// if string starts with ita = italic
		if(setting[1].substring(0,3)=='ita'){
			weight = '400';
			style = 'italic';
		}else{
			// else string starts with weight number [100,300]
			weight = setting[1].substring(0,3);

			// if string has any characters after weight, it's style
			if(setting[1].substring(3)!==''){
				if( setting[1].substring(3) == 'ita' || setting[1].substring(3) == 'italic'){
					style = 'italic';
				}else{
					style = 'normal';
				}
...
wp.customize('font_body',function( value ) {
	value.bind(function(to) {
		if(to){
			apply_typo_setting(to,'body');
		}
	});
});

/*------------------------------------*/
/* Typography: Font Navigation
/*------------------------------------*/

wp.customize('font_navigation',function( value ) {
	value.bind(function(to) {
		if(to){
			apply_typo_setting(to,'h1 .above, h2 .above, .custom-heading, .dropcap, cite, .header__nav a, .sidebar .widget__title, .sidebar__tabs a');
		}
	});
});

/*------------------------------------*/
/* Typography: Custom Heading
/*------------------------------------*/

wp.customize('font_custom_heading',function( value ) {
	value.bind(function(to) {
		if(to){
			apply_typo_setting(to,'.custom-heading');
		}
	});
});
...
wp.customize('font_h3',function( value ) {
	value.bind(function(to) {
		if(to){
			apply_typo_setting(to,'h3:not(.widget__title), .h3');
		}
	});
});
...
wp.customize('font_h5',function( value ) {
	value.bind(function(to) {
		if(to){
			apply_typo_setting(to,'h5, .h5');
		}
	});
});
wp.customize('font_h6',function( value ) {
	value.bind(function(to) {
		if(to){
			apply_typo_setting(to,'h6, .h6');
		}
	});
});

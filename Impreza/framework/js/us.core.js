				...
				this.$fullscreenSections.each(function(index, section){
					...
					if ($section.hasClass('valign_center')) {
						var $sectionH = $section.find('.l-section-h'),
							...
							sectionBottomPadding = parseInt($section.css('padding-bottom')),
							sectionPadding = (sectionBottomPadding > 0) ? sectionBottomPadding : sectionTopPadding,
						...
						if (sectionOverlapped) {
							...
							topMargin = Math.max(0, (sectionHeight - sectionPadding - contentHeight) / 2);
						} else {
		...
		resize: function(){
			...
			if (this.orientation == 'hor') {
				if (this.pos == 'fixed' && ($us.canvas.headerInitialPos == 'bottom' || $us.canvas.headerInitialPos == 'below') && ($us.$body.usMod('state') == 'default')) {
					...
					this.headerTop = $us.canvas.$firstSection.outerHeight() + adminBarHeight;

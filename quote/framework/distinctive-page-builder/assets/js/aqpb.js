...
jQuery(document).ready(function($){
	...
	$('#blocks-archive > li.block-aq_section_block').each(function() {
		$(this).draggable({
			...
			stop: function(event, ui) {
				$('#blocks-archive > li.block-aq_section_block').find('.block-handle').css({'background' : '#1abc9c', 'color' : '#ffffff', 'cursor' : 'default'});
		        $('#blocks-archive > li.block-aq_section_block').find('.block-title').text('You Must Save To Add Another');
		        $('#blocks-archive > li.block-aq_section_block').draggable({ disabled: true });			       
		    }
		});
	});

	$('#blocks-archive > li.block:not(#section-wrapper)').each(function() {
		$(this).draggable({
			connectToSortable: "#blocks-to-edit",
			helper: 'clone',
			revert: 'invalid',
...
// what fish?
	...
	$('a.thickbox').click(function() {
		$(this).closest('.block').addClass('tbopen');
	})

	$('.an-icon').click(function() {
		var icon =  $(this).next().text();
		$('.tbopen #service-icon input').val($(this).val()+icon);
		tb_remove();
		$('.block').removeClass('tbopen');
	});
	...
	$(".section-wrapper-block").resizable('disable');
});
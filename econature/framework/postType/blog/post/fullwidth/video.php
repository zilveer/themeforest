...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	if (!post_password_required()) {
		if ($cmsms_post_video_type == 'selfhosted' && !empty($cmsms_post_video_links) && sizeof($cmsms_post_video_links) > 0) {
			...
			$attrs = array( 
				'preload'  => 'none', 
				'height'   => $video_size['full-thumb']['height'], 
				'width'    => $video_size['full-thumb']['width'] 
			);
			
			
			if (has_post_thumbnail()) {
				$video_poster = wp_get_attachment_image_src((int) get_post_thumbnail_id(get_the_ID()), 'full-thumb');
			...
			echo '<div class="cmsms_video_wrap">' . 
				do_shortcode($wp_embed->run_shortcode('[embed width="' . $video_size['full-thumb']['width'] . '" height="' . $video_size['full-thumb']['height'] . '"]' . $cmsms_post_video_link . '[/embed]')) . 
			'</div>';

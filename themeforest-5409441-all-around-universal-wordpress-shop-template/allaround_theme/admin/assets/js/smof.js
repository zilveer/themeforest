...
jQuery(document).ready(function($){
	...
	socialnetwork_select();
	function socialnetwork_select() {
		$('.socialnetwork-select').change( function() {
			var new_icon = $(this).parent().prev().prev().attr('src');
			var new_url = new_icon.replace( new_icon.substring( new_icon.lastIndexOf('/')+1), $(this).val() );
			$(this).parent().prev().prev().attr('src', new_url);
		});
	}

	//Duplicate Social Networks
	$(".network_add_button").live('click', function() {
		var duplicateContent = $(this).parent().find('.duplicateelement').html();
		$(this).before(duplicateContent);
		objectId = $(this).parent().find('.socialnetwork').length;
		setObjectInput = $(this).prev().prev();
		setObjectSelect = $(this).prev().children('select');
	...
	$(".slide_add_button").live('click', function(){		
		...
		if ( sliderId == 'contact' ) {
				...
				var newIcons = slidesContainer.find('li:first').find('.duplicateelement').html();
				...
				newSlide += '<div class="duplicateelement">'+ changedIconsNew +'</div>';
				newSlide += '<a href="#" class="button network_add_button">Add Network</a><div class="clear"></div><a class="slide_delete_button" href="#">Delete</a><div class="clear"></div></div></li>';
		}		

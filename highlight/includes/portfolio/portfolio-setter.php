...
$html='';
$imgWidth=get_opt('_portfolio_width'.$column_number);
$imgHeight=get_opt('_portfolio_height'.$column_number);
	...
	if(have_posts()) {
		...
		if($title_link=='on'){
			$title='<a href="'.$permalink.'" title="'.get_the_title().'">'.get_the_title().'</a>';
		}else{
			$title=get_the_title();
		}
		
		$description=get_post_meta($post->ID, 'description_value', true);
		$descHtml=$show_desc=='true'?'<div class="item-desc" style="width:'.($imgWidth+10).'px;"><h4>'.$title.'</h4><hr/><p>'.$description.'</p></div>':'';
		...
		if(!$thumbnail){
			$thumbnail=pexeto_get_resized_image($preview,$imgWidth, $imgHeight, 100);
		}

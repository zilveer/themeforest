<?php 
    ...
    if (is_day()) {
        $title = sprintf(__("Archives: %s", BFI_I18NDOMAIN), get_the_date());
    } else if (is_month()) {
        $title = sprintf(__("Archives: %s", BFI_I18NDOMAIN), get_the_date('F, Y'));
    } else if (is_year()) {
        $title = sprintf(__("Archives: %s", BFI_I18NDOMAIN), get_the_date('Y'));
    } else if (is_category()) {
        $title = sprintf(__("Category Archives: %s", BFI_I18NDOMAIN), single_cat_title('', false));
    } else if (is_tag()) {
        $title = sprintf(__("Tag Archives: %s", BFI_I18NDOMAIN), single_cat_title('', false));
    } else if (is_author()) {
        global $author, $author_name;
        $curauth = (isset($_GET['author_name'])) ? get_user_by('slug', $author_name) : get_userdata(intval($author));
        $title = sprintf(__("Author Archives: %s", BFI_I18NDOMAIN), $curauth->nickname);
    } else if (!is_single() && !is_page() && get_post_type() != 'post') { // portfolio categories
        global $wp_query;
        $cat_obj = $wp_query->get_queried_object();
        $tax_obj = get_taxonomy($cat_obj->taxonomy);
        $title = sprintf(__('Category Archives %1$s: %2$s', BFI_I18NDOMAIN), $tax_obj->labels->name, $cat_obj->name);
    } else if (is_archive()) {
    ...
    <?php 
        ...
        while ( $wp_query->have_posts() ) : $wp_query->the_post();
             ...
             */
                ...
                bfi_set_excerpt_readmore('');
                ?>
                <article class='<?php echo get_post_class('', get_the_ID()) ?>'>
                    <a href='<?php echo get_permalink() ?>'><h2><?php the_title() ?></h2></a>
                    <hr>

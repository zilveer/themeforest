...
<?php 
if ( is_page() || is_single() ) :  // Page & Single post header options  ?>
	
	<?php if ( get_post_meta( get_the_ID(), 'pre_content_activation', true ) != 'off' ) : ?>

		<div class="pre-content" <?php if ( get_post_meta( get_the_ID(), 'pre_content_height', true ) ) { echo 'style="height:'. esc_html(get_post_meta( get_the_ID(), 'pre_content_height', true )) .'"'; } ?>>
		
			<?php if ( get_post_meta( get_the_ID(), 'rev_on_off', true ) != 'off' && get_post_meta( get_the_ID(), 'rev_slider_header', true ) != '' ) : ?>
				<div class="pre-content-slider"><?php putRevSlider( wp_kses_post(get_post_meta( get_the_ID(), 'rev_slider_header', true )) ); ?></div>
			<?php endif; ?>		
			
			<?php if ( get_post_meta( get_the_ID(), 'pre_content_html', true ) ) : ?>
				<div class="pre-content-html"><?php echo do_shortcode( wp_kses_post(get_post_meta( get_the_ID(), 'pre_content_html', true )) ); ?></div>
			<?php endif; ?>
			...
			<div class="row-inner">
				<h1 class="page-title">
					...
					elseif ( is_search() ) : // Search title
						printf( __( 'Search Results for: %s', 'craftsman' ), '<span>' . get_search_query() . '</span>' );
						
					elseif ( is_404() ) : // Search title
						echo '404';

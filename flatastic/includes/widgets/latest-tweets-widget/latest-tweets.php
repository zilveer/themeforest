...
function latest_tweets_render( $screen_name, $count, $rts, $ats, $pop = 0 ){
    try {
        ...
        if( ! twitter_api_configured() ){
            throw new Exception( __('Plugin not fully configured','flatastic') );
        }
...
class Latest_Tweets_Widget extends WP_Widget {
    ...
    public function __construct( $id_base = false, $name = '', $widget_options = array(), $control_options = array() ){
        ...
        $this->options = array(
            ...
            array (
                'name'  => 'screen_name',
                'label' => __('Twitter handle','flatastic'),
                'type'  => 'text'
            ),
            array (
                'name'  => 'num',
                'label' => __('Number of tweets','flatastic'),
                'type'  => 'number'
            ),
            array (
                'name'  => 'pop',
                'label' => __('Minimum popularity','flatastic'),
                'type'  => 'number'
            ),
            array (
                'name'  => 'rts',
                'label' => __('Show Retweets','flatastic'),
                'type'  => 'bool'
            ),
            array (
                'name'  => 'ats',
                'label' => __('Show Replies','flatastic'),
                'type'  => 'bool'
            ),
        );
        $name or $name = __('Latest Tweets','flatastic');
    ...
    private function check_instance( $instance ){
        ...
        $instance += array (
            'title' => __('Latest Tweets','flatastic'),
            'screen_name' => '',
            'num' => 5,
            'pop' => 0,
            'rts' => '',
...
if( is_admin() ){
    ...
    function latest_tweets_plugin_row_meta( $links, $file ){
        if( false !== strpos($file,'/latest-tweets.php') ){
            $links[] = '<a href="options-general.php?page=twitter-api-admin"><strong>'.esc_attr__('Connect to Twitter','flatastic').'</strong></a>';
        } 
        return $links;
    }

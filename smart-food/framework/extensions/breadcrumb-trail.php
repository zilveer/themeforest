...
class Breadcrumb_Trail {
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => __( 'Browse:',                             'breadcrumb-trail', 'smartfood' ),
			'home'                => __( 'Home',                                'breadcrumb-trail', 'smartfood' ),
			'error_404'           => __( '404 Not Found',                       'breadcrumb-trail', 'smartfood' ),
			'archives'            => __( 'Archives',                            'breadcrumb-trail', 'smartfood' ),
			/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
			'search'              => __( 'Search results for &#8220;%s&#8221;', 'breadcrumb-trail', 'smartfood' ),
			/* Translators: %s is the page number. */
			'paged'               => __( 'Page %s',                             'breadcrumb-trail', 'smartfood' ),
			/* Translators: Minute archive title. %s is the minute time format. */
			'archive_minute'      => __( 'Minute %s',                           'breadcrumb-trail', 'smartfood' ),
			/* Translators: Weekly archive title. %s is the week date format. */
			'archive_week'        => __( 'Week %s',                             'breadcrumb-trail', 'smartfood' ),
	...
	public function do_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'breadcrumb-trail', 'smartfood' ) ) );
	}
	...
	public function do_minute_archive_items() {
		...
		/* Add the minute item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( _x( 'i', 'minute archives time format', 'breadcrumb-trail', 'smartfood' ) ) );
	}
	...
	public function do_hour_archive_items() {
		...
		/* Add the hour item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( _x( 'g a', 'hour archives time format', 'breadcrumb-trail', 'smartfood' ) ) );
	}
	...
	public function do_day_archive_items() {
		...
		$year  = sprintf( $this->args['labels']['archive_year'],  get_the_time( _x( 'Y', 'yearly archives date format',  'breadcrumb-trail', 'smartfood' ) ) );
		$month = sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'breadcrumb-trail', 'smartfood' ) ) );
		$day   = sprintf( $this->args['labels']['archive_day'],   get_the_time( _x( 'j', 'daily archives date format',   'breadcrumb-trail', 'smartfood' ) ) );
	...
	public function do_week_archive_items() {
		...
		$year = sprintf( $this->args['labels']['archive_year'],  get_the_time( _x( 'Y', 'yearly archives date format', 'breadcrumb-trail', 'smartfood' ) ) );
		$week = sprintf( $this->args['labels']['archive_week'],  get_the_time( _x( 'W', 'weekly archives date format', 'breadcrumb-trail', 'smartfood' ) ) );
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format',  'breadcrumb-trail', 'smartfood' ) ) ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'breadcrumb-trail', 'smartfood' ) ) ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'breadcrumb-trail', 'smartfood' ) ) ) . '</a>';
...
class bbPress_Breadcrumb_Trail extends Breadcrumb_Trail {
	...
	public function do_trail_items() {
		...
		elseif ( bbp_is_topic_tag_edit() ) {
			...
			if ( true === $this->args['show_title'] )
				$this->items[] = __( 'Edit', 'breadcrumb-trail', 'smartfood' );
		}
		...
		elseif ( bbp_is_single_topic() ) {
			...
			if ( bbp_is_topic_split() && true === $this->args['show_title'] )
				$this->items[] = __( 'Split', 'breadcrumb-trail', 'smartfood' );

			/* If viewing a topic merge page. */
			elseif ( bbp_is_topic_merge() && true === $this->args['show_title'] )
				$this->items[] = __( 'Merge', 'breadcrumb-trail', 'smartfood' );

...
?>
	...
	<?php while ( have_posts() ) : the_post(); ?>
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class('tribe-events-event clearfix entry-content'); ?> itemscope itemtype="http://schema.org/Event">
			...
			<div class="entry-content-c">
				...
				<div class="tribe-events-event-entry" itemprop="description">
					<?php if (has_excerpt ()): ?>
						<?php the_excerpt(); ?>
					<?php else: ?>
						<?php the_content(); ?>
	...
	<div id="tribe-events-nav-below" class="tribe-events-nav clearfix">
		...
		if( tribe_is_upcoming() && get_previous_posts_link() ) : ?>
			<?php previous_posts_link( '<span><img style="margin-right: 10px;" src="' . get_template_directory_uri() . '/images/previous-arrows.png" alt="" /> '.__('Previous Events', 'tribe-events-calendar').'</span>' ); ?>
		<?php elseif( tribe_is_upcoming() && !get_previous_posts_link( ) ) : ?>
			<a href='<?php echo tribe_get_past_link(); ?>'><span><img style="margin-right: 10px;" src="<?php echo get_template_directory_uri() . '/images/previous-arrows.png'; ?>" alt="" /> <?php _e('Previous Events', 'tribe-events-calendar' ); ?></span></a>
		<?php elseif( tribe_is_past() && get_next_posts_link( ) ) : ?>
			<?php next_posts_link( '<span><img style="margin-right: 10px;" src="' . get_template_directory_uri() . '/images/previous-arrows.png" alt="" /> '.__('Previous Events', 'tribe-events-calendar').'</span>' ); ?>
		<?php endif; ?>
		</div>
		...
		if( tribe_is_upcoming() && get_next_posts_link( ) ) : ?>
			<?php next_posts_link( '<span>'.__('Next Events', 'tribe-events-calendar').' <img style="margin-left: 10px;" src="' . get_template_directory_uri() . '/images/next-arrows.png" alt="" /></span>' ); ?>
		<?php elseif( tribe_is_past() && get_previous_posts_link( ) ) : ?>
			<?php previous_posts_link( '<span>'.__('Next Events', 'tribe-events-calendar').' <img style="margin-left: 10px;" src="' . get_template_directory_uri() . '/images/next-arrows.png" alt="" /></span>' ); // a little confusing but in 'past view' to see newer events you want the previous page ?>
		<?php elseif( tribe_is_past() && !get_previous_posts_link( ) ) : ?>
			<a href='<?php echo tribe_get_upcoming_link(); ?>'><span><?php _e('Next Events', 'tribe-events-calendar'); ?> <img style="margin-left: 10px;" src="<?php echo get_template_directory_uri() . '/images/next-arrows.png'; ?>" alt="" /></span></a>
		<?php endif; ?>
		</div>

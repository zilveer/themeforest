...
<div id="js_seo_groups_panel">
	...
	<div class="option option-add-form">
		...
		<h4 class="option-title"><?php _e('Add SEO Group', 'cardealer'); ?></h4>

		<div class="controls">
			<input type="text" value="" id="seo_group_name" placeholder="<?php _e("type title here", 'cardealer') ?>">
	...
	<div class="option">
		...
		<h4 class="option-title"><?php _e('SEO Groups', 'cardealer'); ?></h4>
		<ul class="groups seo_groups_list">
			<?php if (!empty($seo_groups) AND is_array($seo_groups)): ?>
				<?php foreach ($seo_groups as $group_id => $seo_group) : ?>
					<?php if ($group_id): ?>
						<li>
							<a data-id="seo_group_<?php echo $group_id; ?>" class="js_edit_seo_group" href="#"><?php echo $seo_group['name']; ?></a>
							<input type="hidden" name="seo_group[<?php echo $group_id; ?>][name]" value="<?php echo $seo_group['name']; ?>" />
							<a href="#" title="<?php _e('Delete', 'cardealer'); ?>" class="remove js_remove_seo_group" data-id="seo_group_<?php echo $group_id ?>"></a>
							<a data-id="seo_group_<?php echo $group_id; ?>" href="#" title="<?php _e('Edit', 'cardealer'); ?>" class="edit js_edit_seo_group"></a>
						</li>
					<?php endif; ?>
				<?php endforeach; ?>
			<?php else: ?>

...
if( !class_exists('THB_AdminPage') ) {
	class THB_AdminPage {
		...
		public function __construct( $title, $slug, $capability=null )
		{
			...
			$this->_capability = apply_filters( 'thb_adminpage_capability', $this->_capability, $slug );
		}
		...
		{
			$page_template = new THB_TemplateLoader($this->_template, array(
				'page' => $this,
				'theme' => thb_theme()
			));
		...
		{
			$data = thb_array_extend($data, array(
				'page' => $this
			));

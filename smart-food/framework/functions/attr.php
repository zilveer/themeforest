...
add_filter( 'tdp_attr_body',    'tdp_attr_body',    5    );
add_filter( 'tdp_attr_header',  'tdp_attr_header',  5    );
add_filter( 'tdp_attr_subheader',  'tdp_attr_subheader',  5    );
add_filter( 'tdp_attr_footer',  'tdp_attr_footer',  5    );
add_filter( 'tdp_attr_content', 'tdp_attr_content', 5    );
add_filter( 'tdp_attr_sidebar', 'tdp_attr_sidebar', 5, 2 );
add_filter( 'tdp_attr_menu',    'tdp_attr_menu',    5, 2 );

/* Header attributes. */
add_filter( 'tdp_attr_branding',         'tdp_attr_branding',         5 );
add_filter( 'tdp_attr_site-title',       'tdp_attr_site_title',       5 );
add_filter( 'tdp_attr_site-description', 'tdp_attr_site_description', 5 );

/* Loop attributes. */
add_filter( 'tdp_attr_loop-meta',        'tdp_attr_loop_meta',        5 );
add_filter( 'tdp_attr_loop-title',       'tdp_attr_loop_title',       5 );
add_filter( 'tdp_attr_loop-description', 'tdp_attr_loop_description', 5 );

/* Post-specific attributes. */
add_filter( 'tdp_attr_post',            'tdp_attr_post',            5    );
add_filter( 'tdp_attr_entry',           'tdp_attr_post',            5    ); // Alternate for "post".
add_filter( 'tdp_attr_entry-title',     'tdp_attr_entry_title',     5    );
add_filter( 'tdp_attr_entry-author',    'tdp_attr_entry_author',    5    );
add_filter( 'tdp_attr_entry-published', 'tdp_attr_entry_published', 5    );
add_filter( 'tdp_attr_entry-content',   'tdp_attr_entry_content',   5    );
add_filter( 'tdp_attr_entry-summary',   'tdp_attr_entry_summary',   5    );
add_filter( 'tdp_attr_entry-terms',     'tdp_attr_entry_terms',     5, 2 );

/* Comment specific attributes. */
add_filter( 'tdp_attr_comment',           'tdp_attr_comment',           5 );
add_filter( 'tdp_attr_comment-author',    'tdp_attr_comment_author',    5 );
add_filter( 'tdp_attr_comment-published', 'tdp_attr_comment_published', 5 );
add_filter( 'tdp_attr_comment-permalink', 'tdp_attr_comment_permalink', 5 );
add_filter( 'tdp_attr_comment-content',   'tdp_attr_comment_content',   5 );
 ...
 * @return void
 */
function tdp_attr( $slug, $context = '' ) {
	echo tdp_get_attr( $slug, $context );
}
 ...
 * @return string
 */
function tdp_get_attr( $slug, $context = '' ) {

	$out    = '';
	$attr   = apply_filters( "tdp_attr_{$slug}", array(), $context );
 ...
 * @return array
 */
function tdp_attr_subheader( $attr ) {

	$attr['data-img']	=	esc_url(get_field('background_image'));

	// For the blog page
	//Get blog page id
	$page_id = get_option('page_for_posts');
	if( is_home() && get_field('background_image', $page_id) )
		$attr['data-img']	=	esc_url( get_field('background_image', $page_id) );
 ...
 * @return array
 */
function tdp_attr_content( $attr ) {
 ...
 * @return array
 */
function tdp_attr_sidebar( $attr, $context ) {
	...
	if ( !empty( $context ) ) {
		...
		$attr['aria-label'] = esc_attr( sprintf( _x( '%s Sidebar', 'sidebar aria label', 'tdp-core', 'smartfood' ), tdp_get_sidebar_name( $context ) ) );
	}
 ...
 * @return array
 */
function tdp_attr_menu( $attr, $context ) {
	...
	if ( !empty( $context ) ) {
		...
		$attr['aria-label'] = esc_attr( sprintf( _x( '%s Menu', 'nav menu aria label', 'tdp-core', 'smartfood' ), tdp_get_menu_location_name( $context ) ) );
	}
 ...
 * @return array
 */
function tdp_attr_branding( $attr ) {
 ...
 * @return array
 */
function tdp_attr_site_title( $attr ) {
 ...
 * @return array
 */
function tdp_attr_site_description( $attr ) {
 ...
 * @return array
 */
function tdp_attr_loop_meta( $attr ) {
 ...
 * @return array
 */
function tdp_attr_loop_title( $attr ) {
 ...
 * @return array
 */
function tdp_attr_loop_description( $attr ) {
...
function tdp_attr_post( $attr ) {
	...
	if ( !empty( $post ) ) {
		...
		elseif ( 'attachment' === get_post_type() && tdp_attachment_is_audio() ) {

			$attr['itemtype'] = 'http://schema.org/AudioObject';
		}

		elseif ( 'attachment' === get_post_type() && tdp_attachment_is_video() ) {
 ...
 * @return array
 */
function tdp_attr_entry_title( $attr ) {
 ...
 * @return array
 */
function tdp_attr_entry_author( $attr ) {
 ...
 * @return array
 */
function tdp_attr_entry_content( $attr ) {
 ...
 * @return array
 */
function tdp_attr_entry_summary( $attr ) {
 ...
 * @return array
 */
function tdp_attr_entry_terms( $attr, $context ) {
 ...
 * @return array
 */
function tdp_attr_comment( $attr ) {
 ...
 * @return array
 */
function tdp_attr_comment_author( $attr ) {
 ...
 * @return array
 */
function tdp_attr_comment_published( $attr ) {

	$attr['class']    = 'comment-published';
	$attr['datetime'] = get_comment_time( 'Y-m-d\TH:i:sP' );

	/* Translators: Comment date/time "title" attribute. */
	$attr['title']    = get_comment_time( _x( 'l, F j, Y, g:i a', 'comment time format', 'tdp-core', 'smartfood' ) );
 ...
 * @return array
 */
function tdp_attr_comment_permalink( $attr ) {
 ...
 * @return array
 */
function tdp_attr_comment_content( $attr ) {

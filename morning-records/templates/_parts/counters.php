...
if ($show_all_counters || morning_records_strpos($post_options['counters'], 'views')!==false) {
	?>
	<<?php echo trim($counters_tag); ?> class="post_counters_item post_counters_views icon-eye" title="<?php echo esc_attr( sprintf(__('Views - %s', 'morning-records'), $post_data['post_views']) ); ?>" href="<?php echo esc_url($post_data['post_link']); ?>"><span class="post_counters_number"><?php echo trim($post_data['post_views']); ?></span><?php if (morning_records_strpos($post_options['counters'], 'captions')!==false) echo ' '.esc_html__('Views', 'morning-records'); ?></<?php echo trim($counters_tag); ?>>
	<?php
}
...
if ($show_all_counters || morning_records_strpos($post_options['counters'], 'comments')!==false) {
	?>
	<a class="post_counters_item post_counters_comments icon-comment" title="<?php echo esc_attr( sprintf(__('Comments - %s', 'morning-records'), $post_data['post_comments']) ); ?>" href="<?php echo esc_url($post_data['post_comments_link']); ?>"><span class="post_counters_number"><?php echo trim($post_data['post_comments']); ?></span><?php if (morning_records_strpos($post_options['counters'], 'captions')!==false) echo ' '.esc_html__('Comments', 'morning-records'); ?></a>
	<?php 
}
...
if ($show_all_counters || morning_records_strpos($post_options['counters'], 'likes')!==false) {
	...
	?>
	<a class="post_counters_item post_counters_likes icon-heart <?php echo !empty($allow) ? 'enabled' : 'disabled'; ?>" title="<?php echo !empty($allow) ? esc_attr__('Like', 'morning-records') : esc_attr__('Dislike', 'morning-records'); ?>" href="#"
		...
		data-title-dislike="<?php esc_attr_e('Dislike', 'morning-records'); ?>"><span class="post_counters_number"><?php echo trim($post_data['post_likes']); ?></span><?php if (morning_records_strpos($post_options['counters'], 'captions')!==false) echo ' '.esc_html__('Likes', 'morning-records'); ?></a>
	<?php
}

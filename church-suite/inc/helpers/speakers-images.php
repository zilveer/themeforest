<?php
/*
 * Based on Plugin: Categories Images - http://zahlan.net/blog/2012/06/categories-images/
*/
?>
<?php	
define('Z_IMAGE_PLACEHOLDER', get_template_directory_uri() ."/images/no_speaker.jpg");
add_action('admin_init', 'z_init');
function z_init() {
	add_action('sermon_speaker_add_form_fields', 'z_add_texonomy_field');
	add_action('sermon_speaker_edit_form_fields', 'z_edit_texonomy_field');
	add_filter( 'manage_edit-sermon_speaker_columns', 'z_taxonomy_columns' );
	add_filter( 'manage_sermon_speaker_custom_column', 'z_taxonomy_column', 10, 3 );
}
...
function z_add_texonomy_field() {
	...
	echo '<div class="form-field">
		<label for="taxonomy_image">' . __('Image', 'webnus_framework') . '</label>
		<input type="text" name="taxonomy_image" id="taxonomy_image" value="" />
		<br/>
		<button class="z_upload_image_button button">' . __('Upload/Add image', 'webnus_framework') . '</button>
	</div>'.z_script();
}
...
function z_edit_texonomy_field($taxonomy) {
	...
	echo '<tr class="form-field">
		<th scope="row" valign="top"><label for="taxonomy_image">' . __('Image', 'webnus_framework') . '</label></th>
		...
		<button class="z_upload_image_button button">' . __('Upload/Add image', 'webnus_framework') . '</button>
		<button class="z_remove_image_button button">' . __('Remove image', 'webnus_framework') . '</button>
		</td>
	</tr>'.z_script();
}
...
function z_taxonomy_columns( $columns ) {
	...
	$new_columns['thumb'] = __('Image', 'webnus_framework');
...
function z_taxonomy_column( $columns, $column, $id ) {
	if ( $column == 'thumb' )
		$columns = '<span><img src="' . z_taxonomy_image_url($id, NULL, TRUE) . '" alt="' . __('Thumbnail', 'webnus_framework') . '" class="wp-post-image" /></span>';

...
if ( strlen( $link ) > 0 && 'none' !== $link ) {
	...
	if ( 'custom' === $link && ! empty( $url ) ) {
		...
		*/
		...
		//$image_block .= '<a href="{{ post_image_url }}" title="{{ post_title }}" class="vc_gitem-link vc-zone-link fancybox action-icons lightbox-icon"><i class="'. $lightbox_type .' fa-lg"></i></a>';
		$image_block .= '<a href="{{ post_image_url }}" title="{{ post_title }}" data-fancybox-group="gallery'. get_the_ID() . '-' . rand() .'" class="vc_gitem-link fancybox vc-zone-link"></a>';
	}

...
( function( $, window ) {
	...
	function import_demodata() {
		...
		var importButton = jQuery( '#wpGrade_import_demodata_button' ),
			container = jQuery( '#customize-control-pile_options-import_demodata_button_control' );
				...
				onQueueChange: function( length ) {
					...
					if ( length == 0 ) {
						if ( res.errors == false ) {
							...
							setTimeout( function() {
								resultcontainer.append( '<i>' + pile_admin_js_texts.import_all_done + '</i><br />' );
							}, 1000 );

							setTimeout( function() {
								resultcontainer.append( '<h3>' + pile_admin_js_texts.import_phew + '</h3><br /><p>' + pile_admin_js_texts.import_success_note + pile_admin_js_texts.import_success_reload + pile_admin_js_texts.import_success_warning + '</p>' );
							}, 1000 );
							...
							setTimeout( function() {
								resultcontainer.append( '<i>' + pile_admin_js_texts.import_failed + '</i><br />' );
							}, 1000 );
						}
		...
		importButton.bind( 'click', {set: saveData}, function( receivedData ) {
			...
			var activate = confirm( pile_admin_js_texts.import_confirm );
			...
			resultcontainer.append( '<br /><i>' + pile_admin_js_texts.import_working + '</i><br />' );
		...
		while ( stepNumber < numberOfSteps ) {
			...
			qInst.Queue( {
				...
				.fail( function( response ) {
					...
					resultcontainer.append( '<i style="color:red">' + pile_admin_js_texts.import_posts_failed + '</i><br />' );
				} )
					...
					if ( res != null && res.errors != null ) {
						if ( res.errors == false ) {
							if ( res.responses[0] != null ) {
								resultcontainer.append( '<i>' + pile_admin_js_texts.import_posts_step + ' ' +  + res.responses[0].supplemental.stepNumber + ' of ' + res.responses[0].supplemental.numberOfSteps + '</i><br />' );
								//for debuging purposes
								resultcontainer.append( '<div style="display:none;visibility:hidden;">Return data:<br />' + res.responses[0].data + '</div>' );
							} else {
								resultcontainer.append( '<i style="color:red">' + pile_admin_js_texts.import_posts_failed + '</i><br />' + pile_admin_js_texts.import_error + ' ' + res.responses[0].data );
							}
						}
						else {
							if ( res.responses[0] != null ) {
								resultcontainer.append( '<i style="color:red">' + pile_admin_js_texts.import_posts_failed + '</i><br />( ' + res.responses[0].errors[0].message + ' )<br/>' );
							} else {
								resultcontainer.append( '<i style="color:red">' + pile_admin_js_texts.import_posts_failed + '</i><br />' + pile_admin_js_texts.import_error + ' ' + res.responses[0].data );
							}
						}
					} else {
						resultcontainer.append( '<i style="color:red">' + pile_admin_js_texts.import_posts_failed + ' ' + pile_admin_js_texts.import_try_reload + ' </i><br />' );
					}
				} );
		}
	}
	...
	function ajax_import_theme_options(resultcontainer, this_data) {
		...
		qInst.Queue( {
			...
			.fail( function( response ) {
				responseRaw = response;
				res = wpAjax.parseAjaxResponse( response, 'notifier' );
				resultcontainer.append( '<i style="color:red">' + pile_admin_js_texts.import_theme_options_failed + '</i><br />' );
			} )
			.done( function( response ) {
				responseRaw = response;
				res = wpAjax.parseAjaxResponse( response, 'notifier' );
				if ( res != null && res.errors != null ) {
					if ( res.errors == false ) {
						resultcontainer.append( '<i>' + pile_admin_js_texts.import_theme_options_done + '</i><br />' );
						//for debuging purposes
						resultcontainer.append( '<div style="display:none;visibility:hidden;">Return data:<br />' + res.responses[0].data + '</div>' );
					}
					else {
						resultcontainer.append( '<i style="color:red">' + pile_admin_js_texts.import_theme_options_error + ': ' + res.responses[0].errors[0].message + ' )<br/><br/>' );
					}
				} else {
					resultcontainer.append( '<i style="color:red">' + pile_admin_js_texts.import_theme_options_failed + '</i><br />' );
				}
	...
	function ajax_import_widgets(resultcontainer, this_data) {
		...
		qInst.Queue( {
			...
			.fail( function() {
				responseRaw = response;
				res = wpAjax.parseAjaxResponse( response, 'notifier' );
				resultcontainer.append( '<i style="color:red">' + pile_admin_js_texts.import_widgets_failed + '</i><br />' );
			} )
			.done( function( response ) {
				responseRaw = response;
				res = wpAjax.parseAjaxResponse( response, 'notifier' );
				if ( res != null && res.errors != null ) {
					if ( res.errors == false ) {
						resultcontainer.append( '<i>' + pile_admin_js_texts.import_widgets_done + '</i><br />' );

						//for debuging purposes
						resultcontainer.append( '<div style="display:none;visibility:hidden;">Return data:<br />' + res.responses[0].data + '</div>' );
					}
					else {
						resultcontainer.append( '<i style="color:red">' + pile_admin_js_texts.import_widgets_error + ': '  + res.responses[0].errors[0].message + ' )<br/><br/>' );
					}
				} else {
					resultcontainer.append( '<i style="color:red">' + pile_admin_js_texts.import_widgets_failed + '</i><br />' );
				}

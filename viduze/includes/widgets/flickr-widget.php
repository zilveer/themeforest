<?php
/**
 * Plugin Name: CrunchPress Flickr Widget
...
class flickr extends WP_Widget {
	...
	function flickr() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'cp_flickr-widget', 'description' => __('A widget that show last flickr photo streams', 'crunchpress') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'cp_flickr-widget' );

		/* Create the widget. */
		parent::__construct( 'cp_flickr-widget', __(THEME_NAME. ' - Flickr', 'crunchpress'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Flickr Widget', 'crunchpress'), 'id' => __('0', 'crunchpress'), 'show_num' => '6', 'how' => 'lastest');
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'id' ); ?>"><?php _e('Flickr id', 'crunchpress'); ?></label>
		...
		<p>
			...
			<label><?php _e('Max is "10"', 'crunchpress'); ?></label>
		</p>
		
        <p>
			<label for="<?php echo $this->get_field_id( 'how' ); ?>"><?php _e('What pictures to display','crunchpress'); ?></label><br />

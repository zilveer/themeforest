...
if( !class_exists( 'YITH_WCWL_Init' ) ) {
    ...
    class YITH_WCWL_Init {
        ...
        public function install() {
            ...
            if( $this->db_version != get_option( 'yith_wcwl_db_version' ) ) {
        ...
        public function enqueue_styles_and_stuffs() {
            ...
            var yith_wcwl_plugin_ajax_web_url = '<?php echo YITH_WCWL_URL . 'yith-wcwl-ajax.php' ?>';
        ...
        public function enqueue_scripts() {
            ...
            $yith_wcwl_l10n = array(
                'out_of_stock' => __( 'Cannot add to the cart as product is Out of Stock!', 'homeshop' ),
            );
        ...
        public function add_page_setting_woocommerce( $settings ) {
            ...
            $settings[] = array(
                'name' => __( 'Wishlist Page', 'homeshop' ),
        		'desc' 		=> __( 'Page contents: [yith_wcwl_wishlist]', 'homeshop' ),
        ...
        public function print_plugin_options() {
            $links = apply_filters( 'yith_wcwl_tab_links', array(
                '<a href="#yith_wcwl_general_settings">' . __( 'General Settings', 'homeshop' ) . '</a>',
                '<a href="#yith_wcwl_styles">' . __( 'Styles', 'homeshop' ) . '</a>',
                '<a href="#yith_wcwl_socials_share">' . __( 'Socials &amp; Share', 'homeshop' ) . '</a>',
            ) );
        ...
        private function _styles_options() {
            ...
            yith_frontend_css_color_picker( __( '"Add to Wishlist" button background', 'homeshop' ), 'yith_wcwl_color_add_to_wishlist_background', $colors['add_to_wishlist']['background'] );
            yith_frontend_css_color_picker( __( '"Add to Wishlist" button text', 'homeshop' ), 'yith_wcwl_color_add_to_wishlist_color', $colors['add_to_wishlist']['color'] );
            yith_frontend_css_color_picker( __( '"Add to Wishlist" button border', 'homeshop' ), 'yith_wcwl_color_add_to_wishlist_border_color', $colors['add_to_wishlist']['border_color'] );
            
            ?><div class="clear" style="height:10px;"></div><?php
            
            yith_frontend_css_color_picker( __( '"Add to Cart" button background', 'homeshop' ), 'yith_wcwl_color_add_to_cart_background', $colors['add_to_cart']['background'] );
            yith_frontend_css_color_picker( __( '"Add to Cart" button text', 'homeshop' ), 'yith_wcwl_color_add_to_cart_color', $colors['add_to_cart']['color'] );
            yith_frontend_css_color_picker( __( '"Add to Cart" button border', 'homeshop' ), 'yith_wcwl_color_add_to_cart_border_color', $colors['add_to_cart']['border_color'] );
            
            ?><div class="clear" style="height:10px;"></div><?php
            
            yith_frontend_css_color_picker( __( 'Wishlist table background', 'homeshop' ), 'yith_wcwl_color_wishlist_table_background', $colors['wishlist_table']['background'] );
            yith_frontend_css_color_picker( __( 'Wishlist table text', 'homeshop' ), 'yith_wcwl_color_wishlist_table_color', $colors['wishlist_table']['color'] );
            yith_frontend_css_color_picker( __( 'Wishlist table border', 'homeshop' ), 'yith_wcwl_color_wishlist_table_border_color', $colors['wishlist_table']['border_color'] );
        ...
        private function _plugin_options() {
            ...
            $options['general_settings'] = array(
                array( 'name' => __( 'General Settings', 'homeshop' ), 'type' => 'title', 'desc' => '', 'id' => 'yith_wcwl_general_settings' ),
                
                array(
                    'name'      => __( 'Enable YITH Wishlist', 'homeshop' ),
                    'desc'      => __( 'Enable all plugin features.', 'homeshop' ), 
                    'id'        => 'yith_wcwl_enabled',
                    'std'       => 'yes',
                    'type'      => 'checkbox'
                ),
                array(
                    'name'      => __( 'Use cookies', 'homeshop' ),
                    'desc'      => __( 'Use cookies instead of sessions. With this feature, the wishlist will be available for each not logged user for 30 days. Use the filter yith_wcwl_cookie_expiration_time to change the expiration time ( needs timestamp ).', 'homeshop' ), 
                    'id'        => 'yith_wcwl_use_cookie',
                    'std'       => 'yes',
                    'type'      => 'checkbox'
                ),
                array(
                    'name'      => __( 'Wishlist title', 'homeshop' ),
            		'id' 		=> 'yith_wcwl_wishlist_title',
            		'std' 		=> sprintf( __( 'My wishlist on %s', 'homeshop' ), get_bloginfo( 'name' ) ),
            		'type' 		=> 'text',
            		'css' 		=> 'min-width:300px;',
                ),
                array(
                    'name'      => __( 'Position', 'homeshop' ),
                    'desc'      => __( 'On variable products you can add it only After "Add to Cart" or use the shortcode [yith_wcwl_add_to_wishlist].', 'homeshop' ),
                    'id'        => 'yith_wcwl_button_position',
                    'type'      => 'select',
                    'class'		=> 'chosen_select',
                    'css' 		=> 'min-width:300px;',
                    'options'   => array(
                        'add-to-cart' => __( 'After "Add to cart"', 'homeshop' ),
                        'thumbnails' => __( 'After thumbnails', 'homeshop' ),
                        'summary'   => __( 'After summary', 'homeshop' ),
                        'shortcode' => __( 'Use shortcode', 'homeshop' )
                    ),
                    'desc_tip'	=>  true
                ),
                array(
                    'name'      => __( 'Redirect to cart', 'homeshop' ),
                    'desc'      => __( 'Redirect to cart page if "Add to cart" button is clicked in the wishlist page.', 'homeshop' ), 
                    'id'        => 'yith_wcwl_redirect_cart',
                    'std'       => 'no',
                    'type'      => 'checkbox'
                ),
                array(
                    'name'      => __( 'Remove if added to the cart', 'homeshop' ),
                    'desc'      => __( 'Remove the product from the wishlist if is been added to the cart.', 'homeshop' ), 
                    'id'        => 'yith_wcwl_remove_after_add_to_cart',
                    'std'       => 'yes',
                    'type'      => 'checkbox'
                ),
                array(
                    'name'      => __( '"Add to Wishlist" text', 'homeshop' ),
            		'id' 		=> 'yith_wcwl_add_to_wishlist_text',
            		'std' 		=> __( 'Add to Wishlist', 'homeshop' ),
            		'type' 		=> 'text',
            		'css' 		=> 'min-width:300px;',
                ),
                array(
                    'name'      => __( '"Add to Cart" text', 'homeshop' ),
            		'id' 		=> 'yith_wcwl_add_to_cart_text',
            		'std' 		=> __( 'Add to Cart', 'homeshop' ),
            		'type' 		=> 'text',
            		'css' 		=> 'min-width:300px;',
                ),
            ...
            $options['styles'] = array(
                array( 'name' => __( 'Styles', 'homeshop' ), 'type' => 'title', 'desc' => '', 'id' => 'yith_wcwl_styles' ),
                
                array(
                    'name'      => __( 'Use buttons', 'homeshop' ),
                    'desc'      => __( 'Use buttons instead of a simple anchors.', 'homeshop' ), 
                    'id'        => 'yith_wcwl_use_button',
                    'std'       => 'yes',
                    'type'      => 'checkbox'
                ),
                array(
                    'name'      => __( 'Custom CSS', 'homeshop' ),
                ...
                array(
                    'name'      => __( 'Use theme style', 'homeshop' ),
                    'desc'      => __( 'Use the theme style.', 'homeshop' ), 
                    'id'        => 'yith_wcwl_frontend_css',
                    'std'       => 'yes',
                    'type'      => 'checkbox'
                ),
                array(
                    'name'      => __( 'Buttons rounded corners', 'homeshop' ),
                    'desc'      => __( 'Make buttons corner rounded', 'homeshop' ), 
                    'id'        => 'yith_wcwl_rounded_corners',
                    'std'       => 'yes',
                    'type'      => 'checkbox'
                ),
                array( 
                    'name'      => __( '"Add to Wishlist" icon', 'homeshop' ),
            		'desc' 		=> __( 'Add an icon to the "Add to Wishlist" button', 'homeshop' ),
            		'id' 		=> 'yith_wcwl_add_to_wishlist_icon',
            		'css' 		=> 'min-width:300px;width:300px;',
            		'std' 		=> apply_filters( 'yith_wcwl_add_to_wishlist_std_icon', 'none' ),
            		'type' 		=> 'select',
            		'class'		=> 'chosen_select',
            		'desc_tip'	=>  true,
                ...
                array( 
                    'name'      => __( '"Add to Cart" icon', 'homeshop' ),
            		'desc' 		=> __( 'Add an icon to the "Add to Cart" button', 'homeshop' ),
            		'id' 		=> 'yith_wcwl_add_to_cart_icon',
            		'css' 		=> 'min-width:300px;width:300px;',
            		'std' 		=> apply_filters( 'yith_wcwl_add_to_cart_std_icon', 'icon-shopping-cart' ),
            		'type' 		=> 'select',
            		'class'		=> 'chosen_select',
            		'desc_tip'	=>  true,
                    'options'   => array( 'none'=>'None' ) + $icons
            ...
            $options['socials_share'] = array(
                array( 'name' => __( 'Socials &amp; Share', 'homeshop' ), 'type' => 'title', 'desc' => '', 'id' => 'yith_wcwl_socials_share' ),
                
                array(
                    'name'      => __( 'Share on Facebook', 'homeshop' ),
                    'id'        => 'yith_wcwl_share_fb',
                    'std'       => 'yes',
                    'type'      => 'checkbox'
                ),
                array(
                    'name'      => __( 'Tweet on Twitter', 'homeshop' ),
                    'id'        => 'yith_wcwl_share_twitter',
                    'std'       => 'yes',
                    'type'      => 'checkbox'
                ),
                array(
                    'name'      => __( 'Pin on Pinterest', 'homeshop' ),
                    'id'        => 'yith_wcwl_share_pinterest',
                    'std'       => 'yes',
                    'type'      => 'checkbox'
                ),
                array(
                    'name'      => __( 'Socials title', 'homeshop' ),
                ...
                array(
                    'name'      => __( 'Socials text', 'homeshop' ),
                    'desc'      => __( 'Will be used by Facebook, Twitter and Pinterest. Use <strong>%wishlist_url%</strong> where you want the URL of your wishlist to appear.', 'homeshop' ),
                ...
                array(
                    'name'      => __( 'Socials image URL', 'homeshop' ),

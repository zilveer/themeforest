...
class Nav_Menu_Images_Admin extends Nav_Menu_Images {
	...
	public function et_update_item_settings($menu_id, $menu_item_db_id, $args) {
		...
		if ( @is_array( $_REQUEST['menu-item-background_repeat']) ) {
			$value = $_REQUEST['menu-item-background_repeat'][$menu_item_db_id];
			update_post_meta( $menu_item_db_id, '_menu-item-background_repeat', $value );
		}
		if ( @is_array( $_REQUEST['menu-item-background_position']) ) {
			$value = $_REQUEST['menu-item-background_position'][$menu_item_db_id];
			update_post_meta( $menu_item_db_id, '_menu-item-background_position', $value );
		}
		if ( @is_array( $_REQUEST['menu-item-anchor']) ) {
			$value = $_REQUEST['menu-item-anchor'][$menu_item_db_id];
			update_post_meta( $menu_item_db_id, '_menu-item-anchor', $value );
		}
		if ( @is_array( $_REQUEST['menu-item-design']) ) {
			$value = $_REQUEST['menu-item-design'][$menu_item_db_id];
			update_post_meta( $menu_item_db_id, '_menu-item-design', $value );
		}
		if ( @is_array( $_REQUEST['menu-item-columns']) ) {
			$value = $_REQUEST['menu-item-columns'][$menu_item_db_id];
			update_post_meta( $menu_item_db_id, '_menu-item-columns', $value );
		}
		if ( @is_array( $_REQUEST['menu-item-icon']) ) {
			$value = $_REQUEST['menu-item-icon'][$menu_item_db_id];
			update_post_meta( $menu_item_db_id, '_menu-item-icon', $value );
		}
		if ( @is_array( $_REQUEST['menu-item-label']) ) {
			$value = $_REQUEST['menu-item-label'][$menu_item_db_id];
			update_post_meta( $menu_item_db_id, '_menu-item-label', $value );
		}

		if ( @is_array( $_REQUEST['menu-item-disable_titles']) ) {
			if( isset($_REQUEST['menu-item-disable_titles'][$menu_item_db_id]) && $_REQUEST['menu-item-disable_titles'][$menu_item_db_id] == 1) {
				$value = $_REQUEST['menu-item-disable_titles'][$menu_item_db_id];
				update_post_meta( $menu_item_db_id, '_menu-item-disable_titles', $value );
			} else {
				update_post_meta( $menu_item_db_id, '_menu-item-disable_titles', 0 );
			}
	...
	public function enqueue_scripts() {
		...
		// Enqueue old script
		wp_enqueue_script( 'nmi-scripts', ET_CODE_3D_URI . 'menu-images/inc/nmi.js', array( 'media-upload', 'thickbox' ), '1', true );
		...
		if ( version_compare( $wp_version, '3.5', '>=' ) ) {
			wp_enqueue_media();
			wp_enqueue_script( 'nmi-media-view', ET_CODE_3D_URI . 'menu-images/inc/media-view.js', array( 'jquery', 'media-editor', 'media-views', 'post' ), '1.1', true );
		}

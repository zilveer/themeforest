...
if (!function_exists('sf_get_share_links')) {
	function sf_get_share_links( $url ) {
		...
		if( get_option( 'yith_wcwl_share_email' ) == 'yes' ) {
		    $html .= '<li class="mail" style="list-style-type: none; display: inline-block;">
		        <a class="mail" href="mailto:?subject=' . apply_filters( 'yith_wcwl_email_share_subject', __( 'I wanted you to see this site', 'swiftframework' ) ) . '&amp;body=' . apply_filters( 'yith_wcwl_email_share_body', $url ) . '&amp;title=' . $title . '" title="' . __( 'Email', 'swiftframework' ) . '"><i class="fa-envelope"></i><i class="fa-envelope"></i></a>
		    </li>';
		}

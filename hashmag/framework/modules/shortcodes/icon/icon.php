<?php
namespace Hashmag\Modules\Shortcodes\Icon;

use Hashmag\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package Hashmag\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface
{
    ...
    public function vcMap() {
        vc_map(array(
            'name' => esc_html__('Mikado Icon', 'hashmag'),
            'base' => $this->base,
            'category' => 'by MIKADO',
            'icon' => 'icon-wpb-icons extended-custom-icon',
            'allowed_container_element' => 'vc_row',
            'params' => array_merge(
                \HashmagMikadoIconCollections::get_instance()->getVCParamsArray(),
                array(
                    array(
                        'type' => 'dropdown',
                        'heading' => 'Size',
                    ...
                    array(
                        ...
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'hashmag'),
    ...
    public function render($atts, $content = null) {
        ...
        $html = hashmag_mikado_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
    ...
    private function generateIconStyles($params) {
        ...
        if (($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
            ...
            if (!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:' . hashmag_mikado_filter_px($params['custom_size']) . 'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if (isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if (!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: ' . hashmag_mikado_filter_px($shapeSize) . 'px';
                $iconHolderStyles[] = 'height: ' . hashmag_mikado_filter_px($shapeSize) . 'px';
                $iconHolderStyles[] = 'line-height: ' . hashmag_mikado_filter_px($shapeSize) . 'px';
            }
            ...
            if (!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
                ...
                $iconHolderStyles[] = 'border-width: ' . hashmag_mikado_filter_px($params['border_width']) . 'px';
            ...
            if ($params['type'] == 'square') {
                if (isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: ' . hashmag_mikado_filter_px($params['border_radius']) . 'px';
                }
            }
        }

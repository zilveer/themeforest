...
class WPBakeryShortCode_VC_mad_gallery extends WPBakeryShortCode {
	...
	private function html() {
		...
		if ($type == 'image_grid') {
			...
			$col_column_class = 'gallery-col-' . $columns;
			$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'gallery-grid wpb_content_element row ' . $col_column_class, $this->settings['base'], $this->atts );
		...
		<div class="portfolio-holder">
			...
			<div <?php echo $layout_type ?> class="wpb_wrapper <?php echo esc_attr($css_class) ?>">
				...
				<?php foreach ($images as $id => $attach_id):
					...
					<div class="<?php echo apply_filters('mad_gallery_el_start_class', 'portfolio-item', '') ?>">

<?php
namespace Flow\Modules\Shortcodes\Icon;

use Flow\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package Flow\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
            ...
            'params'                    => array_merge(
                ...
                array(
                    array(
                        ...
                        'value'       => array(
                            'Tiny'       => 'eltd-icon-tiny',
                            'Small'      => 'eltd-icon-small',
                            'Medium'     => 'eltd-icon-medium',
                            'Large'      => 'eltd-icon-large',
                            'Very Large' => 'eltd-icon-huge'
                        ),
                        'save_always' => true
                    ),
                    array(
                    ...
                    array(
                        ...
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'flow'),
    ...
    public function render($atts, $content = null) {
        ...
        $default_atts = array_merge($default_atts, flow_elated_icon_collections()->getShortcodeParams());
        ...
        $html = flow_elated_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.flow_elated_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.flow_elated_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.flow_elated_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.flow_elated_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.flow_elated_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.flow_elated_filter_px($params['border_radius']).'px';
                }
            }
        }

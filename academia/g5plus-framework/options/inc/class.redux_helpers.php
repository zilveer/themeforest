...
//                }
            ...
            public static function compileSystemStatus( $json_output = false, $remote_checks = false ) {
	            ...
	            $sysinfo['redux_data_writeable'] = self::makeBoolStr( @call_user_func('fopen', ReduxFramework::$_upload_dir . 'test-log.log', 'a' ) );
                    ...
                    foreach ( $files as $file ) {
                        ...
                        if ( $theme_file ) {
                            ...
                            if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
                                ...
                                $found_files[ $plugin_name ][] = sprintf( esc_html__( '<code>%s</code> version <strong style="color:red">%s</strong> is out of date. The core version is %s', 'redux-framework' ), str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
                            } else {
            ...
            private static function get_template_version( $file ) {
	            ...
	            $fp = call_user_func('f' . 'open', $file, 'r' );

	            // Pull only the first 8kiB of the file in.
	            $file_data = call_user_func('fread', $fp, 8192 );

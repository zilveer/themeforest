...
function mom_buttons_sc($atts, $content) {
	extract(shortcode_atts(array(
		...
		return '<span class="mom_button_wrap" '.$wstyle.'>'.$ob_start.'<a class="button mom_button '.$color.$size.$style.'" '. $link.$target.' style="'.$bgcolor.$textcolor.$font.$font_weight.$font_style.$radius.$textcolor.$bordercolor.$width.$center_button.'" data-bg="'.$bgcolorS.'" data-hoverbg="'.$hoverbg.'" data-text="'.$textcolorS.'" data-texthover="'.$texthcolor.'"'.$dborder.$hoverborder.'>'.$icon.do_shortcode($content). '</a>'.$ob_end.'</span>';

    ...
    if ( ! class_exists( 'Redux_Tracking' ) ) {
        ...
        class Redux_Tracking {
            ...
            function tracking_request() {
                ...
                $content = '<h3>' . __( 'Help improve Our Panel', 'mk_framework' ) . '</h3>';
                $content .= '<p>' . __( 'Please helps us improve our panel by allowing us to gather anonymous usage stats so we know which configurations, plugins and themes to test to ensure compatibility.', 'mk_framework' ) . '</p>';
                $opt_arr = array(
                    'content'  => $content,
            ...
            function newsletter_request() {
                ...
                $content = '<h3>' . __( 'Welcome to the Redux Demo Panel', 'mk_framework' ) . '</h3>';
                $content .= '<p><strong>' . __( 'Getting Started', 'mk_framework' ) . '</strong><br>' . sprintf( __( 'This panel demonstrates the many features of Redux.  Before digging in, we suggest you get up to speed by reviewing %1$s.', 'mk_framework' ), '<a href="' . 'http://' . 'docs.reduxframework.com/redux-framework/getting-started/" target="_blank">' . __( 'our documentation', 'mk_framework' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Redux Generator', 'mk_framework' ) . '</strong><br>' . sprintf( __( 'Want to get a head start? Use the %1$s. It will create a customized boilerplate theme or a standalone admin folder complete with all things Redux (with the help of Underscores and TGM). Save yourself a headache and try it today.', 'mk_framework' ), '<a href="' . 'http://' . 'generate.reduxframework.com/" target="_blank">' . __( 'Redux Generator', 'mk_framework' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Redux Extensions', 'mk_framework' ) . '</strong><br>' . sprintf( __( 'Did you know we have extensions, which greatly enhance the features of Redux?  Visit our %1$s to learn more!', 'mk_framework' ), '<a href="' . 'http://' . 'reduxframework.com/extensions/" target="_blank">' . __( 'extensions directory', 'mk_framework' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Like Redux?', 'mk_framework' ) . '</strong><br>' . sprintf( __( 'If so, please %1$s and consider making a %2$s to keep development of Redux moving forward.', 'mk_framework' ), '<a target="_blank" href="' . 'http://' . 'wordpress.org/support/view/plugin-reviews/redux-framework">' . __( 'leave us a favorable review on WordPress.org', 'mk_framework' ) . '</a>', '<a href="' . 'https://' . 'www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=N5AD7TSH8YA5U" target="_blank">' . __( 'donation', 'mk_framework' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Newsletter', 'mk_framework' ) . '</strong><br>' . __( 'If you\'d like to keep up to with all things Redux, please subscribe to our newsletter', 'mk_framework' ) . ':</p>';
                $content .= '<form action="' . 'http://' . 'reduxframework.us7.list-manage2.com/subscribe/post?u=564f5178f6cc288064f332efd&amp;id=ace5bbc1f9&SOURCE=panel" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate><p style="text-align: center;"><label for="mce-EMAIL">' . __( 'Email address', 'mk_framework' ) . ' </label><input type="email" value="" name="EMAIL" class="required email" id="mce-EMAIL"><input type="hidden" value="panel" name="SOURCE">&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="' . __( 'Subscribe', 'mk_framework' ) . '" name="subscribe" id="mc-embedded-subscribe" class="button button-primary"></p></form>';
                $opt_arr = array(
                    'content'      => $content,
                    'position'     => array( 'edge' => 'top', 'align' => 'center' ),
                    'pointerWidth' => 450
                ...
                $this->print_scripts( $id, $opt_arr, __( 'Close', 'mk_framework' ), false, '', $function1 );
            }

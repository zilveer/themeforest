...
while ( $my_query->have_posts() ) {
    ...
    if( $grid_template == 'filtered_grid' ) {
        ...
        $post_categories = wp_get_object_terms($my_query->post->ID, array_keys($taxonomies));
    ...
    if ( $grid_link != 'link_no' ) {
        $link = '<a class="more-link" href="'. esc_url( get_permalink($post_id) ) .'"'.$link_target.' title="'. sprintf( esc_attr__( 'Permalink to %s', 'js_composer' ), the_title_attribute( 'echo=0' ) ).'">'. __("Read more &rsaquo;", "js_composer") .'</a>';
    }

    // Thumbnail logic
    if ( in_array($grid_layout, array('title_thumbnail_text', 'thumbnail_title_text', 'thumbnail_title_date_category_text', 'title_date_category', 'thumbnail_title_date_category', 'thumbnail_text', 'thumbnail_title', 'thumbnail', 'title_text') ) ) {
    ...
    if ( $grid_link != 'link_no' ) {
        ...
        if ( $grid_link == 'link_post' ) {
            $link_image_start = '<a class="link_image" href="'.esc_url( get_permalink($post_id) ).'"'.$link_target.' title="'.sprintf( esc_attr__( 'Permalink to %s', 'js_composer' ), the_title_attribute( 'echo=0' ) ).'">';
            $link_title_start = '<a class="link_title" href="'.esc_url( get_permalink($post_id) ).'"'.$link_target.' title="'.sprintf( esc_attr__( 'Permalink to %s', 'js_composer' ), the_title_attribute( 'echo=0' ) ).'">';
        }
    ...
    // If grid layout is: Thumbnail + Title + Text
        ...
        if ( $content ) {
            $to_filter = '<div class="entry-content">' . $content . '</div><div class="more-link-wrapper">'. $link .'</div>';
            $teasers .= apply_filters('vc_teaser_grid_content', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "content" => $content, "media_link" => $p_link) );
        }
    }
	// If grid layout is: Thumbnail + Title + Date and Category + Text
    else if ( $grid_layout == 'thumbnail_title_date_category_text' ) {
        if ( $thumbnail ) {
            $to_filter = '<div class="post-thumb">' . $link_image_start . $thumbnail . $link_image_end .'</div>';
            $teasers .= apply_filters('vc_teaser_grid_thumbnail', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "thumbnail" => $thumbnail, "media_link" => $p_link) );
        }
        if ( $post_title ) 	{
			$categories = get_the_category_list( __( ', ', 'mega' ) );
            $to_filter = '<h2 class="post-title">' . $link_title_start . $post_title . $link_title_end . '</h2><div class="entry-meta"><span class="entry-date">'.esc_html( get_the_date() ).'</span><span class="sep">|</span>'.$categories.'</div>';
            $teasers .= apply_filters('vc_teaser_grid_title', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "title" => $post_title, "media_link" => $p_link) );
        }	
        if ( $content ) {
            $to_filter = '<div class="entry-content">' . $content . '</div><div class="more-link-wrapper">'. $link .'</div>';
	...
	// If grid layout is: Title + Date and Category
    else if ( $grid_layout == 'title_date_category' ) {
        if ( $post_title ) 	{
			$categories = get_the_category_list( __( ', ', 'mega' ) );
            $to_filter = '<h2 class="post-title">' . $link_title_start . $post_title . $link_title_end . '</h2><div class="entry-meta"><span class="entry-date">'.esc_html( get_the_date() ).'</span><span class="sep">|</span>'.$categories.'</div>';
            $teasers .= apply_filters('vc_teaser_grid_title', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "title" => $post_title, "media_link" => $p_link) );
	...
	// If grid layout is: Thumbnail + Title + Date and Category
    else if ( $grid_layout == 'thumbnail_title_date_category' ) {
        if ( $thumbnail ) {
            $to_filter = '<div class="post-thumb">' . $link_image_start . $thumbnail . $link_image_end .'</div>';
            $teasers .= apply_filters('vc_teaser_grid_thumbnail', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "thumbnail" => $thumbnail, "media_link" => $p_link) );
        }
    ...
    // If grid layout is: Title + Text
        ...
        if ( $post_title ) 	{
            $to_filter = '<div class="date-wrapper"><span>'.esc_html( get_the_date( 'd' ) ).'</span> <span class="entry-date">'.esc_html( get_the_date( 'M' ) ).'</span></div> <div class="post-wrapper"><h3 class="post-title title-text">' . $link_title_start . $post_title . $link_title_end . '</h3>';
            $teasers .= apply_filters('vc_teaser_grid_title', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "title" => $post_title, "media_link" => $p_link) );
		}
        if ( $content ) {
            $to_filter = '<div class="entry-content title-text">' . $content . '</div><div class="more-link-wrapper">'. $link .'</div></div>';
            $teasers .= apply_filters('vc_teaser_grid_content', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "content" => $content, "media_link" => $p_link) );
        }
    }
...
if( $grid_template == 'filtered_grid' && $teasers && !empty($teaser_categories)) {
    ...
    */
    $categories_array = get_terms(array_keys($taxonomies), array(
        'orderby' => 'name',
        'include' => implode(',', $teaser_categories)
    ));

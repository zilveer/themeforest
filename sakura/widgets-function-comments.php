...
function kjgrc_get_option($section,$option_name)
{
	...
	if ($kjgrc_options == NULL || $kjgrc_options['version'] != $version) 
	{
		$default_options = array (
			...
			"grc_format" => "<li>%author_url: <a href=\"%comment_link\" title=\"%post_title, %post_date\">%comment_excerpt</a> on <a href=\"%post_link\">%post_title</a></li>",
...
function kjgrc_is_wordpress_user($comment)
{
	...
	if ($kjgrc_wordpress_users == NULL) {
		...
		if (function_exists("get_users")) {
			$kjgrc_wordpress_users = get_users();
		} else {
			$kjgrc_wordpress_users = kjgrc_get_users();
		}
	}

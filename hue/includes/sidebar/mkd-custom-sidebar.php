...
if(!class_exists('HueMikadoSidebar')) {

	class HueMikadoSidebar {
		...
		function __construct() {
			...
			$this->title  = esc_html__('Custom Widget Area', 'hue');
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_html__('Enter Name of the new Widget Area', 'hue')."' name='mkd-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_html__('Add Widget Area', 'hue')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters('hue_mikado_custom_widget_args', $args);

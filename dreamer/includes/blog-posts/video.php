...
<div class="blog-format format-posts single-post <?php if (is_sticky()) { echo 'sticky'; } ?> ">
    ...
    <div class="twelve columns news-item video-container mobile-two">
        <?php if (isset($smof_data['news_video_post_icon']) && !empty($smof_data['news_video_post_icon'])): ?>
            <section class="news-cat-image format-cat-image">
                <img src="<?php echo $smof_data['news_video_post_icon']; ?>" alt="Icon">
            </section>
        <?php endif ?>
        <?php $embed = get_post_meta(get_the_ID(), 'hex_video_embed', TRUE); ?>
        <?php echo stripslashes(htmlspecialchars_decode($embed)); ?>
        <section style="height: auto;" class="news-details">
...
<div class="blog-format format-posts">
	<div <?php post_class(); ?>>
    <h3 class="news-blog-title format-title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
    <div class="twelve columns news-item video-container item-news paragraph-section mobile-two">
        <?php if (isset($smof_data['news_video_post_icon']) && !empty($smof_data['news_video_post_icon'])): ?>
            <section class="news-cat-image format-cat-image">
                <img src="<?php echo $smof_data['news_video_post_icon']; ?>" alt="Icon">
            </section>

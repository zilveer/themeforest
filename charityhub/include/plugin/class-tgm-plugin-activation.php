	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( $this->tgmpa->plugins as $plugin ) {
				...
				$table_data[ $i ]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'gdlr_translate' ) : __( 'Recommended', 'gdlr_translate' );
		...
		public function column_cb( $item ) {
			...
			if ( __( 'Private Repository', 'gdlr_translate' ) === $item['source'] ) {
				// Escape external URLs
				$plugin_url = esc_url( $plugin_url );
			}
			elseif ( __( 'Pre-Packaged', 'gdlr_translate' ) === $item['source'] ) {
		...
		public function no_items() {
			...
			printf( wp_kses_post( __( 'No plugins to install or activate. <a href="%1$s">Return to the Dashboard</a>', 'gdlr_translate' ) ), esc_url( self_admin_url() ) );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				...
				class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function add_strings() {
						...
						if ( $this->tgmpa->is_automatic ) {
							...
							$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'gdlr_translate' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'tgmpa' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'tgmpa' ) . '</span>.</a>';

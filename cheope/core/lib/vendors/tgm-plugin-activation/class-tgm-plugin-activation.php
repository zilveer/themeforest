    ...
    class TGM_Plugin_Activation {
        ...
        public function install_plugins_page() {
            ...
            <div class="tgmpa wrap">
                ...
                <div class="update-nag">
                    <?php $msg = sprintf(__('Some Pre-Packaged plugins <strong>must be moved</strong> from %s package to theme/plugins directory, inside the theme folder, <strong>before install</strong> plugins from this page.', 'tgmpa'), $this->_marketplace); ?>
                    <?php echo $msg; ?>
                </div>
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'tgmpa' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'tgmpa' ) . '</a>'  : '';
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                ...
                if ( 'Pre-Packaged' == $item['source'] && ! file_exists( $item['url'] ) ) {
                    $msg = sprintf(__('Get the ZIP package of the plugin from %s package, put it inside theme/plugins folder of the theme and reload this page', 'tgmpa'), $this->_marketplace );
                    $actions = array(

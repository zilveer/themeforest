...
$QData['AdsWidName'] = 'AdSense Widget %d';
$QData['Default'] = array(
	'AdsDisp'=>'3',
...
function ads_admin_page() {  
	add_menu_page("Google Adsense", "Google Adsense", 'add_users', basename(__FILE__), "ads_admin_page_inc", get_template_directory_uri().'/includes/quick-adsense/icon.png' );  
} 
  ...
  for($i=1;$i<=$QData['AdsWid'];$i++) {
		if(get_option('WidCode'.$i)!='') {
			$displaystr =
				...
				'}';
			$controlstr	=
				'if (isset($_POST["WidCodeSubmit'.$i.'"]) && $_POST["WidCodeSubmit'.$i.'"]) {'.
			...
			wp_register_sidebar_widget('ads'.$i, $wadsid, $displaycall[$i]);
			wp_register_widget_control('ads'.$i, $wadsid, $controlcall[$i]);
		}			
	}
}	

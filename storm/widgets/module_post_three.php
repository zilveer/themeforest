<?php
/**
 * Plugin Name: BK-Ninja: Module Post Three
 * Plugin URI: http://bk-ninja.com
 * Description: This module displays one main post and four sub posts below
 * Version: 1.0
 * Author: BK-Ninja
 ...
 */
add_action('widgets_init', 'bk_register_module_post_three');

function bk_register_module_post_three(){
	register_widget('bk_module_post_three');
}
 ...
 *
 */ 
class bk_module_post_three extends WP_Widget {
	...
	function __construct(){
		/* Widget settings. */	
		$widget_ops = array('classname' => 'module-post-three', 'description' => __('[Content module] Displays one main post and four sub posts below in content section', 'bkninja'));
		
		/* Create the widget. */
		parent::__construct('bk_module_post_three', __('*BK: Module Posts Three', 'bkninja'), $widget_ops);
	}
			...
			<div class="cat-header">
				<div class="cat-title">
                    <?php if($cat_url != NULL){?>				
                          ...
                          <?php }else{?>
                            <h3 ><?php _e('Latest Post','bkninja');?></h3>
                          <?php }?>
        			...
        			<?php while ( $query -> have_posts() ) : $query -> the_post(); $post_id = get_the_ID();?>	
        				...
        				<?php if ( $query->current_post == 0 ) { ?>		
        					<div class="two-col main-post clear-fix">
                                ...
                                if(($display == 'video')&&( get_post_format( $post_id ) == 'video') && (($bk_parse_url['host'] == 'www.youtube.com')||($bk_parse_url['host'] == 'youtube.com'))){
                                    ...
                                    echo '<div class="thumb-wrap video-thumb one-col">';
                                    ...
                                    echo '<div class="thumb-wrap video-thumb one-col"> <div class="bk-oEmbed-video">';
        				...
        				<?php if ( $query->current_post == 1 ) { ?>
                            ...
                            <?php if (( $query->current_post == 1 ) || ( $query->current_post == 3 )){ ?>
        					   <div class="one-col<?php if ( $query->current_post == 3 ) { echo ' last'; }?>">
                               <?php } ?>
        						<?php if ( $query->current_post >= 1 ) { ?>	
        							<div class="item-post">
                                ...
                                <?php } ?>
                                <?php if (( $query->current_post == 2 ) || ( $query->current_post == 4 ) || (($query->current_post == 1) && ($query->current_post == $last_post)) || (($query->current_post == 3) && ($query->current_post == $last_post))){ ?>
        					   </div>
                               <?php } ?>

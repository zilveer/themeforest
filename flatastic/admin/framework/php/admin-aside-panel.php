...
if (!class_exists('MAD_ADMIN_ASIDE_PANEL')) {
	...
	class MAD_ADMIN_ASIDE_PANEL extends MAD_FRAMEWORK {
			...
			if (!empty($data)) {
				...
				foreach ($data as $key => $value) {
					...
					if (in_array($name, $required_fields)) {
						if (empty($value)) {
							if ($name == "name") {
								$errors[$name] = __('Please enter your name before sending', 'flatastic');
							}
							if ($name == "email") {
								if (!$this->isValidEmail($value)) {
									$errors[$name] = __('Please enter your email before sending', 'flatastic');
								}
							}
							if ($name == "message") {
								$errors[$name] = __('Please enter your message', 'flatastic');
							}
						}
					}
				}
				...
				if ($emailto) {
					...
					if ($mail) {
						$result = array(
							'status' => 'success',
							'text' => __('Your message has been sent successfully!', 'flatastic')
						);
					} else {
						$result['status'] = 'error';
					}
		...
		public function vk_output() {
			...
			if ($show_vk_box):
				$vk_title = htmlspecialchars(mad_custom_get_option('vk_title', __('Join Us on VK', 'flatastic')));
		...
		public function facebook_output() {
			...
			if ($show_facebook_box):
				$facebook_title = mad_custom_get_option('facebook_title', __('Join Us on Facebook', 'flatastic'));
		...
		public function latest_tweets_output() {
			...
			if ($show_latest_tweets):
				...
				$latest_tweets_title = htmlspecialchars(mad_custom_get_option('latest_tweets_title', __('Latest Tweets', 'flatastic')));
						...
						<div class="twitterfeed">
							...
							<?php if ($show_follow_button == 'show_follow_button' && !empty($latest_tweets_username)): ?>
								<a target="_blank" class="follow-button" href="https://twitter.com/<?php echo $latest_tweets_username ?>">
									<?php _e('Follow on Twitter', 'flatastic') ?>
								</a>
							<?php endif; ?>
		...
		public function contact_us_output() {
			...
			if ($show_contact_us):
				$contact_us_title = htmlspecialchars(mad_custom_get_option('contact_us_title', __('Contact Us', 'flatastic')));
				$contact_us_short_text = mad_custom_get_option('contact_us_short_text', __('Lorem ipsum dolor sit amet, consectetuer adipis mauris', 'flatastic'));
			?>
				<li>
					...
					<div class="admin-panel-content">
						...
						<form id="contactform" method="post" class="mini" >
							<input type="text" name="name" placeholder="<?php _e('Your name', 'flatastic') ?>" />
							<input type="text" name="email" placeholder="<?php _e('Your email', 'flatastic') ?>">
							<textarea placeholder="<?php _e('Message', 'flatastic') ?>" name="message"></textarea>
							<button type="submit"><?php _e('Send', 'flatastic'); ?></button>
						</form>
		...
		public function store_location_output() {
			...
			if ($show_store_location):
				$store_location_title = htmlspecialchars(mad_custom_get_option('store_location_title', __('Store Location', 'flatastic')));
				$store_location_address = mad_custom_get_option('store_location_address', __('8901 Marmora Road, Glasgow, D04 89GR.', 'flatastic'));
		...
		public function instagram_output() {
			...
			if ($show_instagram):
				$instagram_title = htmlspecialchars(mad_custom_get_option('instagram_title', __('Instagram', 'flatastic')));
		...
		public function pinterest_output() {
			...
			if ($show_pinterest):
				$pinterest_title = htmlspecialchars(mad_custom_get_option('pinterest_title', __('Pinterest', 'flatastic')));

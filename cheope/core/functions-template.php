...
if( !function_exists( 'yit_og_meta_tags' ) ) {
    ...
    function yit_og_meta_tags( $post_id ) {
        ...
        <meta property="og:title" content="<?php echo function_exists( 'wp_get_document_title' ) ? wp_get_document_title() : wp_title( '-', false, 'right' ) ?>"/>
        ...
        ?><meta property="og:description" content="<?php echo $ogcontent ?>" />

        <?php $type =  apply_filters( 'yit_og_type' , $queried_object );?>
        <?php if($type): ?>
...
if( !function_exists( 'yit_get_title' ) ) {
     ...
     * @since 1.0.0
     */
    function yit_get_title( $title = '', $sep = '', $seplocation = '' ) {
        ...
        if( $post_id ) {
            ...
            if( ! empty( $post_type ) && yit_get_post_meta( $post_id, '_seo-title' ) ){
                $title['title'] = yit_get_post_meta( $post_id, '_seo-title' );
            }

            if( empty( $title['title'] ) ) {
                $title['title'] = yit_get_option( $post_type . '-seo-title' );
            }

            if( empty( $title ) ) {
                $title['title'] = yit_get_option( 'seo-title' );
            }

            if( empty( $title ) && ! is_plugin_active( 'wordpress-seo/wp-seo.php' ) ) {
                ...
                $title = yit_remove_chars_title( $title['title'] );

                // Add description, if is home
                $description =  get_bloginfo( 'description' );
                if ( $description && ( is_home() || is_front_page() ) ) {
                    $title['title'] .= ' | ' . yit_remove_chars_title( $description );
                }

            }
        } else {

            $title['title'] .= yit_remove_chars_title( get_bloginfo('name') );
            ...
            $title['title'] = yit_icl_translate('WP','yit','blog_title',$title);
        }

        // Add a page number if necessary:
        if ( $paged >= 2 || $page >= 2 )
            $title['title'] .= ' | ' . sprintf( __( 'Page %s', 'yit' ), max( $paged, $page ) );
...
if( !function_exists( 'yit_get_attachment_id' ) ) {
	...
	function yit_get_attachment_id( $url ) {
		...
		// remove the protocol
		$dir = str_replace( array( 'http:', 'https:' ), '', $dir );
		...
		foreach( $ids as $id ){
			...
			if( $url == str_replace( array( 'http:', 'https:' ), '', array_shift( $attachment_image ) ) )
				return $id;
		}
		...
		foreach( $ids as $id ) {
			...
			foreach( (array) $meta['sizes'] as $size => $values ){
				...
				if( $values['file'] == $file && $url == str_replace( array( 'http:', 'https:' ), '', array_shift( $attachment_image ) ) ) {
...
//                 echo '<a href="' . $homeLink . '/' . $slug['slug'] . '/">' . $post_type->labels->singular_name . '</a> ' . $delimiter . ' ';
        ...
        } elseif ( !is_single() && !is_page() && get_post_type() != 'post' && !is_404() && !$is_blog) {
            $post_type = get_post_type_object( get_post_type() );
        ...
        elseif ( ( ( is_page() && ! $post->post_parent ) || ( $is_blog && ! $post->post_parent ) ) && ! is_tag() ){
            echo $before . ucfirst( strtolower( get_the_title() ) ) . $after;
        }
        elseif ( ( is_page() && $post->post_parent ) || ( $is_blog && $post->post_parent ) ) {

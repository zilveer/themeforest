...
if(get_post_meta($id, "edgt_page_background_color", true) != ""){
    $full_screen_holder_style .= "background-color:".esc_attr(get_post_meta($id, "edgt_page_background_color", true)).";";
}else{
    $full_screen_holder_style .= "";
}
...
<?php get_header(); ?>
    <div class="full_screen_preloader">
        <?php
        if($edgt_options['loading_animation'] == "on") {
            if($edgt_options['loading_image'] != "") { ?>
                <div class="ajax_loader"><div class="ajax_loader_1"><div class="ajax_loader_2"><img src="<?php echo esc_url($edgt_options['loading_image']) ?>" alt="" /></div></div></div>
            <?php }
            else { ?>
                <div class="ajax_loader"><div class="ajax_loader_1"><?php edgt_loading_spinners(); ?></div></div>
            <?php }
        }
        ?>
    </div>

    <div class="full_screen_holder"<?php edgt_inline_style($full_screen_holder_style); ?>>
        <div class="full_screen_inner" <?php edgt_inline_style($content_style); ?>>

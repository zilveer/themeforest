	...
	class Breadcrumb_Trail
	{
		...
		public function trail()
		{
			...
			if (!empty($this->items) && is_array($this->items)) {
				...
				$breadcrumb = "\n\t\t" . '<' . tag_escape($this->args['container']) . ' class="' . tag_escape($this->args['container_class']) . '"' . $vocabulary . '>';
				...
				foreach ($this->items as &$link) {
					...
					if ($this->args['rich_snippet'] === true) {
						...
						$link = preg_replace( '/<a (.*?)>(.*?)<\/a>/', '<a $1 itemprop="url"><span itemprop="title">$2</span></a>', $link, -1, $count_replace );
						$micro_data = '';
						if ( $count_replace > 0 ) {
							$micro_data = ' itemscope itemtype="http://data-vocabulary.org/Breadcrumb"';
						}

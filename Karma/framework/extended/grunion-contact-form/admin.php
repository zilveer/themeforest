...
add_action( 'admin_print_styles', 'tt_grunion_admin_css' );
function tt_grunion_admin_css() {
...
#icon-edit, #icon-post { background: url("<?php echo TT_GRUNION_PLUGIN_URL; ?>/images/grunion-menu-big.png") no-repeat !important; }
</style>
...
add_action( 'admin_menu', 'tt_grunion_admin_menu' );
function tt_grunion_admin_menu() {
	global $menu, $submenu;
	unset( $submenu['edit.php?post_type=feedback'] );
}

add_filter( 'bulk_actions-edit-feedback', 'tt_grunion_admin_bulk_actions' );
function tt_grunion_admin_bulk_actions( $actions ) {
	global $current_screen;
	if ( 'edit-feedback' != $current_screen->id )
		return $actions;
...
add_filter( 'views_edit-feedback', 'tt_grunion_admin_view_tabs' );
function tt_grunion_admin_view_tabs( $views ) {
	global $current_screen;
	if ( 'edit-feedback' != $current_screen->id )
		return $actions;
...
add_filter( 'manage_feedback_posts_columns', 'tt_grunion_post_type_columns_filter' );
function tt_grunion_post_type_columns_filter( $cols ) {
	$cols = array(
		'cb'	=> '<input type="checkbox" />',
		'feedback_from'		=> __( 'From','truethemes_localize' ),
		'feedback_message'		=> __( 'Message','truethemes_localize' ),
		'feedback_date'			=> __( 'Date','truethemes_localize' )
	);

	return $cols;
}

add_action( 'manage_posts_custom_column', 'tt_grunion_manage_post_columns', 10, 2 );
function tt_grunion_manage_post_columns( $col, $post_id ) {
	global $post;
	...
	switch ( $col ) {
		...
		case 'feedback_message':
			...
			if ( $post->post_status == 'trash' ) {
				...
				echo '">' . __( 'Restore','truethemes_localize' ) . '</a></span> | ';

				echo "<span class='delete'> <a class='submitdelete' title='";
				echo esc_attr( __( 'Delete this item permanently','truethemes_localize' ) );
				echo "' href='" . get_delete_post_link( $post->ID, '', true );
				echo "'>" . __( 'Delete Permanently','truethemes_localize' ) . "</a></span>";
?>
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as spam', 'truethemes_localize' );
				...
				echo '">';
				echo '<a class="submitdelete" title="' . esc_attr__( 'Trash','truethemes_localize' );
				echo '" href="' . get_delete_post_link( $post_id );
				echo '">' . __( 'Trash','truethemes_localize' ) . '</a></span>';
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as NOT spam','truethemes_localize' );
...
<?php
		...
		case 'feedback_date':
			echo get_the_date( __( 'Y-m-d @ g:i:s A','truethemes_localize'));
			break;
	}
}

function tt_grunion_esc_attr( $attr ) {
	$out = esc_attr( $attr );
...
function tt_grunion_sort_objects( $a, $b ) {
...
function tt_grunion_ajax_shortcode() {
	check_ajax_referer( 'grunion_shortcode' );
	
	$atts = '';
	if ( trim( $_POST['subject'] ) )
		$atts .= ' subject="'.tt_grunion_esc_attr($_POST['subject']).'"';
	if ( trim( $_POST['to'] ) )
		$atts .= ' to="'.tt_grunion_esc_attr($_POST['to']).'"';
/*
*Mod by tt, add in [raw] tag 		
*/
	$shortcode = '[raw][contact-form'.$atts.']';
	$shortcode .= "\n";
	if ( is_array( $_POST['fields'] ) ) {
		usort( $_POST['fields'], 'tt_grunion_sort_objects' );
		foreach ( $_POST['fields'] as $field ) {
			...
			if ( isset( $field['options'] ) && $field['options'] ) {
				...
				foreach ( $field['options'] as $option ) {
					...
					$option = tt_grunion_esc_attr( $option );
			...
			$shortcode .= '[contact-field label="'. $field['label'] .'" type="'.tt_grunion_esc_attr($field['type']).'"' . $req . $opts .' /]'."\n";
		}
	}
/*
*Mod by tt, add in [/raw] tag 		
...
function tt_grunion_ajax_shortcode_to_json() {
...
add_action( 'wp_ajax_grunion_shortcode', 'tt_grunion_ajax_shortcode' );
add_action( 'wp_ajax_grunion_shortcode_to_json', 'tt_grunion_ajax_shortcode_to_json' );


// process row-action spam/not spam clicks
add_action( 'wp_ajax_grunion_ajax_spam', 'tt_grunion_ajax_spam' );
function tt_grunion_ajax_spam() {
	global $wpdb;
	...
	if ( !current_user_can("edit_page", $post_id) )
		wp_die( __( 'You are not allowed to manage this item.','truethemes_localize') );
	...
	if ( $_POST['make_it'] == 'spam' ) {
		...
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item out of the Trash.','truethemes_localize') );

		if ( ! wp_untrash_post($post_id) )
			wp_die( __('Error in restoring from Trash.', 'truethemes_localize') );

	} elseif( $_POST['make_it'] == 'trash' ) {
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item to the Trash.','truethemes_localize') );

		if ( ! wp_trash_post($post_id) )
			wp_die( __('Error in moving to Trash.','truethemes_localize') );
	...
	if ( isset( $status['publish'] ) ) {
		...
		$status_html .= '>' . __( 'Messages','truethemes_localize' ) . ' <span class="count">';
	...
	if ( isset( $status['trash'] ) ) {
		...
		$status_html .= '>' . __( 'Trash','truethemes_localize' ) . ' <span class="count">';
	...
	if ( isset( $status['spam'] ) ) {
		...
		$status_html .= '>' . __( 'Spam','truethemes_localize' ) . ' <span class="count">';

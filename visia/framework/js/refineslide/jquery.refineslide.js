...
;(function ($, window, document, undefined) {
	...
	function RS(elem, settings) {
		...
		this.$slides            = this.$slider.find('> div');           // Elem Arr: Slide elements
	...
	RS.prototype = {
        ...
        ,setup: function (clones) {
            ...
            var i = clones.length,c;
            while (i--) {
                //widths.push($(clones[i]).width());
				c = clones[i];
				widths.push(c[0].naturalWidth || c[0].width || c.width());
			...
			// added by pixelentity
			this.$slider.trigger("init.pixelentity");
	...
	Transition.prototype = {
        ...
        ,before: function (callback) {
            ...
            if (this.RS.cssTransitions) {
				...
				// fixes background tab bug on ff (transition end doesn't fire) 
				setTimeout(onEnd,_this.RS.settings['transitionDuration']*2);
				$(this.listenTo).one('webkitTransitionEnd transitionend otransitionend msTransitionend MSTransitionEnd',onEnd);
            }
        }

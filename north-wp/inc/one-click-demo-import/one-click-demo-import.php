...
//		}
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ( false !== has_action( $action ) ) && empty( $this->frontend_error_messages ) ) {
			...
			$this->do_import_action( $action, $this->import_files[ $this->selected_index ], $this->selected_import_files['options'] );
		}
	 ...
	 */
	private function do_import_action( $action, $selected_import, $selected_import_files ) {

		ob_start();
			do_action( $action, $selected_import, $selected_import_files );
		$message = ob_get_clean();

...
if (!class_exists("ThemeUpdater")) {
	...
	class ThemeUpdater {
		...
		public function check($updates) {
			...
			$this->username = apply_filters("theme_updater_username",$this->username);
			$this->apiKey = apply_filters("theme_updater_apiKey",$this->apiKey);
			$this->authors = apply_filters("theme_updater_authors",$this->authors);
			...
			if (!class_exists("Envato_Protected_API")) {
				require_once("envato-protected-api.php");
			}
		...
		public static function init($username = null,$apiKey = null,$authors = null) {
			new ThemeUpdater($username,$apiKey,$authors);
		}

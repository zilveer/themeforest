...
function breadcrumbs() {
	...
	$breadcrumbs = get_post_meta($cmsms_page_id, 'cmsms_breadcrumbs', true);
	$custom_breadcrumbs = get_post_meta($cmsms_page_id, 'cmsms_custom_breadcrumbs', true);
	...
	if ($breadcrumbs == 'custom' && $custom_breadcrumbs != '') {
		foreach ($custom_breadcrumbs as $custom_breadcrumb) {
			echo '<a href="' . $custom_breadcrumb[1] . '">' . $custom_breadcrumb[0] . '</a>' . $sep;
		}
		
		echo '<span>' . cmsms_title($cmsms_page_id, false) . '</span>';
	} elseif ($breadcrumbs != 'disabled') {
		if (is_single()) {
			...
			if ($num_cat < 1) {
				...
				echo get_category_parents($category[0], true, $sep) . ' ' . '<span>' . cmsms_title($cmsms_page_id, false) . '</span>';
			}
		} elseif (is_category()) {
			global $cat;
			...
			echo '<a href="' . $url_year . '">' . $year_format . '</a>' . $sep . '<a href="' . $url_month . '">' . $month_format . '</a>' . $sep . $day_format . ' (' . $day_full_format . ')';
		} elseif (is_month()) {

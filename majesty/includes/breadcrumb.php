	...
	public function __construct( $templates = array(), $options = array(), $strings = array(), $autorun = true ) {
		...
		$this->templates = wp_parse_args(
			...
			array(
				...
				'standard' => '<li>%s</li>',
				'before' => '<nav><ol class="breadcrumb">',
				'after' => '</ol></nav>'
			)
		);
		...
		$this->strings = wp_parse_args( $strings, array(
			'home' => esc_html__('Home', 'theme-majesty'),
			'search' => array(
				'singular' => esc_html__('Search Result for: %s', 'theme-majesty'),
				'plural'   => esc_html__('Search Results for: %s', 'theme-majesty')
			),
			'paged' => esc_html__('Page %d', 'theme-majesty'),
			'404_error' => esc_html__('Error 404', 'theme-majesty')
		) );

		// Generate breadcrumb
		if ( $autorun)
			echo wp_kses( $this->output(), $majesty_allowed_tags );
	}
		...
		if ( is_singular() ) { // Posts, (Sub)Pages, Attachments and Custom Post Types
			...
			if ( $this->options['show_pagenum'] )
				$this->breadcrumb['search'] = $this->template( array(
					'link' => esc_url(home_url( '/?s=' . urlencode( get_search_query( false ) ) )),
					'title' => $text
				) );

...
$grid_content = $el_class = $width = $orderby = $order = $el_position = $isotope_item = $animation = '';
$delay = $mydelay = $delay_time = $delaystart = '';
...
while ( $my_query->have_posts() ) {
    ...
    $delay_time = $delay_time + $delay +$delaystart;
    $delaystart = 0;

    if ( $delay != '') {
    $mydelay = ' style="animation-delay: '.$delay_time.'ms; -moz-animation-delay: '.$delay_time.'ms; -webkit-animation-delay: '.$delay_time.'ms;"';
    }
    ...
    if($grid_content == 'teaser') {
        $content = apply_filters('the_excerpt', wp_trim_words( get_the_content(), 13 ));
    } else {
    ...
    if(empty($thumbnail) || has_post_format( 'video' ) || has_post_format( 'audio' )) {
        $video_url = get_post_meta( get_the_ID(), 'l_url', true );
        $video_url2 = get_post_meta( get_the_ID(), 'v_url', true );
        $audio_url = get_post_meta ( get_the_ID(), 'a_url', true );

        if ( !empty($video_url)) {
            $embed_url = explode("?v=", $video_url);
            $new_url = $embed_url[1];
            $ourthumbnail = '<iframe width="100%" height="208" src="http://www.youtube.com/embed/'.$new_url.'?wmode=opaque" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
        }
        ...
        if ( !empty($video_url2)) {
            ...
            $ourthumbnail  = '<iframe width="100%" height="208" src="http://player.vimeo.com/video/'.$new_url.'" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
        }

        if ( !empty($audio_url)) {
            $ourthumbnail = $audio_url;
        }

        $video_url = $video_url2 = $audio_url = "";
    }

    else {
        $prettyurl = wp_get_attachment_url( get_post_thumbnail_id($post->ID) );
        $ourthumbnail = $thumbnail.'<span class="overthumb"></span><div class="carousel-icon">
                            <a href="'.$prettyurl.'" data-rel="prettyPhoto" class="prettyPhoto lightzoom">
                            <i class="mukam-search"></i>
                            </a>
                            <a href="'.get_permalink($post_id).'" class="postlink">
    ...
    $teasers .= '<li class="'.$isotope_item.apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $li_span_class, 'vc_teaser_grid_li').$categories_css.'"><div class="'.$animation.' latest-item animated"'.$mydelay.'>';
    // If grid layout is: Title + Thumbnail + Text
    if ( $grid_layout == 'title_thumbnail_text' ) {
        if ( $post_title ) 	{
            $to_filter = '<h4 class="post-title">' . $link_title_start . $post_title . $link_title_end . '</h4>';
        ...
        if ( $ourthumbnail ) {
            $to_filter = '<div class="widget-thumb">' . $ourthumbnail .'</div>';
    ...
    $teasers .= '</div></li> ' . $this->endBlockComment('single teaser');
} // endwhile loop
wp_reset_query();
...
if( $grid_template == 'filtered_grid' && $teasers && !empty($teaser_categories)) {
    ...
    */
    ...
    $categories_list_output = '<div class="custom-categories"><ul class="categories_filter clearfix">';
    ...
    $categories_list_output.= '</ul></div><div class="clearfix"></div>';
} else {
...
if ( $grid_template == 'carousel' ) { 
    $output .= apply_filters( 'vc_teaser_grid_carousel_arrows', '<a href="#" class="prev"><span><i class="icon-angle-left icon-3x"></i></span></a> <a href="#" class="next"><span><i class="icon-angle-right icon-3x"></i></span></a>' );
}
$output .= "\n\t\t".'<div class="wpb_wrapper">';
//$output .= ($title != '' ) ? "\n\t\t\t".'<h4 class="wpb_heading wpb_teaser_grid_heading">'.$title.'</h4>' : '';

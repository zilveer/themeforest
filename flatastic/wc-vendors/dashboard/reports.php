...
<div class="woocommerce">
	...
	<h2><?php _e( 'Sales Report', 'flatastic' ); ?></h2>
	...
	<table class="table table-condensed table-vendor-sales-report">
		<thead>
		<tr>
			<th class="product-header"><?php _e( 'Product', 'flatastic' ); ?></th>
			<th class="quantity-header"><?php _e( 'Quantity', 'flatastic' ) ?></th>
			<th class="commission-header"><?php _e( 'Commission', 'flatastic' ) ?></th>
			<th class="rate-header"><?php _e( 'Rate', 'flatastic' ) ?></th>
<!--			<th></th>-->
		</thead>
		<tbody>
		...
		<?php if ( !empty( $vendor_summary ) ) : ?>
			...
			<?php if ( !empty( $vendor_summary[ 'products' ] ) ) : ?>
				...
				<?php foreach ( $vendor_summary[ 'products' ] as $product ) :
					...
					<tr>
						...
						<?php if ( $can_view_orders ) : ?>
							<td>
								<a href="<?php echo $product[ 'view_orders_url' ]; ?>"><?php _e( 'Show Orders', 'flatastic' ); ?></a>
							</td>
						<?php endif; ?>
				...
				<tr>
					<td><strong><?php _e( 'Totals', 'flatastic' ); ?></strong></td>
			...
			<?php else : ?>
				...
				<tr>
					<td colspan="4"
						style="text-align:center;"><?php _e( 'You have no sales during this period.', 'flatastic' ); ?></td>
				</tr>
		...
		<?php else : ?>
			...
			<tr>
				<td colspan="4"
					style="text-align:center;"><?php _e( 'You haven\'t made any sales yet.', 'flatastic' ); ?></td>
			</tr>

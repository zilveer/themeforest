...
class Webnus_LatestPosts extends WP_Widget{
	...
	public function form($instance){
		...
		<p><label for="<?php echo $this->get_field_id('numberOfPosts') ?>">Number of Posts:</label><input type="text" class="widefat" id="<?php echo $this->get_field_id('numberOfPosts') ?>" name="<?php echo $this->get_field_name('numberOfPosts') ?>" value="<?php if( isset($numberOfPosts) )  echo esc_attr($numberOfPosts); ?>" /></p>
		<?php 
	}
	public function widget($args, $instance){
		...
		?>
		  <li>
		  <?php get_the_image( array( 'meta_key' => array( 'Full', 'Full' ), 'size' => 'tabs-img' ) );  ?>

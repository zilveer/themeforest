...
if ( !function_exists( 'morning_records_template_matches_1_theme_setup' ) ) {
	add_action( 'morning_records_action_before_init_theme', 'morning_records_template_matches_1_theme_setup', 1 );
	function morning_records_template_matches_1_theme_setup() {
		morning_records_add_template(array(
			'layout' => 'matches-1',
			'template' => 'matches-1',
			'mode'   => 'matches',
			/*'container_classes' => 'sc_slider_nopagination sc_slider_controls sc_slider_controls_bottom',*/
			'title'  => esc_html__('Matches /Style 1/', 'morning-records'),
...
// Template output
if ( !function_exists( 'morning_records_template_matches_1_output' ) ) {
	function morning_records_template_matches_1_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<li<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if (!$post_data['post_protected']) {
					...
					$match_preview = !empty($post_meta['match_link']) && !morning_records_param_is_inherit($post_meta['match_link']) ? $post_meta['match_link'] : '';	

...
window.CMB = (function(window, document, $, undefined){
	...
	cmb.metabox = function() {
		...
		cmb.$metabox = jQuery('table.cmb_metabox');
		return cmb.$metabox;
	}
	...
	cmb.init = function() {
		...
		$metabox
			.on( 'change', '.cmb_upload_file', function() {
				cmb.formfield = jQuery(this).attr('id');
				jQuery('#' + cmb.formfield + '_id').val('');
			})
		...
		if ( $repeatGroup.length ) {
			$repeatGroup
				.filter('.sortable').each( function() {
					// Add sorting arrows
					jQuery(this).find( '.remove-group-row' ).before( '<a class="shift-rows move-up alignleft" href="#">'+ l10n.up_arrow +'</a> <a class="shift-rows move-down alignleft" href="#">'+ l10n.down_arrow +'</a>' )
				})
		...
		jQuery(window).on( 'resize', cmb.resizeoEmbeds );
	...
	cmb.handleMedia = function(event) {
		...
		var $formfield   = jQuery('#'+cmb.formfield);
		...
		var handlers = {
			list : function( selection ) {
				...
				jQuery('#'+ cmb.formfield +'_id').val(attachment.id);
				...
				jQuery( fileGroup ).each( function() {
	...
	jQuery.fn.replaceText = function(b, a, c) {
		return this.each(function() {
			...
			d.length && jQuery(d).remove()
		})
	}

	jQuery.fn.cleanRow = function( prevNum, group ) {
		var $self = jQuery(this);
	...
	jQuery.fn.newRowHousekeeping = function() {
		var $row         = jQuery(this);
	...
	cmb.addGroupRow = function( event ) {
		...
		var $self    = jQuery(this);
		var $table   = jQuery('#'+ $self.data('selector'));
		...
		var $newRow = jQuery( '<tr class="repeatable-grouping" data-iterator="'+ cmb.idNumber +'">'+ $row.html() +'</tr>' );
		$oldRow.after( $newRow );
	...
	cmb.emptyValue = function( event, row ) {
		jQuery('input:not([type="button"]), textarea', row).val('');
	}
	...
	cmb.removeAjaxRow = function( event ) {
		...
		// cmb.log( 'number of trs', jQuery('tr', $table).length );
	...
	cmb.doAjax = function($obj, e) {
		...
		// only proceed if the user has pasted, pressed a number, letter, or whitelisted characters
			...
			var embed_container = jQuery('.embed_status', $context);
			var oembed_width = $obj.width();
			var child_el = jQuery(':first-child', embed_container);
			...
			setTimeout( function() {
				...
				if (jQuery('.cmb_oembed:focus').val() != oembed_url)
					return;
				jQuery.ajax({
					type : 'post',
					dataType : 'json',
	...
	jQuery(document).ready(cmb.init);

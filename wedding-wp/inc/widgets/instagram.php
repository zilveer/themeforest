...
class WebnusInstagramWidget extends WP_Widget{
	...
	public function form($instance){
		...
		<p><label for="<?php echo $this->get_field_id('type'); ?>">Recent media published by:</label>
		<select id="<?php echo $this->get_field_id('type'); ?>" name="<?php echo $this->get_field_name('type'); ?>" class="widefat" style="width:100%;">
		<option <?php if ('Username' == $instance['type']) echo 'selected="selected"'; ?>>Username</option>
		<option <?php if ('Hashtag' == $instance['type']) echo 'selected="selected"'; ?>>Hashtag</option>
		</select></p>
		<p><label for="<?php echo $this->get_field_id('username') ?>">Instagram Username/Hashtag:</label>
		...
		<p><label for="<?php echo $this->get_field_id('token') ?>">Instagram Access Token:</label>
		...
		<small>Get the this information <a target="_blank" href="http://www.pinceladasdaweb.com.br/instagram/access-token/">here</a>.</small></p>
		<p><label for="<?php echo $this->get_field_id('count') ?>">Feed Count(Max 20):</label>

<?php
	...
	*/
	...
	function print_item_header( $header, $addition = '', $header_class = '' ){
		if(!empty($header)){
			echo '<div class="gdl-header-wrapper ' . $header_class . '">';
			echo '<h3 class="gdl-header-title">' . $header . '</h3>';
			echo '<div class="gdl-header-gimmick-wrapper">';
			if( !empty( $addition ) ){
				echo '<div class="gdl-header-right" >' . $addition . '</div>';
			}			
			echo '<div class="gdl-header-gimmick"></div>';
			echo '</div>'; // gdl-header-gimmick-wrapper
			
			echo '<div class="clear"></div>';
			echo '</div>'; // gdl-header-wrapper
		}
	}
	...
	function print_column_service($item_xml){
		...
		if( !empty($column_service_icon) ){
			$icon = wp_get_attachment_image_src( $column_service_icon , $item_size );
			$alt_text = get_post_meta($column_service_icon , '_wp_attachment_image_alt', true);
			
			echo '<img class="column-service-image" src="' . $icon[0] .'" alt="'. $alt_text .'"/>';
		}
		echo '<h2 class="column-service-title">' . $column_service_title . '</h2>';
		echo '<div class="column-service-title-gimmick"></div>';
	...
	// Print Feature Service 1
	function print_feature_service_1($item_xml){
		$service_media = find_xml_value($item_xml, 'media');
		
		echo '<div class="feature-service-1">';
		
		if( !empty($service_media) ){
			$icon = wp_get_attachment_image_src( $service_media , $item_size );
			$alt_text = get_post_meta($service_media , '_wp_attachment_image_alt', true);
			
			echo '<img class="feature-service-media" src="' . $icon[0] .'" alt="'. $alt_text .'"/>';
		}		
		echo '<div class="clear"></div>';
		echo '<div class="feature-service-description"><span>';
		echo find_xml_value($item_xml, 'description');
		echo '</span></div>';
		
		echo '</div>';
	}	

	// Print Feature Service 2
	function print_feature_service_2($item_xml){
		echo '<div class="feature-service-2">';
		
		echo '<div class="feature-service-title-wrapper">';
		$service_icon = find_xml_value($item_xml, 'icon');
		if( !empty($service_icon) ){
			$icon = wp_get_attachment_image_src( $service_icon , $item_size );
			$alt_text = get_post_meta($service_icon , '_wp_attachment_image_alt', true);
			echo '<img class="feature-service-icon" src="' . $icon[0] .'" alt="'. $alt_text .'"/>';
		}	
		
		echo '<h3 class="feature-service-title">';
		echo find_xml_value($item_xml, 'title');
		echo '</h3>';		
		echo '</div>';
		
		echo '<div class="feature-service-caption">';
		echo find_xml_value($item_xml, 'caption');
		echo '</div>';
		
		echo '<div class="feature-service-description">';
		echo find_xml_value($item_xml, 'description');
		echo '</div>';
		...
		if( !empty($button_link) ){
			...
			echo '<div class="feature-service-button">';
			echo '<a class="gdl-button" href="' . $button_link . '">' . $button_text . '</a>';
			echo '</div>';
		}
	...
	function print_testimonial($item_xml){
		...
		if( $display_type == 'Static Testimonial' ){
			...
			if( !empty($header) ){
				print_item_header( $header, '', 'nav-on' );
			}else{
	...
	function print_slider_item($item_xml){
		...
		switch(find_xml_value($item_xml,'slider-type')){
			...
			case 'Custom Slider':
				print_custom_slider(find_xml_node($item_xml,'slider-item'), $xml_size, $slider_height);
				break;
		}

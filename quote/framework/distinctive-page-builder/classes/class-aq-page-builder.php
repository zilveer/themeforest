...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function __construct( $config = array()) {
			...
			$defaults['page_title'] = __('Distinctive Themes Page Builder', 'framework');
			$defaults['page_slug'] = __('distinctive-page-builder', 'framework');
		...
		function add_admin_bar(){
			global $wp_admin_bar;
			$wp_admin_bar->add_menu( array( 'id' => 'distinctive-page-builder', 'parent' => 'appearance', 'title' => 'Page Builder', 'href' => admin_url('themes.php?page='.$this->args['page_slug']) ) );
		...
		function admin_enqueue() {
			...
			wp_enqueue_style( 'dtpb-css', get_template_directory_uri() . '/framework/distinctive-page-builder/assets/css/aqpb.css' , array(), time(), 'all');
			wp_enqueue_style( 'dtpb-el', get_template_directory_uri() . '/assets/css/font-awesome.min.css' , array(), time(), 'all');
			wp_enqueue_style( 'dtpb-pe-icons', get_template_directory_uri() . '/assets/css/pe-icons.css' , array(), time(), 'all');
			wp_enqueue_style( 'dtpb-blocks-css', get_template_directory_uri() . '/framework/distinctive-page-builder/assets/css/aqpb_blocks.css' , array(), time(), 'all');
			wp_enqueue_script('dtpb-js', get_template_directory_uri().'/framework/distinctive-page-builder/assets/js/aqpb.js', array('jquery'), time(), true);
			wp_enqueue_script('dtpb-fields-js', get_template_directory_uri().'/framework/distinctive-page-builder/assets/js/aqpb-fields.js' , array('jquery'), time(), true);
		...
		function view_enqueue() {
			...
			// front-end css
			wp_register_style( 'aqpb-view-css', get_template_directory_uri() . '/framework/distinctive-page-builder/assets/css/aqpb-view.css', array(), time(), 'all');
			wp_enqueue_style('aqpb-view-css');
				
			// front-end js
			wp_register_script('aqpb-view-js', get_template_directory_uri() . '/framework/distinctive-page-builder/assets/js/aqpb-view.js', array('jquery'), time(), true);
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				<div id="aqpb-iframe-container" style="display: none;">
					<div class="wrap" style="padding: 1em">
						...
						if ( empty( $templates ) ) {
							echo sprintf( __( 'You don\'t have any template yet. Let\'s %s create %s one!', 'framework' ), '<a href="' .admin_url().'themes.php?page=distinctive-page-builder">', '</a>' );
							return;
						}
						?>						
		...
		function contextual_help() {
			...
			if($screen->id == $this->page) {
				...
				$screen->set_help_sidebar(
					'<p><strong>' . __('For more information:' ,'bold') . '</strong></p>' .

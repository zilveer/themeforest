	...
	function submit_button( $value = '', $action = 'action', $class = "button" ) {
		if ( is_array( $value ) ) {
			extract( wp_parse_args( $value, array(
				'value' => __( 'Save Changes', WEBNUS_TEXT_DOMAIN),
				'action' => 'action',
				'class' => 'button',
				'ajax' => true
			) ) );
...
//  ____________INTERNAL METHODS____________
	...
	function page_init() {
		...
		if ( ! $toplevel ) {
			$this->pagehook = add_theme_page( $parent, $page_title, $menu_title, $capability, $page_slug, array( $this, '_page_content_hook' ) );
		} else {
	...
	private function check_args() {
		...
		$this->args = wp_parse_args( $this->args, array(
			...
			'page_slug' => '',
			'nonce' => '',
			'action_link' => __( 'Settings', WEBNUS_TEXT_DOMAIN),
			'ajax_submit' => false,

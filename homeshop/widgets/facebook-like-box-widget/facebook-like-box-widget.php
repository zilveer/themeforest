...
class FacebookLikeBoxWidget extends WP_Widget
{
	...
	*/
	function FacebookLikeBoxWidget(){
		...
		parent::__construct('FacebookLikeBox', 'homeshop'.' - '.__('Facebook Like Box Widget', 'homeshop'), $widget_ops, $control_ops);
	}
	...
	*/
	function widget($args, $instance){
		...
		if (strlen($pageURL) > 23) {	
			...
			$like_box_iframe = "<iframe src=\"http://www.facebook.com/plugins/likebox.php?id=$pageID&amp;width=$width&amp;colorscheme=$colorScheme&amp;show_border=false&amp;border_color=$borderColor&amp;show_faces=$showFaces&amp;connections=$connection&amp;stream=$streams&amp;header=$header&amp;height=$height\" scrolling=\"no\" frameborder=\"0\" style=\"border:none; overflow:hidden; width:" . $width . "px; height:" . $height . "px;\" allowTransparency=\"true\"></iframe>";
...
<?php
		...
		echo '<p style="text-align:right;"><label for="' . $this->get_field_name('twitterButtonStyle') . '"> <select name="' . $this->get_field_name('twitterButtonStyle')  . '" id="' . $this->get_field_id('twitterButtonStyle')  . '">"';
?>

	...
	if(!empty($posts))
	{
		...
		$return_html.= '<ul class="posts">';
			...
			foreach($posts as $key => $post)
			{
				...
				$return_html.= '<strong class="title"><a href="'.get_permalink($post->ID).'">'.pp_substr($post->post_title, 50).'</a></strong><br/><span class="post_attribute">'.get_the_time('F j, Y', $post->ID).'</span><br/>';
				$return_html.= '</li>';
			}	

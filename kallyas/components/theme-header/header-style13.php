...
$flexbox_scheme = array(
    'main' => array(
        'left' => array(
            'stretch' => 'fxb-basis-auto hidden-xs hidden-sm',
        ),
        'right' => array(
            'stretch' => 'fxb-basis-auto hidden-xs hidden-sm',
        ),
...
if(zn_check_components('flags')) add_action( 'zn_head__top_left', 'zn_wpml_language_switcher_alt', 20 ); // WPML LANGUAGE SWITCHER
if(zn_check_components('custom_text')) add_action( 'zn_head__top_left', 'zn_header_head_text', 30 ); // CUSTOM TEXT


/**** TOP RIGHT */

if(zn_check_components('header_nav')) add_action( 'zn_head__top_right', 'zn_add_navigation', 10 ); // HEADER NAVIGATION
if(zn_check_components('login')) add_action( 'zn_head__top_right', 'zn_login_text', 20 ); // LOGIN/LOGOUT TEXT
...
if(zn_check_components('search_box')) add_action( 'zn_head__top_right', 'zn_header_searchbox_min', 50 ); // SEARCH BOX
...
add_action( 'zn_head__main_center', 'zn_header_main_menu', 10 ); // MAIN NAVIGATION
add_action( 'zn_head__main_center', 'zn_header_display_logo', 20 ); // LOGO MARKUP

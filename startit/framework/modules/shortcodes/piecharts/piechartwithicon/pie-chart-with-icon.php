<?php
namespace QodeStartit\Modules\PieCharts\PieChartWithIcon;
...
class PieChartWithIcon implements ShortcodeInterface {
	...
	public function vcMap() {
		vc_map( array(
			'name' => 'Select Pie Chart With Icon',
			'base' => $this->getBase(),
			...
			'params' => array_merge(
				...
				array(
					array(
						...
						'dependency' => Array('element' => 'icon_pack', 'value' => qode_startit_icon_collections()->getIconCollectionsKeys())
					),
					array(
						'type' => 'textfield',
	...
	public function render($atts, $content = null)
	{
		...
		$html = qode_startit_get_shortcode_module_template_part('templates/pie-chart-with-icon', 'piecharts/piechartwithicon', '', $params);
	...
	private function getPieChartData($params) {
		...
		if( $params['active_bar_color'] !== '' ) {
			...
			$pieChartData['data-bar-color'] = qode_startit_options()->getOptionValue('first_color');
		}
	...
	private function getPieChartIcon($params) {
		...
		$pie_chart_icon = qode_startit_icon_collections()->renderIcon( $params[$icon], $params['icon_pack'], $iconStyles );

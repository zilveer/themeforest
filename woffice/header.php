...
<html <?php language_attributes(); ?>>
	...
	$design_update_class = ($design_update == "2.X") ? "woffice-2-x" : "";

	?>

	<!-- START BODY -->
	<body <?php body_class($menu_class . ' ' . $sidebar_show_class . ' ' . $extra_navbar_class .' '.$design_update_class); ?>>
		...
		<div id="page-wrapper" <?php echo (!$nav_opened_state) ? 'class="menu-is-closed"':''; ?>>
			...
			<?php
			if(!is_page_template('page-templates/blank-page.php')):
			?>
				...
				<nav id="navbar" class="<?php echo esc_attr($extra_navbar_class ); ?>">
					<div id="nav-left">
						...
						if ($header_logo_hide == false) { ?> 
							<!-- START LOGO -->
							<div id="nav-logo">
								<a href="<?php echo esc_url( apply_filters('woffice_logo_link_to', home_url( '/' ) ) ); ?>">
									<?php

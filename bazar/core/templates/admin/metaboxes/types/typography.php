...
<div id="<?php echo $id ?>" class="rm_typography rm_option">
    <div class="option">
        ...
        <div class="spinner_container">
        	<input class="number" type="text" name="<?php echo $name ?>[size]" id="<?php echo $id ?>-size" value="<?php echo $value['size'] ?>" />
        </div>
        ...
        <div class="select_wrapper font-family">
            ...
            if( !empty( $web_fonts ) ) {
                ...
                foreach( $web_fonts as $font_name => $rule ) {
                    ?>
                    <option value='<?php echo $rule ?>' <?php selected( stripslashes( $value['family'] ), $rule ) ?>><?php echo $font_name ?></option>
                    <?php
                }
...
//                     $std_human  = trim( stripslashes( end( array_slice( explode( ',', $value['family'] ), 0, 1 ) ) ), "'" );
        ...
        <input type="text" name="<?php echo $name ?>[color]" id="<?php echo $id ?>-color" style="width:150px" value="<?php echo $value['color'] ?>" />
        <?php endif; ?>
    </div>
    <div class="clear"></div>

<?php
// Pagination
if( ! function_exists( 'peekaboo_pagination' ) ) {
	function peekaboo_pagination() {
		global $wp_query;
		...
		$paginate_links = paginate_links( array(
			...
			'mid_size' => 5,
			'prev_next' => True,
		    'prev_text' => __('&laquo;', 'peekaboo'),
		    'next_text' => __('&raquo;', 'peekaboo'),
			'type' => 'list'
		) );
 ...
 */
if( ! function_exists( 'peekaboo_menu_fallback' ) ) {
	function peekaboo_menu_fallback() {
		echo '<div class="alert-box secondary">';
		// Translators 1: Link to Menus, 2: Link to Customize
	  	printf( __( 'Please assign a menu to the primary menu location under %1$s or %2$s the design.', 'peekaboo' ),
	  		sprintf(  __( '<a href="%s">Menus</a>', 'peekaboo' ),
	  			get_admin_url( get_current_blog_id(), 'nav-menus.php' )
	  		),
	  		sprintf(  __( '<a href="%s">Customize</a>', 'peekaboo' ),
...
if( ! function_exists( 'peekaboo_active_nav_class' ) ) {
	function peekaboo_active_nav_class( $classes, $item ) {
...
add_filter( 'nav_menu_css_class', 'peekaboo_active_nav_class', 10, 2 );

/**
 * Use the active class of ZURB Foundation on wp_list_pages output.
 * From required+ Foundation http://themes.required.ch
 */
if( ! function_exists( 'peekaboo_active_list_pages_class' ) ) {
	function peekaboo_active_list_pages_class( $input ) {
...
add_filter( 'wp_list_pages', 'peekaboo_active_list_pages_class', 10, 2 );
 ...
 * From required+ Foundation http://themes.required.ch
 */
// class peekaboo_walker extends Walker_Nav_Menu
// {

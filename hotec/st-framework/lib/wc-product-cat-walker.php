...
class ST_WC_Product_Cat_List_Walker extends Walker {
	...
	function start_el( &$output, $cat, $depth, $args ) {
		...
		$output .=  '"><a href="' . get_term_link( (int) $cat->term_id, 'product_cat' ) . '">' . $cat->name .$count. '</a>';
...
function st_wc_product_cat_widget($args){
   $args['walker'] 			= new ST_WC_Product_Cat_List_Walker;
    return $args;
}

add_filter('woocommerce_product_categories_widget_args','st_wc_product_cat_widget');

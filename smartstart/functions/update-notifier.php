...
define( 'NOTIFIER_THEME_NAME', 'SmartStart' ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', 'smartstart' ); // The theme folder name
define( 'NOTIFIER_XML_FILE', 'http://demo.samuli.me/updates/smartstart-wp.xml' ); // The remote notifier XML file containing the latest version of the theme and changelog
...
function update_notifier_menu() {  
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		$theme_version = update_notifier_theme_version(); // Read theme current version from the style.css
		
		if( (float)$xml->latest > $theme_version) { // Compare current theme version with the remote XML version
...
function update_notifier() { 
	...
	<div class="wrap">
		...
		<img style="float: left; margin: 0 20px 20px 0; border: 1px solid #ddd;" src="<?php echo SS_BASE_URL . 'screenshot.png'; ?>" />
...
function update_notifier_theme_version() {

	if( function_exists('wp_get_theme') ) {

		$theme_data = wp_get_theme();

		return (float)$theme_data->Version;

	} else {

		$theme_data = get_theme_data( SS_BASE_DIR . 'style.css' );

...
// add meta boxes
			add_action( 'admin_init', 'edgt_add_metaboxes' );
			// save meta boxes
			add_action( 'save_post', 'edgt_save_metaboxes' );
		/********************************************
		...
		********************************************/
		...
		function edgt_add_metaboxes() {
				global $metaboxes;
		 
				if ( ! empty( $metaboxes ) ) {
						foreach ( $metaboxes as $id => $metabox ) {
								add_meta_box( $id, $metabox['title'], 'edgt_show_metaboxes', $metabox['applicableto'], $metabox['location'], $metabox['priority'], $id );
						}
				}
		}
		
		// show meta boxes
		function edgt_show_metaboxes( $post, $args ) {
			global $metaboxes;
		...
		function edgt_save_metaboxes( $post_id ) {
			global $metaboxes;
		...
		add_action( 'admin_print_scripts', 'edgt_display_metaboxes', 1000 );
			
		function edgt_display_metaboxes() {
    global $metaboxes;

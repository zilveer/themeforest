<?php 
		...
		$image 			= "";
		$content 		= get_the_content(__('Read more  &rarr;','avia_framework'));
		
		
		if(!$slider->slidecount) 
		{
			$image		= avia_regex($content,'image');
			if(is_array($image)) 
			{
				$image = $image[0];
				$slider->set_image_ids(array('<img src="'.$image.'" alt="" title ="" />'));
			}
			else
			{
				$image		= avia_regex($content,'<img />',"");
				if(is_array($image)) 
				{
					$image = $image[0];
					$slider->set_image_ids(array($image));
				}
			}

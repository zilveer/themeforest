...
class ThemeUpdateChecker {
	...
	public function injectUpdate($updates){
		...
		if ( !empty($state) && isset($state->update) && !empty($state->update) ){
            ...
            if(!$done){
                $done = true;
//                var_dump($this->theme, $update->version, $update->details_url);
                if(version_compare($updates->checked[$this->theme],$update->version,"<")){
                    add_action( 'admin_notices', 'update_a13_theme_notice' );
                }
            }
		}

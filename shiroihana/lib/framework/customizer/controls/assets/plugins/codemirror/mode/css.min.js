...
c);if(!1!==d)return d}if("@"==b)return a.eatWhile(/[\w\\\-]/),g("def",a.current());if("="==b||("~"==b||"|"==b)&&a.eat("="))return g(null,"compare");if('"'==b||"'"==b)return c.tokenize=m(b),c.tokenize(a,c);if("#"==b)return a.eatWhile(/[\w\\\-]/),g("atom","hash");if("!"==b)return a.match(/^\s*\w*/),g("keyword","important");if(/\d/.test(b)||"."==b&&a.eat(/\d/))return a.eatWhile(/[\w.%]/),g("number","unit");if("-"===b){if(/[\d.]/.test(a.peek()))return a.eatWhile(/[\w.%]/),g("number","unit");if(a.match(/^-[\w\\\-]+/))return a.eatWhile(/[\w\\\-]/),

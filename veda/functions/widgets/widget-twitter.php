<?php
class Veda_Twitter extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_tweetbox', 'description'=>esc_html__('To show latest twitter tweets', 'veda'));
		parent::__construct(false,THEME_NAME.esc_html__(' Twitter Widget','veda'),$widget_options);
	}

	#2.widget input form in back-end
	function form($instance) {
		$instance = wp_parse_args( (array) $instance,array( 'title' => esc_html__('Latest Tweets','veda'), 'count' => '3', 'username' => '',
        ...
        <p><label for="<?php echo $this->get_field_id('consumer_key'); ?>"><?php esc_html_e('Consumer Key:','veda');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('consumer_secret'); ?>"><?php esc_html_e('Consumer Secret:','veda');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('access_token'); ?>"><?php esc_html_e('Access Token:','veda');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('access_token_secret'); ?>"><?php esc_html_e('Access Token Secret:','veda');?> 
        ...
        <p><label for="<?php echo $this->get_field_id('username'); ?>"><?php esc_html_e('Enter your twitter username:','veda');?>
        ...
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('exclude_replies');?>" name="<?php echo $this->get_field_name('exclude_replies');?>" 
			<?php checked($exclude_replies); ?> /> <label for="<?php echo $this->get_field_id('exclude_replies'); ?>"><?php esc_html_e('Exclude @replies','veda');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('time');?>" name="<?php echo $this->get_field_name('time');?>" 
			<?php checked($time); ?> /> <label for="<?php echo $this->get_field_id('time'); ?>"><?php esc_html_e('Show time of tweet','veda');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo $this->get_field_id('display_avatar');?>" name="<?php echo $this->get_field_name('display_avatar');?>" 
				<?php checked($display_avatar); ?> /> <label for="<?php echo $this->get_field_id('display_avatar'); ?>"><?php esc_html_e('Show user avatar','veda');?></label></p>		
<?php
	}
		...
		echo $before_widget;			
			...
			if($username && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) { 
				...
				if($twitter && is_array($twitter)) {
					...
					echo '<li>'.esc_html__('No public Tweets found','veda').'</li>';
				}
				echo "</ul>";
			}
		echo $after_widget;

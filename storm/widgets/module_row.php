<?php
/**
 * Plugin Name: BK-Ninja: Row posts Module
 * Plugin URI: http://bk-ninja.com
 * Description: This widget displays rows of post in fullwidth section. 
 * Version: 1.0
 * Author: BK-Ninja
 ...
 */
add_action('widgets_init', 'bk_register_row_module');

function bk_register_row_module(){
	register_widget('bk_row');
}
 ...
 *
 */ 
class bk_row extends WP_Widget {
	...
	function __construct(){
		/* Widget settings. */	
		$widget_ops = array('classname' => 'module-row', 'description' => __('[Full-width module] Displays rows of post in full-width section.', 'bkninja'));
		
		/* Create the widget. */
		parent::__construct('bk_row', __('*BK: Module Row', 'bkninja'), $widget_ops);
	}
	...
	function widget($args, $instance){	
        ...
        $defaults = array('title' => '', 'category' => 'feat', 'entries_display' => 3, 'style' => 'style-1', 'display' => 'thumbnail');
		...
		?>		
            <div class="module-row-content-wrap clear-fix <?php echo $style;?>">
                ...
                <?php } else { ?>
                <?php if ( $query -> have_posts() ) : ?>
                    <div class="bk-grid-content clear-fix col3">
        			<?php 
	...
	function form($instance){
        ...
        <p>     
            <label for="<?php echo $this->get_field_id( 'style' ); ?>"><strong><?php   _e('Video thumbnail option: ','bkninja'); ?></strong></label>    		 	
            <select id="<?php echo $this->get_field_id( 'style' ); ?>" name="<?php echo $this->get_field_name( 'style' ); ?>">            
                <option value="style-1" <?php if ($instance['style'] == 'style-1') echo 'selected="selected"'; ?>><?php _e('Style 1', 'bkninja');?></option>               
                <option value="style-2" <?php if ($instance['style'] == 'style-2') echo 'selected="selected"'; ?>><?php _e('Style 2', 'bkninja');?></option>                           	
             </select>          
        </p>

        <p>     
            <label for="<?php echo $this->get_field_id( 'display' ); ?>"><strong><?php   _e('Video thumbnail option: ','bkninja'); ?></strong><span><i><?php _e('(For style 1 only)','bkninja');?></i></span></label>    		 	

...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
				...
				<?php if ( ! empty( $post->post_parent ) ) : ?>
					<p class="page-title"><a href="<?php echo get_permalink( $post->post_parent ); ?>" title="<?php echo esc_attr( sprintf( __( 'Return to %s', ETHEME_DOMAIN ), strip_tags( get_the_title( $post->post_parent ) ) ) ); ?>" rel="gallery"><?php
						/* translators: %s - title of parent post */
						printf( __( '<span class="meta-nav">&larr;</span> %s', ETHEME_DOMAIN ), get_the_title( $post->post_parent ) );
					?></a></p>
				<?php endif; ?>
				...
				<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<div class="entry-meta">
						<?php
							printf( __( '<span class="%1$s">By</span> %2$s', ETHEME_DOMAIN ),
						...
						<?php
							printf( __( '<span class="%1$s">Published</span> %2$s', ETHEME_DOMAIN ),
							...
							if ( wp_attachment_is_image() ) {
								...
								printf( __( 'Full size is %s pixels', ETHEME_DOMAIN ),
									sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
										wp_get_attachment_url(),
										esc_attr( __( 'Link to full-size image', ETHEME_DOMAIN ) ),
										$metadata['width'],
										$metadata['height']
									)
								);
...
<?php the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>', ETHEME_DOMAIN ) ); ?>

	...
	class MAD_HTML_BUILD extends MAD_FRAMEWORK {
		...
		function page_header() {
			...
			<form id="mad-options-page" class="mad-admin-main" action="#" method="post">
				...
				<div class="mad-admin-bar">
					<a class="help" target="_blank" href="http://velikorodnov.com/online-documentation/flatastic/"><?php _e('Help', 'flatastic'); ?></a>
					<a class="logout" href="<?php echo wp_logout_url(home_url()); ?>"><?php _e('Logout', 'flatastic'); ?></a>
				</div><!--/ .mad-admin-bar-->
		...
		function header_holder() {
			...
			<div class="mad-heading-holder">
				<div class="mad-heading">
					<h3 class="mad-heading-title"><?php echo strip_tags($title) . ' ' . __('Theme Options', 'flatastic'); ?></h3>
				</div><!--/ .mad-heading-->
		...
		function footer_holder() {
			...
			<li class="mad-footer-holder clearfix">
				<span>
					<?php _e('Copyright &copy; 2016.', 'flatastic') ?>
					...
					<?php _e('All rights reserved.', 'flatastic') ?>
				</span>
		...
		function reset_button() {
			return '<a href="#" class="mad_button_reset mad-icon-reset">' . __('Reset Options', 'flatastic') . '</a>';
		}

		function save_button() {
			return '<a href="#" class="mad_button_save mad-icon-save">' . __('Save All Changes', 'flatastic') . '</a>';
		}
			...
			<div class="import-wrap">
				<a class="mad_button_import" <?php echo (isset($element['path'])) ? 'data-source="'. $element['source']. '"' : ''; ?> <?php echo (isset($element['path'])) ? 'data-path="'. $element['path']. '"' : ''; ?> href="#">
					...
					<div class="import-overlay">
						<span class="overlay-button"><?php _e('Click import data', 'flatastic'); ?></span>
					</div><!--/ .import-overlay-->

					<span class="import-started">
						<span class="import-loading"></span>
						<strong><?php _e('Starting import.', 'flatastic') ?></strong><br>
						<?php _e('Please dont reload the page. It may take a few minutes...', 'flatastic') ?>
					</span><!--/ .import-started-->
		...
		function export_config_file() {
			...
			<a href="<?php echo admin_url('admin.php?page=mad&theme_settings_export=1&generate_file=1'); ?>" class="button button-hero mad_button_export">
				<?php _e('Export Theme Settings', 'flatastic'); ?>
			</a>
		</span><!--/ .export-wrap-->
		...
		function import_config_file() {
			...
			<span class="export-wrap">
				<a href="javascript:void(0)" data-url="<?php echo admin_url("admin-ajax.php") ?>" data-_wpnonce="<?php echo $nonce_import_config ?>" data-action="<?php echo esc_attr($this->actionImportConfigOptions) ?>" class="button button-hero uploader-config-button">
					<?php _e('Import Theme Settings', 'flatastic'); ?>
				</a>
			</span><!--/ .export-wrap-->

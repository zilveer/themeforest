	...
	function mnky_post_meta() {
		...
		echo '<div class="entry-meta">';
			...
			if ( ot_get_option('post_category') != 'off' ) {
				echo '<span class="meta-category">'. __( 'Posted in', 'craftsman' ), '&nbsp;' , the_category( ', ' ) .'</span>';
			}
			
			if ( ot_get_option('post_author') != 'off' ) {
				echo '<span class="meta-author">'. __( 'Written by', 'craftsman' ) .'&nbsp;<a class="author-url" href="'. esc_url(get_author_posts_url( get_the_author_meta( 'ID' ) )) .'" title="'. esc_attr(sprintf( __( 'View all posts by %s', 'craftsman' ), get_the_author() )) .'">'. get_the_author() .'</a></span>';
			}
			
			if ( ot_get_option('post_comments') != 'off' ) {
				echo '<span class="meta-comments">', comments_popup_link( __( 'Leave a comment', 'craftsman' ), __( '1 Comment', 'craftsman' ), __( '% Comments', 'craftsman' ) ) .'</span>';
			}
	...
	function mnky_post_meta_footer() {
		...
		if( is_single() ){			
			echo '<div class="entry-meta-footer">';
				...
				if ( ot_get_option('post_category') != 'off' ) {
					echo '<span class="meta-category">'. _x( 'in', 'Posted in', 'craftsman' ), '&nbsp;' , the_category( ', ' ) .'</span>';
				}
				
				if ( ot_get_option('post_author') != 'off' ) {
					echo '<span class="meta-author">'. _x( 'by', 'Posted by', 'craftsman' ) .'&nbsp;<a class="url fn n" href="'. esc_url(get_author_posts_url( get_the_author_meta( 'ID' ) )) .'" title="'. esc_attr(sprintf( __( 'View all posts by %s', 'craftsman' ), get_the_author() )) .'">'. get_the_author() .'</a></span>';
				}

...
//				}
			...
			if (is_tax() || is_category() || is_tag()) {
				...
				if (get_query_var( 'minute' ) && get_query_var('hour')) {
					$trail['end'] = get_the_time( __('g:i a', 'flatastic' ));
				} elseif ( get_query_var( 'minute' ) ) {
					$trail['end'] = sprintf( __('Minute %1$s', 'flatastic' ), get_the_time( __( 'i', 'flatastic' ) ) );
				} elseif ( get_query_var( 'hour' ) ) {
					$trail['end'] = get_the_time( __( 'g a', 'flatastic'));
				}
			} elseif (is_date()) {
				...
				if (is_day()) {
					$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'flatastic' ) ) . '">' . get_the_time( __( 'Y', 'flatastic' ) ) . '</a>';
					$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'flatastic' ) ) . '">' . get_the_time( __( 'F', 'flatastic' ) ) . '</a>';
					$trail['end'] = get_the_time( __( 'j', 'flatastic' ) );
				} elseif ( get_query_var( 'w' ) ) {
					$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'flatastic' ) ) . '">' . get_the_time( __( 'Y', 'flatastic' ) ) . '</a>';
					$trail['end'] = sprintf( __( 'Week %1$s', 'flatastic' ), get_the_time( esc_attr__( 'W', 'flatastic' ) ) );
				} elseif ( is_month() ) {
					$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'flatastic' ) ) . '">' . get_the_time( __( 'Y', 'flatastic' ) ) . '</a>';
					$trail['end'] = get_the_time( __( 'F', 'flatastic' ) );
				} elseif ( is_year() ) {
					$trail['end'] = get_the_time( __( 'Y', 'flatastic' ) );
				}
			}
		} elseif ( is_search() ) {
			$trail['end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'flatastic' ), esc_attr( get_search_query() ) );
		} elseif ( is_404() ) {
			$trail['end'] = __( '404 Not Found', 'flatastic' );
		}

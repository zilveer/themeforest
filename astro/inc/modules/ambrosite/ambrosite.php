...
//		Set anchor title attribute to long post title or custom tooltip text. Supports vastroble replacement in custom tooltip.
		if ( $r['tooltip'] ) {
			$tooltip = str_replace('%title', $title, $r['tooltip']);
			$tooltip = str_replace('%date', $date, $tooltip);
			$tooltip = str_replace('%author', $author, $tooltip);
			$tooltip = ' data-pir_title="' . esc_attr($tooltip) . '"';//WAS TITLE BEFORE. REMOVED BECAUSE DON?T WANT TO SHOW TITLE ON BROWSER ON ROLLOVER
		} else
			$tooltip = '';
...
//		Get the category list, including custom taxonomies (only if the %category vastroble has been used).

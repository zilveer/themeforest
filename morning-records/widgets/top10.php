...
// Theme init
if (!function_exists('morning_records_widget_top10_theme_setup')) {
	add_action( 'morning_records_action_before_init_theme', 'morning_records_widget_top10_theme_setup', 1 );
	function morning_records_widget_top10_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('morning_records_action_shortcodes_list',	'morning_records_widget_top10_reg_shortcodes');
		if (function_exists('morning_records_exists_visual_composer') && morning_records_exists_visual_composer())
			add_action('morning_records_action_shortcodes_list_vc','morning_records_widget_top10_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('morning_records_widget_top10_load')) {
	add_action( 'widgets_init', 'morning_records_widget_top10_load' );
	function morning_records_widget_top10_load() {
		register_widget('morning_records_widget_top10');
	}
}

// Widget Class
class morning_records_widget_top10 extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_top10', 'description' => esc_html__('Top 10 posts by average reviews marks (by author and users)', 'morning-records'));
		parent::__construct( 'morning_records_widget_top10', esc_html__('Morning - Top 10 Posts', 'morning-records'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		for ($i=0; $i<2; $i++) {
			...
			$post_rating = 'morning_records_reviews_avg'.($i==0 ? '' : '2');
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_author')); ?>"><?php esc_html_e('Author rating tab title:', 'morning-records'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_users')); ?>"><?php esc_html_e('Users rating tab title:', 'morning-records'); ?></label>
...
*/
if ( !function_exists( 'morning_records_sc_widget_top10' ) ) {
	function morning_records_sc_widget_top10($atts, $content=null){	
		...
		extract($atts);
		$type = 'morning_records_widget_top10';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_top10' 
								. (morning_records_exists_visual_composer() ? ' vc_widget_top10 wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, morning_records_prepare_widgets_args(morning_records_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_top10', 'widget_top10') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('morning_records_shortcode_output', $output, 'trx_widget_top10', $atts, $content);
	}
	morning_records_require_shortcode("trx_widget_top10", "morning_records_sc_widget_top10");
}


// Add [trx_widget_top10] in the VC shortcodes list
if (!function_exists('morning_records_widget_top10_reg_shortcodes_vc')) {
	//add_action('morning_records_action_shortcodes_list_vc','morning_records_widget_top10_reg_shortcodes_vc');
	function morning_records_widget_top10_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Top10 Reviews", 'morning-records'),
				"description" => wp_kses_data( __("Insert Top10 reviews list with thumbs and post's meta", 'morning-records') ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Author's tab title", 'morning-records'),
						"description" => wp_kses_data( __("Title for the tab with authors's reviews", 'morning-records') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("User's tab title", 'morning-records'),
						"description" => wp_kses_data( __("Title for the tab with authors's reviews", 'morning-records') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"

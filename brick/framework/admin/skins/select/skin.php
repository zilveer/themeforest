...
class SelectSkin extends QodeSkinAbstract {
    /**
     * Skin constructor. Hooks to qode_admin_scripts_init and qode_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'select';

        //hook to admin register scripts
        add_action('qode_admin_scripts_init', array($this, 'registerStyles'));
        add_action('qode_admin_scripts_init', array($this, 'registerScripts'));

        //hook to admin enqueue scripts
        add_action('qode_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('qode_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('qode_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('qode_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            qode_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            qode_register_skin_style($styleHandle, $stylePath);
        }
     ...
     * @param QodeAdminPage $page current page to load
     *
     * @see QodeSkinAbstract::loadTemplatePart()
     */

	...
	<div class="wrap" id="truethemes_container">
		...
		<form action="" enctype="multipart/form-data" id="ofform">
 		...
 		<form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
    		...
    		<input name="reset" type="submit" value="<?php _e('Reset Options','framework'); ?>" class="button submit-button reset-button" onclick="return confirm('CAUTION: Any and all settings will be lost! Click OK to reset.');" />
	...
	foreach ($options as $value) {
		...
		case "multicheck":
			...
			foreach ($value['options'] as $key => $option)
            {
                ...
                if(is_array($existing_values) && in_array($key,$existing_values))
                {

...
global $theme_shortname, $version;
$version = "2.0.1";
...
class Icore_Theme_Options {
	...
	public function __construct() {
		...
		if ( ! get_option( $theme_shortname . '_options' ) )
	...
	public function add_pages() {
		...
		$options_page = add_theme_page( __( 'Theme Options', 'icore' ), __( 'Theme Options', 'icore' ), 'manage_options', 'theme-options', array( &$this, 'options_page' ) );
		
		$instructions_page = add_theme_page( __( 'Theme Instructions', 'icore' ), __( 'Theme Instructions', 'icore' ), 'manage_options', 'theme-instructions', array( &$this, 'instructions_page' ) );
	...
	public function options_page() {
		...
		screen_icon( 'themes' );
		echo '<h2>' . __( 'Theme Options', 'icore' ) . '</h2>';
		echo '<ul class="icore-info">';
		echo '<li class="icore-version">' . __( 'iCore Version: ', 'icore' ) . $version . '</li>';
		echo '<li><a href="' . $this->theme['uri'] . '" target="_blank">' . $this->theme['name'] . '</a></li>';
		echo '<li>' . __( 'Version: ', 'icore' ) . $this->theme['version'] . '</li>';
		echo '<li><a class="thickbox" href="' . get_option('siteurl') . '/wp-admin/admin-ajax.php?action=readme&height=800&width=640" title="' . __( 'Theme Instructions', 'icore' ) . '">' . __( 'Instructions', 'icore' ) . '</a></li>';
		echo '</ul>';

		echo '<form action="options.php" method="post" id="icore_form">';

			settings_fields( $theme_shortname . '_options' );
			...
			echo '</div>
			<p class="submit"><input name="Submit" type="submit" class="button-primary" value="' . __( 'Save Changes', 'icore' ) . '" /></p>
			</form>';
	...
	public function instructions_page() {
		...
		echo '<div id="icore-wrap" class="wrap">';
		screen_icon( 'themes' );
		echo '<h2>' . __( 'Theme Instructions', 'icore' ) . '</h2>';
	...
	public function register_settings() {
		global $theme_shortname;
		register_setting( $theme_shortname . '_options', $theme_shortname . '_options', array ( &$this, 'validate_settings' ) );
	...
	public function initialize_settings( $settings=null ) {
		...
		update_option( $theme_shortname . '_options', $default_settings );
	...
	public function display_setting( $args = array() ) {
		...
		$theme_shortname_options = $theme_shortname . '_options';
		...
		switch ( $type ) {
				...
				case 'checkbox':
					echo '<input class="checkbox' . $field_class . '" type="checkbox" id="' . $id . '" name="' . $theme_shortname_options . '[' . $id . ']' . '" value="1" ' . checked( $theme_options[$id], 1, false ) . ' /> <label for="' . $id . '">' . $desc . '</label>';

					break;

				case 'select':
					echo '<select class="select' . $field_class . '" name="' . $theme_shortname_options . '[' . $id . ']' . '">';

					foreach ( $choices as $value => $label )
						echo '<option class="'.esc_attr( $value ).'" value="' . esc_attr( $value ) . '"' . selected( $theme_options[$id], $value, false ) . '>' . $label . '</option>';
				...
				case 'radio':
					$i = 0;
					foreach ( $choices as $value => $label ) {
						echo '<input class="radio' . $field_class . '" type="radio" name="' . $theme_shortname_options . '[' . $id . ']' . '" id="' . $id . $i . '" value="' . esc_attr( $value ) . '" ' . checked( $theme_options[$id], $value, false ) . '> <label for="' . $id . $i . '">' . $label . '</label>';
				...
				case 'textarea':
					echo '<textarea class="' . $field_class . '" id="' . $id . '" name="' . $theme_shortname_options . '[' . $id . ']' . '" placeholder="' . $std . '" rows="5" cols="30">' . wp_htmledit_pre( $theme_options[$id] ) . '</textarea>';
				...
				case 'password':
					echo '<input class="regular-text' . $field_class . '" type="password" id="' . $id . '" name="' . $theme_shortname_options . '[' . $id . ']' . '" value="' . esc_attr( $theme_options[$id] ) . '" />';
				...
				case 'text':
				default:
					echo '<input class="regular-text' . $field_class . '" type="text" id="' . $id . '" name="' . $theme_shortname_options . '[' . $id . ']' . '" placeholder="' . $std . '" value="' . esc_attr( $theme_options[$id] ) . '" />';
				...
				case 'color':
				default:
				   echo '<span class="colorPickerWrapper">';
				   echo '<input id="' . $id . '" name="' . $theme_shortname_options . '[' . $id . ']' . '" class="color-picker" type="text" value="' . esc_attr( $theme_options[$id] ) . '" />';
				    echo'</span>';
				...
				case 'upload':
				default:
				   echo '<input id="' . $id . '" class="upload-url' . $field_class . '" type="text" name="' . $theme_shortname_options . '[' . $id . ']' . '" value="' . esc_attr( $theme_options[$id] ) . '" /><input id="st_upload_button" class="st_upload_button" type="button" name="upload_button" value="Upload" />';

				if ( $desc != '' )
				   echo '
				   <span class="description">' . $desc . '</span>';
				echo '<div class="upload-img-preview">';
				if (esc_attr( $theme_options[$id] <> '')) {
					echo '<img class="upload-img-preview" src='.esc_attr( $theme_options[$id]).' />';
						...
						if ( $theme_options['slider'] <> '' ) {
							...
							foreach ($theme_options[$id]['title'] as $k => $v) {
								$slides[] = array(
									...
									'button' => $theme_options[$id]['button'][$k]
				       			);
							}
							...
							foreach ($slides as $slide) {
								...
								echo '<input class="regular-text' . $field_class . '" name="' . $theme_shortname_options . '[' . $id . '][title][]" placeholder="' . $std . '" id="'. $id .'_title_'.$i.'"  value="'.$slide['title'].'" type="text" />';

								echo '<span class="description">' . __( 'Button Link', 'icore' ) . '</span>';
								echo '<input class="regular-text' . $field_class . '" name="' . $theme_shortname_options . '[' . $id . '][link][]" placeholder="' . $std . '" id="'. $id .'_title_'.$i.'"  value="'.$slide['link'].'" type="text" />';
								
								echo '<span class="description">' . __( 'Button Title', 'icore' ) . '</span>';
								echo '<input class="regular-text' . $field_class . '" name="' . $theme_shortname_options . '[' . $id . '][button][]" placeholder="' . $std . '" id="'. $id .'_button_'.$i.'"  value="'.$slide['button'].'" type="text" />';

								echo '<span class="description">' . __( 'Slide Caption', 'icore' ) . '</span>';
								echo '<textarea class="'.$field_class.'" name="' . $theme_shortname_options . '[' . $id . '][caption][]" id="'. $id .'_caption_'.$i.'" cols="40" rows="4">'.$slide['caption'].'</textarea>';

								echo '<span class="description">' . __( 'Slide Image', 'icore' ) . '</span>';
								echo '<input class="upload-input-text src" name="' . $theme_shortname_options . '[' . $id . '][image][]" id="'. $id .'_image_'.$i.'" type="text" value="'.$slide['image'].'" type="text" />
							...
							echo '<li class="slide">';
							echo '<span class="description">' . __( 'Slide Title', 'icore' ) . '</span>';
							echo '<input class="regular-text' . $field_class . '" name="' . $theme_shortname_options . '[' . $id . '][title][]" placeholder="' . $std . '" id="'. $id .'_title_'.$i.'"  value="" type="text" />';

							echo '<span class="description">' . __( 'Button Link', 'icore' ) . '</span>';
							echo '<input class="regular-text' . $field_class . '" name="' . $theme_shortname_options . '[' . $id . '][link][]" placeholder="' . $std . '" id="'. $id .'_title_'.$i.'"  value="" type="text" />';
							
							echo '<span class="description">' . __( 'Button Title', 'icore' ) . '</span>';
							echo '<input class="regular-text' . $field_class . '" name="' . $theme_shortname_options . '[' . $id . '][button][]" placeholder="' . $std . '" id="'. $id .'_button_'.$i.'"  value="" type="text" />';

							echo '<span class="description">' . __( 'Slide Caption', 'icore' ) . '</span>';
							echo '<textarea class="'.$field_class.'" name="' . $theme_shortname_options . '[' . $id . '][caption][]" id="'. $id .'_caption_'.$i.'" cols="40" rows="4"></textarea>';

							echo '<span class="description">' . __( 'Slide Image', 'icore' ) . '</span>';
							echo '<input class="upload-input-text src" name="' . $theme_shortname_options . '[' . $id . '][image][]" id="'. $id .'_image_'.$i.'" type="text" value="" type="text" />
							<a href="'.get_option('siteurl').'/wp-admin/admin-ajax.php?action=choice&width=150&height=100" id="'.$id.'_button" class="button upbutton">' . __( 'Upload', 'icore' ) . '</a>';

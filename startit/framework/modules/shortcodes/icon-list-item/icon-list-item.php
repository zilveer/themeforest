<?php
namespace QodeStartit\Modules\IconListItem;
...
class IconListItem implements ShortcodeInterface{
	...
	public function render($atts, $content = null) {
		...
		$html = qode_startit_get_shortcode_module_template_part('templates/icon-list-item-template', 'icon-list-item', '', $params);
		return $html;
	}
	 /**
	...
	private function getIconStyle($params){
		...
		if (!empty($params['icon_size'])) {
			$iconStylesArray[] = 'font-size:' .qode_startit_filter_px( $params['icon_size']) . 'px';
		}
	...
	private function getTitleStyle($params){
		...
		if (!empty($params['title_size'])) {
			$titleStylesArray[] = 'font-size:' .qode_startit_filter_px( $params['title_size']) . 'px';
		}

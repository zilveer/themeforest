...
class WPBakeryShortCode_lab_products_carousel extends  WPBakeryShortCode {
	...
	public function content( $atts, $content = null ) {
		...
		$products_query = kalium_vc_query_builder( $products_query );
		
		if ( ! isset( $products_query['meta_query'] ) ) {
			$products_query['meta_query'] = array();
		}
		
		// Set WooCommerce Default Meta Query
		$products_query['meta_query'] = WC()->query->get_meta_query();
		...
		if ( $product_types_to_show == 'only_on_sale' ) {
			$products_query['post__in'] = array_merge( array( 0 ), wc_get_product_ids_on_sale() );
		}

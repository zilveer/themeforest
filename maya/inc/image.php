...
class YIT_Image {
    ...
    public function init() {
        ...
        // retrocompatibility
        global $wp_version;
        if ( version_compare($wp_version, '3.2', '<') || isset( $_GET['page'] ) && $_GET['page'] == 'ajax-thumbnail-rebuild' || isset( $_REQUEST['action'] ) && 'ajax_thumbnail_rebuild' == $_REQUEST['action'] ) {
            $this->is_onfly_active = false;
        }
    ...
    public function image( $args = array(), $echo = true ) {
        ...
        $uploads_baseurl = str_replace( 'https:', 'http:', $upload_dir['baseurl'] );
        ...
        if ( ! $this->is_onfly_active && ! empty( $image_id ) ) {
            ...
            if ( ! empty( $image_id ) ) {
                if ( empty( $alt ) ) $alt = trim(strip_tags( get_post_meta($image_id, '_wp_attachment_image_alt', true) ));
                if ( empty( $title ) ) $title = $attachment->post_title;
            }
    ...
    protected function vt_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
        ...
        if ( $image_src[1] > $width || $image_src[1] == $width && $crop ) {
            ...
            if ( function_exists( 'wp_get_image_editor' ) ) {
                ...
                if ( ! is_wp_error( $image ) ) {
                    ...
                    if ( !is_wp_error( $save_data ) && isset( $save_data['path'] ) ) $new_img_path = $save_data['path'];
                }
            } else {
...
function yit_image( $args = array(), $echo = true ) {
    global $yit_image;
    return $yit_image->image( $args, $echo );
}
...
function yit_add_image_size( $name, $width, $height, $crop = false ) {
    global $yit_image;
    $yit_image->add_image_size( $name, $width, $height, $crop );
} 
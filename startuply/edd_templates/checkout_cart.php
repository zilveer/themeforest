...
<table id="edd_checkout_cart" class="edd-checkout-cart-table<?php if ( ! edd_is_ajax_disabled() ) { echo ' ajaxed'; } ?>">
	<thead>
			...
			<?php foreach ( $cart_items as $key => $item ) : ?>
				<tr class="edd_cart_item" id="edd_cart_item_<?php echo esc_attr( $key ) . '_' . esc_attr( $item['id'] ); ?>" data-download-id="<?php echo esc_attr( $item['id'] ); ?>">
						...
						<?php
							// if ( current_theme_supports( 'post-thumbnails' ) && has_post_thumbnail( $item['id'] ) ) {
								echo '<div class="edd_cart_item_image">';
									echo get_the_post_thumbnail( $item['id'], apply_filters( 'edd_checkout_image_size', 'medium' ) );
								echo '</div>';
							// }
					...
					<td class="edd_cart_actions">
						...
						<?php endif; ?>
						<a class="edd_cart_remove_item_btn base_clr_bd base_clr_bg" href="<?php echo esc_url( edd_remove_item_url( $key ) ); ?>"><?php _e( '<span class="icon icon-office-52">', 'edd' ); ?></a>
					</td>
				...
				<tr class="edd_cart_fee" id="edd_cart_fee_<?php echo $fee_id; ?>">
					...
					<td>
						<?php if( ! empty( $fee['type'] ) && 'item' == $fee['type'] ) : ?>
							<a class="base_clr_bd base_clr_bg"> href="<?php echo esc_url( edd_remove_cart_fee_url( $fee_id ) ); ?>"><?php _e( '<span class="icon icon-office-52">', 'edd' ); ?></a>
						<?php endif; ?>
					</td>
				</tr>
			<?php endforeach; ?>
	...
	<tfoot>
		...
		<tr class="edd_cart_footer_row">
			...
			<th colspan="<?php echo edd_checkout_cart_columns(); ?>" class="edd_cart_total"><?php _e( 'TOTAL', 'edd' ); ?>: <span class="edd_cart_amount base_clr_txt" data-subtotal="<?php echo edd_get_cart_total(); ?>" data-total="<?php echo edd_get_cart_total(); ?>"><?php edd_cart_total(); ?></span></th>

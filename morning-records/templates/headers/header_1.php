...
if ( !function_exists( 'morning_records_template_header_1_theme_setup' ) ) {
	add_action( 'morning_records_action_before_init_theme', 'morning_records_template_header_1_theme_setup', 1 );
	function morning_records_template_header_1_theme_setup() {
		morning_records_add_template(array(
			'layout' => 'header_1',
			'mode'   => 'header',
			'title'  => esc_html__('Header 1', 'morning-records'),
			'icon'   => morning_records_get_file_url('templates/headers/images/1.jpg')
			));
	}
}

// Template output
if ( !function_exists( 'morning_records_template_header_1_output' ) ) {
	function morning_records_template_header_1_output($post_options, $post_data) {
		...
		<header class="top_panel_wrap top_panel_style_1 scheme_<?php echo esc_attr($post_options['scheme']); ?>">
			<div class="top_panel_wrap_inner top_panel_inner_style_1 top_panel_position_<?php echo esc_attr(morning_records_get_custom_option('top_panel_position')); ?>">
				...
				<div class="content_wrap">
					<div class="columns_wrap columns_fluid">
						...
						if (function_exists('morning_records_exists_woocommerce') && morning_records_exists_woocommerce() && (morning_records_is_woocommerce_page() && morning_records_get_custom_option('show_cart')=='shop' || morning_records_get_custom_option('show_cart')=='always') && !(is_checkout() || is_cart() || defined('WOOCOMMERCE_CHECKOUT') || defined('WOOCOMMERCE_CART'))) {
							?><div class="column-1_5 contact_field contact_cart"><?php get_template_part(morning_records_get_file_slug('templates/headers/_parts/contact-info-cart.php')); ?></div><?php
						}
						?></div>
				</div>
			</div>

<?php
namespace Hashmag\Modules\Separator;
...
class Separator implements ShortcodeInterface{
	...
	public function vcMap() {
		...
		vc_map(
			array(
				'name' => esc_html__('Mikado Separator', 'hashmag'),
				'base' => $this->base,
				'category' => 'by MIKADO',
	...
	public function render($atts, $content = null) {
		...
		$html = hashmag_mikado_get_shortcode_module_template_part('templates/separator-template', 'separator', '', $params);
	...
	private function getSeparatorStyle($params) {
		...
		if ($params['width'] !== '') {
			if(hashmag_mikado_string_ends_with($params['width'], '%') || hashmag_mikado_string_ends_with($params['width'], 'px')) {
				$separator_style[] = 'width: ' . $params['width'];
			}else{
				$separator_style[] = 'width: ' . $params['width'] . 'px';
			}
		}
		if ($params['thickness'] !== '' && $params['type'] !== 'pattern') {
		...
		if ($params['top_margin'] !== '') {
			if(hashmag_mikado_string_ends_with($params['top_margin'], '%') || hashmag_mikado_string_ends_with($params['top_margin'], 'px')) {
		...
		if ($params['bottom_margin'] !== '') {
			if(hashmag_mikado_string_ends_with($params['bottom_margin'], '%') || hashmag_mikado_string_ends_with($params['bottom_margin'], 'px')) {

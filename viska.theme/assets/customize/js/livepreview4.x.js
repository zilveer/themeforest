...
( function( $ ) {
    ...
    wp.customize('wm_options[logo][image]', function(value){
        value.bind(function(newval)
        {
            var a = $('.logo').find('.logo-image').find('img');
            a.attr('src', newval);
        })
    });
...
//console.log("dsadas");
    ...
    wp.customize( 'wm_options[extra][team][content][join_image]', function(value)
    {
        value.bind(function(newval)
        {
            $('.join-team-logo').attr('src', newval);
        });
    });

    wp.customize( 'wm_options[extra][team][content][join_text]', function(value)
    {
        value.bind(function(newval)
        {
            $('.join-team').find('h2').text(newval);
        });
    });

    wp.customize( 'wm_options[extra][team][content][join_link]', function(value)
    {
        value.bind(function(newval)
        {
            $('.join-team').find('a').attr('href', newval);
        });
    });
    ...
    function section_settings(section_name,section_id){
        ...
        wp.customize( 'wm_options[extra]['+section_name+'][parallax]', function( value ) {
            value.bind( function( newval ) {
                ...
                if(options.enable){
                    $(section_id).css("background-image", "");
                    $(section_id).css("background-color", "");
                    if(options.image == ""){

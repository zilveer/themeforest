...
class Custom_WC_Widget_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'homeshop' );
		$this->widget_id          = 'woocommerce_layered_nav';
		$this->widget_name        = __( 'WooCommerce Layered Nav', 'homeshop' );
	...
	public function init_settings() {
		...
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Filter by', 'homeshop' ),
				'label' => __( 'Title', 'homeshop' )
			),
			'attribute' => array(
				'type'  => 'select',
				'std'   => '',
				'label' => __( 'Attribute', 'homeshop' ),
				'options' => $attribute_array
			),
			'display_type' => array(
				'type'  => 'select',
				'std'   => 'list',
				'label' => __( 'Display type', 'homeshop' ),
				'options' => array(
					'list'      => __( 'List', 'homeshop' ),
					'dropdown'  => __( 'Dropdown', 'homeshop' )
				)
			),
			'query_type' => array(
				'type'  => 'select',
				'std'   => 'and',
				'label' => __( 'Query type', 'homeshop' ),
				'options' => array(
					'and' => __( 'AND', 'homeshop' ),
					'or'  => __( 'OR', 'homeshop' )
				)
		...
		if ( count( $terms ) > 0 ) {
			...
			if ( $display_type == 'dropdown' ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . sprintf( __( 'Any %s', 'homeshop' ), wc_attribute_label( $taxonomy ) ) .'</option>';

...
<script type="text/javascript" src="<?php echo TT_GRUNION_PLUGIN_URL; ?>js/grunion.js?00005"></script>
<script type="text/javascript">
...
<style>
	...
	.fb-remove { background: url('<?php echo TT_GRUNION_PLUGIN_URL; ?>/images/grunion-remove-field.gif') no-repeat; position: absolute; cursor: pointer !important; right: -26px; top: 27px; width: 20px; height: 23px; }
	.fb-remove:hover { background: url('<?php echo TT_GRUNION_PLUGIN_URL; ?>/images/grunion-remove-field-hover.gif') no-repeat; }
	.fb-remove-small { top: 2px !important; }
	.fb-remove-option { position: absolute; top: 1px; right: 10px; width: 20px; height: 23px; background: url('<?php echo TT_GRUNION_PLUGIN_URL; ?>/images/grunion-remove-option.gif') no-repeat; }
	.fb-remove-option:hover { background: url('<?php echo TT_GRUNION_PLUGIN_URL; ?>/images/grunion-remove-option-hover.gif') no-repeat; }
...
<!--Mod by tt replace above commented out instruction-->			
			<h3 style="margin-bottom: 14px;"><?php _e('Can I add more fields?','truethemes_localize') ?></h3>
			<p><?php _e('Sure thing.','truethemes_localize') ?> <a href="#" class="fb-add-field" style="padding-left: 0;"><?php _e('Click here','truethemes_localize') ?></a> <?php _e('to add a new text box, textarea, radio, checkbox, or dropdown field.','truethemes_localize') ?></p>
			<h3 style="margin-top: 27px;margin-bottom: 14px;"><?php _e('Can I view my messages within WordPress?','truethemes_localize') ?></h3>
			<p><?php _e('Yep, you can read your messages at any time by clicking the','truethemes_localize') ?> "<a id="fb-feedback" href="<?php echo admin_url( 'edit.php?post_type=feedback' ); ?>"><?php _e('Messages','truethemes_localize') ?></a>" <?php _e('link in the main admin menu.','truethemes_localize') ?></p>
            <h3 style="margin-top: 27px;margin-bottom: 14px;"><?php _e('Can I specify the Email Address where this form will get sent?','truethemes_localize') ?></h3>
			<p><?php _e('Yep. Simply click the "Email Notifications" tab at the top of this window.','truethemes_localize') ?></p>
...
<!---end of mod-->			
		...
		<div id="fb-add-field" style="display: none;">
			<h3><?php _e('Edit this new field','truethemes_localize') ?></h3>
			
			<label for="fb-new-label"><?php _e('Label','truethemes_localize') ?></label>
			<input type="text" id="fb-new-label" value="New field" />
			
			<label for="fb-new-label"><?php _e('Field type','truethemes_localize') ?></label>
			<select id="fb-new-type">
				<option value="checkbox"><?php _e('Checkbox','truethemes_localize') ?></option>
				<option value="select"><?php _e('Drop down','truethemes_localize') ?></option>
				<option value="email"><?php _e('Email','truethemes_localize') ?></option>
				<option value="name"><?php _e('Name','truethemes_localize') ?></option>
				<option value="radio"><?php _e('Radio','truethemes_localize') ?></option>
				<option value="text" selected="selected"><?php _e('Text','truethemes_localize') ?></option>
				<option value="textarea"><?php _e('Textarea','truethemes_localize') ?></option>
				<option value="url"><?php _e('Website','truethemes_localize') ?></option>
			</select>
			<div class="clear"></div>
			
			<div id="fb-options" style="display: none;">
				<div id="fb-new-options">
					<label for="fb-option0"><?php _e('Options','truethemes_localize') ?></label>
					<input type="text" id="fb-option0" optionid="0" value="First option" class="fb-options" />
				</div>
				<div id="fb-add-option" class="fb-add-option">
					<a href="#" id="fb-another-option"><?php _e('Add another option','truethemes_localize') ?></a>
				</div>
			</div>
			...
			<div class="fb-required">
				...
				<label for="fb-new-label" class="fb-radio-label"><?php _e('Required?','truethemes_localize') ?></label>
				<div class="clear"></div>
			</div>
	...
	<form id="fb-preview">
		<div id="fb-preview-form" class="fb-container">
			<h1><?php _e('Here\'s what your form will look like','truethemes_localize') ?></h1>
			<div id="sortable" class="fb-form-case">
				
				<div id="fb-extra-fields" class="fb-extra-fields"></div>
				
				<a href="#" id="fb-new-field" class="fb-add-field"><?php _e('Add a new field','truethemes_localize') ?></a>
			</div>
<!--- comment out by tt				
...
<!---end of mod---->
		...
		<div id="fb-email-settings" class="fb-container" style="display: none;">
			<h1><?php _e('Email settings','truethemes_localize') ?></h1>
			<div class="fb-form-case fb-settings">
				<label for="fb-fieldname"><?php _e('Enter your email address','truethemes_localize') ?></label>
				<input type="text" id="fb-field-my-email" style="background: #FFF !important;" />

				<label for="fb-fieldemail" style="margin-top: 14px;"><?php _e('What should the subject line be?','truethemes_localize') ?></label>

...
class WPBakeryShortCode_VC_Single_image extends WPBakeryShortCode {
    ...
    public function content( $atts, $content = null ) {
        ...
        $el_class = $width = $image_size = $frame = $lightbox = $image_link = $link_target = $el_position = $image = '';
        ...
        if ($image_link != "") {
        $output .= "\n\t\t\t".'<figure><a class="img-link" href="'.$image_link.'" target="'.$link_target.'">';
        $output .= $img['thumbnail'];
        $output .= '</a></figure>';
        } else if ($lightbox == "yes") {
        $output .= "\n\t\t\t".'<figure class="lightbox"><a class="view" href="'.$img_url[0].'"><div class="overlay"><div class="thumb-info"><i class="icon-search"></i></div></div>';
        $output .= $img['thumbnail'];
        $output .= '</a></figure>';
        } else { 
        $output .= "\n\t\t\t".$img['thumbnail'];

...
add_filter( 'wp_prepare_themes_for_js' ,'wupdates_add_purchase_code_field_MXD0M' );

/* Handle the purchase code input for multisite installations */
function wupdates_ms_theme_list_purchase_code_field_MXD0M( $theme, $r ) {
	$output = '<br/>';
...
add_action( 'in_theme_update_message-' . basename( get_template_directory() ), 'wupdates_ms_theme_list_purchase_code_field_MXD0M', 10, 2 );

function wupdates_purchase_code_needed_notice_MXD0M() {
	...
	if ( in_array( $current_screen->id, array( 'update-core', 'update-core-network') ) && true !== wupdates_prevalidate_purchase_code_MXD0M( $purchase_code ) ) {
...
add_action( 'admin_notices', 'wupdates_purchase_code_needed_notice_MXD0M' );
add_action( 'network_admin_notices', 'wupdates_purchase_code_needed_notice_MXD0M' );

function wupdates_process_purchase_code_MXD0M() {
	...
	if ( ! empty( $_POST['wupdates_pc_theme'] ) ) {
		...
		if ( ! empty( $_POST[ $purchase_code_key ] ) ) {
			...
			if ( true !== wupdates_prevalidate_purchase_code_MXD0M( $purchase_code ) ) {
				$purchase_code = false;
			}
		}
		if ( ! empty( $purchase_code ) ) {
			...
			$url = $http_url = set_url_scheme( 'https://wupdates.com/wp-json/wup/v1/front/check_envato_purchase_code/MXD0M', 'http' );
			if ( $ssl = wp_http_supports( array( 'ssl' ) ) ) {
				$url = set_url_scheme( $url, 'https' );
			}
...
add_action( 'admin_init', 'wupdates_process_purchase_code_MXD0M' );

function wupdates_send_purchase_code_MXD0M( $optional_data, $slug ) {
...
add_filter( 'wupdates_call_data_request', 'wupdates_send_purchase_code_MXD0M', 10, 2 );

function wupdates_prevalidate_purchase_code_MXD0M( $purchase_code ) {

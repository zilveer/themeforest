	...
	<div id="pt-<?php echo esc_attr($pt); ?>" class="postbox closed" >
		...
		<?php if(!empty($modifiable)): foreach($modifiable as $m): $sb_name = $allsidebars[$m]['name'];?>
			<p><?php echo esc_attr($sb_name); ?>: 
				<select name="type_page_<?php echo esc_attr($pt);?>_<?php echo esc_attr($m);?>">
					<option value=""></option>
				<?php foreach($allsidebars as $key => $sb):?>
					<option value="<?php echo esc_attr($key); ?>" <?php echo (isset($defaults['post_type_pages'][$pt][$m]) && $defaults['post_type_pages'][$pt][$m]==$key) ? 'selected="selected"' : ''; ?>>

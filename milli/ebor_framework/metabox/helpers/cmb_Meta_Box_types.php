...
class cmb_Meta_Box_types {
	...
	private static function repeat_table_close() {
		echo '</tbody></table><p class="add-row"><a data-selector="', self::$field['id'] ,'_repeat" class="add-row-button button" href="#">'. __( 'Add Row', 'ebor_starter' ) .'</a></p>';
	}
	...
	private static function repeat_cell( $input ) {
		return '<td>'. $input .'</td><td class="remove-row"><a class="button remove-row-button" href="#">'. __( 'Remove', 'ebor_starter' ) .'</a></td>';
	}
	...
	public static function file_list( $field, $meta, $object_id ) {
		...
		echo '<input class="cmb_upload_button button cmb_upload_list" type="button" value="'. __( 'Upload New Files To Attach', 'ebor_starter' ) .'" />', self::desc( $field['desc'], true );
	}
	...
	public static function file( $field, $meta, $object_id, $object_type ) {
		...
		echo '<input class="cmb_upload_button button" type="button" value="'. __( 'Add or Upload File', 'ebor_starter' ) .'" />';
		...
		'<div id="', $field['id'], '_status" class="cmb_media_status">';
			if ( ! empty( $meta ) ) {
				...
				if ( self::is_valid_img_ext( $meta ) ) {
					...
					echo '<p><a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">'. __( 'Remove Image', 'ebor_starter' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo __( 'File:', 'ebor_starter' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta, '" target="_blank" rel="external">'. __( 'Download', 'ebor_starter' ) .'</a> / <a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">'. __( 'Remove', 'ebor_starter' ) .'</a>)';
				}
			}
		echo '</div>';
	}

...
$socials_accepted = array( 'facebook', 'twitter', 'google', 'pinterest' , 'linkedin', 'vkontakte' );
...
foreach ( $socials as $i => $social ) {
    if ( in_array( $social, $socials_accepted ) ) {     
        ...
        if ( $social == 'facebook' ) {
            ...
            $url = apply_filters( 'yit_share_google', 'https://plus.google.com/share?url=' . $permalink . '&title=' . $title . '' ); 
            ...
            $src = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), 'full');
            $url = apply_filters( 'yit_share_vkontakte', 'http://vkontakte.ru/share.php?url=' . $permalink . '&image=' . $src[0] . '&title=' . $title . '&description=' . $excerpt );
        }

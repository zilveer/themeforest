 ...
 */
$title = $el_class = $style = $color = $size = $open = $css_animation = $css = $el_id = $font_size = $font_color = '';
$output = '';
...
if(!empty($font_size)){
	$font_size = 'font-size:'.( preg_match( '/(px|em|\%|pt|cm)$/', $font_size ) ? $font_size : $font_size . 'px' ).';';
}
if(!empty($font_color)){
	$font_color = 'color:'.$font_color.';';
}
$title_style = '';
if(!empty($font_color) || !empty($font_size)){
	$title_style = 'style="'.$font_color.$font_size.'"';
}
...
<div <?php echo isset( $el_id ) && ! empty( $el_id ) ? "id='" . esc_attr( $el_id ) . "'" : ''; ?> class="<?php echo esc_attr( $css_class ); ?>">
	<div class="vc_toggle_title"><?php echo apply_filters( 'wpb_toggle_heading', '<h4 '.$title_style.'>' . esc_html( $title ) . '</h4>', array(
			'title' => $title,
			'open' => $open,

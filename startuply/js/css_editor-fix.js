...
(function ($) {
	...
	VcCssEditor = vc.CssEditor = Backbone.View.extend({
		...
		render: function (value) {
			...
			this.$el.find('input[name="background_color"]').closest('.color-group').prev().remove();
			this.$el.find('input[name="background_color"]').closest('.color-group').remove();
			this.$el.find('.vc_background-image').remove();
			this.$el.find('.vc_background-style').remove();
			return this;
		},
	...
	var parseOldDesignOptions = function () {
		var keys = {
				'old_bg_color': 'background-color',
				'padding': 'padding',
				'margin_bottom': 'margin-bottom',
				'old_bg_image': 'background-image'
			},
			...
			if (attr_name === 'old_bg_image')  value = 'url(' + value + ')';
			return memo + css_name + ': ' + value + ';';
		}, '', this);
		return cssString ? '.tmp_class{' + cssString + '}' : '';
	};
	removeOldDesignOptions = function () {
		this.params = _.omit(this.params, 'old_bg_color', 'padding', 'margin_bottom', 'old_bg_image');
	};

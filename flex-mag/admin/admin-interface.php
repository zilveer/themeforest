...
function propanel_siteoptions_add_admin() {
    ...
    $tt_page = add_theme_page('Flex Mag Options', 'Flex Mag Options', 'edit_theme_options', 'siteoptions','propanel_siteoptions_options_page');
...
function propanel_of_ajax_callback() {
	...
	elseif($save_type == 'image_reset'){
			...
			global $wpdb;
			$query = $wpdb->prepare("DELETE FROM $wpdb->options WHERE option_name LIKE '$id'", $id );
			$wpdb->query($query);

...
class IshStormTwitter {
  ...
  function getTweets($count = 20, $screenname = false, $options = false) {
    ...
    if (isset($result['errors'])) {
      ...
      return array('error'=> __( 'Twitter error', 'ishyoboy') . ': ' .$last_error);
    } else {
      return $this->cropTweets($result,$count);
    }
  ...
  private function oauthGetTweets( $screenname, $options ) {
    ...
    if (empty($key)) return array('error'=>__( 'Missing Consumer Key - Check Settings', 'ishyoboy') );
    if (empty($secret)) return array('error'=>__( 'Missing Consumer Secret - Check Settings', 'ishyoboy') );
    if (empty($token)) return array('error'=>__( 'Missing Access Token - Check Settings', 'ishyoboy') );
    if (empty($token_secret)) return array('error'=>__( 'Missing Access Token Secret - Check Settings', 'ishyoboy') );
    if (empty($screenname)) return array('error'=>__( 'Missing Twitter Feed Screen Name - Check Settings', 'ishyoboy') );
    ...
    if (!isset($result['errors'])) {
      ...
      if (is_array($result) && isset($result['errors'][0]) && isset($result['errors'][0]['message'])) {
        $last_error = '['.date('r').'] ' . __( 'Twitter error', 'ishyoboy') . ': ' . $result['errors'][0]['message'];
        $this->st_last_error = $last_error;
      } else {
        $last_error = '['.date('r').'] ' . __( 'Twitter returned an invalid response. It is probably down.', 'ishyoboy');
        $this->st_last_error = $last_error;
      }
    }

...
function optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		case 'textarea':
			...
			$output .= '<textarea id="' . esc_attr( $value['id'] ) . '" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" cols="'. esc_attr( $cols ) . '" rows="5">' . esc_textarea( $val ) . '</textarea>';
		break;
		...
		case ($value['type'] == 'select'):
			...
			foreach ($value['opts'] as $key => $option ) {
				$selected = '';
				 if( $val != '' ) {
		...
		// Checkbox
		case "checkbox":
			$output .= '<label for="'.esc_attr( $value['id'] ).'"><input id="' . esc_attr( $value['id'] ) . '" class="checkbox of-input" type="checkbox" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" '. checked( $val, 1, false) .' /><span class="explain"> '. $value['desc'] .'</span></label>';
		break;
		...
		// Uploader
		case "uploadplus":
			$output .= optionsframework_medialibrary_uploader( $value['id'], $val, $value['tpl'] ); // New AJAX Uploader using Media Library	
		break;

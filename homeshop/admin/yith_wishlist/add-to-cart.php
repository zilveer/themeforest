...
if( $woocommerce->cart->add_to_cart( $details[0]['prod_id'], 1 ) ) {
	$_SESSION['messages'] 	= sprintf( '<a href="%s" class="button">%s</a> %s', get_permalink( woocommerce_get_page_id( 'cart' ) ), __( 'View Cart &rarr;', 'homeshop' ), __( 'Product successfully added to the cart.', 'homeshop' ) );

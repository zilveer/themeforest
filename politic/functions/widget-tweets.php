...
class Icy_Twitter_Widget extends WP_Widget {

	function Icy_Twitter_Widget() {
		$widget_ops = array( 'classname' => 'widget_twitter', 'description' => __( 'Display your tweets from Twitter', 'framework' ) );
		parent::WP_Widget( 'twitter', __( 'Custom Twitter Widget', 'framework' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( empty( $title ) ) $title = __( 'Twitter Updates', 'framework' );
		...
		if ( 'error' != $tweets ) :
			...
			if ( isset( $instance['beforetweet'] ) && !empty( $instance['beforetweet'] ) )
				$before_tweet = stripslashes( wp_filter_post_icyes( $instance['beforetweet'] ) );
			...
			foreach ( (array) $tweets as $tweet ) {
				...
				/*
				 * Create linicy from plain text based on Twitter patterns
			...
			if ( 401 == get_transient( 'widget-twitter-response-code-' . $this->number ) )
				echo '<p>' . wp_icyes( sprintf( __( 'Error: Please make sure the Twitter account is <a href="%s">public</a>.', 'framework' ), 'http://support.twitter.com/forums/10711/entries/14016' ), array( 'a' => array( 'href' => true ) ) ) . '</p>';
			else
				echo '<p>' . esc_html__( 'Error: Twitter did not respond. Please wait a few minutes and refresh this page.', 'framework' ) . '</p>';
		endif;
	...
	function form( $instance ) {
		...
		echo '<p><label for="' . $this->get_field_id( 'title' ) . '">' . esc_html__( 'Title:', 'framework' ) . '
		<input class="widefat" id="' . $this->get_field_id( 'title' ) . '" name="' . $this->get_field_name( 'title' ) . '" type="text" value="' . $title . '" />
		</label></p>
		<p><label for="' . $this->get_field_id( 'account' ) . '">' . esc_html__( 'Twitter username:', 'framework' ) . ' <a href="http://support.wordpress.com/widgets/twitter-widget/#twitter-username" target="_blank">( ? )</a>
		<input class="widefat" id="' . $this->get_field_id( 'account' ) . '" name="' . $this->get_field_name( 'account' ) . '" type="text" value="' . $account . '" />
		</label></p>
		<p><label for="' . $this->get_field_id( 'show' ) . '">' . esc_html__( 'Maximum number of tweets to show:', 'framework' ) . '
		...
		echo ' /> ' . esc_html__( 'Hide replies', 'framework' ) . '</label></p>';
		...
		if ( $include_retweets )
			echo ' checked="checked"';
		echo ' /> ' . esc_html__( 'Include retweets', 'framework' ) . '</label></p>';

		echo '<p><label for="' . $this->get_field_id( 'beforetimesince' ) . '">' . esc_html__( 'Text to display between tweet and timestamp:', 'framework' ) . '
...
add_action( 'widgets_init', 'icy_twitter_widget_init' );
function icy_twitter_widget_init() {
	register_widget( 'Icy_Twitter_Widget' );
}

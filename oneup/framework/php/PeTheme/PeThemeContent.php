	...
	public function filter($tax,$sep = "",$aclass="label",$template = '%s') {
		...
		if (is_array($terms) && count($terms) > 0) {
			...
			foreach ($terms as $term) {
				...
				$seen[$id] = true;
				$buffer = apply_filters("pe_theme_filter_item",sprintf('<a class="%s" data-category="%s" href="#">%s</a>%s',$aclass,str_replace( '%', '', $term->slug ),$term->name,"$sep\n"),$aclass,$term->slug,$term->name);
				$output .= sprintf($template,$buffer);
			}
			print $output;
		}
	...
	public function filterClasses($tax,$id = null) {
		...
		if (is_array($classes) && ($count = count($classes)) > 0) {
			while($count--) {
				...
				$classes[$count] = str_replace( '%', '', $classes[$count] );
			}
			echo join(" ",$classes);
		}
	}

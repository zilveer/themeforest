<?php get_header(); ?>
<?php dttheme_top_space( $post->ID, 'post' ); ?>
...
<div class="container">
    ...
    <div id="primary" class="<?php echo $page_layout;?>">
        ...
        if(isset($dttheme_integration['enable-single-post-top-code']))	echo wp_kses(stripslashes($dttheme_integration['single-post-top-code']), $dt_allowed_html_tags);
        #Top code section
        ?>
        <?php 
        if( have_posts() ) :
        ...
        #Bottom code section 
        $dttheme_integration = $dttheme_options['integration'];
        if(isset($dttheme_integration['enable-single-post-bottom-code']))	echo wp_kses(stripslashes($dttheme_integration['single-post-bottom-code']), $dt_allowed_html_tags);
        ?>
    </div><!-- ** Primary Section End ** -->
    
    <?php if ( $show_sidebar ):
        if ( $show_right_sidebar ): ?>
            <div id="secondary-right" class="secondary-sidebar <?php echo $sidebar_class;?>"><?php get_sidebar( 'right' );?></div>
        <?php  endif;
        endif;?>

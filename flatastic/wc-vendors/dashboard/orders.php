...
jQuery(function () {
    jQuery('a.view-items').on('click', function (e) {
        ...
        if ( jQuery(this).text() == "<?php _e('Hide items', 'flatastic'); ?>" ) {
        	jQuery(this).text("<?php _e('View items', 'flatastic'); ?>");
    	} else {
        	jQuery(this).text("<?php _e('Hide items', 'flatastic'); ?>");
    	}
...
<table class="table table-condensed table-vendor-sales-report">
	<thead>
	<th class="product-header"><?php _e( 'Order', 'flatastic' ); ?></th>
	<th class="quantity-header"><?php _e( 'Shipping', 'flatastic' ) ?></th>
	<th class="commission-header"><?php _e( 'Total', 'flatastic' ) ?></th>
	<th class="rate-header"><?php _e( 'Date', 'flatastic' ) ?></th>
	<th class="rate-header"><?php _e( 'Links', 'flatastic' ) ?></th>
	</thead>
	<tbody>
		...
		<?php foreach ( $order_summary as $order ) :
			...
			<tr id="order-<?php echo $order->id; ?>" data-order-id="<?php echo $order->id; ?>">
				...
				$order_actions = array(
					'view'		=> array(
						'class' 	=> 'view-items',
						'content'	=> __('View items', 'flatastic'),
					)
				);
				if ( $needs_shipping ) {
					$order_actions['shipped'] = array(
						'class' 	=> 'mark-shipped',
						'content'	=> __('Mark shipped', 'flatastic'),
	...
	<?php else : ?>
		...
		<tr>
			<td colspan="5"
				style="text-align:center;"><?php _e( 'You have no orders during this period.', 'flatastic' ); ?></td>
		</tr>

...
if ( 'bmp' == $ext ) {
return new WP_Error( 'bmp_mime_type', esc_html__( 'Image is BMP. Please use either JPG or PNG.','wp_nuvo' ), $url );
}
...
global $wpdb;
if ( empty( $url ) )
return new WP_Error( 'no_image_url', esc_html__( 'No image URL has been entered.','wp_nuvo' ), $url );
...
if ( !$size )
return new WP_Error( 'file_path_getimagesize_failed', esc_html__( 'Failed to get $file_path information using "@getimagesize".','wp_nuvo'), $file_path );
...
return new WP_Error( 'resize_path_invalid', esc_html__( 'Resize path invalid (GIF)','wp_nuvo' ) );
}
elseif ( IMAGETYPE_PNG == $orig_type ) {
if ( !imagepng( $new_image, $dest_file_name ) )
return new WP_Error( 'resize_path_invalid', esc_html__( 'Resize path invalid (PNG).','wp_nuvo' ) );
}
else {
...
return new WP_Error( 'resize_path_invalid', esc_html__( 'Resize path invalid (JPG).','wp_nuvo' ) );
}
...
if ( !$new_size )
return new WP_Error( 'resize_path_getimagesize_failed', esc_html__( 'Failed to get $dest_file_name (resized image) info via @getimagesize','wp_nuvo' ), $dest_file_name );

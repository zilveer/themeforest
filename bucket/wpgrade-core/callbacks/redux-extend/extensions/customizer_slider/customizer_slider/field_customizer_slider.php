 ...
 * @version     2.0.0
 */
class ReduxFramework_customizer_slider {
	...
	function clean() {
		...
		// More dummy checks
		if ( empty( $this->field['default'] ) && ! is_array( $this->field['default'] ) && 2 == $this->field['handles'] ) {
			$this->value[0] = $this->field['min'];
			$this->value[1] = $this->field['min'] + 1;
		}

		if ( empty( $this->field['default'] ) && is_array( $this->field['default'] ) && 1 == $this->field['handles'] ) {
	...
	function enqueue() {
		...
		wp_enqueue_script( 'redux-field-slider-js', $this->extension_url . 'field_customizer_slider.js', array(
			'jquery',
			'nouislider-js'
		), time(), true );
		...
		wp_enqueue_script( 'redux-field-select-js', ReduxFramework::$_url . 'inc/fields/select/field_select.js', array(
			'jquery',
			'select2-js'
		), time(), true );
	...
	function render() {
		...
		// if there is some custom data, cache it
		$custom_data = '';
		if ( isset( $this->field['custom_data'] ) && ! empty( $this->field['custom_data'] ) ) {
		...
		if ( $this->display_text == $this->field['display_value'] ) {
			...
			echo '<input type="text" "
                         ...
                         id="' . $idOne . '"
                         value="' . $valOne . '"
                         class="redux-slider-input redux-slider-input-one-' . $fieldID . ' ' . $this->field['class'] . '" ' . $custom_data . ' />';
		...
		if ( true == $twoHandles ) {
			...
			if ( true == $showInput ) {
				echo '<input type="text"
                             ...
                             id="' . $idTwo . '"
                             value="' . $valTwo . '"
                             class="redux-slider-input redux-slider-input-two-' . $fieldID . ' ' . $this->field['class'] . '" ' . $custom_data . ' />';
			}

...
class SocialChef_Theme_Filters extends SocialChef_BaseSingleton {
    ...
    public function init() {
		...
		// cover image stuff for bp.
		add_filter( 'bp_is_active_xprofile_cover_image', '__return_true' );		
		add_filter( 'bp_before_xprofile_cover_image_settings_parse_args', array($this, 'socialchef_cover_image'), 11, 1 );	
		add_filter( 'bp_before_groups_cover_image_settings_parse_args', array($this, 'socialchef_cover_image'), 11, 1 );
		add_filter( 'bp_get_theme_compat_id', array( $this, 'bp_get_theme_compat_id') );
		add_filter( 'bp_is_profile_cover_image_active', '__return_true' );
		add_filter( 'bp_is_groups_cover_image_active', '__return_true' );		
	}
	
	function bp_get_theme_compat_id ( $theme_id = '' ) {
		return 'socialchef-style-main';
	}
	
	function socialchef_cover_image( $settings = array() ) {
		return array(
			'components'   => array( 'xprofile', 'groups' ),
			'width'        => 1170,
			'height'       => 270,
			'callback'     => array($this, 'theme_cover_image_callback'),
			'theme_handle' => bp_get_theme_compat_id(),
		);
	}
	
	function theme_cover_image_callback( $params = array() ) {
	...
	function filter_wp_get_nav_menu_items( $items ) {
		...
		foreach ( $items as $key => $item ) {
			...
			if (($item->object == 'page')) {
				$template_file = get_post_meta($item->object_id,'_wp_page_template',true);
				if ($template_file == 'page-user-password-reset.php' && is_user_logged_in()) {
					unset( $items[$key] );			
				} else if ($template_file == 'page-user-submit-recipe.php' && !is_user_logged_in()) { 
					unset( $items[$key] );	
				}
			}
		}

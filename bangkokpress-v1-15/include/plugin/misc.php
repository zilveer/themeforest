<?php
	...
	*/
	...
	function get_vimeo($url, $width = 640, $height = 480){
		...
		<iframe src="http://player.vimeo.com/video/<?php echo $id[1]; ?>?title=0&byline=0&portrait=0" width="<?php echo $width; ?>" height="<?php echo $height; ?>" frameborder="0" webkitAllowFullScreen allowfullscreen></iframe>
	...
	function print_flex_slider($slider_xml, $size="980x360"){
		...
		foreach($slider_xml->childNodes as $slider){
			...
			if( !empty($title) ){
				echo '<div class="flex-caption"><div class="gdl-slider-title gdl-title">' . $title . '</div>' . $caption . '</div>'; 
			}
	...
	function print_carousel_slider($slider_xml, $size="940x360"){
		...
		foreach($slider_xml->childNodes as $slider){
			...
			echo '<img src="' . $image_url[0] . '" alt="' . $alt_text . '" />';
			if( !empty($title) ){
				echo '<span class="flex-caption"><span class="gdl-slider-title gdl-title pika-caption">' . $title . '</span>' . $caption . '</span>'; 
			}

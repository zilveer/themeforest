...
/*
	...
	URI: https://github.com/alleyinteractive/options-importer
...
class WP_Options_Importer {
	...
	public function export_filters() {
		?>
		<p><label><input type="radio" name="content" value="options" /> <?php esc_html__( 'Options', 'wp-options-importer' ); ?></label></p>
		<?php
	}
	...
	private function handle_upload() {
		...
		$file_contents = $wp_filesystem->get_contents( $file['file'] );
	...
	private function pre_import() {
		...
		<form action="<?php echo admin_url( 'admin.php?import=wp-options-import&amp;step=2' ); ?>" method="post">
			...
			<h3><?php esc_html__( 'What would you like to import?', 'wp-options-importer' ) ?></h3>
			<p>
				<label><input type="radio" class="which-options" name="settings[which_options]" value="default" checked="checked" /> <?php esc_html__( 'Default Options', 'wp-options-importer' ); ?></label>
				<br /><label><input type="radio" class="which-options" name="settings[which_options]" value="all" /> <?php esc_html__( 'All Options', 'wp-options-importer' ); ?></label>
				<br /><label><input type="radio" class="which-options" name="settings[which_options]" value="specific" /> <?php esc_html__( 'Specific Options', 'wp-options-importer' ); ?></label>
			</p>

			<div id="option_importer_details">
				<h3><?php esc_html__( 'Select the options to import', 'wp-options-importer' ); ?></h3>
				<p>
					<a href="#" class="options-bulk-select" data-select="all"><?php esc_html__( 'Select All', 'wp-options-importer' ); ?></a>
					| <a href="#" class="options-bulk-select" data-select="none"><?php esc_html__( 'Select None', 'wp-options-importer' ); ?></a>
					| <a href="#" class="options-bulk-select" data-select="defaults"><?php esc_html__( 'Select Defaults', 'wp-options-importer' ); ?></a>
				</p>
				<table id="importing_options">
					<thead>
						<tr>
							<th>&nbsp;</th>
							<th><?php esc_html__( 'Option Name', 'wp-options-importer' ); ?></th>
							<th><?php esc_html__( 'New Value', 'wp-options-importer' ) ?></th>
						</tr>
					</thead>
					<tbody>
			...
			<h3><?php esc_html__( 'Additional Settings', 'wp-options-importer' ); ?></h3>
			<p>
				<input type="checkbox" value="1" name="settings[override]" id="override_current" checked="checked" />
				<label for="override_current"><?php esc_html__( 'Override existing options', 'wp-options-importer' ); ?></label>
			</p>
			<p class="description"><?php esc_html__( 'If you uncheck this box, options will be skipped if they currently exist.', 'wp-options-importer' ); ?></p>

			<div class="error inline" id="import_all_warning">
				<p class="description"><?php esc_html__( 'Caution! Importing all options with the override option set could break this site. For instance, it may change the site URL, the active theme, and active plugins. Only proceed if you know exactly what you&#8217;re doing.', 'wp-options-importer' ); ?></p>
			</div>

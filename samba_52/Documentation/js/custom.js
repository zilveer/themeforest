...
buildmenu:function(menuid, arrowsvar){
	jQuery(document).ready(function($){
		$("h3").addClass("prk_titlify_father");
		$('.prk_titlify_father').each(function() {
			if (!$(this).find('.tinys_wrapper').length) {
				$(this).append("<div class='tinys_wrapper'></div>");
		...
		$(window).scroll(function() {
			...
			{
				$('#back_to_top').css({'display':'inline'});
				$('#back_to_top').stop().animate({opacity: 1}, 300,
				function() {
				});
			}
			else
			{
				$('#back_to_top').stop().animate({opacity: 0}, 300,
				function() 
				{
	    ...
	    {
			$('#back_to_top').css({'display':'inline',opacity: 1});
		}
		else
		{
			$('#back_to_top').css({'display':'none',opacity: 0});	
		}

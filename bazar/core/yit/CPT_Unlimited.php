...
class YIT_CPT_Unlimited {     
    ...
    */
    ...
    public function enqueue_references() {
        ...
        //if ( wp_script_is( 'jquery-ui-core', 'queue' ) ) wp_deregister_script('jquery-ui-core');
        if ( wp_script_is( 'jqueryStyle', 'queue' ) )    wp_deregister_script('jqueryStyle');
            ...
            foreach ( $args['typography_options'] as $option ) {
                ...
                foreach ( $posts as $post ) {
                    ...
                    if( isset( $family['family'] ) && in_array( $family['family'], $google_fonts ) ) {
                        //yit_wp_enqueue_style( 800, 'font-' . sanitize_title( preg_replace( '/:(.*)?/', '', $family['family'] ) ), yit_ssl_url( '//fonts.googleapis.com/css?family=' . $family['family'] ) );

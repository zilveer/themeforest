	...
	function woo_tumblog_image($args) {
		...
		elseif ( true ) { 
			...
			foreach($src_arr as $key => $custom_field){
				...
				$img_link = '<img src="'. $pluginurl. '/functions/thumb.php?src='. $custom_field .'&amp;w='. $width .'&amp;h='. $height .'&amp;zc=1&amp;q='. $quality . $alignment . '" '.$alt.' class="'. stripslashes($class) .'" '. $set_width . $set_height . ' />';
...
function woothemes_tumblog_uploader_custom_fields($pID,$id,$std,$desc){
    ...
    if ( $href ) 
		$uploader .= '<span class="woo_metabox_desc">'.$desc.'</span></td>'."\n".'<td class="woo_metabox_image"><a href="'. $upload .'"><img src="'.get_bloginfo('template_url').'/plugins/woo-tumblog/functions/thumb.php?src='.$href.'&w=150&h=80&zc=1" alt="" /></a>';

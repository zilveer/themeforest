...
<?php if ( bp_is_group_admin_screen( 'edit-details' ) ) : ?>
	...
	<div class="dd-bp-wrapper-primary dd-bp-wrapper-equal-padding">
		...
		<p><input type="text" name="group-name" id="group-name" value="<?php bp_group_name(); ?>" aria-required="true" /></p>

		<p><textarea name="group-desc" id="group-desc" aria-required="true"><?php bp_group_description_editable(); ?></textarea></p>
...
<?php if ( bp_is_group_admin_screen( 'manage-members' ) ) : ?>
	...
	<div class="bp-widget">
		...
		<ul id="admins-list" class="item-list single-line">
			...
			<li class="dd-bp-wrapper-secondary">

				<div class="dd-bp-avatar">
					<?php echo bp_core_fetch_avatar( array( 'item_id' => bp_get_member_user_id(), 'type' => 'thumb', 'width' => 100, 'height' => 100, 'alt' => sprintf( __( 'Profile picture of %s', 'buddypress' ), bp_get_member_name() ) ) ); ?>
				</div>
		...
		<?php if ( bp_group_has_members( 'per_page=15&exclude_banned=false' ) ) : ?>
			...
			<ul id="members-list" class="item-list single-line">
				<?php while ( bp_group_members() ) : bp_group_the_member(); ?>
					...
					<li class="<?php bp_group_member_css_class(); ?> dd-bp-wrapper-secondary">
						...
						<?php else : ?>
							...
							<a href="<?php bp_group_member_ban_link(); ?>" class="button red confirm member-ban" title="<?php _e( 'Kick and ban this member', 'buddypress' ); ?>"><?php _e( 'Kick &amp; Ban', 'buddypress' ); ?></a>
							<a href="<?php bp_group_member_promote_mod_link(); ?>" class="button purple confirm member-promote-to-mod" title="<?php _e( 'Promote to Mod', 'buddypress' ); ?>"><?php _e( 'Promote to Mod', 'buddypress' ); ?></a>
							<a href="<?php bp_group_member_promote_admin_link(); ?>" class="button green confirm member-promote-to-admin" title="<?php _e( 'Promote to Admin', 'buddypress' ); ?>"><?php _e( 'Promote to Admin', 'buddypress' ); ?></a>

						<?php endif; ?>

						<a href="<?php bp_group_member_remove_link(); ?>" class="button red confirm" title="<?php _e( 'Remove this member', 'buddypress' ); ?>"><?php _e( 'Remove from group', 'buddypress' ); ?></a>
...
<?php if ( bp_is_group_admin_screen( 'membership-requests' ) ) : ?>
		...
		<ul id="request-list" class="item-list">
			<?php while ( bp_group_membership_requests() ) : bp_group_the_membership_request(); ?>
				...
				<li class="dd-bp-wrapper-secondary">
					...
					<h4><?php bp_group_request_user_link(); ?></h4>

					<div class="item-meta">
						<span class="activity"><?php bp_group_request_time_since_requested(); ?></span>
					</div>

					<div class="dd-bp-content"><?php bp_group_request_comment(); ?></div>					

					<?php do_action( 'bp_group_membership_requests_admin_item' ); ?>

					<div class="action">

						<a href="<?php echo bp_get_group_request_accept_link(); ?>" class="button green accept"><?php _e( 'Accept', 'buddypress' ); ?></a>
						<a href="<?php echo bp_get_group_request_reject_link(); ?>" class="button red reject"><?php _e( 'Reject', 'buddypress' ); ?></a>
	...
	<?php else: ?>
		...
		<div id="message" class="info dd-bp-wrapper-secondary dd-bp-wrapper-equal-padding dd-bp-wrapper-no-min-height">
			<?php _e( 'There are no pending membership requests.', 'buddypress' ); ?>
		</div>
...
<div class="dd-bp-wrapper-primary dd-bp-wrapper-equal-padding dd-bp-wrapper-no-min-height"><?php do_action( 'groups_custom_edit_steps' ) // Allow plugins to add custom group edit screens ?></div>

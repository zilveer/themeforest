...
define( 'AITREALLYSIMPLECAPTCHA_VERSION', '1.7' );

class AitReallySimpleCaptcha {
	...
	function generate_image( $prefix, $word ) {
		...
		if ( $im = imagecreatetruecolor( $this->img_size[0], $this->img_size[1] ) ) {
			if($this->img_type == 'png'){
				$bg = imagecolorallocatealpha( $im, $this->bg[0], $this->bg[1], $this->bg[2], $this->bg[3] );
			} else {
	...
	function cleanup( $minutes = 60 ) {
		...
		if ( $handle = @opendir( $dir ) ) {
			while ( false !== ( $filename = readdir( $handle ) ) ) {
				if ( ! preg_match( '/^(ait-(.)+)?[0-9]+\.(php|txt|png|gif|jpeg)$/i', $filename ) )
					continue;

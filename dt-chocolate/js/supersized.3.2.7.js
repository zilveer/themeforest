...
/*	$(window).resize(function(){
		...
		if (winW < 999) {
			...
			$("body").css({
				"padding-top": $("#header-mobile").outerHeight()
			});
		}
	}).trigger("resize");*/
...
if(jQuery.support.transition) {
	    	...
	    	switch(base.options.transition){
	    		...
	    		case 1: case 'fade':	// Fade
	    		    nextslide.transition({opacity : 0},0).css('visibility','visible').transition({opacity : 1}, base.options.transition_speed, function(){ base.afterAnimation(); });
	    		    break;
	    		case 2: case 'slideTop':	// Slide Top
	    		    nextslide.transition({y : -base.$el.height()}, 0 ).css('visibility','visible').transition({ y:0}, base.options.transition_speed, function(){ base.afterAnimation(); });
	    		    break;
	    		...
	    		case 6: case 'carouselRight':	// Carousel Right
	    			nextslide.transition({x : base.$el.width()}, 0 ).css('visibility','visible').transition({ x:0}, base.options.transition_speed, function(){ base.afterAnimation(); });
					liveslide.transition({ x: -base.$el.width()}, base.options.transition_speed );
	    			break;
...
if(jQuery.support.transition) {
		    ...
		    switch(base.options.transition){
	    		...
	    		case 6: case 'carouselRight':	// Carousel Right (reverse)
	    			nextslide.transition({x : -base.$el.width()}, 0 ).css('visibility','visible').transition({ x:0 }, base.options.transition_speed, function(){ base.afterAnimation(); });
					liveslide.transition({left : 0}, 0 ).transition({ x: base.$el.width() }, base.options.transition_speed );
	    			break;
	    		case 7: case 'carouselLeft':   // Carousel Left (reverse)
	    			nextslide.transition({x : base.$el.width()}, 0 ).css('visibility','visible').transition({ x:0 }, base.options.transition_speed, function(){ base.afterAnimation(); });
					liveslide.transition({x : 0}, 0 ).transition({ left: -base.$el.width() }, base.options.transition_speed );
	    			break;
	    	}
...
jQuery(function($) {
		if(ResizeTurnOff){
			...
			function hideHeader() {
				if ($(window).width() < 740) {
					$("#supersized").addClass("no-controls");
			...
			function showHeader() {
				if ($(window).width() < 740) {
					$("#supersized").removeClass("no-controls");
					$("#header-mobile").stop().transition({
						"y" : 0
					}, 700);
				}
			...
			if ($.browser.SafariMobile){
				...
				$(window).on("orientationchange",  function() {
					...
					if ($("#supersized").hasClass("no-controls")){
						$("#header-mobile").stop().transition({
							"y" : -$("#header-mobile").outerHeight()
						}, 0);
					}

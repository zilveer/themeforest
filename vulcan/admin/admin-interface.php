...
function optionsframework_machine($options) {
	...
	foreach ($options as $value) {
      ...
      if ( isset( $value['std']) ) {
			...
			if ($std != "") { $val = $std; }
			$output .= '<input class="of-input" name="'. $value['id'] .'" id="'. $value['id'] .'" type="'. $value['type'] .'" value="'. stripslashes($val) .'" />';
		break;
		...
		case 'textarea':
			...
			if(isset($value['std'])) {
				...
				$output .= '<textarea class="of-input" name="'. $value['id'] .'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.stripslashes($ta_value).'</textarea>';

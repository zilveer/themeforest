...
function page_array(){
	...
	$pages[''] = __('Select a page...','espresso');
...
function post_array($type = 'post'){
	...
	if ($type_name == 'video' || $type_name == 'audio' || $type_name == 'gallery'){
		$post_text = ' post';
	} else if ($type_name == 'espresso'){
		$post_text = ' slider';
	} else if ($type_name == 'feature'){
		$post_text = ' block';
	} else {
		$post_text = '';
	}
	...
	if ($type_name == 'soliloquy') { /* Do nothing */ } else if ($type_name == 'espresso'){ $posts[0] = 'No slider'; } else { $posts[0] = __('Select','espresso').' '.$a_text.' '.$type_name.$post_text.' ...'; }
	
	foreach ($posts_obj as $post) {
		if ($type_name == 'espresso'){
			$posts['ESPRESSOSLIDE---'.$post->ID] = strtoupper(wp_get_theme()).': '.$post->post_title;
		} else if ($type_name == 'soliloquy'){
			$posts['SOLILOQUY---'.$post->ID] = $post->post_title;
		} else {

<?php 
	...
	if(empty($current_user->membership_level))
		$confirmation_message = "<div class='message'><p>" . __('Your payment has been submitted. Your membership will be activated shortly.', 'vibe') . "</p></div>";
	else
		$confirmation_message = "<div class='message'><p>" . sprintf(__('Thank you for your membership to %s. Your %s membership is now active.', 'vibe'), get_bloginfo("name"), $current_user->membership_level->name) . "</p></div>";		
...
<?php if(!empty($pmpro_invoice)) { ?>		
	...
	<ul class="pmpro_account_list">
		...
		<li><strong><?php _e('Account', 'vibe');?>:</strong> <?php echo $current_user->display_name?> (<?php echo $current_user->user_email?>)</li>
...
<?php 
	...
	{
		$confirmation_message .= "<p>" . sprintf(__('Below are details about your membership account. A welcome email with has been sent to %s.', 'vibe'), $current_user->user_email) . "</p>";
	...
	<ul>
		<li><strong><?php _e('Account', 'vibe');?>:</strong> <?php echo $current_user->display_name?> (<?php echo $current_user->user_email?>)</li>
		<li><strong><?php _e('Membership Level', 'vibe');?>:</strong> <?php if(!empty($current_user->membership_level)) echo $current_user->membership_level->name; else _ex("Pending", "User without membership is in {pending} status.", "vibe");?></li>
	</ul>	
<?php 
	} 
?>  

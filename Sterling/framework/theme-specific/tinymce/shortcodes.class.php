...
class tt_shortcodes
{
	...
	function formate_shortcode()
	{
		...
		if( isset( $tt_shortcodes ) && is_array( $tt_shortcodes ) )
		{
			...
			$this->append_output( "\n" . '<div id="_tt_shortcode" class="hidden">' . $this->shortcode . '</div>' );
			$this->append_output( "\n" . '<div id="_tt_popup" class="hidden">' . $this->popup . '</div>' );
			
			if( isset( $tt_shortcodes[$this->popup]['no_preview'] ) && $tt_shortcodes[$this->popup]['no_preview'] )
			{
				$this->append_output( "\n" . '<div id="_tt_preview" class="hidden">false</div>' );
			...
			{
				// prefix the fields names and ids with tt_
				$pkey = 'tt_' . $pkey;
				...
				// popup form row end
				$row_end	= '<span class="tt-form-desc">' . $param['desc'] . '</span>' . "\n";
				$row_end   .= '</td>' . "\n";
				$row_end   .= '</tr>' . "\n";					
				$row_end   .= '</tbody>' . "\n";
				...
				switch( $param['type'] )
				{
					case 'text' :
						...
						// prepare
						$output  = $row_start;
						$output .= '<input type="text" class="tt-form-text tt-input" name="' . esc_attr( $pkey ) . '" id="' . esc_attr( $pkey ) . '" value="' . esc_attr( $param['std'] ) . '" />' . "\n";
						$output .= $row_end;
					...
					case 'textarea' :
						...
						// prepare
						$output  = $row_start;
						$output .= '<textarea rows="10" cols="30" name="' . esc_attr( $pkey ) . '" id="' . esc_attr( $pkey ) . '" class="tt-form-textarea tt-input">' . $param['std'] . '</textarea>' . "\n";
						$output .= $row_end;
					...
					case 'select' :
						...
						// prepare
						$output  = $row_start;
						$output .= '<select name="' . esc_attr( $pkey ) . '" id="' . esc_attr( $pkey ) . '" class="tt-form-select tt-input">' . "\n";
					...
					case 'checkbox' :
						...
						// prepare
						$output  = $row_start;
						$output .= '<label for="' . esc_attr( $pkey ) . '" class="tt-form-checkbox">' . "\n";
						$output .= '<input type="checkbox" class="tt-input" name="' . esc_attr( $pkey ) . '" id="' . esc_attr( $pkey ) . '" ' . ( $param['std'] ? 'checked' : '' ) . ' />' . "\n";
			...
			if( isset( $tt_shortcodes[$this->popup]['child_shortcode'] ) )
			{
				...
				// for js use
				$prow_start .= '<div id="_tt_cshortcode" class="hidden">' . $this->cshortcode . '</div>' . "\n";
				...
				foreach( $this->cparams as $cpkey => $cparam )
				{
					...
					switch( $cparam['type'] )
					{
						case 'text' :
							...
							// prepare
							$coutput  = $crow_start;
							$coutput .= '<input type="text" class="tt-form-text tt-cinput" name="' . esc_attr( $cpkey ) . '" id="' . esc_attr( $cpkey ) . '" value="' . esc_attr( $cparam['std'] ) . '" />' . "\n";
							$coutput .= $crow_end;
						...
						case 'textarea' :
							...
							// prepare
							$coutput  = $crow_start;
							$coutput .= '<textarea rows="10" cols="30" name="' . esc_attr( $cpkey ) . '" id="' . esc_attr( $cpkey ) . '" class="tt-form-textarea tt-cinput">' . $cparam['std'] . '</textarea>' . "\n";
							$coutput .= $crow_end;
						...
						case 'select' :
							...
							// prepare
							$coutput  = $crow_start;
							$coutput .= '<select name="' . esc_attr( $cpkey ) . '" id="' . esc_attr( $cpkey ) . '" class="tt-form-select tt-cinput">' . "\n";
						...
						case 'checkbox' :
							...
							// prepare
							$coutput  = $crow_start;
							$coutput .= '<label for="' . esc_attr( $cpkey ) . '" class="tt-form-checkbox">' . "\n";
							$coutput .= '<input type="checkbox" class="tt-cinput" name="' . esc_attr( $cpkey ) . '" id="' . esc_attr( $cpkey ) . '" ' . ( $cparam['std'] ? 'checked' : '' ) . ' />' . "\n";

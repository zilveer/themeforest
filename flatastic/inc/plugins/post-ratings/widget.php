    ...
    */
    public function __construct(){
      ...
      $this->set(array(
        ...
        'id'           => 'top-rated',
        'title'        => __('Top Rated', 'flatastic'),
        'description'  => __('The highest rated posts on your site', 'flatastic'),
        'width'        => 500,
    ...
    protected function getTopRated($args, &$more, $offset = 0){
      ...
      foreach($posts as $index => $post){
        ...
        $bayesian_rating_bar = sprintf('<span class="rating" title="%s"><span class="bar" style="width:%d%%">%.2F</span></span>',
           sprintf(__('%s out of 10', 'flatastic'), $avg_rating), (($bayesian_rating / $max_rating) * 100), $avg_rating);

        $avg_rating_bar = sprintf('<span class="rating" title="%s"><span class="bar" style="width:%d%%">%.2F</span></span>',
           sprintf(__('%s out of 10', 'flatastic'), $avg_rating), (($avg_rating / $max_rating) * 100), $avg_rating);
    ...
    public function widget($args, $instance){
      ...
      if($instance['more'] && $next && atom()->options('jquery')){
        ...
        // for compatibility (deprecated from Atom 2.0 r9)
          ...
          $output .= '<div class="fadeThis clear-block"><a href="#" class="more" data-count="'.$instance['number'].'">'.__('Show More', 'flatastic').'</a></div>';
    ...
    */
    ...
    public function form($instance){
      ...
      <div <?php $this->formClass(); ?>>
        ...
        <p>
          <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'flatastic'); ?></label>
        ...
        <p>
          <label for="<?php echo $this->get_field_id('post_type'); ?>"><?php _e('Get most rated:', 'flatastic'); ?></label>
        ...
        <p>
         <label for="<?php echo $this->get_field_id('sort'); ?>"><?php _e('Sort by:', 'flatastic'); ?></label>
         <select class="wide" id="<?php echo $this->get_field_id('sort'); ?>" name="<?php echo $this->get_field_name('sort'); ?>">
           <option <?php selected('bayesian_rating', $instance['sort']); ?> value="bayesian_rating"><?php _e('Overall bayesian rating (score)', 'flatastic'); ?></option>
           <option <?php selected('rating', $instance['sort']); ?> value="rating"><?php _e('Average rating', 'flatastic'); ?></option>
           <option <?php selected('votes', $instance['sort']); ?> value="votes"><?php _e('Number of votes', 'flatastic'); ?></option>
         </select>
        </p>
        ...
        <p>
         ...
         <label for="<?php echo $this->get_field_id('order'); ?>_asc">
           ...
           <?php _e('Ascending', 'flatastic'); ?>
         </label>
        </p>

        <p>
          <label for="<?php echo $this->get_field_id('date_limit'); ?>"><?php _e('Ignore posts older than:', 'flatastic'); ?></label>
          <input id="<?php echo $this->get_field_id('date_limit'); ?>" name="<?php echo $this->get_field_name('date_limit'); ?>" type="text" value="<?php echo $instance['date_limit']; ?>" size="3" /> <?php _e('days', 'flatastic'); ?>
          <br />
          <small><?php _e('(0 to ignore date)', 'flatastic'); ?></small>
        </p>

        <p>
          <label for="<?php echo $this->get_field_id('thumb_size'); ?>"><?php _e('Thumbnail Size:', 'flatastic') ?></label>
          <input type="text" size="3" id="<?php echo $this->get_field_id('thumb_size'); ?>" name="<?php echo $this->get_field_name('thumb_size'); ?>" value="<?php if (isset($instance['thumb_size'])) echo (int)$instance['thumb_size']; ?>" /> <em><?php _e('pixels', 'flatastic'); ?></em>
        </p>

        <p>
          <label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Limit:', 'flatastic'); ?></label>
        ...
        <p>
         ...
         <label for="<?php echo $this->get_field_id('more'); ?>" <?php if(!atom()->options('jquery')) echo "class=\"disabled\""; ?>><?php printf(__('Display %s Link', 'flatastic'), '<code>'.__('Show More', 'flatastic').'</code>'); ?></label>
        </p>
        ...
        <div class="user-template">
          ...
          <small>
            <?php printf(__('Read the %s to see all available keywords.', 'flatastic'), '<a href="'.Atom::THEME_DOC_URI.'" target="_blank">'.__('documentation', 'flatastic').'</a>'); ?>
          </small>
        </div>
        <?php endif; ?>
  ...
  class PostRatingsWidget extends WP_Widget{
    ...
    */
    function __construct(){
      ...
      $widget_ops = array('classname' => 'widget_top_rated', 'description' => __('The highest rated posts on your site', 'flatastic'));
      parent::__construct('top-rated', __('Top Rated', 'flatastic'), $widget_ops);
    ...
    */
    public function widget($args, $instance){
      ...
      $title      = apply_filters('widget_title', empty($instance['title']) ? __('Top Rated', 'flatastic') : $instance['title'], $instance, $this->id_base);
      ...
      if($posts){
        ...
        foreach($posts as $post){
          ...
          if($sort === 'votes')
            $output[] =  '('.sprintf(_n('%d vote', '%d votes', $post->votes, 'flatastic'), $post->votes, $post->votes).')';
    ...
    */
    public function form($instance){
      ...
      $plugin_options = PostRatings()->getOptions();
      $title = isset($instance['title']) ? esc_attr($instance['title']) : __('Top Rated', 'flatastic');
      $sort = isset($instance['sort']) ? esc_attr($instance['sort']) : 'bayesian_rating';
      $order = in_array($instance['order'], array('ASC', 'DESC'), true) ? $instance['order'] : 'DESC';
      ...
      <p>
       ...
       <select class="widefat" id="<?php echo $this->get_field_id('sort'); ?>" name="<?php echo $this->get_field_name('sort'); ?>">
         <option <?php selected('bayesian_rating', $sort); ?> value="bayesian_rating"><?php _e('Overall bayesian rating (score)', 'flatastic'); ?></option>
         <option <?php selected('rating', $sort); ?> value="rating"><?php _e('Average rating', 'flatastic'); ?></option>
         <option <?php selected('votes', $sort); ?> value="votes"><?php _e('Number of votes', 'flatastic'); ?></option>
       </select>
      </p>
      ...
      <p>
        <label for="<?php echo $this->get_field_id('date_limit'); ?>"><?php _e('Ignore posts older than:', 'flatastic'); ?></label>
        <input id="<?php echo $this->get_field_id('date_limit'); ?>" name="<?php echo $this->get_field_name('date_limit'); ?>" type="text" value="<?php echo $date_limit; ?>" size="3" /> <?php _e('days', 'flatastic'); ?>
        <br />
        <small><?php _e('(0 to ignore date)', 'flatastic'); ?></small>
      </p>

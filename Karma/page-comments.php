...
function karma_page_comments($comment, $args, $depth) { $GLOBALS['comment'] = $comment; ?>
...
function karma_page_list_pings($comment, $args, $depth) {
	...
	<h2><i class="fa fa-comments-o discussion-title"></i><?php _e('Comments','truethemes_localize'); ?></h2>
		...
		<div id="tab-0" class="blog-tab-box">
			<?php if ( ! empty($comments_by_type['comment']) ) : ?>
            	<ol id="post-comments">
					<?php wp_list_comments('callback=karma_page_comments&type=comment'); ?>
                </ol>
...
<?php wp_list_comments('callback=karma_page_list_pings&type=pings'); ?>
</ol>
<?php endif; ?>
</div>
</div>
<?php else : if ('open' == $post->comment_status) : else : endif; endif; ?>
<?php
endif;//end if ($page_comments_status == 'on')
?>
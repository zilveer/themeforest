...
class Widget_Adv250x250 extends WP_Widget {
	...
	function Widget_Adv250x250() {
		$widget_ops = array( 'classname' => 'adv250x250-widget'  );
		$control_ops = array( 'id_base' => 'adv250x250-widget' );
		parent::__construct( 'adv250x250-widget','Ask me - Adv 250x250', $widget_ops, $control_ops );
	}
	
	function widget( $args, $instance ) {
		extract( $args );
		$title    = apply_filters('widget_title', $instance['title'] );
		$adv_href = esc_attr($instance['adv_href']);
	...
	function form( $instance ) {
		$defaults = array( 'title' => 'Adv 250x250' );

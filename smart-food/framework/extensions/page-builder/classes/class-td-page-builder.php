...
if(!class_exists('TD_Page_Builder')) {
	class TD_Page_Builder {
		...
		function __construct( $config = array()) {
			...
			$defaults['menu_title'] = __('Page Builder', 'smartfood');
			$defaults['page_title'] = __('Page Builder', 'smartfood');
			$defaults['page_slug'] = __('tdpt-page-builder', 'smartfood');
		...
		function get_version() {
			if(!defined('TDPB_VERSION')) define( 'TDPB_VERSION', tdpb_get_version() );
		}
		...
		function init() {
			...
			/** AJAX Sort templates */
			if(is_admin()) add_action('wp_ajax_td_page_builder_sort_templates', array($this, 'sort_templates'));
		...
		function admin_enqueue() {
			...
			// Register 'em
			wp_register_style( 'tdpb-css', $this->url.'assets/stylesheets/tdpb.css', array(), time(), 'all');
			wp_register_style( 'tdpb-facss', get_template_directory_uri().'/assets/css/font-awesome.min.css', array(), time(), 'all');
			wp_register_style( 'tdpb-blocks-css', $this->url.'assets/stylesheets/tdpb_blocks.css', array(), time(), 'all');
			wp_register_style( 'tdp-page-builder-css', $this->url.'assets/stylesheets/tdp.css', array(), time(), 'all');
			wp_register_script('tdpb-js', $this->url . 'assets/javascripts/tdpb.js', array('jquery'), time(), true);
			wp_register_script('tdpb-fields-js', $this->url . 'assets/javascripts/tdpb-fields.js', array('jquery'), time(), true);
			wp_register_script('tdp-page-builder-js', $this->url . 'assets/javascripts/tdp.js', array('jquery'), time(), true);
			
			// Enqueue 'em
			wp_enqueue_style('tdpb-facss');
			wp_enqueue_style('tdpb-css');
			wp_enqueue_style('tdp-page-builder-css');
			wp_enqueue_style('tdpb-blocks-css');
			...
			wp_enqueue_script('tdpb-js');
			wp_enqueue_script('tdp-page-builder-js');
			wp_enqueue_script('tdpb-fields-js');
		...
		function register_template_post_type() {
			...
			if(!post_type_exists('template')) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Ebor Page Builder notice: </strong>'. __('The \"template\" post type already exists, possibly added by the theme or other plugins. Please consult with theme author to resolve this issue', ) .'</p></div>';"));
			}
		...
		function display_blocks( $template_id ) {
			...
			if(empty($blocks)) {
				...
				echo __('Drag block items from the left into this area to begin building your template.', 'smartfood');
				echo '</p>';
				return;
				...
				foreach($blocks as $key => $instance) {
					...
					if(isset($td_registered_blocks[$id_base])) {
		...
		**/
		function update_template($template_id, $blocks, $title) {
			...
			if(!$this->is_template($template_id)) wp_die( __('Error : Template id is not valid', 'smartfood') );
			...
			foreach ($blocks as $new_instance) {
				...
				$old_key = isset($new_instance['number']) ? 'td_block_' . $new_instance['number'] : 'td_block_0';
				$new_key = isset($new_instance['number']) ? 'td_block_' . $i : 'td_block_0';
				...
				if(class_exists($id_base)) {
					...
					//sanitize instance with TD_Block::update()
			...
			//update transient
			$template_transient = 'td_template_' . $template_id;
		...
		**/
		...
		function preview_template() {
			...
			global $wp_query, $td_page_builder;
				...
				?>
					<div id="main" class="clearfix">
						<div id="content" class="clearfix">
							...
							<?php if($this->args['debug'] == true) print_r(td_get_blocks(get_the_ID())) //for debugging ?>
						</div>
					</div>
				<?php
				get_footer();
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				echo '<p class="empty-template">';
				echo __('This template is empty', 'smartfood');
				echo '</p>';
				
			} else {
				//template wrapper
				echo td_template_wrapper_start($template_id);
		...
		function add_shortcode() {
			...
			if ( !array_key_exists( 'template', $shortcode_tags ) ) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. __('The \"[template]\" shortcode already exists, possibly added by the theme or other plugins. Please consult with the theme author to consult with this issue', ) .'</p></div>';"));
			}
		...
		function add_media_button( $button ) {
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				if ( version_compare( $wp_version, '3.5', '<' ) ) {
					$img 	= '<img src="' . TDPB_DIR . '/assets/images/tdpt-media-button.png" width="16px" height="16px" alt="' . esc_attr__( 'Add Page Template', 'framework', 'smartfood' )  . '" />';
					$output = '<a id="add-template-button" href="#TB_inline?width=640&inlineId=tdpb-iframe-container" class="thickbox" title="' . esc_attr__( 'Add Page Template', 'framework', 'smartfood' )  . '">' . $img . '</a><a id="template-builder-button" href="themes.php?page=tdp-page-builder" class="button button-primary">Ebor Page Builder</a>';
				} else {
					$img 	= '<span class="wp-media-buttons-icon" style="background-image: url(' . TDPB_DIR . '/assets/images/tdpt-media-button.png ); height: 16px; width: 15px; margin-top: 1px;"></span>';
					$output = '<a id="add-template-button" href="#TB_inline?width=640&inlineId=tdpb-iframe-container" class="thickbox button" title="' . esc_attr__( 'Add Page Template', 'framework', 'smartfood' ) . '" style="padding-left: .4em;">' . $img . ' ' . esc_attr__( 'Add Template', 'framework', 'smartfood' ) . '</a><a id="template-builder-button" href="themes.php?page=tdp-page-builder" class="button button-primary">Template Builder</a>';
				}
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				<script type="text/javascript">
					function insertTemplate() {
						var id = jQuery( '#select-tdpb-template' ).val(),
							name = jQuery( '#select-tdpb-template :selected' ).text();

						/** Alert user if there is no template selected */
						if ( '' == id ) {
							alert("<?php echo esc_js( __( 'Please select your template first!', 'smartfood') ); ?>");
							return;
						}
				...
				<div id="tdpb-iframe-container" style="display: none;">
					<div class="wrap" style="padding: 1em">

						<?php do_action( 'tdpb_before_iframe_display', $templates ); ?>	

						<?php
						/** If there is no template created yet */
						if ( empty( $templates ) ) {
							echo sprintf( __( 'You don\'t have any template yet. Let\'s %s create %s one!', 'smartfood'), '<a href="' .admin_url().'themes.php?page=tdpt-page-builder">', '</a>' );
						} else { ?>						

							<h3><?php _e( 'Choose Your Page Template', 'smartfood'); ?></h3><br />
							<select id="select-tdpb-template" style="clear: both; min-width:200px; display: inline-block; margin-right: 3em;">
							<?php
							...
							<input type="button" id="tdpb-insert-template" class="button-primary" value="<?php echo esc_attr__( 'Insert Template', 'smartfood'); ?>" onclick="insertTemplate();" />
							<a id="tdpb-cancel-template" class="button-secondary" onclick="tb_remove();" title="<?php echo esc_attr__( 'Cancel', 'smartfood'); ?>"><?php echo esc_attr__( 'Cancel', 'smartfood'); ?></a>
							
						<?php } ?>

						<?php do_action( 'tdpb_after_iframe_display', $templates ); ?>
		...
		function builder_page_show(){
			...
			require_once(TDPB_PATH . 'view/view-builder-page.php');
		...
		function sort_templates(){
			...
			if (! wp_verify_nonce($nonce, 'tdpb-settings-page-nonce') ) die('-1');

...
if (have_posts()) :
	...
	if($postlist->have_posts()) : ?>
		<section class="columns">
			<?php while($postlist->have_posts()): $postlist->the_post(); ?><article <?php post_class('col col2'); ?>>
				<?php 
				$iframe_video = get_post_meta(get_the_id(), 'single_meta_video_iframe', true);
				if($iframe_video):
					echo '<p>'.$iframe_video.'</p>'; 
				else :
					if(has_post_thumbnail()): ?><div class="img"><a href="<?php the_permalink() ?>"><span class="img-border"><?php the_post_thumbnail('thumbnail-medium'); ?></span></a></div><?php endif;
				endif; ?>

...
/* Royal Recent Category Post Widget for Footer */

 
class Royal_Recent_Category_Post_Footer_Widget extends WP_Widget {

    function Royal_Recent_Category_Post_Footer_Widget() {
		global $themename;
		$widget_ops = array('classname' => 'custom-recent-category-widget', 'description' => __( "Recent category post widget - For Footer", 'my-text-domain') );
		$control_ops = array('width' => 250, 'height' => 200);
		$this->WP_Widget('recentcategorypostsfooter', __('8) Royal Recent Category Post - For Footer', 'my-text-domain'), $widget_ops, $control_ops);
    }

    function widget($args, $instance) {
		global $wpdb, $shortname;
        extract( $args );
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Recent Category Posts', 'my-text-domain') : $instance['title'], $instance, $this->id_base);
		...
		<ul class="widget_footer_posts">
			...
			<li>
				...
				<div class="widget_footer_thumbnail">
				<?php $image = aq_resize( $thumbnailSrc, 60, 40, true ); ?>
                <img src="<?php echo $image ?>" alt="<?php echo $post_title; ?>" title="<?php echo $post_title; ?>" />
				</div>
				</a>
				<?php } ?>
				<div class="widget_footer_info">
				<a class="widget_footer_title" href="<?php echo $permalink; ?>" rel="bookmark"><?php echo $post_title; ?></a>
			    </div>
			<div class="clear"></div>
			</li>
				<?php } ?>
...
add_action('widgets_init', create_function('', 'return register_widget("Royal_Recent_Category_Post_Footer_Widget");'));

  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $c = 0;
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '" alt="'.esc_html__('Remove','candidate').'" title="'.esc_html__('Remove','candidate').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '" alt="'.esc_html__('Add','candidate').'" title="'.esc_html__('Add','candidate').'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  if (!isset($value['id']))
	...
	echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
	if ($has_image)
		echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".esc_html__('Remove Image', 'candidate')."' type='button'/>";
	else
		echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".esc_html__('Upload Image', 'candidate')."' type='button'/>";
	$this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_color( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . esc_html__( 'Select a color','candidate' ) . "'/>";

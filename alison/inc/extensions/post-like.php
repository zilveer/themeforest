...
function like_scripts() {
	wp_enqueue_script( 'jm_like_post', get_template_directory_uri().'/assets/js/post-like.min.js', array(), NULL, true );
...
function jm_post_like() {
	...
	if ( isset( $_POST['jm_post_like'] ) ) {
		...
		if ( is_user_logged_in() ) { // user is logged in
			...
			if ( !AlreadyLiked( $post_id ) ) { // like the post
				setcookie("alison-".$post_id, true, time() + (86400 * 365));
				update_post_meta( $post_id, "_post_like_count", ++$post_like_count ); // +1 count post meta
				echo esc_html($post_like_count); // update count on front end
				
			} else { // unlike the post
				setcookie("alison-".$post_id, false, time() + (86400 * 365));
				update_post_meta( $post_id, "_post_like_count", --$post_like_count ); // -1 count post meta
				echo "already".esc_html($post_like_count); // update count on front end
...
function AlreadyLiked( $post_id ) { // test if user liked before
	if ( is_user_logged_in() ) { // user is logged in
		...
		if(isset($_COOKIE["alison-".$post_id])){
			$is_post_like = $_COOKIE["alison-".$post_id];
...
function show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'alison' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				echo "<p>" . _e( 'You don\'t like anything yet.', 'alison' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
...
function frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . __( 'You Like:', 'alison' ) . "</h3>\n";
			$like_list .= "<ul>\n";
...
function jm_most_popular_today() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . _e( 'Today\'s Most Popular Posts', 'alison' ) . "</h3>\n";
		echo "<ul>\n";
...
function jm_most_popular_month() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . _e( 'This Month\'s Most Popular Posts', 'alison' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular_week() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . _e( 'This Week\'s Most Popular Posts', 'alison' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular() {
	global $post;
	echo "<aside>\n";
	echo "<h3>" . __( 'Most Popular Posts', 'alison' ) . "</h3>\n";
	echo "<ul>\n";
	$args = array(
		 'post_type' => array( 'post', 'enter-your-comma-separated-post-types-here' ),
		 'meta_query' => array(

...
if ( !isset( $pexeto->content_buttons ) ) {
	$pexeto->content_buttons = array( 'pexetotwocolumns', 'pexetothreecolumns',
		...
		'pexcirclecta', 'pexetonivoslider', 'pexetoposts', 'pexetopricing', 'pexetobgsection' );
}
...
if(!function_exists('pexeto_load_tinymce_styles')){
	...
	function pexeto_load_tinymce_styles(){
		...
		if ( $current_screen->base=='post' ) {
			wp_enqueue_style('pexeto-buttons', PEXETO_FUNCTIONS_URL.'formatting-buttons/buttons.css', array(), PEXETO_VERSION);
		}
	}
}
...
if ( !function_exists( 'pexeto_load_button_data_sets' ) ) {
	...
	function pexeto_load_button_data_sets() {
		...
		if ( $current_screen->base=='post' ) {
			...
			$service_terms = get_terms( PEXETO_SERVICES_POSTTYPE.PexetoCustomPage::term_suffix, array( 'hide_empty'=>false ) );
			$service_sets = array();
			foreach ( $service_terms as $term ) {
				$service_sets[]=array( 'id'=>$term->slug, 'name'=>$term->name );
			}

			//load the Nivo sliders
			$nivo_terms = get_terms( PEXETO_NIVOSLIDER_POSTTYPE.PexetoCustomPage::term_suffix, array( 'hide_empty'=>false ) );
			$nivo_sliders = array();
			foreach ( $nivo_terms as $term ) {
				$nivo_sliders[]=array( 'id'=>$term->term_id, 'name'=>$term->name );
			}

			//load the testimonials
			$testimonial_terms = get_terms( PEXETO_TESTIMONIALS_POSTTYPE.PexetoCustomPage::term_suffix, array( 'hide_empty'=>false ) );
			$testimonial_sets = array();
			foreach ( $testimonial_terms as $term ) {
				$testimonial_sets[]=array( 'id'=>$term->slug, 'name'=>$term->name );
			}

			//load the pricing tables
			$pricing_terms = get_terms( PEXETO_PRICING_POSTTYPE.PexetoCustomPage::term_suffix, array( 'hide_empty'=>false ) );
			$pricing_sets = array();
			foreach ( $pricing_terms as $term ) {
				$pricing_sets[]=array( 'id'=>$term->term_id, 'name'=>$term->name );
			}

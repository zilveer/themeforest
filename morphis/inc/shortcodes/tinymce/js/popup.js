...
jQuery(document).ready(function($) {
    var zillas = {
    	...
    	resizeTB: function()
    	{
			...
			$('#pulp-popup').addClass('no_preview');
    	},
    	load: function()
    	{
    		var	zillas = this,
    		...
    		$('.zilla-insert', form).click(function() {    		 			
    			if(window.tinyMCE)
				{
					//window.tinyMCE.execCommand( 'mceInsertContent', false, $('#_zilla_ushortcode', form).html() );
					tinymce.get(window.tinyMCE.activeEditor.id).execCommand( 'mceInsertContent', false, $('#_zilla_ushortcode', form).html() );
					tb_remove();
				}
    		});
    	}
    ...
    // run
    $('#pulp-popup').livequery( function() { zillas.load(); } );
});

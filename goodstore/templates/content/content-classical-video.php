...
?>
<article id="post-<?php the_ID(); ?>"  <?php post_class(array('element', $class, 'col-lg-'.jaw_template_get_var('box_size','max'), 'content-classical', 'format-video')); ?>   
        ...
        <div class="image">
            ...
            if (!jwUtils::has_post_thumbnail()) {
                ...
                if (isset($video->thumbnails['thumbnail_medium'])) {
                    echo '<img src="' . esc_url($video->thumbnails['thumbnail_medium']) . '"  width = "' . esc_attr(jwUtils::get_size('4') - 40) . '" height="193"   alt="' . esc_attr(get_the_title()) . '" / >';
                }
            } else {

...
remove_filter('the_excerpt', 'startuply_excerpt_read_more_link');
...
while ( $my_query->have_posts() ) {
	...
	$post_comments_count = wp_count_comments($post->id);
	$post->meta_data = get_the_date('j M, Y');
	$post->meta_comments_count = $post_comments_count->approved;
	$post->meta_comments_link = get_comments_link();
	$post->meta_author = get_the_author();
	$post->meta_cat = get_the_category_list(", ");
	...
	if ( $vc_teaser_box->getTeaserData( 'enable', $post->id ) === '1' ) {
		...
		$post->image_link = empty( $video ) && $post->thumbnail && isset( $post->thumbnail_data['p_img_large'][0] ) ? $post->thumbnail_data['p_img_large'][0] : $video;
		if ( $grid_columns_count < 6 ) {
			$post->thumbnail = $post->thumbnail_data['p_img_large'][0] ? '<img src="' . $post->thumbnail_data['p_img_large'][0] . '" />' : '';
		}
	}

	$post->categories_css = $this->getCategoriesCss( $post->id );

	if (mb_strlen($post->title) > $max_title_chars) {
		$post->title = mb_substr($post->title, 0, $max_title_chars - mb_strlen($title_remaind_str)).$title_remaind_str;
	}
...
?>
<div class="vc_post_grid_container <?php echo apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $css_class, $this->settings['base'], $atts ) ?>">
	<div class="wpb_wrapper">
		<?php echo wpb_widget_title( array( 'title' => $title, 'extraclass' => 'wpb_teaser_grid_heading' ) ) ?>
		<div class="teaser_grid_container">
			<?php if ( $filter === 'yes' && ! empty( $this->filter_categories ) ): $categories_array = $this->getFilterCategories(); ?>

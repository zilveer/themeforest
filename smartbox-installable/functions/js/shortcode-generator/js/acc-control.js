function desAccMaker(h, i, f) {
    ...
    this.buildSelectControl = function () {
        ...
        this.selectControl = jQuery( "<select></select>").attr( "id", "des-acc-select").addClass(f ? f : "" );
        var a = jQuery( "<option></option>").attr( "value", "select").attr( "selected", "selected").text( "Number of Sections..." );
        a.appendTo(this.selectControl);
        for (var b = 2; b <= this.maxTabs; b++) {
            a = jQuery( "<option></option>").attr( "value", b).text(b + " sections" );
            a.appendTo(this.selectControl)
        }
    ...
    this.buildTextInputControl = function ( id ) {
    	...
    	var labelElement = '<label for="des_acc_title">Section ' + id + ' Title</label>';
    	var inputElement = '<input type="text" id="des_acc_title_' + id.toString() + '" class="txt input-text" name="des_acc_title" />';
    ...
    this.buildTabButtons = function (a) {
        if (this.buttonsControl) {
			...
			jQuery( 'label[for="des_acc_title"]').parents( 'tr').remove();

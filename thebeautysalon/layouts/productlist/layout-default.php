...
/** Default Product List
  ...
  */
  ...
  $disable_links = get_post_meta( $post->ID, 'disable_links', true );
  $disable_links = ( empty($disable_links) ) ? 'no' : $disable_links;
...
<div class='category productlist-layout-default'>
	...
	<div class='products'>
		...
		<div class='product <?php echo $thumbnail_class ?>'>
			...
			<?php if( !empty( $has_thumbnail ) ) : ?>
				<div class='image'>
					<?php if( $disable_links == 'yes' ) : ?>
					<?php the_post_thumbnail( 'eb_thumb' ) ?>
					<?php else : ?>
					<a href='<?php the_permalink() ?>' class='hoverlink'><?php the_post_thumbnail( 'eb_thumb' ) ?></a>
					<?php endif ?>
				</div>
			<?php endif ?>

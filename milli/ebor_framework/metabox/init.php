...
class cmb_Meta_Box {
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear' => __( 'Clear', 'ebor_starter' ),
					'defaultString' => __( 'Default', 'ebor_starter' ),
					'pick' => __( 'Select Color', 'ebor_starter' ),
					'current' => __( 'Current Color', 'ebor_starter' ),
				) );
			}
		} else {
	...
	function post_metabox() {
		...
		self::show_form( $this->_meta_box, $post->ID, 'post' );
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'ebor_starter' ) );

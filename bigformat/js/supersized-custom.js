/*
	...
	============================================================
	Customized by Andre Gagnon for the BigFormat Wordpress Theme
	============================================================	
*/
...
(function($){ 
	...
	$(document).ready(function() {
		$('.mainbody').append('<div id="supersized-loader"></div><ul id="supersized"></ul>');
	});
...
/*-----------------------------------------------------------------------------------*/
  ...
  jQuery(function($) {
	...
	$('.navhandle, #tray-button, .page-template-template-home-php .lines, .single-portfolio .lines').click(function(e) { e.preventDefault();

    $navnow = $('.navhandle').css('left');
    
    if ($navnow == navhandleinit) {
		document.getElementById('wrapper').style.marginLeft = "10px";
			base.resizeNowAnimate();
    ...
    if (($thisid == 'tray-button')) {
		...
		/* ----------------------------------------------------------------------------------------------
		/ Resize Images Animate
		/-----------------------------------------------------------------------------------------------*/
		
		base.resizeNowAnimate = function(){
		  		...
		  		$('img', base.el).each(function(){
					...
					if (base.options.fit_always){	// Fit always is enabled
						...
						if ((browserheight <= base.options.min_height) && (browserwidth <= base.options.min_width)){	// If window smaller than minimum width and height
							...
							if ((browserheight/browserwidth) > ratio){
								base.options.fit_landscape && ratio < 1 ? resizeWidthAnimate(true) : resizeHeightAnimate(true);	// If landscapes are set to fit
							} else {
								base.options.fit_portrait && ratio >= 1 ? resizeHeightAnimate(true) : resizeWidthAnimate(true);		// If portraits are set to fit
							}
						
						} else if (browserwidth <= base.options.min_width){		// If window only smaller than minimum width
						
							if ((browserheight/browserwidth) > ratio){
								base.options.fit_landscape && ratio < 1 ? resizeWidthAnimate(true) : resizeHeightAnimate();	// If landscapes are set to fit
							} else {
								base.options.fit_portrait && ratio >= 1 ? resizeHeightAnimate() : resizeWidthAnimate(true);		// If portraits are set to fit
							}
							
						} else if (browserheight <= base.options.min_height){	// If window only smaller than minimum height
						
							if ((browserheight/browserwidth) > ratio){
								base.options.fit_landscape && ratio < 1 ? resizeWidthAnimate() : resizeHeightAnimate(true);	// If landscapes are set to fit
							} else {
								base.options.fit_portrait && ratio >= 1 ? resizeHeightAnimate(true) : resizeWidthAnimate();		// If portraits are set to fit
							}
						
						} else {	// If larger than minimums
							
							if ((browserheight/browserwidth) > ratio){
								base.options.fit_landscape && ratio < 1 ? resizeWidthAnimate() : resizeHeightAnimate();	// If landscapes are set to fit
							} else {
								base.options.fit_portrait && ratio >= 1 ? resizeHeightAnimate() : resizeWidthAnimate();		// If portraits are set to fit
							}
					...
					/*-----Resize Animate Functions-----*/
					
					/*-----Resize Functions-----*/
				function resizeWidthAnimate(minimum){
						if (minimum){	// If minimum height needs to be considered
							if(thisSlide.width() < browserwidth || thisSlide.width() < base.options.min_width ){
								if (thisSlide.width() * ratio >= base.options.min_height){
									thisSlide.animate({width: base.options.min_width, height: (thisSlide.width() * ratio)}, 500, 'easeOutCubic');
						    	}else{
						    		resizeHeightAnimate();
						    	}
						    }
							...
							if (base.options.min_height >= browserheight && !base.options.fit_landscape){	// If minimum height needs to be considered
								if (browserwidth * ratio >= base.options.min_height || (browserwidth * ratio >= base.options.min_height && ratio <= 1)){	// If resizing would push below minimum height or image is a landscape
									thisSlide.animate({width: browserwidth, height: (browserwidth * ratio)}, 500, 'easeOutCubic');
								} else if (ratio > 1){		// Else the image is portrait
									thisSlide.animate({height: (base.options.min_height), width: (thisSlide.height() / ratio)}, 500, 'easeOutCubic');
								} else if (thisSlide.width() < browserwidth) {
									thisSlide.animate({width: (browserwidth), height: (thisSlide.width() * ratio)}, 500, 'easeOutCubic');
								}
							}else{	// Otherwise, resize as normal
						
							slideOrigWidth = thisSlide.width();
							slideOrigHeight = thisSlide.height();
							...
							thisSlide.width(slideOrigWidth);
							thisSlide.height(slideOrigHeight); 
					...
					function resizeHeightAnimate(minimum){
						if (minimum){	// If minimum height needs to be considered
							if(thisSlide.height() < browserheight){
								if (thisSlide.height() / ratio >= base.options.min_width){
									thisSlide.animate({height: (base.options.min_height), width: (thisSlide.height() / ratio)}, 500, 'easeOutCubic');
								}else{
									resizeWidthAnimate(true);
								}
							}
						}else{	// Otherwise, resized as normal
							if (base.options.min_width >= browserwidth){	// If minimum width needs to be considered
								if (browserheight / ratio >= base.options.min_width || ratio > 1){	// If resizing would push below minimum width or image is a portrait
									thisSlide.animate({height: (browserheight), width: (browserheight / ratio)}, 500, 'easeOutCubic');

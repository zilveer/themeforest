<?php
/**
 * WP_Importer class from wp importer plugin
 *  Modified a bit to be able to import menu meta as well
 */
...
class WP_Import extends WP_Importer {
	...
	function import_start( $file ) {
		if ( ! is_file($file) ) {
			echo '<p><strong>' . __( 'Sorry, there has been an error.', 'wolf' ) . '</strong><br />';
			echo __( 'The file does not exist, please try again.', 'wolf' ) . '</p>';
			$this->footer();
			die();
		}
	...
	function import_end() {
		...
		echo '<p>' . __( 'All done.', 'wolf' ) . ' <a href="' . admin_url() . '">' . __( 'Have fun!', 'wolf' ) . '</a>' . '</p>';
		//echo '<p>' . __( 'Remember to update the passwords and roles of imported users.', 'wolf' ) . '</p>';
	...
	function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			echo '<p><strong>' . __( 'Sorry, there has been an error.', 'wolf' ) . '</strong><br />';
			printf( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'wolf' ), esc_html( $file['file'] ) );
			echo '</p>';
			return false;
		}
		...
		if ( $this->version > $this->max_wxr_version ) {
			...
			printf( __( 'This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'wolf' ), esc_html($import_data['version']) );
			echo '</strong></p></div>';
		}
	...
	function get_authors_from_import( $import_data ) {
		...
		// no author information, grab it from the posts
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					//printf( __( 'Failed to import author %s. Their posts will be attributed to the current user.', 'wolf' ), esc_html( $post['post_author'] ) );
					//echo '<br />';
					continue;
				}
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					//printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'wolf' ), esc_html($this->authors[$old_login]['author_display_name']) );
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				//printf( __( 'Failed to import category %s', 'wolf' ), esc_html($cat['category_nicename']) );
				//if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG )
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				//printf( __( 'Failed to import post tag %s', 'wolf' ), esc_html($tag['tag_name']) );
				//if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG )
				//	echo ': ' . $id->get_error_message();
				//echo '<br />';
				continue;
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				//printf( __( 'Failed to import %s %s', 'wolf' ), esc_html($term['term_taxonomy']), esc_html($term['term_name']) );
				//if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG )
				//	echo ': ' . $id->get_error_message();
				//echo '<br />';
				continue;
	...
	function header() {
		echo '<div class="wrap">';
		screen_icon();
		//echo '<h2>' . __( 'Import WordPress', 'wolf' ) . '</h2>';
		...
		if ( isset( $updates[$basename] ) ) {
			...
			// echo '<div class="error"><p><strong>';
			// printf( __( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'wolf' ), $update->update->new_version );
			// echo '</strong></p></div>';
		}
	}
	...
	function greet() {
		...
		//echo '<p>'.__( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'wolf' ).'</p>';
		//echo '<p>'.__( 'Choose a WXR (.xml) file to upload, then click Upload file and import.', 'wolf' ).'</p>';

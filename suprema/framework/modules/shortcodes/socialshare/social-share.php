<?php
namespace SupremaQodef\Modules\Shortcodes\SocialShare;
...
class SocialShare implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		vc_map(array(
			'name' => esc_html__('Select Social Share', 'suprema'),
			'base' => $this->getBase(),
	...
	public function render($atts, $content = null) {
		...
		$params['enable_social_share'] = (suprema_qodef_options()->getOptionValue('enable_social_share') == 'yes') ? true : false;

		//Is social share enabled for post type
		$post_type = get_post_type();
		$params['enabled'] = (suprema_qodef_options()->getOptionValue('enable_social_share_on_'.$post_type)) ? true : false;
		...
		if ($params['enable_social_share']) {
			if ($params['enabled']) {
				$html .= suprema_qodef_get_shortcode_module_template_part('templates/' . $params['type'], 'socialshare', '', $params);
			}
		}
	...
	private function getSocialNetworksParams($params) {
		...
		foreach ($this->socialNetworks as $net) {
			...
			$html = '';
			if (suprema_qodef_options()->getOptionValue('enable_'.$net.'_share') == 'yes') {
				...
				$params['custom_icon'] = (suprema_qodef_options()->getOptionValue($net.'_icon')) ? suprema_qodef_options()->getOptionValue($net.'_icon') : '';
				$html = suprema_qodef_get_shortcode_module_template_part('templates/parts/network', 'socialshare', '', $params);
	...
	private function getSocialNetworkShareLink($net, $image) {
		...
		switch ($net) {
			...
			case 'tumblr':
				...
				break;
			case 'pinterest':
				$link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . suprema_qodef_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
				break;
			case 'vk':
	...
	private function getSocialNetworkLabel($net) {
		...
		switch ($net) {
			case 'facebook':
				$label = esc_html__("Share on facebook", "suprema");
				break;
			case 'twitter':
				$label = esc_html__("Share on twitter", "suprema");
				break;
			case 'google_plus':
				$label = esc_html__("Share on google+", "suprema");
				break;
			case 'linkedin':
				$label = esc_html__("Share on linkedin", "suprema");
				break;
			case 'tumblr':
				$label = esc_html__("Share on tumblr", "suprema");
				break;
			case 'pinterest':
				$label = esc_html__("Share on pinterest", "suprema");
				break;
			case 'vk':
				$label = esc_html__("Share on vk", "suprema");
				break;
			default:
				$label = "";
		}

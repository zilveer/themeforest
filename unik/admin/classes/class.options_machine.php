...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($smof_data) || !empty($unik_data)){
			...
			switch ( $value['type'] ) {
				...
				case 'file':
					$output .= '<div class="slider"><ul id="'.$value['id'].'">';
					$files = $smof_data[$value['id']];
					$count = count($files);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_file_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button file_add_button">Add New</a></div>';
					
				break;

				//drag & drop audio manager
				case 'audio':
					$output .= '<div class="slider"><ul id="'.$value['id'].'">';
					$files = $smof_data[$value['id']];
					$count = count($files);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_audio_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
						foreach ($files as $file) {
							$oldorder = $file['order'];
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button audio_add_button">Add New Audio</a></div>';
				...
				case 'sidebar':
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button sidebar_add_button">Add New</a></div>';
				...
				case 'sorter':
				    ...
				    $sortlists = isset($unik_data[$value['id']]) && !empty($unik_data[$value['id']]) ? $unik_data[$value['id']] : $value['std'];
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ',THEMENAME).'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
...
/**
	 ...
	 * @return string
	 */
	public static function optionsframework_audio_function($id,$std,$oldorder,$order){
		...
		$val['artist'] = isset($val['artist']) ? $val['artist'] : '';
		$slider .= '<label>Artist</label>';
		$slider .= '<input class="of-input" name="'. $id .'['.$order.'][artist]" id="'. $id .'_'.$order .'_slide_artist" value="'. stripslashes($val['artist']) .'" />';

		$slider .= '<div class="section-media"><label>Audio URL</label>';
		$slider .= '<input class="upload slide of-input" name="'. $id .'['.$order.'][url]" id="'. $id .'_'.$order .'_slide_url" value="'. $val['url'] .'" />';
		
		$slider .= '<div class="upload_button_div"><span class="button media_upload_button_2" id="'.$id.'_'.$order .'">Upload</span>';
		
		if(!empty($val['url'])) {$hide = '';} else { $hide = 'hide';}
		$slider .= '<span class="button remove-image-2 '. $hide.'" id="reset_'. $id .'_'.$order .'" title="' . $id . '_'.$order .'">Remove</span>';
		$slider .='</div></div>' . "\n";

		$val['poster_url'] = isset($val['poster_url']) ? $val['poster_url'] : '';
		$slider .= '<div class="section-media"><label>Poster Image URL</label>';
		$slider .= '<input class="upload slide of-input" name="'. $id .'['.$order.'][poster_url]" id="'. $id .'_'.$order .'_poster_url" value="'. $val['poster_url'] .'" />';
		
		$slider .= '<div class="upload_button_div"><span class="button media_upload_button_2" id="'.$id.'_'.$order .'">Upload</span>';
		
		if(!empty($val['poster_url'])) {$hide = '';} else { $hide = 'hide';}
		$slider .= '<span class="button remove-image-2 '. $hide.'" id="reset_'. $id .'_'.$order .'" title="' . $id . '_'.$order .'">Remove</span>';
		$slider .='</div>' . "\n";
		$slider .= '<div class="screenshot">';
		if(!empty($val['poster_url'])){
			
	    	$slider .= '<a class="of-uploaded-image" href="'. $val['poster_url'] . '">';
	    	$slider .= '<img class="of-option-image" id="image_'.$id.'_'.$order .'" src="'.$val['poster_url'].'" alt="" />';
	    	$slider .= '</a>';
	 ...
	 * @return string
	 */
	public static function optionsframework_file_function($id,$std,$oldorder,$order){
	    ...
	    if (isset($file[$oldorder])) { $val = $file[$oldorder]; } else {$val = $std;}
		...
		$files = '';
		//begin files interface	
		if (!empty($val['title'])) {
			$files .= '<li><div class="slide_header"><strong>'.stripslashes($val['title']).'</strong>';
		} else {
			$files .= '<li><div class="slide_header"><strong>Item '.$order.'</strong>';
		}
		
		$files .= '<input type="hidden" class="slide of-input order" name="'. $id .'['.$order.'][order]" id="'. $id.'_'.$order .'_slide_order" value="'.$order.'" />';
	
		$files .= '<a class="slide_edit_button" href="#">Edit</a></div>';
		
		$files .= '<div class="slide_body">';
		
		$files .= '<label>Title</label>';
		$files .= '<input class="slide of-input of-slider-title" name="'. $id .'['.$order.'][title]" id="'. $id .'_'.$order .'_slide_title" value="'. stripslashes($val['title']) .'" />';
		
		$files .= '<label>File URL</label>';
		$files .= '<input class="upload slide of-input" name="'. $id .'['.$order.'][url]" id="'. $id .'_'.$order .'_slide_url" value="'. $val['url'] .'" />';
		
		$files .= '<div class="upload_button_div"><span class="button media_upload_button" id="'.$id.'_'.$order .'">Upload</span>';
		
		if(!empty($val['url'])) {$hide = '';} else { $hide = 'hide';}
		$files .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'_'.$order .'" title="' . $id . '_'.$order .'">Remove</span>';
		$files .='</div>' . "\n";
		$files .= '<div class="screenshot">';
		if(!empty($val['url'])){
			
	    	$files .= '<a class="of-uploaded-image" href="'. $val['url'] . '">';
	    	$files .= '</a>';
			
			}
		$files .= '</div>';			
	
		$files .= '<a class="file_delete_button" href="#">Delete</a>';
	...
	public static function optionsframework_sidebar_function($id,$std,$oldorder,$order){
		...
		$sidebars = '';
		//begin sidebars interface	
		if (!empty($val['title'])) {
			$sidebars .= '<li><div class="slide_header"><strong>'.stripslashes($val['title']).'</strong>';
		} else {
			$sidebars .= '<li><div class="slide_header"><strong>Item '.$order.'</strong>';
		}

...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
	...
	function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
		...
		if ( !file_exists( $dest_file_name ) ) {
			...
			if ( !$size )
				return new WP_Error( 'file_path_getimagesize_failed', esc_html__('Failed to get $file_path information using "@getimagesize".','wta'), $file_path );
			...
			if ( IMAGETYPE_GIF == $orig_type ) {
				if ( !imagegif( $new_image, $dest_file_name ) )
					return new WP_Error( 'resize_path_invalid', esc_html__('Resize path invalid (GIF)','wta' ) );
			}
			elseif ( IMAGETYPE_PNG == $orig_type ) {
				if ( !imagepng( $new_image, $dest_file_name ) )
					return new WP_Error( 'resize_path_invalid', esc_html__('Resize path invalid (PNG).','wta' ) );
			}
			else {
				...
				if ( !imagejpeg( $new_image, $dest_file_name, apply_filters( 'resize_jpeg_quality', 90 ) ) )
					return new WP_Error( 'resize_path_invalid', esc_html__('Resize path invalid (JPG).','wta' ) );
			...
			$stat = stat( call_user_func('di' . 'rname',  $dest_file_name ));
			...
			if ( !$new_size )
				return new WP_Error( 'resize_path_getimagesize_failed', esc_html__('Failed to get $dest_file_name (resized image) info via @getimagesize','wta' ), $dest_file_name );
			list( $resized_width, $resized_height, $resized_type ) = $new_size;

			// Get the new image URL
			$resized_url = str_replace( wp_basename( $url ), wp_basename( $dest_file_name ), $url );

...
class TB_Categories extends WP_Widget {
	...
	function TB_Categories() {
		$widget_ops = array('classname' => 'tb_categories widget_categories', 'description' => __( 'Simple widget: shows categories', 'grace') );		
		$this->WP_Widget('TB_Categories', __('ThemeBlossom: Categories', 'grace'), $widget_ops);
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('About Us', 'grace') : $instance['title'], $instance, $this->id_base);
	...
	?>
        ...
        <p>
        	<label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of Categories:', 'grace'); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo $this->get_field_id('count'); ?>"><?php _e( 'Order by count of posts?', 'grace' ); ?></label></p>
        </p>
	<?php
	}
}

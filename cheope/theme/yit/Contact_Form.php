...
class YIT_Contact_Form {
        ...
        if ( isset( $_POST['yit_action'] ) AND ( $_POST['yit_action'] == 'sendemail' OR $_POST['yit_action'] == 'sendemail-ajax' ) AND wp_verify_nonce( $_REQUEST['_wpnonce'], 'yit-sendmail' ) )
        {
            ...
            if( isset( $_FILES['yit_contact']['tmp_name'] ) )
            {
                foreach( $_FILES['yit_contact']['tmp_name'] as $id => $a_file )
                {
                    ...
                    list( $file_name, $file_ext ) = explode( '.', $new_path );
    ...
    public function module( $name_or_id, $action, $echo = true )
    {
        ...
        foreach( $fields as $id => $field )
        {
            ...
            $is_required = (isset( $field['required'] ) AND intval( $field['required'] ));
            $is_email = (isset( $field['is_email'] ) AND intval( $field['is_email'] )) ;
            // errors
            $error_msg = '';
            $error = false;

            if(empty($field['error']) && $is_email) $field['error'] =  stripslashes_deep( $field['title'], 'highlight-text' ).' '.__( 'is not a valid address' , 'yit' );
            if(empty($field['error']) && $is_required) $field['error'] =  stripslashes_deep( $field['title'], 'highlight-text' ).' '.__( 'is a required field' , 'yit' );

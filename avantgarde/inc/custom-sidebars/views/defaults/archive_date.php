...
<h3 class="csh3title" title="<?php _e('Click to toogle', 'custom-sidebars'); ?>"><?php _e('Date archives','custom-sidebars'); ?></h3>
...
<div class="defaultsContainer"><?php if(!empty($modifiable)): foreach($modifiable as $m): $sb_name = $allsidebars[$m]['name'];?>
                <p><?php echo esc_attr($sb_name); ?>: 
                    <select name="date_page_<?php echo esc_attr($m);?>">
                        <option value=""></option>
                    <?php foreach($allsidebars as $key => $sb):?>
                        <option value="<?php echo esc_attr($key); ?>" <?php echo (isset($defaults['date'][$m]) && $defaults['date'][$m]==$key) ? 'selected="selected"' : ''; ?>>

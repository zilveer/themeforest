...
function twitter_api_admin_render_page(){
    ...
    try {
        ...
        if( ! $consumer_key || ! $consumer_secret ){
            throw new Exception( __('Twitter application not fully configured', 'candidate') );
        }
        ...
        if( ! $access_key || ! $access_secret ){
            twitter_api_admin_render_header( __('Plugin not yet authenticated with Twitter', 'candidate'), 'error' );
        ...
        // else we have auth - verify that tokens are all still valid
            ...
            twitter_api_admin_render_header( sprintf( __('Authenticated as @%s', 'candidate'), $me['screen_name'] ), 'updated' );
        }
...
function twitter_api_admin_menu() {
    add_theme_page( __('Twitter API', 'candidate'), __('Twitter API', 'candidate'), 'manage_options', 'twitter-api-admin', 'twitter_api_admin_render_page');
}

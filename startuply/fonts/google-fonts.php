...
function startuply_get_google_fonts_list() {
  global $startuply_google_fonts;

  return json_decode($startuply_google_fonts, true);
}
 ...
 */
function startuply_typography_get_google_fonts() {
    // Google Font Defaults
    global $google_font_json;

    $fonts_data = startuply_get_google_fonts_list();
    $google_faces = array('' => __('Theme font', 'js_composer'));

    foreach ($fonts_data['items'] as $font) {
        $font_family = $font['family'].', '.$font['category'];
        $font_display = $font['family'].', '.$font['category'];

        $google_faces[$font_family] = $font_display;
    }
    return $google_faces;
}
 ...
 */
function startuply_typography_enqueue_google_font($font) {
    $font = explode(',', $font);
    $font = $font[0];
    ...
    $found = null;
    foreach ($fonts_data['items'] as $target) {
        if($target['family'] == $font) {
            $found = $target;
            break;
        }
    }
    ...
    if ($found) {
        $family = str_replace(" ", "+", $found['family']);
        $variants = implode( ',', $found['variants']);
        $subset = implode( ',', $found['subsets']);

        $url = is_ssl() ? 'https' : 'http';
        $url .= '://fonts.googleapis.com/css?family='.$family;
        ...
        wp_enqueue_style( "options_typography_$font", $url, false, null, 'all' );
    }
}
 ...
 */
function startuply_typography_get_google_font_styles($option, $selectors) {
    $font_parts = explode(',', $option, 2);
    $font_family = '\''.trim($font_parts[0]).'\'';
    if(count($font_parts) > 0) {
        $font_family .= ', '.trim($font_parts[1]);
    }
 ...
 * If so, startuply_typography_google_fonts is called to enqueue the font.
 * Ensures that each Google font is only enqueued once.
 */
if ( !function_exists( 'startuply_typography_google_fonts' ) ) {
    function startuply_typography_google_fonts() {
        $all_google_fonts = array_keys( startuply_typography_get_google_fonts() );
        // Define all the options that possibly have a unique Google font

        $font_options = array('custom_fonts_h16', 'custom_fonts_content', 'custom_fonts_menu_item');

        $selected_fonts = array();

        foreach ($font_options as $font_option) {
            $option = startuply_option($font_option, '');
            if (strlen($option)) {
                $selected_fonts[] = $option;
            }
        }
        ...
        foreach ( $selected_fonts as $font ) {
            if ( in_array( $font, $all_google_fonts ) ) {
                startuply_typography_enqueue_google_font($font);
            }
        }
    }
}
add_action( 'wp_enqueue_scripts', 'startuply_typography_google_fonts' );

/**
 * Outputs the selected option panel styles inline into the <head>
 */
function startuply_typography_google_fonts_styles() {
    $output = '';

    $font_selector_options = array(
        'custom_fonts_h16' => 'h1, h2, h3, h4, h5, h6, .h1, .h2, .h3, .h4, .h5, .h6, .h7, .heading-font',
        'custom_fonts_content' => 'body',
        'custom_fonts_menu_item' => 'nav.navigation-bar a');

    foreach ($font_selector_options as $font_option => $selector) {
        $option = startuply_option($font_option, '');

        if (strlen($option)) {
            $output .= startuply_typography_get_google_font_styles($option , $selector);
        }
    }
...
add_action('wp_head', 'startuply_typography_google_fonts_styles');

/**
 * DesThemes Typography Live Preview
 * 2011-10-07.
 *
 * @description The code below is designed to generate a live preview using the
 * setting specified in a "custom typography" field in the DesFramework.
 *
 * @since 4.7.0
 */
...
(function ($) {
  ...
  desTypographyPreview = {
    ...
    loadPreviewButtons: function () {
     ...
     var previewButtonHTML = '<a href="#" class="des-typography-preview-button button submit-button" title="' + 'Preview your customized typography settings' + '"><span>' + '+' + '</span></a>';
     
     $( 'input.des-typography-color' ).each( function ( i ) {
     	$( this ).after( previewButtonHTML );
     });
     
     // Register event handlers.
     desTypographyPreview.handleEvents();
    ...
    handleEvents: function () {
    	$( 'a.des-typography-preview-button' ).live( 'click', function () {
    		desTypographyPreview.generatePreview( $( this ) );
    		return false;
    	});
    	
    	$( 'a.preview_remove' ).live( 'click', function () {
    		desTypographyPreview.closePreview( $( this ) );
    		return false;
    	});
    }, 
     ...
     */
     ...
     closePreview: function ( target ) {
		target.parents( '.section' ).find( '.des-typography-preview-button .refresh' ).removeClass( 'refresh' );
    ...
    generatePreview: function ( target ) {
    	...
    	var fontUnit = controls.find( '.des-typography-unit' ).val();
    	
    	var sizeSelector = '.des-typography-size-px';
    	if ( fontUnit == 'em' ) { sizeSelector = '.des-typography-size-em'; }
    	
    	var fontSize = controls.find( sizeSelector ).val();
    	
    	var fontFace = controls.find( '.des-typography-face' ).val();
    	var fontStyle = controls.find( '.des-typography-style' ).val();
    	var fontColor = controls.find( '.des-typography-color' ).val();
    	...
    	controls.find( '.des-typography-preview-button span' ).addClass( 'refresh' );
    }

   
  }; // End desTypographyPreview Object // Don't remove this, or the sky will fall on your head.

/*-----------------------------------------------------------------------------------*/
/* Execute the above methods in the desTypographyPreview object.
/*-----------------------------------------------------------------------------------*/
  
	$(document).ready(function () {

		desTypographyPreview.loadPreviewButtons();

 ...
 */
add_action( 'widgets_init', 'icy_facebook_likebox_init' );

function icy_facebook_likebox_init() {
...
class WPCOM_Widget_Facebook_LikeBox extends WP_Widget {
	...
	function __construct() {
		parent::__construct( 'facebook-likebox', __( 'Facebook Like Box', 'framework' ), array( 'classname' => 'widget_facebook_likebox', 'description' => __( 'Display a Facebook Like Box to connect visitors to your Facebook Page', 'framework' ) ) );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( empty( $like_args['href'] ) || ! $this->is_valid_facebook_url( $like_args['href'] ) ) {
			if ( current_user_can('edit_theme_options') ) {
				echo $before_widget;
				echo '<p>' . sprintf( __( 'It looks like your Facebook URL is incorrectly configured. Please check it in your <a href="%s">widget settings</a>.', 'framework' ), admin_url( 'widgets.php' ) ) . '</p>';
				echo $after_widget;
			}
		...
		do_action( 'icy_stats_extra', 'widget', 'facebook-likebox' );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'href' ); ?>">
				<?php _e( 'Facebook Page URL', 'framework' ); ?>
				...
				<br />
				<small><?php _e( 'The Like Box only works with <a href="http://www.facebook.com/help/?faq=174987089221178">Facebook Pages</a>.', 'framework' ); ?></small>
			</label>
		</p>
		
		<p>
			<label for="<?php echo $this->get_field_id( 'width' ); ?>">
				<?php _e( 'Width', 'framework' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'colorscheme' ); ?>">
				<?php _e( 'Color Scheme', 'framework' ); ?>
				<select name="<?php echo $this->get_field_name( 'colorscheme' ); ?>" id="<?php echo $this->get_field_id( 'colorscheme' ); ?>">
					<option value="light" <?php selected( $like_args['colorscheme'], 'light' ); ?>><?php _e( 'Light', 'framework' ); ?></option>
					<option value="dark" <?php selected( $like_args['colorscheme'], 'dark' ); ?>><?php _e( 'Dark', 'framework' ); ?></option>
				</select>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_faces' ); ?>">
				...
				<?php _e( 'Show Faces', 'framework' ); ?>
				<br />
				<small><?php _e( 'Show profile photos in the plugin.', 'framework' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'stream' ); ?>">
				...
				<?php _e( 'Show Stream', 'framework' ); ?>
				<br />
				<small><?php _e( 'Show the profile stream for the public profile.', 'framework' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'force_wall' ); ?>">
				...
				<?php _e( 'Show Wall', 'framework' ); ?>
				<br />
				<small><?php _e( 'Show the wall for a Places page rather than friend activity.', 'framework' ); ?></small>
			</label>
		</p>
	...
	function get_default_args() {
		...
		return apply_filters( 'icy_facebook_likebox_defaults', $defaults );
	}

(function( window, $, undefined ) {
	...
	$.elastislide.prototype 	= {
		...
		_configure			: function() {
			...
			this.$slider.parents('.es-carousel-wrapper').addClass('no-slide'); 
		...
		_initEvents			: function() {
			...
			$(window).bind('resize.elastislide', function( event ) {
				...
				if( instance.options.infinite ) {
					clearInterval(this.interval);
	    			this.interval = setInterval(function(){
	    				instance._slideInfinite('right');
...
*/			
        ...
        _slideInfinite      : function( dir, val, anim, callback ) {  
			
    		this.$slider.parents('.es-carousel-wrapper').removeClass('no-slide').addClass('sliding'); 
            ...
            if( val === undefined ) {
                ...
                if( dir === 'right' && this.sliderW - ( Math.abs( ml ) + amount ) < this.visibleWidth ) {
                    for (var i=0;i<this.fitCount;i++) { // add elements
                        ...
                        this.$slider.append( this.$slider.children('li:first').clone(true) ) ;
                        this.$slider.children('li:first').remove();
                    }
                } else if ( dir === 'left' && Math.abs( ml ) - amount < 0 ) {
                    for (var i=0;i<this.fitCount;i++) { // add elements
                        this.$slider.css('margin-left', '-=' + this.itemW );
                        this.$slider.prepend( this.$slider.children('li:last').clone(true) ) ;
            ...
            this.$slider.applyStyle( sliderCSS, $.extend( true, [], { duration : this.options.speed, easing : this.options.easing, complete : function() {
    			...
    			instance.$slider.parents('.es-carousel-wrapper').removeClass('sliding').addClass('no-slide');
            } } ) );
            
            this.$slider.find('li').trigger('elastiautoslide');
		...
		_slideToCurrent		: function( anim ) {
			...
			if( this.options.infinite) {
                this._slideInfinite('', -amount, anim);
            } else {
		...
		_destroy 			: function( callback ) {
			...
			this.$el.find('.products').attr('style','');

...
class DropDown_Nav_Menu extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth, $args ) {
		...
		if(isset($item->url)) { $output .= $indent . '<option' . $class_names .' value="'. $item->url .'"'. $selected .'>'; }

		// push sub-menu items in as we can't nest optgroups
		$indent_string = null;
		if(isset($args->indent_string)) { $indent_string = str_repeat( apply_filters( 'dropdown_menus_indent_string', $args->indent_string, $item, $depth, $args ), ( $depth ) ? $depth : 0 ); }
		$indent_string .= !empty( $indent_string ) ? apply_filters( 'dropdown_menus_indent_after', $args->indent_after, $item, $depth, $args ) : '';

		if(isset($args->before)) { $item_output = $args->before . $indent_string; }
		if(isset($item->title)) { $item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after; }
		if(isset($args->after)) { $item_output .= $args->after; }

		if(isset($item_output)) { $output .= apply_filters( 'walker_nav_menu_dropdown_start_el', $item_output, $item, $depth, $args ); }
	}
	...
	function end_el( &$output, $item, $depth ) {
		if(has_nav_menu('header-menu') || has_nav_menu('category-menu')) { $output .= apply_filters( 'walker_nav_menu_dropdown_end_el', "</option>\n", $item, $depth); }
	}
}

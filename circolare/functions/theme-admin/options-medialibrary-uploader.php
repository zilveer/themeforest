...
function optionsframework_mlu_init () {
	register_post_type( 'optionsframework', array(
		'labels' => array(
			'name' => __( 'Theme Options Media', 'circolare' ),
		),
		'public' => true,
		'show_ui' => false,
...
function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
	...
	$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload', 'circolare' ) . '" rel="' . $int . '" />' . "\n";
...
function optionsframework_mlu_modify_tabs ( $tabs ) {
	$tabs['gallery'] = str_replace( __( 'Gallery', 'circolare' ), __( 'Previously Uploaded', 'circolare' ), $tabs['gallery'] );
	return $tabs;
}

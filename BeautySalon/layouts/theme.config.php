...
if ($this['config']->get('headerbg')) {
    $css[] .= sprintf('.header-wrapper { background-image: url('.$this['config']->get('headerbg').' ) !important;}');
}
if ($this['config']->get('footerbg')) {
    $css[] .= sprintf('.bottom-wrapper { background-image: url('.$this['config']->get('footerbg').' ) !important;}');
}
if ($this['config']->get('bodybackground')) {
    $css[] .= sprintf('body { background-image: url('.$this['config']->get('bodybackground').' ) !important;}');
}
if ($this['config']->get('boxedversion')) {
    $css[] .= sprintf('.header-wrapper, .body-wrapper, .bottom-wrapper{ max-width: %dpx; margin: 0 auto; }',  '1200px');
    
    if ($this['config']->get('bodybackground')) {
    $css[] .= sprintf('body { background-image: url('.$this['config']->get('bodybackground').'); background-position: top center;}'); 
    }
}

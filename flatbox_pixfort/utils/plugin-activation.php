...
function options_panel()
{
    ...
    if (isset($_POST['panel_name'])) {
        $settings_display = (isset($_POST['panel_name'])) ? trim($_POST['panel_name']) : '';
        $answer = options_html($settings_display);
    }

    $all = get_option('crum_settings');

    if ($all === false || $all == 0) {

?>
     <?php  echo get_option('purchase-form'); ?>
     <?php  echo '<p><strong style="color:red">' . $answer . '</strong></p>' ?>
     <?php  echo get_option('purchase-message'); ?>

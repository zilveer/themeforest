...
if ( !function_exists( 'get_pps_post_ids' ) ) {
	...
	*/
	function pps_add_custom_box() {
		...
		if ( current_user_can( 'edit_theme_options' ) ) {
			add_meta_box( 'pps_sectionid', __( 'Custom Sidebar', 'craftsman' ), 'pps_inner_custom_box', 'post', 'advanced', 'high' );
			add_meta_box( 'pps_sectionid', __( 'Custom Sidebar', 'craftsman' ), 'pps_inner_custom_box', 'page', 'advanced', 'high' );
		}
	}
	...
	*/
	function pps_inner_custom_box( $post ) {
		...
		if ( function_exists( 'wp_nonce_field' ) ) {
			wp_nonce_field( 'pps_nonce_action', 'pps_nonce' );
		}
		echo '<p style="margin-bottom:0px;"><input type="checkbox" value="1" id="tcc-pps-active" name="tcc-pps-active" class="tcc-pps-checkbox" ' .
			tcc_pps_add_checked( $pps_active_on_post, true ) . '/><label for="tcc-pps-active">' . __( 'Activate Custom Sidebar?', 'craftsman' ) . 
			'</label></p><p style="margin-top:5px; margin-bottom:25px; color: #888888;" ><i>New sidebar will be created in the Appearance > Widgets after you save the page.</i></p>';
		echo '<div class="tcc-pps-radio-replace-buttons">';
		echo '<p><strong>' . __( 'Select a sidebar to replace:', 'craftsman' ) . '</strong></p>';
		...
		foreach ( $wp_registered_sidebars as $id => $sidebar ) {
			...
			if ( substr( $name, 0, strlen( SIDEBAR_PREFIX ) ) != SIDEBAR_PREFIX ) {
				...
				if ( $screen->post_type == 'page' && $id != 'blog-sidebar' ){
					echo '<p><span class="tcc-pps-radio-option"><input type="radio" name="tcc-pps-replace" value="' . $id . 
					'" class="tcc-pps-radio" ' . tcc_pps_add_checked( $replaced_sidebar, $id ) . '/> ' . $name . '</span></p>';
				} elseif ( $screen->post_type == 'post' && $id != 'deafult-sidebar' ) {
					echo '<p><span class="tcc-pps-radio-option"><input type="radio" name="tcc-pps-replace" value="' . $id . 
					'" class="tcc-pps-radio" ' . tcc_pps_add_checked( $replaced_sidebar, $id ) . '/> ' . $name . '</span></p>';
				}	
			}
	...
	*/
	function pps_save_postdata( $post_id ) {
		...
		if( !isset( $_POST['pps_nonce'] ) || !wp_verify_nonce( $_POST['pps_nonce'], 'pps_nonce_action' ) ) return $post_id;
	...
	/*	
	*	---------------------------------------------------------------------
	*	Adds the custom sidebar to the sidebar array as a page is loaded.
	*	--------------------------------------------------------------------- 
	*/

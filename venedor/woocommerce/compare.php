...
<head>
    ...
    <title><?php _e( 'Product Comparison', 'venedor' ) ?></title>
    ...
    <style type="text/css">
        ...
        <?php venedor_print_hbg('btn') ?>
            color: <?php echo $venedor_design['btn-hcolor'] ?>;
        }
    </style>
</head>
<!-- END HEAD -->
...
<h1 class="page-title">
    ...
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php _e( 'Close window [X]', 'venedor' ) ?></a><?php endif; ?>
</h1>
...
<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php _e( 'No products added in the compare table.', 'venedor' ) ?></td>
        </tr>
        ...
        <tr class="remove">
            ...
            <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id ?>
                <td class="<?php echo $product_class; ?>">
                    <a href="<?php echo esc_url( add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->id ) ) ) ?>" data-product_id="<?php echo $product->id; ?>"><?php _e( 'Remove', 'venedor' ) ?> <span class="remove">x</span></a>
                </td>
            <?php endforeach ?>
        </tr>

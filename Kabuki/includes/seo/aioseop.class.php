...
//			$keywords =	$this->get_unique_keywords($keywords);
			...
			if ( isset ($aioseop_options['aiosp_front_meta_tags']) ) {
...
//		if (!wp_verify_nonce($nonce, 'edit-aioseop-nonce')) die ( 'Security Check - If you receive this in error, log out and back in to WordPress');
	...
	function admin_menu() {
		...
		add_menu_page('SEO Options','SEO Options', 'manage_options', $file, array($this, 'options_panel'), get_template_directory_uri().'/includes/seo/images/seo-icon.png' );
	}
	...
	function options_panel() {
		...
		if(!$aioseop_options['aiosp_cap_cats']) {
			...
			if (isset($_POST['action']) && $_POST['action'] == 'aiosp_update' && isset($_POST['Submit_Default'])) {
				...
				$message = __("SEO Options have been reset to default values.", 'all_in_one_seo_pack');
...
?>
	...
	<div class="wrap">
		...
		<h2><?php _e('SEO Options', 'all_in_one_seo_pack'); ?></h2>
...
<?php
_e('The SEO Options must be enabled for use.', 'all_in_one_seo_pack');
 ?>
</div>
</td>
</tr>

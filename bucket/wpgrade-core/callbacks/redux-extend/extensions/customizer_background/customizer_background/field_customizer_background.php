...
// Don't duplicate me!
if ( ! class_exists( 'ReduxFramework_customizer_background' ) ) {

	/**
	 * Main ReduxFramework_customizer_background class
	 * @since       3.1.5
	 */
	class ReduxFramework_customizer_background {
		...
		public function enqueue() {
			wp_enqueue_script( 'redux-field-background-js', $this->extension_url . 'field_customizer_background.js', array(
				'jquery',
				'wp-color-picker',
				'select2-js'
			), time(), true );

			wp_enqueue_script( 'field-select-js', ReduxFramework::$_url . 'inc/fields/select/field_select.js', array(
				'jquery',
				'select2-js'
			), time(), true );

			wp_enqueue_style( 'redux-field-background-css', $this->extension_url . 'field_customizer_background.css', time(), true );

			wp_enqueue_style( 'redux-field-select-css', ReduxFramework::$_url . 'inc/fields/select/field_select.css', time(), true );
		...
		public function getCSS() {
			...
			if ( isset( $GLOBALS['wp_customize'] ) && wpgrade::customizer_option_exists( $field['id'] ) ) {
				$this->value = wpgrade::get_customizer_option( $field['id'] );
			}

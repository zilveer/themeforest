    ...
    if ( ! class_exists( 'ReduxFramework_extension_metaboxes' ) ) {
        ...
        class ReduxFramework_extension_metaboxes {
            ...
            public function __construct( $parent ) {
                ...
                if ((int)$this->post_id == (int)get_option('page_for_posts')) {
                    //return;
                }
            ...
            public function init() {
                ...
                foreach ( $this->boxes as $bk => $box ) {
                    ...
                    // if ((int)$this->post_id == (int)get_option('page_for_posts')) {
                    //     if ( isset($box['page_template']) && ($box['page_template'][0] != 'default') ) {
                    //         continue;
                    //     }
                    // }
                    ...
                    if ( ! empty( $box['sections'] ) ) {
                        ...
                        if ( $addField
                            ...
                            if ( isset( $box['page_template'] ) && $this->post_type == "page" ) {
                                ...
                                if ((int)$this->post_id == (int)get_option('page_for_posts')) {
                                    foreach ($box['page_template'] as $temp) {
                                        //var_dump($temp);
...
//                        );
            ...
            function get_post_ID() {
                if(is_archive() && !is_admin()) {
                    ...
                    if($postType == 'post' || $postType == 'portfolio') {
                        if(isset($redux['arkona_'.$postType.'_archive_parent'])) {

                            $id = $redux['arkona_'.$postType.'_archive_parent'];
                            $id = intval($id);
                            // echo 'Metaboxes id:  '.$id .'</br>';
                            return $id;
                        } else {
                    ...
                    // arkona_post_archive_parent;
                    // arkona_portfolio_archive_parent;
                ...
                foreach ( (array) $rewrite as $match => $query ) {
                    ...
                    if ( preg_match( "!^$match!", $request_match, $matches ) ) {
                        ...
                        if ( ! empty( $query->posts ) && $query->is_singular ) {
                            ...
                            global $wpdb;
                            // Custom post_type with rewrite permalinks enabled
                            // Take slug name from url and fetch id from database
                            $parts = explode("/", $url);
                            $slug_name = $parts[count($parts)-1];

                            $post_id = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_name = %s", $slug_name ) );
...
//print_r($data);
                ...
                <div
                    class="redux-container redux-has-sections redux-box-<?php echo $metabox['args']['position']; ?>">
                            ...
                            foreach ( $sections as $sKey => $section ) :
                                ...
                                if ( isset( $section['fields'] ) && ! empty( $section['fields'] ) ) {
                                    ...
                                    foreach ( $section['fields'] as $fKey => $field ) {
                                        ...
                                        // $post_meta = get_post_meta( $this->post_id, $field['id']);

                                        // if(!empty($post_meta)) {
                                        //     echo '<span class="reset-this-field hidden"></span>';
                                        // }
...
// print_r($this->options[$field['id']]);
            ...
            function meta_boxes_save( $post_id, $post ) {
                ...
                foreach ( $_POST[ $this->parent->args['opt_name'] ] as $key => $value ) {
                    ...
                    // if($key == 'arkona_post_header_meta_elements') {
                    //     // print_r($key);
                    //     print_r($this->options_defaults[$key]);
                    //     print_r($value);
                    //     print_r($this->parent_options);
                    //     print_r($this->parent_defaults[$key]);
                    // }
                    ...
                    if ( isset( $this->options_defaults[ $key ] ) && $value == $this->options_defaults[ $key ] ) {
                        if(is_array($value)) {
                            foreach ($value as $firstKey => $firstValue) {
                                if(isset($firstValue['placebo'])) {
                                    if($value === $this->options_defaults[ $key ] ) {
                                        // $save = false;
                                    }
                                } else {
                                    // $save = false;
                    ...
                    // if(!$save) {
                    //     echo '<h3>'.$key.'</h3>';
                    //     var_dump($this->parent_defaults[ $key ]);
                    //     var_dump($this->options_defaults[ $key ]);
                    //     var_dump($this->options_defaults[ $key ]);
                    //     var_dump($value);
                    //     echo '<hr/>';
                    // }
                ...
                // print_r($toSave['arkona_post_header_title_typography']);
                // die();

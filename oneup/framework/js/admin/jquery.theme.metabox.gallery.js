(function ($) {
	...
	function PeMetaboxGallery(target, conf) {
		...
		//var mtags=target.find('input[name^="%0[tags]"]'.format(id.replace(/_([^_]+)_$/,"[$1]")));
		var mtags=target.find('div[id^="%0_tags_"]'.format(id));
		...
		function typeChange() {
			...
			default:
				...
				tags = $.makeArray(mtags.find('input[type=hidden]').map(getValue));

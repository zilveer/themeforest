...
if(!function_exists('libero_mikado_single_portfolio')) {
    function libero_mikado_single_portfolio() {
        $portfolio_template = libero_mikado_get_portfolio_single_type();
        ...
        if ($portfolio_template == 'gallery') {
            $params['holder_class'][] = 'mkd-portfolio-gallery-' . libero_mikado_options()->getOptionValue('portfolio_single_numb_columns');
        }

        libero_mikado_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
    }
}

if(!function_exists('libero_mikado_get_portfolio_single_type')) {
    function libero_mikado_get_portfolio_single_type() {
        return libero_mikado_get_meta_field_intersect('portfolio_single_template');
    }
}

if(!function_exists('libero_mikado_get_portfolio_single_media')) {
    function libero_mikado_get_portfolio_single_media() {
        ...
        if(is_array($videos) && count($videos)) {
            usort($videos, 'libero_mikado_compare_portfolio_videos');
            foreach($videos as $video) {
                ...
                if(!empty($video['portfoliovideotype'])) {
                    ...
                    $media['video_url']   = libero_mikado_portfolio_get_video_url($video);
...
if(!function_exists('libero_mikado_portfolio_get_video_url')) {
    function libero_mikado_portfolio_get_video_url($video) {
...
if(!function_exists('libero_mikado_portfolio_get_media_html')) {
    function libero_mikado_portfolio_get_media_html($media) {
        global $wp_filesystem;
        $params = array();

        //For adding image overlay in gallery template type
        $params['gallery'] = (libero_mikado_get_portfolio_single_type() == 'gallery') ? true : false;

        if($media['type'] == 'image') {
            $params['lightbox'] = libero_mikado_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
        ...
        if(in_array($media['type'], array('youtube', 'vimeo'))) {
            $params['lightbox'] = libero_mikado_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
        ...
        libero_mikado_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
    }
}

if(!function_exists('libero_mikado_compare_portfolio_videos')) {
    /**
     ...
     */
    function libero_mikado_compare_portfolio_videos($a, $b) {
...
if(!function_exists('libero_mikado_compare_portfolio_options')) {
    /**
     ...
     * @return int result of comparison
     */
    function libero_mikado_compare_portfolio_options($a, $b) {
...
if(!function_exists('libero_mikado_portfolio_get_info_part')) {
    function libero_mikado_portfolio_get_info_part($part) {
        $portfolio_template = libero_mikado_get_portfolio_single_type();

        libero_mikado_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template);
    }
}
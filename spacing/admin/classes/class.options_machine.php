...
class Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$output .= '<textarea class="of-input" name="'.$value['id'].'" id="'. $value['id'] .'" cols="'. $cols .'" rows="'. $value['rows'] .'">'.$ta_value.'</textarea>';		
				break;
				...
				case "radio":
					 ...
					 foreach($value['options'] as $option=>$name) {
						$output .= '<p><input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($data[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label></p>';				
					}
				break;
				...
				case 'sidebarmanager':
					...
					$int = '';
					$int = optionsframework_mlu_get_silentpost( $_id );
					$output .= '<div class="sidebarmanager"><ul id="'.$value['id'].'" rel="'.$int.'">';
					$sidebars = $data[$value['id']];
					$count = count($sidebars);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_sidebarmanager_function($value['id'],$value['std'],$oldorder,$order,$int);
					} else {
						$i = 0;
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slide_add_button">New Sidebar</a></div>';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'spacing_backend').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
	 ...
	 * @return string
	 */
	public static function optionsframework_sidebarmanager_function($id,$std,$oldorder,$order,$int){
		...
		//begin slider interface	
		if (!empty($val['title'])) {
			$sidebarmanager .= '<li><div class="slide_header"><strong>'.stripslashes($val['title']).'</strong>';
		} else {
			$sidebarmanager .= '<li><div class="slide_header"><strong>Slide '.$order.'</strong>';
		}
		
		$sidebarmanager .= '<input type="hidden" class="slide of-input order" name="'. $id .'['.$order.'][order]" id="'. $id.'_'.$order .'_slide_order" value="'.$order.'" />';
	
		$sidebarmanager .= '<a class="slide_edit_button" href="#">Edit</a></div>';
		
		$sidebarmanager .= '<div class="slide_body">';
		
		$sidebarmanager .= '<label>Title</label>';
		$sidebarmanager .= '<input class="slide of-input of-slider-title" name="'. $id .'['.$order.'][title]" id="'. $id .'_'.$order .'_slide_title" value="'. stripslashes($val['title']) .'" />';	
			
		$sidebarmanager .= '<label>Link URL (optional)</label>';
		$sidebarmanager .= '<input class="slide of-input" name="'. $id .'['.$order.'][link]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['link'] .'" />';
	
		$sidebarmanager .= '<a class="slide_delete_button" href="#">Delete</a>';
	    $sidebarmanager .= '<div class="clear"></div>' . "\n";

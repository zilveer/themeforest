...
class Generic_Admin_Object extends Runway_Admin_Object {
	...
	function do_error_message( $field_information ) {
		...
		if ( !$message_template ) {
			if ( isset( $field_information->validationMessage ) && !empty( $field_information->validationMessage ) ) {
				$message_template = $field_information->validationMessage;
			} else {
				$message_template = $default_errors_messages[$field_information->validation];
			}
		}

		if ( $field_information->required == 'true' ) {
			$message_template = $field_information->requiredMessage . '<br>' .$message_template;
		}

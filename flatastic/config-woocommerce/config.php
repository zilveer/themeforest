...
if (!class_exists('MAD_WOOCOMMERCE_CONFIG')) {
	...
	class MAD_WOOCOMMERCE_CONFIG {
		...
		function __construct() {
			...
			if (!$this->woocommerce_enabled()) { return false; }
		...
		public function woocommerce_global_config() {
			...
			if (empty($mad_config['shop_overview_column_count']))
				$mad_config['shop_overview_column_count'] = 3;

			if (empty($mad_config['shop_overview_product_count']))
				$mad_config['shop_overview_product_count'] = 9;
...
//			remove_action('woocommerce_single_product_summary', 'woocommerce_template_single_rating', 10);
		...
		public function add_enqueue_scripts() {
			...
			$woo_variation_file = $this->assetUrl('js/add-to-cart-variation.js');
			...
			if ($goahead == 1) {
				...
				wp_enqueue_script('wc-add-to-cart-variation', $woo_variation_file , array( 'jquery', 'wp-util' ), 1, true );
			} else {
		...
		public function woocommerce_ordering_products() {
			$ordering = new MAD_CATALOG_ORDERING($this->custom_get_option('products_filter'));
		...
		public function woocommerce_template_loop_product_title() {
			echo '<h3><a href="'. esc_url(get_the_permalink()) .'">'. mad_post_content_truncate(get_the_title(), mad_custom_get_option('product_title_count'), " ", "...") .'</a></h3>';
		}
		...
		public function mad_woocommerce_before_thumbnail () {
			...
			$has_thumb = ($this->woocommerce_second_thumbnail() != '') ? 'has-second-thumb' : '';
			?>

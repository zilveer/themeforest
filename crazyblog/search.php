...
$show_banner = crazyblog_set( $general_settings, 'search_page_title_section' );
$title = (crazyblog_set( $general_settings, 'search_page_title' )) ? crazyblog_set( $general_settings, 'search_page_title' ) : "";
$sidebar = (crazyblog_set( $general_settings, 'search_page_sidebar' )) ? crazyblog_set( $general_settings, 'search_page_sidebar' ) : "";
$layout = (crazyblog_set( $general_settings, 'search_sidebar_layout' )) ? crazyblog_set( $general_settings, 'search_sidebar_layout' ) : "";
$cols = ($layout != "full" && $sidebar ) ? "col-md-8" : 'col-md-12';
$bg = (crazyblog_set( $general_settings, 'search_title_section_bg' )) ? 'background:url(' . crazyblog_set( $general_settings, 'search_title_section_bg' ) . ')' : "";
$limit = crazyblog_set( $general_settings, 'search_character_limit' );
                        ...
                        <div class="texty-style">
							<div class="masonary">
								...
								if ( have_posts() ) : while ( have_posts() ) : the_post();
										...
										$innter_col = ($format == 'gallery') ? 'col-md-12' : crazyblog_set( $general_settings, 'search_post_column' );
										$gridCol = crazyblog_set( $general_settings, 'search_post_column' );
										...
										<div class="<?php echo esc_attr( $innter_col ) ?>">
											...
											<div <?php post_class( 'texty-post ' . $no_image ); ?>>
												<div class="texty-post-img">
													...
													<?php if ( crazyblog_set( $general_settings, 'search_post_author' ) == 'show' ) : ?>
												...
												<div class="texty-post-detail">
													<?php if ( crazyblog_set( $general_settings, 'search_post_category' ) == 'show' ) : ?>
														<div class="categories">
															<?php echo crazyblog_get_post_categories( get_the_ID() ); ?>
														</div>
													<?php endif; ?>
													<h2><a title="" href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h2>
													<?php if ( crazyblog_set( $general_settings, 'search_post_date' ) == 'show' || crazyblog_set( $general_settings, 'search_post_author' ) == 'show' ) : ?>
														<ul class="meta">
															<?php if ( crazyblog_set( $general_settings, 'search_post_date' ) == 'show' ) : ?>
													...
													<?php if ( crazyblog_set( $general_settings, 'search_post_comments' ) == 'show' || crazyblog_set( $general_settings, 'search_post_view' ) == 'show' || crazyblog_set( $general_settings, 'search_post_social_share' ) == 'show' ) : ?>
														<div class="post-info">
															<?php if ( crazyblog_set( $general_settings, 'search_post_comments' ) == 'show' ) : ?>
																<span class="view"><i class="fa fa-comments"></i><span><?php echo crazyblog_restyle_text( get_comments_number( get_the_ID() ) ) ?></span></span>
															<?php endif; ?>
															<?php if ( crazyblog_set( $general_settings, 'search_post_view' ) == 'show' ) : ?>
																<span class="view"><i class="fa fa-eye"></i><span><?php echo crazyblog_restyle_text( $view ) ?></span></span>
															<?php endif; ?>
															<?php if ( crazyblog_set( $general_settings, 'search_post_social_share' ) == 'show' ) : ?>
																<span>
								...
								<?php else: ?>
									<div class="search-page">
										<h5><?php esc_html_e( 'Search Results found', 'crazyblog' ); ?> <span>"<?php echo get_search_query(); ?>"</span></h5>
										<?php if ( crazyblog_set( $general_settings, 'search_form' ) == 'show' ) : ?>
											<div class="comment-form">
												<form action="<?php echo esc_url( home_url( '/' ) ); ?>">
													<div class="field">
														<input type="text" id="name" name="s" placeholder="<?php esc_html_e( 'Enter Your Search Keyword', 'crazyblog' ); ?>">
													</div>
													<button id="submit" class="submit" type="submit"><?php esc_html_e( 'Search', 'crazyblog' ); ?></button>
												</form>
											</div>
										<?php endif; ?>
									</div>
							...
							<?php _the_pagination( array( 'total' => $wp_query->max_num_pages ), 1, true ); ?>
						</div>
					</div>
                </div>

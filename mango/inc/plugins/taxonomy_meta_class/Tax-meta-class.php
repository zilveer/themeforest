...
class Tax_Meta_Class {
  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme'])){
      if ($meta_box['use_with_theme'] === true){
        ...
        $this->SelfPath = plugins_url( 'Tax-meta-class', plugin_basename( mango_plugins.'/taxonomy_meta_class' ) );
  ...
  public function show($term_id) {
    ...
    //@todo: for multiple instances of class
    //echo '</table>';
  }
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_radio( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      foreach ( $field['options'] as $key => $value ) {
        echo "<span class='mango_checkbox'><input style='{$field['style']}' type='radio' class='at-radio' name='{$field['id']}' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) . " /> <span class='at-radio-label'>{$value}</span></span>";
      }
    $this->show_field_end( $field, $meta );
  }
  ...
  if (!isset($value['id']))
	...
	echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
	if ($has_image)
		echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".__('Remove Image','mango')."' type='button'/>";
	else
		echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".__('Upload Image','mango')."' type='button'/>";
	$this->show_field_end( $field, $meta );
  }

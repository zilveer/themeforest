...
if(have_posts()){
	...
	$hideNav=get_opt('_fullh_hide')=='on'?'true':'false';
	
		?>
		
<script type="text/javascript">
jQuery(document).ready(function($){
	$('#full-height-slider').pexetoSlideshow({fullWidth:false, thumbContainerId:'slider-navigation', autoplay:<?php echo($autoplay); ?>, interval:<?php echo($interval);?>, pauseInterval:<?php echo($pauseInterval);?>, hideContent:<?php echo($hideNav);?>, hideText:"<?php echo(pex_text('_hide_menu_text')); ?>", showText:"<?php echo(pex_text('_show_menu_text')); ?>"});
});
</script>
    ...
    <div id="slider-img-wrapper">
		...
		foreach ( $attachments as $attachment ) {
			$img_src = wp_get_attachment_image_src($attachment->ID, 'full'); ?>
	  		<div class="img-wrapper"><img src="<?php echo $img_src[0]; ?>" /></div>
	   <?php } ?>

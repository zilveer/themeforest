<?php
namespace Hue\Modules\PieCharts\PieChartDoughnut;
...
class PieChartDoughnut implements ShortcodeInterface {
    ...
    public function vcMap() {
        ...
        for($i = 1; $i <= $this->chartFields; $i++) {
            ...
            $chart_fields[] = array(
                'type'       => 'textfield',
                'heading'    => esc_html__('Chart Value ', 'hue').$i,
                'param_name' => "chart_value_".$i,
            );
            $chart_fields[] = array(
                'type'       => 'colorpicker',
                'heading'    => esc_html__('Chart Color ', 'hue').$i,
                'param_name' => "chart_color_".$i,
            );
            $chart_fields[] = array(
                'type'       => 'textfield',
                'heading'    => esc_html__('Chart Legend ', 'hue').$i,
        ...
        vc_map(array(
            'name'                      => esc_html__('Pie Chart 3 (Doughnut)', 'hue'),
            'base'                      => $this->getBase(),
            'category'                  => 'by MIKADO',
    ...
    public function render($atts, $content = null) {
        ...
        $html = hue_mikado_get_shortcode_module_template_part('templates/pie-chart-doughnut', 'piecharts/piechartdoughnut', '', $params);

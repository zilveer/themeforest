<div class="<?php echo esc_attr( $cols ); ?> column">
    ...
    <div <?php post_class( 'default-template ' . $no_image ); ?> itemscope itemtype="http://schema.org/Article">
		...
		$meta_opt = crazyblog_set( crazyblog_set( $post_meta, 'crazyblog_post_format_options' ), '0' );
		$view = (get_post_meta( get_the_ID(), 'crazyblog_post_views', true )) ? get_post_meta( get_the_ID(), 'crazyblog_post_views', true ) : '0';
		?>


		<div class="single-image <?php echo esc_attr( (crazyblog_set( $meta_opt, 'crazyblog_post_toggle_audio_own' )) ? " audio" : ""  ); ?>">
			<?php
			if ( crazyblog_set( $meta_opt, 'crazyblog_post_toggle_audio_own' ) == '1' ) :
				$url = esc_url( crazyblog_set( $meta_opt, 'crazyblog_post_audio_own' ) );
				echo do_shortcode( '[audio src="' . $url . '"]' );
				?>
			<?php elseif ( crazyblog_set( $meta_opt, 'crazyblog_post_toggle_audio_link' ) ) : ?>
				<?php echo '<iframe class="iframe-width" height="450" src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/' . crazyblog_set( $meta_opt, 'crazyblog_post_audio_link' ) . '&amp;auto_play=false&amp;hide_related=false&amp;show_comments=true&amp;show_user=true&amp;show_reposts=false&amp;visual=true"></iframe>'; ?>
			<?php endif; ?>
			<ul class="short-info">

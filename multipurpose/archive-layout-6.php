...
<div class="columns masonry m4">

	<?php
		$current_category = get_query_var('cat');
		$options = array('hierarchical' => false, 'parent' => $current_category);
		$categories = get_terms('category', $options);
		if (count($categories)) :
		?>
		<div class="filters">
			<p><?php esc_attr_e('Categories', 'multipurpose'); ?>:</p>
			<ul>
	...
	<?php if (have_posts()) :
		while(have_posts()): the_post(); ?>
			<!-- Start: Post -->
			<article <?php post_class('brick'); ?>>
				<?php 
				$iframe_video = get_post_meta(get_the_id(), 'single_meta_video_iframe', true);
				if($iframe_video):
					echo '<p>'.$iframe_video.'</p>'; 
				else :
					if(has_post_thumbnail()): ?><div class="img"><a href="<?php the_permalink() ?>"><?php the_post_thumbnail('thumbnail-masonry'); ?></a></div><?php endif;
				endif; ?>

...
<article id="post-<?php the_ID(); ?>" <?php post_class($post_class) ?>>
	...
	<div class="post-content">
		...
		<?php if (is_sticky()): ?>
			<?php printf( '<span class="sticky-post">%s</span>', __( 'Featured', 'flatastic' ) ); ?>
		<?php endif; ?>
		...
		<?php if (mad_custom_get_option('blog-listing-meta-ratings')): ?>
			...
			<div class="rating-box">
				<span class="rate-desc"><?php _e('Rate this item', 'flatastic'); ?></span>
				<div class="rating readonly-rating" data-score="<?php echo esc_attr($post->rating); ?>"></div>
				<span>(<?php printf(_n('%d vote', '%d votes', $post->votes, 'flatastic'), $post->votes); ?>)</span>
			</div>
		...
		<?php
		wp_link_pages( array(
			'before'      => '<div class="page-links"><span class="page-links-title">' . __( 'Pages:', 'flatastic') . '</span>',
			'after'       => '</div>',
			'link_before' => '<span>',
			'link_after'  => '</span>',
			'pagelink'    => '<span class="screen-reader-text">' . __( 'Page', 'flatastic') . ' </span>%',
			'separator'   => '<span class="screen-reader-text">, </span>',
		) );
		?>

		<a href="<?php echo esc_url($link); ?>" class="read-more">
			<?php _e('Read More', 'flatastic'); ?>
		</a>

		<?php edit_post_link( __( '[ Edit ]', 'flatastic' ), '<span class="edit-link">', '</span>' ); ?>

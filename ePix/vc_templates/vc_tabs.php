<?php
$output = $title = $interval = $el_class = $icon = '';
...
preg_match_all( '/vc_tab title="([^\"]+)"(\stab_id\=\"([^\"]+)\")(?: icon="([^\"]+)\"){0,1}/i', $content, $matches, PREG_OFFSET_CAPTURE ); // themeva
...
foreach ( $tab_titles as $tab ) {
	...
	preg_match('/vc_tab title="([^\"]+)"(\stab_id\=\"([^\"]+)\")(?: icon="([^\"]+)\"){0,1}/i', $tab[0], $tab_matches, PREG_OFFSET_CAPTURE );

	// themeva add font icons
	$icon = '';
	
 	if( isset( $tab_matches[4][0] ) ) 
	{
		$icon = '<i class="fa '. $tab_matches[4][0] .'"></i>';
	}

    if( isset( $tab_matches[1][0] ) ) {
		//$tabs_nav .= '<li><a href="#tab-'. (isset($tab_matches[3][0]) ? $tab_matches[3][0] : sanitize_title( $tab_matches[1][0] ) ) .'">' . $tab_matches[1][0] . '</a></li>';
        $tabs_nav .= '<li><a href="#tab-'. (isset($tab_matches[3][0]) ? $tab_matches[3][0] : sanitize_title( $tab_matches[1][0] ) ) .'">'. $icon .' ' . $tab_matches[1][0] . '</a></li>'; // themeva $icon
    }
}
$tabs_nav .= '</ul>'."\n";

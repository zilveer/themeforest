			...
			<fieldset class="bbp-form">
				<legend>
					...
					<?php
						if ( bbp_is_forum_edit() )
							printf( __( 'Now Editing &ldquo;%s&rdquo;', 'mk_framework' ), bbp_get_forum_title() );
						else
							bbp_is_single_forum() ? printf( __( 'Create New Forum in &ldquo;%s&rdquo;', 'mk_framework' ), bbp_get_forum_title() ) : _e( 'Create New Forum', 'mk_framework' );
					?>
				...
				<?php if ( !bbp_is_forum_edit() && bbp_is_forum_closed() ) : ?>
					...
					<div class="bbp-template-notice">
						<p><?php _e( 'This forum is closed to new content, however your account still allows you to do so.', 'mk_framework' ); ?></p>
					</div>
				...
				<div>
					...
					<p>
						<label for="bbp_forum_title"><?php printf( esc_html__( 'Forum Name (Maximum Length: %d):', 'mk_framework' ), bbp_get_title_max_length() ); ?></label><br />
					...
					<?php if ( !function_exists( 'wp_editor' ) ) : ?>
						...
						<p>
							<label for="bbp_forum_content"><?php _e( 'Forum Description:', 'mk_framework' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_type"><?php _e( 'Forum Type:', 'mk_framework' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_status"><?php _e( 'Status:', 'mk_framework' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_visibility"><?php _e( 'Visibility:', 'mk_framework' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_parent_id"><?php _e( 'Parent Forum:', 'mk_framework' ); ?></label><br />

						<?php
							bbp_dropdown( array(
								'select_id' => 'bbp_forum_parent_id',
								'show_none' => esc_html__( '(No Parent)', 'mk_framework' ),
					...
					<div class="bbp-submit-wrapper">
						...
						<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_forum_submit" name="bbp_forum_submit" class="button submit"><?php _e( 'Submit', 'mk_framework' ); ?></button>
...
<?php elseif ( bbp_is_forum_closed() ) : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php printf( esc_html__( 'The forum &#8216;%s&#8217; is closed to new content.', 'mk_framework' ), bbp_get_forum_title() ); ?></p>
		</div>
	</div>
...
<?php else : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php is_user_logged_in() ? _e( 'You cannot create new forums at this time.', 'mk_framework' ) : _e( 'You must be logged in to create new forums.', 'mk_framework' ); ?></p>
		</div>
	</div>

...
function twitter_api_relative_date( $strdate ){
    ...
    if( $tdiff < 60 ){
        return __('Just now','flatastic');
    }
    // within last hour? X minutes ago
    if( $tdiff < 3600 ){
        $idiff = (int) floor( $tdiff / 60 );
        return sprintf( _n( '%u minute ago', '%u minutes ago', $idiff, 'flatastic' ), $idiff );
    }
    ...
    if( ! empty($samed) ){
        ...
        return sprintf( _n( 'About an hour ago', 'About %u hours ago', $hdiff, 'flatastic' ), $hdiff );
    }
    $tf = get_option('time_format') or $tf = 'g:i A';
    // within 24 hours?
    if( $tdiff < 86400 ){
        return __('Yesterday at','flatastic').date_i18n(' '.$tf, $tt );
    }

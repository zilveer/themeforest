...
<p><label>
<input type="checkbox" name="use_lb" value="1" '.($this->get_lb() ? ' checked="checked"' : '').' />
Enlarge pictures on click
</label></p>
		...
		{
		   $cat=intval($cat);
		   add_post_meta($this->post_id, '_cat_portfolio_lb', $cat, true);
		   update_post_meta($this->post_id, '_cat_portfolio_lb', $cat);
		}
		...
		function get_lb()
		{
         ...
         if (!$this->post_id) $this->post_id=$post->ID;
		   $d=get_post_meta($this->post_id, '_cat_portfolio_lb', true);
		   return $d;
		}
